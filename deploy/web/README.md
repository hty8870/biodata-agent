# BioData Agent 网页版部署手册（T1 灰度，2026-08-25）

> 范围：把仓库以**版本化 Docker 镜像**跑在 biodata-prod（<server-ip>）的 80 端口，
> 纯 HTTP 灰度；回退 = 换 tag 重新部署，分钟级。域名 + HTTPS + 备案 + CI/CD 是后续任务，
> 不在本手册范围。设计依据：《网页版部署设计_2026-08-25.md》（仓库外，与仓库同级的设计备忘）。

## 1. 形态一览

| 项 | 值 |
|---|---|
| 公网入口 | `http://<server-ip>/`（ufw 80 已放行） |
| 容器 | `biodata-web`（单容器、单进程单 worker、非 root uid 1000） |
| 端口 | 宿主 `80` → 容器 `8510`（run_web.py 读 `PORT=8510`、`BIODATA_WEB_HOST=0.0.0.0`） |
| 镜像 | `biodata-web:<WEB_API_VERSION>-g<短sha>`，如 `biodata-web:2.6.0-gae5156f`；本机保留最近 5 个 |
| 语料/前端 | `src/ web/ database/`（tracked 快照）**烤进镜像**——与代码同版本，回退天然一致 |
| 运行态数据 | 宿主卷 `/data/biodata-web` → 容器 `/data`（`BIODATA_DATA_ROOT=/data`，portable 双根分离）：账号 `.userdata/`、上传 `database/external/`、trace、日志、run；回退不动数据 |
| 密钥 | `/opt/biodata-web/.env`（root:600，内容取自本机 `agent/.env.zhipu` + `IMAGE_TAG` + `BIODATA_TRUSTED_HOSTS` 两行部署参数） |
| 内存 | `mem_limit: 1536m`（服务器预算 ≤1.5G；2C4G 同机还有遥测栈） |
| 遥测 | 前端 meta 已硬编码 `http://<server-ip>:8471`，与 80 端口应用互不影响 |

服务器路径边界（本任务的一切改动以此为止）：

- `/opt/biodata-web/`——部署物：`build/<tag>/`（构建上下文）、`.env`、`docker-compose.web.yml`、`deploy.sh`、`RELEASES.log`
- `/data/biodata-web/`——应用运行态数据（宿主每日 03:00 自动备份 `/data`，见 §6）
- `/opt/OPS_NOTES.md`——运维记录（只追加自己的小节）

## 2. 代码同步到服务器（灰度期临时桥）

原则：**一个 tag 一份完整构建上下文，禁止手工散拷单文件**。上下文 = worktree 的 git tracked
快照（`.dockerignore` 在构建时进一步瘦身）。

在**本机 worktree**（T1 施工分支所在目录，下文记为 `<worktree>`）执行：

```bash
TAG=2.6.0-g<短sha>          # 短sha = 分支当前 commit 的 git rev-parse --short HEAD
ssh -i ~/.ssh/glm.pem ops@<server-ip> "sudo mkdir -p /opt/biodata-web/build && sudo chown ops:ops /opt/biodata-web /opt/biodata-web/build"
git archive --format=tar.gz -o /tmp/biodata-ctx-$TAG.tar.gz HEAD
scp -i ~/.ssh/glm.pem /tmp/biodata-ctx-$TAG.tar.gz ops@<server-ip>:/tmp/
ssh -i ~/.ssh/glm.pem ops@<server-ip> "mkdir -p /opt/biodata-web/build/$TAG && tar -xzf /tmp/biodata-ctx-$TAG.tar.gz -C /opt/biodata-web/build/$TAG && rm /tmp/biodata-ctx-$TAG.tar.gz"
```

注意：git archive 只含 tracked 内容——`database/trace/`、`database/external/upload_*.json`
等 gitignored 运行产物天然不进上下文（镜像也不需要它们）。

## 3. 生成依赖锁定文件（改依赖时才需要重跑）

`requirements/requirements.txt` 只有 5 个下界约束；镜像必须按**精确锁定**安装。网页版镜像额外装
「AI 执行」扩展链 `langgraph` + `langchain-openai`（2026-08-26 产品所有者拍板装入镜像；
版本与 requirements/requirements-ci.lock 对齐），本机形态不受影响（扩展在本地仍可选装）。
锁定文件在服务器上用与镜像完全相同的基础环境生成（本机 Windows Python 版本不同，不在此生成）：

```bash
# $CTX = /opt/biodata-web/build/$TAG（构建上下文已按 §2 同步）
ssh -i ~/.ssh/<ssh-key>.pem ubuntu@<server-ip> "
  sudo docker run --rm -v $CTX:/ctx -w /ctx python:3.12-slim bash -lc \
    'pip install --no-cache-dir -q -r requirements/requirements.txt \
       langgraph==<ci锁版本> langchain-openai==<ci锁版本> && pip freeze' \
    | sudo tee $CTX/deploy/web/requirements-web.lock >/dev/null
"
scp -i ~/.ssh/<ssh-key>.pem ubuntu@<server-ip>:/opt/biodata-web/build/$TAG/deploy/web/requirements-web.lock deploy/web/requirements-web.lock
```

回填 worktree 后 `deploy/web/requirements-web.lock` 随分支 commit（Dockerfile COPY 它安装）。
若曾生成过锁定文件且依赖未变，直接复用 build 目录里的上一份即可。

## 4. 首次部署（one-time）

```bash
# 1) 服务器目录与数据卷（ops 属主 /opt/biodata-web；数据卷属主 = 容器内 biodata uid 1000）
ssh -i ~/.ssh/glm.pem ops@<server-ip> "
  sudo mkdir -p /opt/biodata-web /data/biodata-web
  sudo chown ops:ops /opt/biodata-web
  sudo chown 1000:1000 /data/biodata-web
  sudo chmod 700 /data/biodata-web
"

# 2) .env：密钥从本机 agent/.env.zhipu 上传（绝不打印内容），再加两行部署参数
scp -i ~/.ssh/glm.pem "<本机仓库根>/.env.zhipu" ops@<server-ip>:/tmp/biodata-web.env.upload
ssh -i ~/.ssh/glm.pem ops@<server-ip> "
  sudo install -m 600 -o root -g root /tmp/biodata-web.env.upload /opt/biodata-web/.env
  rm /tmp/biodata-web.env.upload
  printf 'BIODATA_TRUSTED_HOSTS=<server-ip>\nIMAGE_TAG=\n' | sudo tee -a /opt/biodata-web/.env >/dev/null
"

# 3) compose 与部署脚本就位（来自构建上下文）
ssh -i ~/.ssh/glm.pem ops@<server-ip> "
  cp /opt/biodata-web/build/$TAG/deploy/web/docker-compose.web.yml /opt/biodata-web/
  cp /opt/biodata-web/build/$TAG/deploy/web/deploy.sh /opt/biodata-web/deploy.sh
  chmod +x /opt/biodata-web/deploy.sh
"

# 4) 构建镜像（上下文 = build/<tag> 目录；tag 规则 <WEB_API_VERSION>-g<短sha>）
ssh -i ~/.ssh/glm.pem ops@<server-ip> "
  sudo docker build -f /opt/biodata-web/build/$TAG/deploy/web/Dockerfile \
    -t biodata-web:$TAG /opt/biodata-web/build/$TAG
"

# 5) 部署
ssh -i ~/.ssh/glm.pem ops@<server-ip> "sudo /opt/biodata-web/deploy.sh $TAG"
```

`BIODATA_TRUSTED_HOSTS` 是 webapp Host 守卫的显式白名单（本机形态默认仅 loopback）；
未来上域名时把该行改成逗号分隔追加域名即可（重启生效）。

## 5. 日常发布与回退

发布新版 = §2 同步 → §3（依赖变了才跑）→ §4-4 构建 → `sudo /opt/biodata-web/deploy.sh <new-tag>`。
deploy.sh 自动：写 `IMAGE_TAG` → `up -d` → 等 healthy（≤120s）→ 失败自动切回上一 tag →
成功记 `/opt/biodata-web/RELEASES.log` 并清理镜像只留最近 5 个。

**手工回退**（不依赖 deploy.sh 的自动路径时）：

```bash
ssh -i ~/.ssh/glm.pem ops@<server-ip> "sudo /opt/biodata-web/deploy.sh <old-tag>"
# 或最原始形态：改 /opt/biodata-web/.env 的 IMAGE_TAG=<old-tag> 后
#   sudo docker compose --env-file /opt/biodata-web/.env -f /opt/biodata-web/docker-compose.web.yml -p biodata-web up -d
```

## 6. 备份核对

宿主 `/data` 每日 03:00 由 `/opt/scripts/backup-data.sh` 自动备份。新增 `/data/biodata-web`
后核对脚本是否覆盖该目录（本任务已核对并把结论写入 ACCEPTANCE-T1.md 与 OPS_NOTES.md）；
若脚本按目录白名单备份而不涵盖，补一行并记录。

## 7. 故障排查入口

```bash
sudo docker ps                                   # 容器状态（应 Up ... (healthy)）
sudo docker inspect --format '{{.State.Health.Status}} {{.State.Health.FailingStreak}}' biodata-web
sudo docker logs --tail 100 biodata-web          # 应用日志（json-file 10m×3 轮转）
sudo docker exec -it biodata-web bash            # 进容器（slim，无 curl；用 python urllib）
curl -s http://localhost/api/health              # 服务器本机过 80 端口验证（Host: localhost→127.0.0.1 守卫放行）
curl -s -H 'Host: <server-ip>' http://<server-ip>/api/health   # 模拟公网 Host（白名单放行路径）
tail /opt/biodata-web/RELEASES.log               # 部署/回退流水
```

常见症状：

- **403 仅接受本机 loopback Host**：`.env` 的 `BIODATA_TRUSTED_HOSTS` 缺失或没包含访问用的
  Host；改后 `sudo docker compose ... up -d` 重建容器（env_file 变化需重建）。
- **healthy 起不来**：`docker logs` 看是否依赖缺失（锁定文件与镜像不符）或 8510 未监听
  （`BIODATA_WEB_HOST`/`PORT` 被覆盖）。
- **注册/上传报错且日志含 Permission denied**：宿主 `/data/biodata-web` 属主不是 1000:1000。

## 8. 验收证据

逐条命令与输出摘要见同目录 `ACCEPTANCE-T1.md`（每次部署后更新）。

## 9. 账号护栏（T3，2026-08-25）

公网形态的三道闸：**登录强制 + 注册邀请 + LLM 日配额**，全部 additive——缺省（不设任何
下列变量）时与本机单机形态逐字节一致，本机安装包用户零感知。只拦 `/api/` 前缀，静态前端
与登录页天然可达；未登录调非白名单 `/api/*` 返回 401 `{"ok":false,"error":"auth_required"}`，
前端据此自动锁定整页只留登录框。

### 9.1 环境变量（改 .env 后需 `up -d` 重建容器生效）

| 变量 | 缺省 | 语义 |
|---|---|---|
| `BIODATA_REQUIRE_ACCOUNT` | 关 | `1` 开启登录强制门（白名单：health/register/login/logout/whoami） |
| `BIODATA_INVITE_CODE` | 空 | 护栏模式下注册必填；**未配置 = 注册整体关闭**（宁可关死不留缝） |
| `BIODATA_LLM_DAILY_PER_USER` | 100 | 正式通道每账号每日 LLM 轮数（0=不限） |
| `BIODATA_LLM_DAILY_GLOBAL` | 1000 | 正式通道全站每日熔断（0=不限） |
| `BIODATA_LLM_QUOTA_EXEMPT` | 空 | 豁免用户名（逗号/空白分隔，产品所有者用） |
| `BIODATA_TRIAL_API_KEY` | 空 | 「限量试用」通道专用 key；**未设时回落 `BIODATA_EMBED_API_KEY`**（2026-08-27 起——试用与 embedding 召回共用同一把智谱 key，部署侧只维护一份；两变量都只在进程环境，请求级覆盖链从不注入） |
| `BIODATA_TRIAL_DAILY_PER_USER` | 30 | 试用通道每账号每日轮数（独立桶，更紧） |
| `BIODATA_TRIAL_DAILY_GLOBAL` | 500 | 试用通道全站每日熔断 |
| `BIODATA_TRIAL_BASE_URL` / `BIODATA_TRIAL_MODEL` | 官方默认 | 试用通道端点/模型覆盖（默认 `https://open.bigmodel.cn/api/paas/v4/` / `glm-5.3-flash`，2026-08-27 起与 embedding 通道同源） |
| `BIODATA_TRIAL_THINKING` | 空 | 试用通道思考档逃逸口（`enabled`/`disabled`）。**缺省不发 thinking 参数**：glm-5.3-flash 始终思考、拒收 disabled（2026-08-27 实测 400），思考开启下 tool_choice 强制/自动档均正常 |
| `BIODATA_LLM_QUOTA_FILE` | userdata 层 `llm_quota.json` | 配额账本路径（一般不动） |

### 9.2 计数口径（哪一发请求会烧服务端 LLM）

埋闸端点全集：`/api/recommend`（润色/AI 重排/动作审核意图为真时）、`/api/utterance`、
`/api/action·plan`、`/api/agent·search-rescue`、`/api/act·summary`、`/api/dream`。
**不计**：BYOK（请求自带 key）、mock、LLM 未启用、服务端无 key——这些烧不到服务端配额。
超限返回 429 中文文案；按 UTC 日重置（北京时间每日 08:00 归零）。账本只留当日、故障
放行（可用性优先，provider 侧消费上限是最后防线）。

试用通道（前端「限量试用」预设）：端点/模型锁定服务端托管值，请求级 key/地址/模型
**一律忽略**（防注入防偷换）；凭据只认 `BIODATA_TRIAL_API_KEY`（未设回落
`BIODATA_EMBED_API_KEY`），绝不回落 `LLM_API_KEY`；默认 `glm-5.3-flash`
（2026-08-27 起）**不发** thinking 参数——该模型始终思考、拒收 disabled，
思考开启下 tool_choice="required" 实测正常（探针 `scripts/probe_glm53flash_trial.py`）。

### 9.3 日常运营动作

```bash
# 生成邀请码（私下发给用户；泄漏了换一个新的重启即可）
python -c "import secrets; print(secrets.token_urlsafe(8))"

# 改限额/豁免/邀请码：编辑 .env 对应行后重建
sudo docker compose --env-file /opt/biodata-web/.env -f /opt/biodata-web/docker-compose.web.yml -p biodata-web up -d

# 查看当日用量账本（容器内 /data/.userdata/llm_quota.json，只留当日）
sudo docker exec biodata-web cat /data/.userdata/llm_quota.json
```
