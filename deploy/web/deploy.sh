...
new_tag="${1:-}"
if [ -z "$new_tag" ]; then
  echo "usage: $0 <image-tag>   # e.g. 2.6.0-gae5156f" >&2
  exit 2
fi
...
# 部署后断言（公网护栏硬化，）：healthy 只证明进程活着，不证明账号护栏开着——
