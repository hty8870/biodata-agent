...
def _count_unit(item: dict[str, Any]) -> tuple[str, str]:
    """取样本量的原始 count / unit（N6 语义提醒用）。与 `_sample_size` 同源：
...
def _raw_status(item: dict[str, Any]) -> str:
    ...
    if item.get("has_raw_data") is False:
        if raw_data_false_is_guess(item):
            return "⚪ 未确认"   # G-08（2026-08-15 ta2）：猜测的 False 不许印成「无 FASTQ」
        return "❌ 无 FASTQ"
    return "⚪ 未说明"
...
def build_dataset_introduction(item: dict[str, Any]) -> dict[str, Any]:
    ...
    # 体裁路由（2026-07-17）：`if description:` 把五种**根本不是同一种东西**的字符串
    ...
    # 补充表的样本量口径说明（如「样本量为 12 张切片的合计」）——与 N6 提醒同列展示，先于定论。
    ...
    return {
        ...
        "facts": facts,
        "caveat": caveat,
        # N6 样本量语义提醒（确定性、单一真源 units.sample_size_note）：
