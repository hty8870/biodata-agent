...
裸标识符 fail-closed（2026-08-04 普查 P1-2：裸贴 DOI 曾被词面解析拆成数字残片 → 全库冒充结果）。
...
#: 本目录索引的 11 个来源（人读，用于 fail-closed 文案）。2026-08-06 接入 HuBMAP / Broad Single Cell Portal / NCBI GEO；
#: 2026-08-14 Zenodo 首批入库（通用仓储人工甄别切片，第 10 源）；
#: 2026-08-14 refine.bio 首批入库（GEO/SRA/AE 统一加工镜像的单细胞/空间切片，第 11 源）。
INDEXED_SOURCES_ZH = (
...
# CELLxGENE/HCA 的 UUID、collection DOI、GEO Series（GSE…，2026-08-06 起随 geo.json 收录）、
...
#: 指路文案反过来教用户把 DOI 拆掉（2026-08-04 第二轮对抗评审 R2-1 P2-2）。
...
def classify(text: str) -> dict[str, Any] | None:
    ...
    GEO Series(GSE) 自 2026-08-06 随 geo.json 收录转为 True。
    """
    t = (text or "").strip()
    if not t:
        return None
