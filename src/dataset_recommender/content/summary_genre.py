...
但「description」在五个源里**根本不是同一种东西**（2026-07-17 全库实测）：
...
def _genre_for_source(source: str) -> str:
    ...
    if not src:
        # 2026-08-15 G-06：无 source 的记录此前静默默认按 10x（→ scaffold）处理，
    ...
    if src == provenance.SOURCE_ARRAYEXPRESS or src == provenance.SOURCE_HCA:
        return GENRE_PROSE
    # 2026-08-06：GEO 的 description 是官方 ESummary 的 submitter summary（T35 逐条对照缓存原文核验）；
    ...
    if src in (provenance.SOURCE_GEO, provenance.SOURCE_SCP):
        return GENRE_PROSE
    # 2026-08-15 G-06：Zenodo 的 description 是官方 metadata.description 剥 HTML
...
def strip_scaffolding(description: str, source: str) -> str:
    ...
    if not text:
        return ""
    # 2026-08-15 G-06：不再把缺 source 静默默认成 10x——认不出的来源一律不剥（不猜）。
    src = meaningful_metadata_text(source)
