...
def constraint_satisfied(record: DatasetRecord, dim: str, terms: list[str], lenient: bool = False) -> bool:
    ...
    if dim == "modality":
        ...
        # spatial 记录会漏进单细胞结果、违反裁判 → 冻结门当场 FAIL（验证 Top1 97.6→92.7）。精确等值匹配存字段。
...
class DatasetRetriever:
    ...
    def _rank_score(self, record: DatasetRecord, intent: QueryIntent) -> float:
        ...
        #
        # **短 ASCII 自由词按词边界判定**（夜场景覆盖测试）：此前是裸子串，于是
        ...
        # 抽样维不计（xdc1 追加发现 A，）：SCEA 的 tissue/disease 可能取自 0.1%~2.5%
