...
# ==================== 解析失败的错误回灌（2026-08-08 B3 调研六候选批）====================
...
# ContextVar 而不是模块级 dict（2026-08-15 触发点审计 D-01，与下方 _LAST_LLM_ERROR 同型）：
...
# ContextVar 而不是模块级槽（2026-08-04 对抗评审 docs-arch P2-7）：`/api/recommend` 是 sync def
...
# 但异常本体必须至少留一行 stderr——否则「重排静默失效」事后无从归因（2026-08-15 审计 D-02）。
_WARNED: set[str] = set()
...
def rerank_candidates(
    ...
    *,
    ...
    # B3（2026-08-08 调研六候选批）：非空但机械结构无效的输出 → 错误回灌重问一次；
...
# （agent 显式调用 + 机械择优闸，见 设计_检索工具化_search_rerun_2026-08-16.md），
...
def audit_action_markers(
    ...
    *,
    ...
    if not text:
        return None, [], ""
    # B3（2026-08-08）：非空但解析不出 → 错误回灌重问一次；仍败照旧 (None, [], "") fail-open。
...
def judge_drop_terms(
    ...
    *,
    ...
    try:
        ...
        if not text:
            return None, ""
        # B3（2026-08-08）：非空但解析不出 → 错误回灌重问一次；仍败照旧 (None, "") 保持弃权
