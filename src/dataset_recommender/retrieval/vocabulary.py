...
# 背景（codex 反馈 + 3 视角对抗评审收敛）：`单细胞` 原硬映射成 platform=chromium，多源检索会静默排除
...
CATALOG: dict[str, list[dict[str, object]]] = {
    "species": [
        ...
        # 玉米（2026-08-06 R6，dev 集 dv28/dv29/dv33）：base 有 2 条 species="Maize" 记录，
    ...
    "disease": [
        ...
        # 2026-08-06 R6（dev 集 dv34）：霍奇金亚型拆出独立条目（见下）——此前「霍奇金淋巴瘤」泛化到 lymphoma，
        # base 里 rank-1 被 small lymphocytic lymphoma（非霍奇金亚型）抢走，是 holdout 首跑唯一硬违规的病灶。
        ...
        # 2026-08-06 R6（dev 集 dv37/dv38）：aml / acute myeloid 从泛称条目移入下方急髓专条——
        ...
        # 改名（2026-07-22 夜）：原别名是裸「子宫内膜」，把**组织**说法吞进了癌症维度。
        ...
        # 非整倍体（2026-08-06 T37）：GEO 试点切片标志性条件词（GSE3642 单细胞 array-CGH 非整倍体
    ...
    "modality": [
        # scrnaseq/snrnaseq（2026-08-06 T37）：既有 scrna-seq/snrna-seq 的无连字符写法，
    ...
    "assay": [
        ...
        # 2026-08-06 R6（dev 集 dv23-dv27）：① 补「V(D)J」带括号形、「免疫受体（库）」、「TCR/BCR」别名——
        ...
        # ── 2026-08-06 批次（T37 SCP/GEO NL 检索补盲）：两源 description 里实测高频、此前一律
...
#
# 2026-07-18（N1 静默丢词诚实层）：把原单表 FILLER_TOKENS 拆成两组，语义不同、用途不同。
...
# 实测（2026-07-22 夜）：「人类肺数据，帮我打包前20条」「人类肺癌数据，生成下载脚本」「导出引文」——
...
#: 管护类操作短语（2026-08-03 agent2）：「AI 执行」关闭时路由层的**规则**降级检测用
...
FILLER_GRAMMAR = [
    ...
    # ── 2026-08-06 R6（dev 集）：口语虚词，此前各自把整句卡进 unresolved_term 弃权 ──────────
...
# 旧注释（已作废）说「最好 / 尽量」改起来属于受控重基线、需要单独授权 —— 2026-07-25 产品所有者
...
EXEC_NEG_PREFIX_CN = (
    ...
    "排除", "剔除", "去除", "去掉", "排掉",
    "拒绝",
    # 2026-08-06 R6（dev 集 dv07/dv08）：「别带X」口语排除（「就是别带脑组织的」）。
...
# 可执行后缀（X 除外）
# 2026-08-06 R6（dev 集 dv03-dv06）：否定后缀两族——「X的不要」（淋巴瘤的不要）与
