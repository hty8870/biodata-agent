...
# alias 覆盖第一批（tissue+species）：并入 Uberon(组织)/NCBI Taxonomy(物种) 常见同义词 + 中文口语/临床变体（如
...
# alias 覆盖第二批（外部平台库对齐）：基础库(10x)为 base、外部平台库(CELLxGENE/HCA/EBI SCEA/ArrayExpress/ENCODE，约4900条)
...
CATALOG: dict[str, list[dict[str, object]]] = {
    ...
    "disease": [
        ...
        # R6（dev 集 dv34）：霍奇金亚型拆出独立条目（见下）——此前「霍奇金淋巴瘤」泛化到 lymphoma，
        ...
        # R6（dev 集 dv37/dv38）：aml / acute myeloid 从泛称条目移入下方急髓专条——
        ...
        # 改名（夜）：原别名是裸「子宫内膜」，把**组织**说法吞进了癌症维度。
        ...
        # 夜验证：「血管瘤」此前命中裸「血管」→ 组织=Blood Vessel/Artery/Vein，
        ...
        # 非整倍体（T37）：GEO 试点切片标志性条件词（GSE3642 单细胞 array-CGH 非整倍体
        # 检测），SCP 亦有 1 条；此前整句 unresolved_term 弃权。染色体拷贝数异常状态、非癌，
        # 独立成条；alias「aneuploid」兼形容词形，中文「非整倍体」是四字专用术语、无碰撞。
        # 提升后 GEO/SCP 各有 1 条记录 disease 字段含该词（T37 反标落笔）→ 不需要 absent_ok。
    ...
    "modality": [
        # scrnaseq/snrnaseq（T37）：既有 scrna-seq/snrna-seq 的无连字符写法，
    ...
    "assay": [
        ...
        # R6（dev 集 dv23-dv27）：① 补「V(D)J」带括号形、「免疫受体（库）」、「TCR/BCR」别名——
        ...
        # 批：此前一律 unresolved_term 弃权的单细胞/空间技术专名。
        ...
        # 批（HuBMAP 接入）：HuBMAP chemistry 高频的空间/多重成像技术专名。
        ...
        # ── 批（T37 SCP/GEO NL 检索补盲）：两源 description 里验证高频、此前一律
...
RAW_REQUIRED_ALIASES = [
    ...
    # 夜：「要原始文件 / 原始序列」此前不是 raw 说法，「原始」两字落进残差门 →
...
#
# （N1 静默丢词诚实层）：把原单表 FILLER_TOKENS 拆成两组，语义不同、用途不同。
...
# 验证（夜）：「人类肺数据，帮我打包前20条」「人类肺癌数据，生成下载脚本」「导出引文」——
...
FILLER_GRAMMAR = [
    ...
    # ── 夜：上一轮补了「来自/出自/基于」，但**同一类**的还剩一批仍在整句弃权。
    ...
    # ── 夜 85 条查询电池：仍在整句弃权的**纯功能词/已落维度的头词** ──────────
    ...
    # ── 产品哲学修正：「或」与 hedge 从「整句弃权」改为「照做」，两者的**残留**
    ...
    # ── R6（dev 集）：口语虚词，此前各自把整句卡进 unresolved_term 弃权 ──────────
...
FILLER_DOMAIN = [
    ...
    # ── 夜：验证里仍在整句弃权、但结构上确实没有对应筛选维度的研究主题词。
    ...
    "成体", "胚系",
    # ── 夜 85 条查询电池：病名**限定语**。系统只按病名落维，限定语没有对应字段。
...
ALIAS_PROTECTED_COMPOUNDS = (
    ...
    # ── 夜验证：上面只收了「胰岛素」一个，**同一族**的其余成员全在裸奔。
...
# 产品哲学修正后重新审视：`OR_MARKERS` 曾让整句弃权（`unsupported_boolean_or`），
...
# 旧注释（已作废）说「最好 / 尽量」改起来属于受控重基线、需要单独授权 —— 产品侧
...
# ============================================================================
# 否定 / 排除语法（v2：小白名单执行 + 大兜底弃权；与 实现 两轮对抗辩论收敛）
# ----------------------------------------------------------------------------
...
EXEC_NEG_PREFIX_CN = (
    ...
    "排除", "剔除", "去除", "去掉", "排掉",
    "拒绝",
    # R6（dev 集 dv07/dv08）：「别带X」口语排除（「就是别带脑组织的」）。
...
# 可执行后缀（X 除外）
# R6（dev 集 dv03-dv06）：否定后缀两族——「X的不要」（淋巴瘤的不要）与
