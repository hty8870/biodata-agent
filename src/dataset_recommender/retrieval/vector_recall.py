...
# 纪律（2026-08-15 触发点审计 D-06）：只缓存**成功**加载的模型（正缓存）；加载失败
...
#: cr1（并发分流 r3 P1-5 / 设计 §4.6）：模块级单飞锁——`_setup_determinism`（写全局
...
def default_model_dir(model_name: str = DEFAULT_EMBEDDING_MODEL) -> Path:
    """dense 嵌入模型目录 = model_root/embeddings/<name>（W1 起经 runtime_paths 单一真源：
...
def load_embedder(
    ...
    装上依赖）后同进程即可恢复，无需重启（2026-08-15 D-06）。
    """
    ...
    # cr1（r3 P1-5 / 设计 §4.6）：check-then-load 全程持模块级单飞锁——并发请求/线程
    # 首次加载模型时只有第一个线程真加载，其余等待后命中缓存（避免重复加载与
    # _setup_determinism 的 os.environ 写入竞态）。
    # cr1 修复（2026-08-19 P0 死锁）：`_setup_determinism` **先无锁调用**（它自带模块级
...
def load_cross_encoder(
    ...
    失败**不缓存**：下次调用会重试加载，故障消除后同进程即恢复，无需重启（2026-08-15 D-06）。
    """
    ...
    # cr1（r3 P1-5 / 设计 §4.6）：同 load_embedder，check-then-load 全程持单飞锁。
    # cr1 修复（2026-08-19 P0 死锁）：`_setup_determinism` 先无锁调用（自带单飞锁），
