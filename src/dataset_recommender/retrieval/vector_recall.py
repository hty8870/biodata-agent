...
#: dense 融合法（五机制批，调研-检索质量与RAG评测 §候选2）：
...
# 经基准+held-out 选型胜出：cross-encoder 重排器，MIT、短中文查询甜区、可审计。
...
# 纪律（触发点审计 D-06）：只缓存**成功**加载的模型（正缓存）；加载失败
...
#: （并发分流 r3 / 设计 §4.6）：模块级单飞锁——`_setup_determinism`（写全局
...
def load_embedder(
    ...
    # （r3 / 设计 §4.6）：check-then-load 全程持模块级单飞锁——并发请求/线程
...
def load_cross_encoder(
    ...
    失败**不缓存**：下次调用会重试加载，故障消除后同进程即恢复，无需重启（D-06）。
    """
    ...
    # （r3 / 设计 §4.6）：同 load_embedder，check-then-load 全程持单飞锁。
