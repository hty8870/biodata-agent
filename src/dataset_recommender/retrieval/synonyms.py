...
背景（2026-08-18）：评测侧（scripts/evaluate_search_live.py 的 must_match 嵌套列表）
...
5. 本表刻意**不**做成配置文件/外部数据：词目需要逐条人工评审与测试回归，
   放代码里让每次改动都走代码评审 + 质量门，而不是静默改配置。
"""
...
CONTROLLED_SYNONYMS: dict[str, tuple[str, ...]] = {
    # 词目 1（首条，2026-08-18）：cancer ↔ carcinoma。
