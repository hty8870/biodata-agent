# -*- coding: utf-8 -*-
"""联网工具组（2026-08-03 P1-B5）：免 key 通用搜索（主力）+ 官方源轻量适配器（对照），统一出口形态。
...
def _raw_get(url: str, *, timeout: int, min_interval: float, headers: dict[str, str],
    ...
    attempts（2026-08-15 G-10）：可选出参（单元素列表），回填实际请求次数——重试发生在函数内部，
...
def fetch_json(
    ...
    *,
    ...
    G-10（2026-08-15）：JSON 解析失败是对端改了返回形状，属确定性失败——不再当瞬时错误
...
def fetch_text_logged(
    ...
    *,
    ...
    G-10（2026-08-15）：发生重试时条目带 attempts 计数（形状只增不减）。"""
...
def search_duckduckgo(
    ...
    *,
    ...
    if not items:
        ...
        # G-07（2026-08-15）：原式 `"anomaly.js" in page or "anomaly" in page.lower() and "result__a" not in page`
...
def refinebio_organism_param(species: str) -> str:
    ...
    G-04（2026-08-15）：这是两个入口（本模块 search_refinebio 与
...
def search_refinebio(
    ...
    *,
    ...
    if sp and not organism:
        # G-04（2026-08-15）：词表外物种不过滤但必须用户可见——否则无法区分
