...
- 其余（GEO 等 page_only / direct_unsized / 语料查不到）：进 `unsupported` 并给中文 reason。
  本批**不新增任何网络抓取代码**：B 级 API 接入不在本批范围。

## 安全红线（与 executor 同口径；SEC-C03 2026-08-21 加固）
...
- **用户上传记录代下开关**（SEC-C03 裁决）：env `BIODATA_DOWNLOAD_BLOCK_USER_UPLOADED`
...
def _block_user_uploaded() -> bool:
    ...
    默认（未设置/0/false）= **允许**——单机用户场景要靠上传记录的直链直接下载（SEC-C03 裁决，
    见认领 kimi-sec-s1-download）。注意这只是一个策略便利：攻击者改记录 source 标签即可绕过，
...
def _make_job(job_dir: Path, plan: dict) -> dict:
    ...
    return {
        ...
        # dl-auto-1：在途队列的权威 uid 列表（按进队顺序，随 add/remove 增删）。
        ...
        # dl-auto-1（在途增删）：逐行取消信号 + 被用户移除的行下标集合。
...
def start_job(uids: Sequence[str], *, records: "dict | None" = None,
    ...
    # **先预检、后建目录**（dl2 回归钉）：磁盘预检用「父目录」做（同卷任一路径的
    # disk_usage 结果相同），预检不通过就抛 507、**不创建任务目录**——此前先 mkdir 再预检，
    # 507 路径会在真实下载目录（~/Downloads/BioData数据-*）留下空目录（dl1 批留下 8 个的根源）。
...
def _finalize_file(job: dict, entry: dict, result: DE.FileResult, row: dict) -> None:
    ...
    if status == FILE_SKIPPED:
        ...
        if result.error and "硬上限" in result.error:
            # SEC-C03：超过单文件硬上限（服务器实际输出远超声明大小/未声明大小）——给专门文案，
...
def _run_job(job: dict, *, opener: "Callable | None" = None,
    ...
    dl-auto-1：支持在途增删——`update_job` 移除正在下载的行时只置该行的 `_row_events[idx]`
    ...
    try:
        for idx, row in enumerate(rows):
            ...
            # ——测试全部注入 opener，这个洞在真机上才现形（dl1 真机 sanity 抓到）。
            inject = {}
