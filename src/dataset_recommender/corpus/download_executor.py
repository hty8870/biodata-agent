...
- **SEC-C03（2026-08-21 安全修复批 S1）SSRF / 磁盘耗尽防线**：生产下载**不再使用裸
...
#: 可重试的 HTTP 状态码（与 corpus_net._RETRYABLE_HTTP 同口径，2026-08-15 G-12）：429 限流 /
#: 503 临时不可用是「稍后可能好」而非确定答案，走指数退避；其余 4xx 仍是确定答案不重试。
RETRYABLE_HTTP = frozenset({429, 503})

# ---- SSRF / 磁盘耗尽防线（SEC-C03，2026-08-21）----
...
class DownloadPolicyError(Exception):
    """下载被安全策略拒绝（SEC-C03）：非 https / 端口不在白名单 / 主机不在白名单 /
    IP 解析闸 / 重定向超限。
...
def _protected_dirs() -> "tuple[Path, ...]":
    ...
    - 单根（source / portable 同根）：整个 database/（base 冻结基准 + external 元数据库）
      + 顶层 research/（2026-08-27 迁移批：原 database 下 workstream 流水线整体上移，
...
def url_policy_error(url: str, allowed_hosts: Sequence[str]) -> "str | None":
    ...
    SEC-C03 起端口只放 443（台账 15215 条直链实测全部 443/无端口）；带 userinfo 的 URL
...
def _hard_max_bytes(row: dict) -> int:
    ...
    SEC-C03：声明大小是攻击者可控的，不能只信它——×1.05 系数兜住「实际输出略多于声明」的
...
def download_one(row: dict, out_root: Path, allowed_hosts: Sequence[str], *,
    ...
    `opener` 为 None 时使用生产默认 `_policy_opener(allowed_hosts)`（SEC-C03：每一跳
    ...
    while attempts < max(1, max_attempts):
        ...
        try:
            ...
            # 429/503（限流/临时不可用）按指数退避重试（G-12，与 corpus_net/corpus_curation 同一
...
def provision(dataset_uids: Sequence[str], out_dir: "str | os.PathLike", *,
    ...
    `opener` 默认 None → 每行用带计划白名单的 `_policy_opener`（SEC-C03 防线）；
