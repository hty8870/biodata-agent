...
#: 单文件条数上限（**写入汇真源**， 验证 迁移：原 corpus_curation
...
#: external 全库累计条数上限（**写入汇真源**，验证/K5/N8）：单文件上限挡不住
...
#: `UploadError.code` 的机器码全集（schema 加固顺手项：从本模块**实际 raise 点**
...
def _ingest_write_and_journal(
    *,
    ...
    账写不进去 → 删 `.tmp` 回滚 + UploadError(journal_failed)——「文件在、账不在」正是  要灭的
    状态。 起落盘改为 `.tmp` → 记账 → `os.replace` 正名：杀进程只留 `.tmp` 残留
...
def ingest_dataset(
    ...
    **强制流水账** → 清外部库缓存。
    ...
    # 任务 3（基线+补丁包）：绑定补丁作用域（请求持有登录会话）时，写入改落
