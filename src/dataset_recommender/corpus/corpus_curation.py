...
设计蓝本：`agent/设计_对话式数据库管护_2026-08-01.md`（2026-08-01 用户明确授权）。本模块是纯函数模块，
...
#: 单文件导入条数上限（2026-08-05 夜班 E 路 P1）：实测 150 万条合法 JSON 入库后 /api/datasets
#: 单请求 37.8s、并发下线程池饿死。上传语义是「用户自己的数据集元数据」，20 万条已是极宽上限。
#: 2026-08-09 codex P0-2：真源迁入写入汇 `uploads.MAX_INGEST_RECORDS`（闸必须住在所有入口
...
#: 2026-08-22 engagement B2 增 sync_busy=另一个 sync_updates 正在跑（整任务文件锁被占，
...
def _parse_payload(payload_bytes: bytes, filename: Any) -> tuple[str, Any, list[dict]]:
    ...
    # 2026-08-05 夜班 E 路 P1：条数上限。实测 150 万条合法 JSON 能入库，随后 /api/datasets
...
#: external 全库累计条数上限（2026-08-08 codex 裁决 K5/N8；2026-08-09 P0-2 真源在写入汇
...
def plan_import(payload_bytes: bytes, filename: Any, source: Any = None, *, project_root: Path) -> dict:
    ...
    return {
        ...
        "duplicate": {
            ...
            # 网页对话通道撞重时会自动带 force 重确认，用户根本不该看见参数名，copy 评审 P2-3）。
            "force_param": "force",
...
# 同时通过、同时打出 N 倍红线速率（对抗评审 R2-8 实测 8 线程 40 调用 35 次违规）。
    ...
    *,
    ...
    for attempt in range(_FETCH_RETRIES):
        ...
        try:
            ...
            # G-10（2026-08-15）：JSON 解析失败是对端改了返回形状，属确定性失败——
...
def _search_hubmap(
    ...
    *,
    ...
    分页补齐（登记册 R9：08-06 过夜批遗留⑤，2026-08-08 v2 施工）：species 本地过滤若发生在
...
def _refinebio_organism_param(species: str) -> str:
    ...
    词表外 → ""（不做服务端过滤，不乱猜映射）。
    G-04（2026-08-15）：映射真源收敛到 corpus_net.refinebio_organism_param，两个入口同口径——
...
def _refinebio_to_record(hit: dict) -> dict | None:
    ...
    字段口径（2026-08-14 实测 /v1/search/ 列表项与 /v1/experiments/{acc}/ 详情两形状）：
...
def _search_refinebio(
    ...
    *,
    ...
    if str(species or "").strip() and not organism:
        # G-04（2026-08-15）：词表外物种不过滤但必须用户可见（与 corpus_net.search_refinebio
...
#: 源适配器注册表（设计 §2 闸 1）。arrayexpress 为首发；2026-08-06 接入 cellxgene / hubmap /
...
# ==============================================================================================
# 实体级去重（2026-08-10 P1-4，codex 二轮评审）：search_online / sync 共用
#
# 病灶：search_online 此前**零去重**——同一数据集换关键词反复搜会反复入库（import 的内容 hash
...
def _record_identity_keys(record: dict, fallback_source: str = "") -> set[str]:
    """单条记录的实体身份键集合（P1-4）。返回空集 = 这条记录无法核验身份，不参与去重。"""
...
def _external_identity_index(project_root: Path) -> tuple[set[str], list[str]]:
    ...
    对齐（2026-08-15 G-02 同族的 fix-corpus 遗留）：跳过不炸，但坏文件名必须随结果带回、
...
def plan_search_online(
    ...
    *,
    ...
    P1-4 实体级去重：已在库中的候选（同来源同编号/同页面链接）过滤并回显 `skipped_existing`——
    ...
    # P1-4 实体级去重：已在库中的候选（同来源同编号、或同页面链接）诚实过滤并如实回显——
...
def apply_search_online(plan_result: dict, *, confirm_token: Any, project_root: Path) -> dict:
    """curate.search_online 第二步：token 比对 → 实体级重检（P1-4）→ 候选经 uploads.ingest_dataset 管线入库。
    ...
    # P1-4：零新候选（全部已在库中）是 plan 的合法产出——仅当 plan 带 skipped_existing 标记时放行，
    ...
    # P1-5：重检 → 落盘全程住进摄取临界区（跨进程 OS 文件锁）——重检与落盘之间
    # 不再有任何通路能插队（P1-4 残留的跨进程微竞态在此收口）。
...
# ---- 任务 3 补丁作用域分支（2026-08-26）：登录账户的删除/恢复改打本人补丁包 ----------------
#
# 语义映射：
...
# ==============================================================================================
# curate.check_updates：检查来源更新（2026-08-03 agent1 新能力）
# 设计蓝本：设计_执行侧Agent化_langgraph_2026-08-03.md §1.3/§2.3。**只读、不落盘、不抛**——
...
#: 在线通道两支（2026-08-03 P1-B5 扩）：arrayexpress 走 `_fill_online_check`（既有 BioStudies 支，
...
def _snapshot_local_info(path: Path, _err: list[str] | None = None) -> tuple[int, str | None]:
    ...
    G-27（2026-08-15 ta2）：损坏 ≠ 空库——解析失败经 _err 出参如实带出（调用方写进 note），
...
def _snapshot_corrupt_note(errs: list[str]) -> str:
    """D5（2026-08-15）：快照损坏 ≠ 空库——diff 是把损坏快照当空集比出来的，新增数字可能虚报，
...
def _ae_local_accessions(path: Path, _err: list[str] | None = None) -> set[str]:
    ...
    D5（2026-08-15）：损坏 ≠ 不存在——解析失败经 _err 出参如实带出（调用方写进 note），
    不再与「空快照」同形。"""
    if not path.is_file():
        return set()
    try:
...
#: AE 版本监控小钉（2026-08-07）：BioStudies arrayexpress/search 顶层键的已知快照。
...
def _fill_online_check(entry: dict, spec: dict, local_count: int, path: Path, root: Path) -> None:
    ...
    try:
        ...
        # G-14（2026-08-15）：与 net 支统一降级语义——在线比对没完成就如实降级 mode="snapshot"，
...
# ---- 2026-08-03 P1-B5：encode/10x 在线比对（走 corpus_net 工具组；失败如实降级 snapshot）----
...
def _fill_online_check_net(entry: dict, spec: dict, local_count: int, path: Path, root: Path) -> None:
    ...
    if kind == "encode":
        ...
        # G-13（2026-08-15）：未接线的 net_kind（拼错/新增源忘接线）不许静默按 10x 通道比对——
...
# ==============================================================================================
# curate.sync_updates（2026-08-06「工作流即工具」批）：检查更新 → 有新增则自动入库的复合流
#
...
#: 同步加全请求总预算 `_SYNC_TOTAL_MAX_IMPORT`（codex 裁决 R5 修正件——每源放宽的
#: 放大效应由总闸兜底，多源连跑也不失控）。
_SYNC_MAX_IMPORT = 10
...
def _external_dataset_uids(root: Path) -> tuple[set[str], list[str]]:
    ...
    坏文件口径（2026-08-15 G-02）：解析失败**跳过、不炸掉整个 sync**（与
...
def _uid_matches_accession(uid: str, probe: str) -> bool:
    ...
    G-03（2026-08-15）：此前用裸 `uid.endswith(probe)` 后缀匹配，互为首尾缀的编号会误判
...
def _sync_collect_records(
    ...
    seen: set[str] = set()
    skipped = 0
    identity_index, dedup_skipped = _external_identity_index(root)   # P1-4：实体级库态（一趟扫）
    if dedup_skipped:
    ...
    for cand in candidates[:max_import]:
        ...
        if not acc:
            continue
        probe = _cmp_key_cached(acc)   # P1-4：与实体身份同一归一化真源
        ...
        for record in picked:
            # P1-4：同实体（同来源编号/同页面链接）已在外部库 → 跳过（盖到编号后缀比对
            # 抓不到的「换皮重复」——编号不同、链接相同）。
...
def sync_updates(sources: Any = None, *, max_import: Any = _SYNC_MAX_IMPORT, project_root: Path) -> dict:
    ...
    外部库去重扫描遇坏 JSON：跳过不炸（2026-08-15 G-02），坏文件名在返回的
    ...
    # 2026-08-22 engagement B2：整任务跨进程锁（检查+联网+入库全程）。锁被占（另一进程/线程的
    ...
    with sync_updates_critical_section(root):
        ...
        for ent in checked.get("sources") or []:
            ...
            if key not in importable:
                ...
                entries.append(out)
                continue
            # 全请求总预算闸（2026-08-08 约束放松批 C）：跨来源累计写入了了 `_SYNC_TOTAL_MAX_IMPORT`
            ...
            # P0-3（2026-08-09 codex 对抗评审）：逐源局部容错——本源搜回/写入抛错只毁本源条目
            ...
            try:
                ...
                notes: list[str] = []
                if records:
                    # P1-5：搜回（联网逐编号查询）与落盘之间的窗口——进临界区后按届时库态终检一遍
        ...
        # 2026-08-22 engagement B2：operation receipt 落账（append-only JSONL）——recall 按
...
def write_boundary_zh(action: str, *, dry_run: bool) -> str:
    ...
    等结构化字段；但诚实语义一个字不能丢：可逆、没真删、绝不碰冻结基准（copy 评审 P1-1）。

    2026-08-22 engagement B2：补 check_updates（只读）与 sync_updates（写盘 + 可整次撤回）
...
# ==============================================================================================
# curate.sync_updates 加固（2026-08-22 engagement 落地包 B2）：整任务跨进程锁 + operation receipt
# + 按 operation_id 批量撤回 + 实例级同步状态
#
# 设计蓝本：docs/工作记录/设计_engagement落地包_2026-08-22.md §7（评审①阻断4 裁决）——
...
def recall_sync_operation(operation_id: Any, *, project_root: Path) -> dict:
    ...
    if _patch_scope():
        # 任务 3（2026-08-26）：绑定账户作用域时，sync 入库经写漏斗改落**该账户补丁包 adds**
