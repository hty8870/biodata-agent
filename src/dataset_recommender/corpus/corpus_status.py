# -*- coding: utf-8 -*-
"""数据库状态汇报（corpus_status，-B6）。

「汇报数据库的当前状态」类指令的**只读状态工具**——通用化 agent 的第一个非更新检查用例
（langgraph execute 节点的 LOOP_TOOLS 注册表首项， 起取代 READ_TOOLS/observe；
...
def _sources_status(root: Path) -> list[dict[str, Any]]:
    ...
    G-11：单源统计失败必须带 error 字段——失败与「真空快照」不同形，
    不许把「库坏了」呈现成「库里没东西」。"""
...
def db_status(*, project_root: Path | None = None) -> dict[str, Any]:
    ...
    if curations_error:
        # G-11：清单失败与「真空清单」不同形——带 error 字段如实区分。
