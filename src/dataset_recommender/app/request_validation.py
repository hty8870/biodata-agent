# -*- coding: utf-8 -*-
"""检索类请求入参的跨端校验单一真源（sm1 批，2026-08-23）。
...
``_require_iso_date``（2026-08-08 D12 严格档裁决），文案取两端的公共措辞——测试钉的
...
def validate_iso_date(value: "str | None", *, name: str) -> str:
    ...
    丢掉，结果和预期对不上却无任何提示；更糟的是 "2026-02-30" 这种不存在的日期曾被
    当作已生效条件回显上屏。诚实方向只有一个：给了就校验，不合法就明说。
    （2026-08-08 D12 严格档裁决；原 webapp 与 mcp 两份实现逐行同构，本函数是其唯一真源。）"""
...
def validate_date_window(date_from: str, date_to: str) -> None:
    ...
    「没数据」。feasibility 与 task-pack 曾缺这道闸（sm1 批补齐，与 /api/recommend
    和 MCP 同口径）。"""
...
def validate_sources(sources: "list[str] | tuple[str, ...] | str | None", *, known: "list[str] | set[str]") -> None:
    ...
    retrieval_params 是自由 dict，不先判型会把字符串逐字符枚举报出滑稽文案（R2-3 P2-4）；
