...
# 同一句话的长度上限与 action_plan/utterance 入口同源（2026-08-08 约束放松批由本地 300 并轨：
# 三个入口三个数是病灶——board 300 / action_plan 500 / webapp 500 的不一致本身就是 bug 源）。
...
COPY: dict[str, str] = {
    ...
    "zone_lenient": "已放宽",
    "zone_lenient_note": (
        # 与前端 board.js CB_ZONE_NOTE.lenient 同一句（2026-08-04 文案统一批）：空值纳入/
...
def parse_command(utterance: str, *, implicit_restrict: bool = False) -> dict:
    ...
    `implicit_restrict`（量词约束档，2026-08-02 cur3）只在**对话上下文**（屏上已有结果）
...
def classify_utterance(utterance: str) -> str:
    ...
    动机（2026-07-23）：「继续对话」此前只认「改条件」这一种说法，其余一律回
    ...
    路由三档（互斥，按优先级）：
      · ``identifier`` —— 话里点名了某个数据集编号/直链（`identifiers.classify` 是 `search`、
                          编号出现在句中任意位置都算）。复用与首个搜索框 N8 直链反查同一套判据。
    ...
    **编号必须排在执行词前面**（2026-07-25 修）：`identifiers.classify` 用的是 `search`，
...
def search_shaped(text: str, keyword_mapping: dict | None = None) -> bool:
    ...
    （2026-08-02 cur3 架构定稿：关键词匹配 → LLM 判断是唯一管线，此前还有一个
...
def _blank(
    ...
    return {
        ...
        # 说一个 GSE 编号也不再是听不懂——分别路由到任务包与整句重搜（后者天然带 N8 直链反查）。
...
def _payload_text(intent: "QueryIntent | None", dim: str, fallback: str) -> str:
    ...
    走 `active_filters` 生成的那句人话（需要 FASTQ / ≥ 2020-01-01），
...
def plan_edit(
    ...
    *,
    ...
    if op == "add" and positive_payload and by_dim.get(target_dim):
        ...
        # 那会出现「Human、Human 都算」这种不成话的按钮。照实说没有改到任何条件（cur3）。
