...
# 同一句话的长度上限与 action_plan/utterance 入口同源（约束放松批由本地 300 并轨：
# 三个入口三个数是问题——board 300 / action_plan 500 / webapp 500 的不一致本身就是 bug 源）。
...
def parse_command(utterance: str, *, implicit_restrict: bool = False) -> dict:
    ...
    `implicit_restrict`（量词约束档， cur3）只在**对话上下文**（屏上已有结果）
...
def search_shaped(text: str, keyword_mapping: dict | None = None) -> bool:
    ...
    （cur3 架构定稿：关键词匹配 → LLM 判断是唯一管线，此前还有一个
