# -*- coding: utf-8 -*-
"""卡片行投影与 /api/recommend 响应载荷的真源（2026-08-16 检索工具化 Phase 1）。
...
def rows_from_retrieved(payload: "list[dict[str, object]]") -> "list[dict[str, object]]":
    ...
    2026-08-16 自 webapp._rows_from_retrieved 迁入（先落 workflow、再随冻结闭包隔离
    ...
    for item in payload:
        row: dict[str, object] = {
                ...
                # N11 国内可达性启发（workflow 序列化已算好，透传给卡片；非实测速度）。
