...
供 Claude Code / Codex 这类支持 MCP 的 agent 直接调用（与网页端能力对齐）：
 ...
 19. curate_datasets(action, …, dry_run/confirm_token)  —— 【写工具 ③】对话式数据库管护
     ...
     **check_updates / sync_updates**（2026-08-22 engagement B2 补齐）：只读检查来源更新 / 检查+自动
...
from pydantic import StrictBool, StrictInt  # noqa: E402  # 严格类型：拒绝 "3"/True/"yes" 的宽松强转（codex 反馈 P2；D 路夜班增补 StrictBool）
...
# 全库浏览分页上限单一真源（ov1-mcp1 收口交接 kimi-sec-s3-webapp-遗留.md 第 4 项）：
...
# 检索入参校验单一真源（sm1 批）：query 四道闸/ISO 日期/倒挂窗口/来源校验与 Web 同一份，
...
# 1.4.0：入参加固（codex 二轮反馈）——top_k/rerank_top_n 上限 100（防响应放大）+ 严格整数（拒 "3"/True 强转）；
...
#        /api/curate/* 与 CLI scripts/curate_datasets.py；设计蓝本 设计_对话式数据库管护_2026-08-01.md，
...
# 1.32.0：日期口径拉齐（登记册 D12 清零，2026-08-08 产品所有者拍板）——recommend_datasets /
...
#        清扫；行为零变化）。
# 1.33.0（ov1-mcp1）：① browse_datasets 硬上限 200 → 100，与 Web /api/datasets 同源同一常量
...
# 1.34.0（eng-b2，2026-08-22；Wave 3 教程批统一收口 bump）：curate_datasets 补齐
...
# 设计（codex 试用反馈 #2/#3/#4）：**非法请求**（空 query / 枚举越界 / 非正 top_k / 未知来源）
...
def _clip(value: object, limit: int = 120) -> str:
    """错误消息回显入参的统一截断（审计 S-15，2026-08-21）：调用方贴多长的输入，
...
# 单细胞检索查询通常几十字；超长上限的真源自 sm1 批起在 app/request_validation.MAX_QUERY_CHARS，
# 此处保留别名供本文件历史引用。
_MAX_QUERY_CHARS = 2000
# top_k / rerank_top_n 硬上限：防响应放大（codex 反馈 P0：top_k=999999999 → 155 万字符响应，吃爆上下文/token）。
...
def _validate_positive(name: str, value: "int | None") -> None:
    ...
    # 上限：防响应放大（codex 反馈 P0）。超限即非法请求，而非静默返回全量。
    if value > _MAX_TOP_K:
        raise _bad_request(
            "bad_param",
...
def _require_iso_date(value: "str | None", *, name: str) -> str:
    ...
    2026-08-08 产品所有者拍板：MCP 与 Web 日期口径**拉齐为严格档**（登记册 D12 清零）——
    旧宽松口径「非年份打头一律忽略＝不限」把用户给了的条件悄悄丢掉；日历不存在的日期
    （如 2026-02-30）曾按字面值透传检索层。sm1 批起闸体下沉：真源在
...
def _validate_date_window(date_from: str, date_to: str) -> None:
    ...
    sm1 批起 Web 的 recommend/feasibility/task-pack 与本端同一份闸）。"""
    try:
...
# ov1-mcp1 起**也是**埋点中继的数据源（Web `/api/telemetry/mcp-calls` 按行号增量读取，
...
# schema 版本：v0 → v1（ov1-mcp1，**additive**：每行新增 `call_id`=uuid4().hex 随机唯一；
...
# redactor（2026-08-10 P0-2：secret 被误粘进 query/自由文本时按值兜住——src 公共安全模块
...
def _internal_error(exc: Exception) -> ToolError:
    ...
    对抗评审 p0-record P2-c 实测外泄），MCP 客户端不该看到。完整堆栈打到本进程 stderr
...
def _record_tool_call(tool: str, params: dict, start: float, *, ok: bool, error: "str | None") -> None:
    ...
    try:
        ...
        record = {
            ...
            "call_id": uuid.uuid4().hex,   # ov1-mcp1：随机唯一调用 ID（中继/去重用；additive）
    ...
    **全程确定性、离线、无需任何 API key**。
    ...
    参数：
      ...
      date_from / date_to: 发表时间范围（ISO YYYY-MM-DD，含端点；对齐网页年份选择器）。**显式传入优先**，
               ...
               严格校验（与网页端同口径，2026-08-08 产品所有者拍板拉齐、登记册 D12 清零）：给了就
    ...
    try:
        ...
        # 发表时间范围校验（2026-08-08 产品所有者拍板拉齐为 Web 严格档，登记册 D12 清零——
...
def get_file_manifest(dataset_uid: str) -> dict:
    ...
    try:
        ...
        # 复制粘贴常带首尾空白 → 自动 strip 再查（codex 反馈：合法 uid 带空格误报 bad_uid）。
...
# 调用方上下文。故 limit 默认 50、硬上限 100（ov1-mcp1 起与 Web /api/datasets 同源同一常量
...
# unresolved 原样回显、响应同步放大（R2-5 P2-3）。200 与 top_k 等既有上限同量级，足够宽松。
_MAX_UID_CHARS = 200
...
def build_task_pack(
    ...
    try:
        ...
        settings = _settings()
        # 发表时间校验（与 recommend_datasets 同一把尺；2026-08-08 产品所有者拍板拉齐 Web 严格档，
        # 登记册 D12 清零）：给了就必须是合法 YYYY-MM-DD，非法/倒挂 → bad_param 点名参数。
        # 病形链：垃圾日期直传检索层做字面比较（'zzz' 字典序大于一切 ISO 日期）会把全库静默归零，
        # 再谎称「没有命中」（R2-5 P1-2）；其后的宽松档「忽略＝不限」又把条件悄悄丢掉。两个都消灭。
        eff_date_from = _require_iso_date(date_from, name="date_from")
        eff_date_to = _require_iso_date(date_to, name="date_to")
        _validate_date_window(eff_date_from, eff_date_to)
        ...
        if not candidates:
            ...
            # 否则垃圾入参（如非法日期）会被包装成「没有命中」的正常业务结论（R2-5 P1-2）。
        ...
        if mode == "build":
            ...
            # （Web 侧同族病 R2-3 P2-2 T2，这里同型收口）。文档口径本就是 build 必须带全。
...
def plan_action(utterance: str, has_results: StrictBool = False, result_total: StrictInt = 0) -> dict:
    ...
    - `verb`：封闭词表里的一个（执行类 `pack.download` / `pack.preview` / `cite.export` /
      ...
      词表——它们只属于多步 agent 环；万一出现会进 `rejected` 并降 `none`（2026-08-17 nl2）。
...
def curate_datasets(
    ...
    try:
        ...
        # plan→execute 断链修复（2026-08-22 engagement B2，评审①#9）：`plan_action` 的封闭动词表
...
# path 入参的敏感位置闸（审计 S-1，2026-08-21）：upload 的读取面必须是调用方显式提供的**数据**
