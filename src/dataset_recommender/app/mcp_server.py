...
供 Claude Code / Codex 这类支持 MCP 的 agent 直接调用（与网页端能力对齐）：
 ...
 19. curate_datasets(action, …, dry_run/confirm_token)  —— 【写工具 ③】对话式数据库管护
     ...
     **check_updates / sync_updates**（engagement B2 补齐）：只读检查来源更新 / 检查+自动
...
from pydantic import StrictBool, StrictInt  # noqa: E402 # 严格类型：拒绝 "3"/True/"yes" 的宽松强转（验证反馈 ；D 路夜班增补 StrictBool）
...
# 1.4.0：入参加固（验证 二轮反馈）——top_k/rerank_top_n 上限 100（防响应放大）+ 严格整数（拒 "3"/True 强转）；
...
# 1.32.0：日期口径拉齐（登记册 D12 清零， 产品侧拍板）——recommend_datasets /
...
# 1.34.0（；教程批统一收口 bump）：curate_datasets 补齐
...
# 设计（验证 试用反馈 #2/#3/#4）：**非法请求**（空 query / 枚举越界 / 非正 top_k / 未知来源）
...
def _clip(value: object, limit: int = 120) -> str:
    """错误消息回显入参的统一截断（审计 S-15，）：调用方贴多长的输入，
...
def _require_iso_date(value: "str | None", *, name: str) -> str:
    """发表时间入参：空 → ""（不限）；否则必须是格式与日历都合法的 YYYY-MM-DD，非法 → bad_param。
     ...
     产品侧拍板：MCP 与 Web 日期口径**拉齐为严格档**（登记册 D12 清零）——
    旧宽松口径「非年份打头一律忽略＝不限」把用户给了的条件悄悄丢掉；日历不存在的日期
    （如）曾按字面值透传检索层。 批起闸体下沉：真源在
...
def _internal_error(exc: Exception) -> ToolError:
    ...
    验证-record -c 验证外泄），MCP 客户端不该看到。完整堆栈打到本进程 stderr
    ...
    **全程确定性、离线、无需任何 API key**。
    ...
    参数：
      ...
      date_from / date_to: 发表时间范围（ISO YYYY-MM-DD，含端点；对齐网页年份选择器）。**显式传入优先**，
               ...
               严格校验（与网页端同口径， 产品侧拍板拉齐、登记册 D12 清零）：给了就
    ...
    try:
        ...
        # 发表时间范围校验（产品侧拍板拉齐为 Web 严格档，登记册 D12 清零——
...
# 调用方上下文。故 limit 默认 50、硬上限 100（起与 Web /api/datasets 同源同一常量
...
# unresolved 原样回显、响应同步放大（R2-5）。200 与 top_k 等既有上限同量级，足够宽松。
_MAX_UID_CHARS = 200
...
def build_task_pack(
    ...
    try:
        ...
        settings = _settings()
        # 发表时间校验（与 recommend_datasets 同一把尺； 产品侧拍板拉齐 Web 严格档，
        # 登记册 D12 清零）：给了就必须是合法 YYYY-MM-DD，非法/倒挂 → bad_param 点名参数。
        # 病形链：垃圾日期直传检索层做字面比较（'zzz' 字典序大于一切 ISO 日期）会把全库静默归零，
        # 再谎称「没有命中」（R2-5）；其后的宽松档「忽略＝不限」又把条件悄悄丢掉。两个都消灭。
        eff_date_from = _require_iso_date(date_from, name="date_from")
        eff_date_to = _require_iso_date(date_to, name="date_to")
        _validate_date_window(eff_date_from, eff_date_to)
        # 审计 C-2：facet/suppressed/lenient 此前**原样透传** run_with_meta，而
        ...
        if not candidates:
            ...
            # 否则垃圾入参（如非法日期）会被包装成「没有命中」的正常业务结论（R2-5）。
        ...
        if mode == "build":
            ...
            # （Web 侧同族病 R2-3  T2，这里同型收口）。文档口径本就是 build 必须带全。
...
def verify_local_assets(directory: str, max_files: int = 20000) -> dict:
    ...
    **只算 md5、绝不读数据内容**（md5 本就是来源清单里的校验和）；巨文件（>8 GiB）跳 md5、只按名+大小
    ...
    try:
        ...
        if mf <= 0:
            mf = lab_ledger.MAX_SCAN_FILES
        # 审计 S-2：硬上限——此前仅挡 <=0，调用方传 10**9 可对任意目录逐文件算
...
def curate_datasets(
    ...
    try:
        ...
        # plan→execute 断链修复（engagement B2，验证#9）：`plan_action` 的封闭动词表
...
# path 入参的敏感位置闸（审计 S-1，）：upload 的读取面必须是调用方显式提供的**数据**
