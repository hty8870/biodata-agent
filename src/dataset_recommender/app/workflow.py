...
#: 喂给 curator LLM 的候选条数上限（2026-08-08 约束放松批，codex 实测：50 条候选序列化
...
class WorkflowResult:
    ...
    # 2026-07-25 起「或」不再整句弃权。引擎能表达的「或」只有同维度多值，所以三档必须如实播报：
    ...
    # 空池独立审核档（原 mode="empty"）2026-08-16 随「检索工具化」删除——空池救回改由
...
def intent_projection(intent: QueryIntent) -> dict:
    ...
    return {
        ...
        # 卡住这句话的未收录词（仅 unresolved_term 弃权时非空；2026-08-09 五机制批加进投影——
        ...
        # 2026-07-25 起「或」不再整句弃权，而是按引擎真实语义（同维度多值）执行——
...
_TRACE_REASON_LABELS = {
    ...
    # 真故障再分档（2026-08-04 C3）：401/403 是密钥无效/无权——重试永不自愈，指路去改设置；
    # 留在 llm_call_failed 里的超时/5xx/空回才是「稍后再试」的临时故障。
    "llm_auth_failed": "API 密钥无效或没有权限（401/403），请到「设置」里检查并更新密钥",
    # 历史值（2026-07-26 之前 rerank 把「没配」和「调用失败」合成一个 reason 写下的）。
...
def _fallback_note(step: dict) -> str:
    ...
    「未启用」是一个选择，「没能完成」是一次故障。2026-07-26 抓到前端把两者合成
...
def _raw_false_is_guess(record: DatasetRecord) -> bool:
    """G-08（2026-08-15 ta2）：record 的 has_raw_data=False 是否为抓取时的猜测占位——
...
def _raw_data_status(record: DatasetRecord) -> dict[str, object]:
    ...
    G-08（2026-08-15 ta2）：猜测性 False（ArrayExpress 抓取占位）归 unknown——「我们没查」不许编码成「它没有」。"""
    if record.has_raw_data is True:
        code = "has_fastq"
...
def _same_hard_filter(a: QueryIntent, b: QueryIntent) -> bool:
    ...
    # 否则「人和小鼠」vs「人类和小鼠」这类同存活集查询会被列表顺序绕过、误采纳空转改写（对抗评审 #1）。
...
def build_degraded_search(
    ...
    **`prepare(degraded_query)` 由调用方注入**，返回 `(relaxed_intent, candidates, total)`
    ...
    只要它自己**另跑一次**检索，就必然和用户点下去之后真跑的那次分叉。2026-07-22 夜的对抗评审
...
# 的真源在 `app/recommend_rows.py`（2026-08-16）：它们必须调 content.introduction，而
...
class DatasetRecommendationWorkflow:
    ...
    def run_with_meta(self, p: "RecommendParams | None" = None, **kwargs) -> WorkflowResult:
        # sm1 批（2026-08-23）：主入口收参数对象（见 RecommendParams docstring）。`**kwargs`
        ...
        # cr1（并发分流 r3 P1-5）：llm_available 显式为 False 时短路**不构造**——pre-loop
        ...
        # 空池/弃权档的独立审核（原 rerank.audit_query_only）2026-08-16 删除——空池救回改由
        ...
        if not candidates:
            ...
            #（手抄过一次就是 2026-07-22 夜对抗评审抓到的「条数虚高 4.3 倍、预览卡全在时间窗外」）。
            ...
            if degrade_with_llm and degraded_search and rerank_llm_config is not None:
                ...
                if drop_ok is True:
                    ...
                    # 才发生，用户更没有理由怀疑。2026-07-22 夜对抗评审两条独立发现都指向这里。
        ...
        # cr1（并发分流 r3 P1-5）：llm_available 显式为 False 时短路——pre-loop 确定性
        ...
        # 喂 LLM 的文本只取前 _LLM_PROMPT_CANDIDATE_CAP 条（codex 实测：50 条候选序列化
        ...
        if not llm_result.succeeded or not llm_result.text:
            ...
            # 故障再分档（2026-08-04 C3）：401/403=密钥无效/无权（重试不自愈，指路改设置），
            # 超时/5xx/空回=临时故障（稍后再试）。
            _update_trace_step(
