...
def _validate_or_400(fn, *args, **kwargs):
    ...
    + X-Error-Code 头（批：机器码此前只在 MCP 端保留，Web 端一律被剥掉，前端无法按码
...
# webobs（触发点审计 app-routes F2 / cross-trace D9）：webapp 此前整体无
...
def _redacted_validation_error(request: Request, exc: RequestValidationError) -> JSONResponse:
    ...
    剔除 pydantic 默认带回的 `input` / `ctx`（验证；覆盖全部端点）。"""
...
class UtteranceRequest(_ExperimentContract):
    ...
    # ---- 课题上下文卡（engagement B4，additive；设计 §3.3）----
    ...
    # ---- 下一步行动建议动作（engagement P6，additive；设计 §5.2）----
    ...
    # ---- 当前检索参数（prelim1，additive；缺省=现状行为）----
...
class SearchRescueRequest(BaseModel):
    """检索救回端点 `/api/agent/search-rescue` 的入参（检索工具化 Phase 1）。
...
class CurateSyncUpdatesRequest(BaseModel):
    """`POST /api/curate/sync-updates` 入参（「工作流即工具」批 `curate.sync_updates`）。
...
class CurateRecallRequest(BaseModel):
    """`POST /api/curate/recall` 入参（engagement B2）：要撤回的 sync operation_id。
...
class WatchCheckRequest(BaseModel):
    """`POST /api/watch/check` 入参（engagement F3）：课题保存的**确定性检索 spec**。
...
class ExportPackRequest(BaseModel):
    """`POST /api/artifacts/export-pack` 入参（engagement P5 导出中心）。
...
def _require_iso_date(value: str | None, *, name: str) -> str:
    ...
    条件回显上屏（backend-probe）。诚实方向只有一个：给了就校验，不合法就明说。
    （批起为薄委托：真源在 app/request_validation.validate_iso_date，与 MCP 同一份。）"""
...
def _normalize_provider(provider: str | None) -> str:
    ...
    if normalized == "mock":
        return "mock"
    if normalized == "trial":
        # T3 限量试用通道：端点/模型锁定、凭据只认服务端 BIODATA_TRIAL_API_KEY
        # （起缺省回落 BIODATA_EMBED_API_KEY——与 embedding 共用智谱 key）。
...
# 检索工具化 Phase 1：卡片行投影的真源已迁入 app.recommend_rows.rows_from_retrieved
...
def _raw_request_host(request: Request) -> tuple[str, int | None] | None:
    ...
    合法 loopback，基于 base_url 的守卫恒放行（backend-probe  真服实测 200）。
...
_AUTH_OPEN_PATHS = frozenset({
    ...
    "/api/account/logout",
    "/api/account/whoami",
    # corpus-sync 批：管理端点「开放但自认证」——token（BIODATA_ADMIN_TOKEN，
...
async def _account_gate(request: Request, call_next):
    ...
    if path.startswith("/api/"):
        # 任务 3（基线+补丁包）：会话解析提前到闸前、且不受护栏开关控制——只要
...
async def _security_headers(request: Request, call_next):
    ...
    安全头的档位（R2-3）。这里自己接住：完整堆栈留 stderr 供排查，客户端只见通用
    ...
    # 静态资源缓存头（夜班 E 路）：此前裸 StaticFiles 只发 etag/last-modified，
...
async def _request_logging(request: Request, call_next):
    """统一请求日志（webobs；触发点审计 F2/D9 "webapp 整体无 logging" 收口）。
...
def _quota_exempt_users() -> frozenset[str]:
    """豁免名单（逗号/空白分隔用户名，产品侧用）；比较一律小写（用户名本就规范化小写）。"""
    return frozenset(
        part.strip().lower()
...
# 混跑 = 新功能静默退回旧样式（p10 后集成踩到「过时的侧边栏样式」正是这一族）。
...
def api_health(request: Request) -> JSONResponse:
    ...
    body = {
            ...
            # additive（T3 账号护栏，）：前端登录门与注册邀请框的判定源——
            ...
            # additive（corpus-sync 批）：语料代变更哨兵——前端登录后拿它比对
...
def _corpus_gen_sentinel() -> "str | None":
    """语料代变更哨兵（corpus-sync 批）：**无绑定** corpus_cache_generation 元组
...
_SESSION_MAX_AGE = accounts.SESSION_TTL_DAYS * 24 * 3600   # 与服务端 _SESSION_TTL 同源（起 30 天）
...
def _set_session_cookie(resp: JSONResponse, token: str, *, remember: bool = True) -> None:
    ...
    # Secure 口（公网护栏硬化，）：`BIODATA_COOKIE_SECURE=1` 时置 Secure——独立于护栏
...
# 公网护栏硬化：护栏模式下登录/注册的 per-IP 进程内节流（复用 的
...
def api_account_trial_quota(request: Request) -> JSONResponse:
    """T3 限量试用通道的当日额度回显（设置界面「今日剩余」数据源， 夜）。
...
def api_dream(payload: DreamRequest, request: Request) -> JSONResponse:
    ...
    if payload.api_key:
        ...
        # 与 /api/introduction 同口径（验证实锤漏网）：ENV_LOCK 内加载
...
class CurateExamplesRequest(BaseModel):
    """成功经验库候选的勾选/忽略（起用户挑选入库）：候选 id 清单 + 端点坐标。
...
def api_interpret(payload: InterpretRequest, request: Request) -> JSONResponse:
    ...
    # 批：与 /api/recommend 同源的四道闸（原「空+2000」弱口径升级，见 request_validation）。
...
def api_recommend(payload: RecommendRequest, request: Request) -> JSONResponse:
    _require_same_origin(request)
    query = payload.query.strip()
    # 批：query 四道闸（空/控制字符/纯符号/超长）收敛到共享真源——Web 侧原只有「空+2000」
    ...
    # 夜班 A 路 ：倒挂窗口（from > to）此前被静默接受、恒零结果还冒充合法生效条件
    # 上屏——用户在替一个不可能的条件读「没数据」。起晚于止必须当场 400 点名。
    # （批：闸体下沉共享真源，与 feasibility/task-pack/MCP 同一份。）
    _validate_or_400(validate_date_window, date_from, date_to)
    # 夜班 A 路 （其二）：来源校验对齐 MCP `_validate_sources`——未知/空白来源名
    # 此前被静默过滤成恒零结果，用户无法区分「来源写错」与「来源里真没数据」。显式 400 并列出合法名。
    # （批：闸体下沉共享真源 app/request_validation.validate_sources。）
    if payload.sources:
    ...
    # 触发点审计 F4：use_llm=true 但 provider 缺省未传 → 被静默拽回 mock，
    ...
    try:
        ...
        #（触发点审计 F5）——留一行 stderr（含异常类型），失败与「没有 OOV」可区分。
    ...
    return _json_utf8(
        ...
        # 起「或」不再整句弃权；引擎只能表达同维度的「或」，落到哪一档必须如实说。
...
def api_feasibility(payload: RecommendRequest, request: Request) -> JSONResponse:
    ...
    # 批补漏（漂移修复）：倒挂窗口与来源校验——此前只有 /api/recommend 与 MCP 的
...
def _task_pack_retrieval(params: dict) -> dict:
    ...
    # 批补漏（漂移修复）：倒挂窗口在检索层单点拦截（预览与产包共用本函数）——MCP
...
def api_task_pack_preview(payload: TaskPackPreviewRequest, request: Request) -> JSONResponse:
    ...
    **刻意不是一句话直接产包。** 用户没勾过任何东西就打包，等于替他决定了「哪 10 条算数」；
    ...
    # 缺这道闸时垃圾日期会被拿去字面比较，再谎称「没有命中」（R2-3）。
...
def api_task_pack_build(payload: TaskPackBuildRequest, request: Request) -> Response:
    ...
    **用回传的同一套检索参数重跑一遍，再算一次指纹。** 对不上就明确说哪里变了、
    ...
    # （latin-1 编不了），英文引号曾注入第二个响应头参数（R2-3  T3/T4）。先按 ISO 校验，
    ...
    try:
        ...
        # （R2-3  T2）。先 preview 拿真指纹，是产包的唯一入口。
    ...
    try:
        ...
        if (payload.format or "zip").lower() == "json":
            # K6（验证决定）：json 通道此前完全绕过 MAX_ZIP_BYTES——zip 要过
...
def _pack_mismatch(payload: "TaskPackBuildRequest", rebuilt: dict, run: dict) -> "dict | None":
    ...
    不再有「if 给了才比」的短路（那曾把「没核对」当成「核对通过」，R2-3  T2）。"""
...
def _utterance_request_fp(text: str, payload: Any, provider: str,
    ...
    """占用条目的请求指纹（验证）：同 req_id 但**内容不同**的请求不得
    共享幂等槽（此前只看 req_id——两个不同 utterance/config 复用同号会错等/错收响应；
    撞指纹 → 409 如实说明，不当成同一次重发）。

     验证 补全：指纹材料加入**现场字段**（has_results/result_total/query/
...
def _prune_utterance_idem(now: float) -> None:
    ...
    TTL 分态（验证）：done 1h 淘汰（缓存体到期）；running 只在 24h 泄漏兜底线上
    淘汰（owner 崩掉占坑）——正常长链执行/锁排队远低于 24h，绝不按 1h 误杀在途 owner。
    上限淘汰只动 done 条目（验证决定 K9）：FIFO 淘汰 running owner =
...
def _utterance_idem_claim(req_id: str, fp: str = "") -> tuple[dict, bool]:
    ...
    同号但请求指纹不同（fp 非空且不匹配）→ 409（验证：那不是重发，是撞号）。"""
    now = time.monotonic()
    with _UTT_IDEM_LOCK:
...
def _utterance_response_body(result: dict[str, Any]) -> dict[str, Any]:
    ...
    **共用这一个真源**——SSE 契约要求 final 体与非流式逐位同形，抄两份必漂移。"""
    ...
    body = {
        ...
        # 婉拒候选 chips（五机制批，additive）：仅 LLM 真判 none 时非空，
        ...
        # prelim1（additive）：result_payload=环内 search.rerun 采纳的
    ...
    # （并发分流 r3 / 设计 §4.1，breaking）：tool 路线 retrieval 恒 None +
    ...
    # M3（多批检索结果，additive）：route_turn 组了批次才透传——flag OFF 或
    ...
    # M4 波2（可追溯性，additive）：trace 开启时 route_turn 回了 trace_turn_id
    ...
    # （设计 §5.2，additive）：suggested_recipe 处理结论如实记录（非法 → 忽略说明 /
...
def _utterance_event_stream(
    ...
    *,
    ...
    """`/api/utterance` 流式分支的 SSE 生成器（设计 §5.2）。
    ...
    **tool_start(共识) → step(共识) → tool_start(understand) → preliminary? → … → final**
    ...
    路线永不发射（路由内部出错则只发 error）。prelim1 起 on_event 的 kind
    ...
    def run() -> None:
        ...
        try:
            result = turn.route_turn(
                ...
                # （设计 §3.3）：课题上下文独立透传，只进 agent prompt。
                artifact_context=str(payload.artifact_context or ""),
                # （设计 §5.2）：建议动作 hint（allowlist 校验在 action_plan 单一真源）。
            ...
            # webobs（F2/D9）：此前 exc 接住即弃、零日志零堆栈，触发即无法定位。
...
def api_utterance(payload: UtteranceRequest, request: Request) -> Response:
    ...
    **规则匹配（一切指令都过；零命中/弃权 ≠ 无效）→ LLM 分流 →
    ...
    tool_start* → step* → final（prelim1 ，设计 §2.3），
    ...
    try:
        result = turn.route_turn(
            ...
            # （设计 §3.3）：课题上下文独立透传，只进 agent prompt。
            artifact_context=str(payload.artifact_context or ""),
            # （设计 §5.2）：建议动作 hint 独立透传——allowlist 校验在 action_plan
        ...
        # webobs（F2/D9）：与流式 worker 同口径——完整堆栈进日志，客户端只见通用文案。
...
def api_agent_search_rescue(payload: SearchRescueRequest, request: Request) -> JSONResponse:
    """检索救回（检索工具化 Phase 1）：零命中查询的「换词重检」入口。
    ...
    当前结果保持不变。 用户拍板：**改空（新条件 0 命中）也采纳**——空结果集
    ...
    **绝不 500**：agent 缺席 / LLM 未配 / 图内任何异常都回 200 fail-open（reason 如实），
    ...
    # M4 波2（可追溯性，设计 §6-9）：rescue 不经 route_turn——端点侧自建
...
def api_act_summary(payload: ActSummaryRequest, request: Request) -> JSONResponse:
    ...
    `brief:true`（设计 §5.3）走一句话模式（≤35 字），响应形状不变。
    """
    _require_same_origin(request)
    provider = _normalize_provider(payload.provider)
    requested_base_url = "" if provider in ("mock", "trial") else _validate_endpoint_url(payload.base_url)
    ...
    if payload.brief:
        # 一句话模式（设计 §5.3）：与长总结同一通道同一 fail-open
...
# 超出的只可能是误操作或本机 DoS 试探（/api/upload 此前无闸，68.7MB 整读内存照收，R2-3）。
...
def api_curate_sync_updates(payload: CurateSyncUpdatesRequest, request: Request) -> JSONResponse:
    """检查更新 → 有新增则自动入库的复合流（「工作流即工具」批 `curate.sync_updates`）。
    ...
    写侧经 uploads 管线 + 账本，可经回收站撤回；网络失败不 5xx——该来源的 note_zh 如实写明。
     ...
     engagement B2：整任务跨进程锁——另一进程/线程的 sync 正在跑 → CurateError
    ...
    （operation_id / created_files[] / failed_sources[] / skipped_existing，既有字段兼容）。
     ...
     corpus-sync 批：**仅 guard on（网页形态）**改为异步——启动/附着进程内单飞
...
def api_curate_sync_updates_status(request: Request) -> JSONResponse:
    """语料同步 job 状态轮询（corpus-sync 批；登录即可——中间件闸，无 token 闸）。
...
def api_curate_sync_status(request: Request) -> JSONResponse:
    """实例级同步状态（engagement B2 §7）：上次同步时间 / 上次 operation / 是否 busy。

    **只读、不写盘**（busy 实时探测 sync 整任务锁，跨进程准确；「上次同步」是**实例级事实**，
    不得存 per-profile localStorage——验证#6 裁决）。"""
    _require_same_origin(request)
    from ..corpus import corpus_curation as cc
...
def api_curate_recall(payload: CurateRecallRequest, request: Request) -> JSONResponse:
    """按 operation_id **整次撤回**一次 sync 的全部成功写入（engagement B2 §7）。
...
# 设计：engagement 落地包 §4.1/§4.2（验证阻断3 裁决）。
...
def api_watch_check(payload: WatchCheckRequest, request: Request) -> JSONResponse:
    """课题更新检查的**确定性重跑**端点（engagement F3 §4.1/§4.2，验证阻断3）。
...
def api_artifacts_export_pack(payload: ExportPackRequest, request: Request) -> Response:
    """课题导出中心（engagement P5，设计 §6「研究包」的落地形态）。
...
def api_diagnose(payload: DiagnoseRequest, request: Request) -> JSONResponse:
    ...
    #（触发点审计 F1：一次死端点诊断曾占锁数个超时长，全进程 LLM 端点静默排队）。
...
def api_introduction(
    ...
    **双层门**：需服务端 `ENABLE_LLM` 开 **且** 本请求显式 `llm=1` 才会真调；否则 `llm_summary=None`、
    ...
    if llm:
        ...
        # 介绍端点本身不接收请求级 key，故只用服务端配置（验证：唯一没被 ENV_LOCK 罩住的 LLM 读取点）。
        with ENV_LOCK:
...
async def api_reuse_pack(request: Request) -> JSONResponse:
    ...
    **为什么是 POST 而不是 GET**：GET 会把用户勾选的每个 dataset_uid 打进 uvicorn 的
    access log —— 那是**事实上的埋点**，而「要不要做使用数据采集」是产品侧明确保留的
