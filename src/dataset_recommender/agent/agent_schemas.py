# -*- coding: utf-8 -*-
"""执行侧 agent 的 pydantic 契约层（schema 加固批：硬约束收到 schema 层）。
...
def _registry_source_labels() -> tuple[tuple[str, ...], tuple[str, ...]]:
    ...
    注册表（验证决定 N1：旧实现所有 verb 共用检索词表枚举，ENCODE/EBI SCEA
...
def source_names_for(verb: str) -> tuple[str, ...]:
    """source 槽按 verb 取真枚举（验证决定 N1）：联网搜给能搜的、检查/同步给能查的，
...
#: source 槽描述按 verb 分派（验证决定 N1 的提示层半）：候选清单不抄进文字（名单真源是
#: 本字段的 enum，两处同出 `source_names_for`，文字再抄一份必漂移）；「接不了如实说」的
#: 口径按动作写清。多点名只填第一个的纪律不变（集成验证 b05/d08 问题的既有锚）。
...
#: B1a 调研六候选批：补具体值示例与出处锚（k09 多来源只检一个、j03/l07 条件成立后
...
_SLOT_DESCRIPTIONS_ZH: dict[str, str] = {
    ...
    # 环内四工具批：compare.datasets 的 a/b 与 compat.find / fair.check 的 uid
    ...
    # 批（设计 §4）：cite.export 的 uids 数组槽——按编号清单导出（真实消费）；
...
#: query 槽的**按动词专职描述**（M2）：「改写后的检索句…」那句只贴合
...
def _args_model_for(spec: "_ap.VerbSpec") -> type[BaseModel]:
    ...
    for slot in spec.slots:
        if slot == "limit":
            ...
            # 验证决定 N1：枚举与描述都按 verb 分派（联网搜给能搜的、检查/同步给能查的），
...
class SearchOnlineResult(BaseModel):
    ...
    filename 例外可为 None（H1）：候选全部已在库中的零写入是
...
class SearchRerunResult(BaseModel):
    ...
    构造）。
     用户拍板：**命中 0 条也采纳**（空结果集照常上屏，是条件变更重检的诚实
...
class RankResult(BaseModel):
    """`rank`（`agent_exec._loop_rank`）的出口契约（M2）：六键恒在——
    ...
    top 条目 批扩字段：dataset_uid + rank（1 起序号）——同批依赖占位的
...
class RerankResult(BaseModel):
    """`rerank`（`agent_exec._loop_rerank`）的出口契约（M2）：在 RankResult
    口径上把 query 拆成三键——original_query（原始坏 query）/ rewritten_query（实际生效
    的检索句；改写未通过机械健全性检查时 = 原句）/ rewritten（改写是否被采纳，如实标注）。
    batch 的 label 取生效的 rewritten_query。top 条目同 RankResult 的 批扩字段
...
class RouteRequestResult(BaseModel):
    """`route.request`（`agent_exec._loop_route_request`）的出口契约（M1
...
class CompareResult(BaseModel):
    """`compare.datasets`（`agent_exec._loop_compare_datasets`）的出口契约（四工具批）：a / b（条目 digest：dataset_uid + dataset_name + source）/ assumption_zh
...
class CiteExportResult(BaseModel):
    """`cite.export`（`agent_exec._loop_cite_export`）的出口契约（四工具批）：
...
class CompatFindResult(BaseModel):
    """`compat.find`（`agent_exec._loop_compat_find`）的出口契约（四工具批）：
...
class FairCheckResult(BaseModel):
    """`fair.check`（`agent_exec._loop_fair_check`）的出口契约（四工具批）：
...
LOOP_RESULT_MODELS: dict[str, type[BaseModel]] = {
    ...
    # M2 RAG 工具组与 M1 逃生口（转正常驻，原开关登记分支摘除）。
    "rank": RankResult,
    "rerank": RerankResult,
    "route.request": RouteRequestResult,
    # 回滚动词化。
    "curate.rollback": RollbackResult,
    # 环内四工具批：compare.datasets / cite.export / compat.find / fair.check。
...
class Step(BaseModel):
    """execute 节点 step 实录的**字段齐整**契约（schema 加固第一刀）。
