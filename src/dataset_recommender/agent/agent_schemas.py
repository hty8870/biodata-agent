# -*- coding: utf-8 -*-
"""执行侧 agent 的 pydantic 契约层（2026-08-06 schema 加固批：硬约束收到 schema 层）。
...
def _registry_source_labels() -> tuple[tuple[str, ...], tuple[str, ...]]:
    ...
    注册表（2026-08-08 codex 裁决 N1：旧实现所有 verb 共用检索词表枚举，ENCODE/EBI SCEA
...
def source_names_for(verb: str) -> tuple[str, ...]:
    """source 槽按 verb 取真枚举（codex 裁决 N1）：联网搜给能搜的、检查/同步给能查的，
...
#: source 槽描述按 verb 分派（codex 裁决 N1 的提示层半）：候选清单不抄进文字（名单真源是
#: 本字段的 enum，两处同出 `source_names_for`，文字再抄一份必漂移）；「接不了如实说」的
#: 口径按动作写清。多点名只填第一个的纪律不变（真机探针 b05/d08 病灶的既有锚）。
...
#: 槽位描述必须**专职**（2026-08-03 agent1b，病灶：「检查10x更新」被填成 source=ArrayExpress）。
...
#: 搜索步被放弃两处病灶的描述层锚；与 decide 铁律 rule 4/9 互文——提示词规则负责解释、
#: 工具描述当 checklist）。
...
_SLOT_DESCRIPTIONS_ZH: dict[str, str] = {
    ...
    # 通用兜底句（2026-08-17 nl1 缝合）：声明 reason 槽的动词都应登记下方专职描述；
    ...
    # 2026-08-20 pbt1 批（设计 §4）：cite.export 的 uids 数组槽——按编号清单导出（真实消费）；
...
#: reason 槽的**按动词专职描述**（2026-08-17 nl1 缝合，与 `_QUERY_SLOT_DESCRIPTIONS_ZH`
...
#: query 槽的**按动词专职描述**（2026-08-17 M2）：「改写后的检索句…」那句只贴合
...
def _args_model_for(spec: "_ap.VerbSpec") -> type[BaseModel]:
    ...
    if spec.verb in _ap.ROUTE_QUERY_VERBS:
        # nl2 低4 留痕（2026-08-17）：ROUTE_QUERY 三动词在 agent 环内的投影已退役——
    ...
    for slot in spec.slots:
        if slot == "limit":
            ...
            # codex 裁决 N1：枚举与描述都按 verb 分派（联网搜给能搜的、检查/同步给能查的），
            ...
            if slot == "reason":
                # reason 槽按动词取专职描述（rerank/route.request；2026-08-17 nl1 缝合）。
                description = _REASON_SLOT_DESCRIPTIONS_ZH.get(spec.verb, description)
            fields[slot] = (str, Field(
                # 专职描述优先（见 _SLOT_DESCRIPTIONS_ZH 的病灶说明）；词表将来新增
...
class SearchOnlineResult(BaseModel):
    ...
    filename 例外可为 None（2026-08-15 fix-exec H1）：候选全部已在库中的零写入是
...
class SearchRerunResult(BaseModel):
    ...
    构造）。
    2026-08-23 用户拍板：**命中 0 条也采纳**（空结果集照常上屏，是条件变更重检的诚实
    答案）——原「改空拒」档 rewrite_empty_kept_original 退役，拒绝只剩同集/条件丢失两档。
    rescue2（2026-08-17）additive 两键：dropped_terms（采纳时改写句里消失的未收录词，
    机械子串比对；未采纳恒 []）、disclosure_zh（采纳档确定性披露句，未采纳为 None）。
    nl-A（2026-08-17）additive 两键：n_before_total/n_after_total（**屏口径**未截断命中
...
class RankResult(BaseModel):
    """`rank`（`agent_exec._loop_rank`）的出口契约（2026-08-17 M2）：六键恒在——
    ...
    displayed（是否上屏）/ batch（display=true 时的 M3 批次原料 dict——kind/label/
    query_raw/query_effective/payload，batch_id/seq/created_at 由轮级装配补齐；否则 None）。
    top 条目 2026-08-20 pbt1 批扩字段：dataset_uid + rank（1 起序号）——同批依赖占位的
...
class RerankResult(BaseModel):
    """`rerank`（`agent_exec._loop_rerank`）的出口契约（2026-08-17 M2）：在 RankResult
    口径上把 query 拆成三键——original_query（原始坏 query）/ rewritten_query（实际生效
    的检索句；改写未通过机械健全性检查时 = 原句）/ rewritten（改写是否被采纳，如实标注）。
    batch 的 label 取生效的 rewritten_query。top 条目同 RankResult 的 pbt1 批扩字段
...
class CiteExportResult(BaseModel):
    """`cite.export`（`agent_exec._loop_cite_export`）的出口契约（2026-08-18 四工具批）：
...
class CompatFindResult(BaseModel):
    """`compat.find`（`agent_exec._loop_compat_find`）的出口契约（2026-08-18 四工具批）：
...
class FairCheckResult(BaseModel):
    """`fair.check`（`agent_exec._loop_fair_check`）的出口契约（2026-08-18 四工具批）：
...
#: 同步漂移在此被闸住（tests.md P2-2 的补位：替身副本与真表从此共享同一份形状闸）。
LOOP_RESULT_MODELS: dict[str, type[BaseModel]] = {
    ...
    # 2026-08-17 M2 RAG 工具组与 M1 逃生口（nl1 转正常驻，原开关登记分支摘除）。
    "rank": RankResult,
    "rerank": RerankResult,
...
class Step(BaseModel):
    """execute 节点 step 实录的**字段齐整**契约（2026-08-06 schema 加固第一刀）。
