...
#: `设计_对话式数据库管护_2026-08-01.md` §0 决策 2。历史教训保留：不可逆的删除类仍不得进表，
#: 管护对象的删除只准走回收站真源（corpus_curation），且本层依旧只出 plan、不执行。
#: 2026-08-17 过夜批（M1 scoped 路由 / M2 RAG 工具组 / M3 多批结果；蓝本
#: `设计_scoped路由_rag工具_多批检索结果_2026-08-17.md`）已**转正**（2026-08-17 nl1）：
...
VERB_SPECS: tuple[VerbSpec, ...] = (
    ...
    # curate.check_updates（2026-08-03 agent1）：「检查更新」语义从 search_online 剥出、各自专职——
    # 此前动词表把它并进 search_online 的 when_zh，「检查10x是否有更新」被判成联网搜数据集，
    # 前端只能靠正则剥词打补丁（病灶见 设计_执行侧Agent化_langgraph_2026-08-03.md §0.1）。
    # 对应能力是只读的 corpus_curation.check_updates（在线比对限有适配器的源，离线快照源如实报告）。
    # 2026-08-08 探针 v4 病灶 j05：主题问句（「那个肺癌的网上有新的吗」）误选它——
    ...
    # curate.sync_updates（2026-08-06「工作流即工具」批）：「检查更新，若有则下载/入库」这类
    ...
    # 2026-08-08 探针 v4 病灶 j03：劣质指令（无标点五件事）里「有新增的人类肺数据就搜来入库」
    ...
    # curate.db_status（2026-08-03 agent2 P1-B6）：通用化 agent 的第一个非更新检查用例——
    ...
    # 原结果；2026-08-23 用户拍板：命中 0 条也采纳上屏——空结果集就是新条件的诚实答案，
    # 绝不「结果不如当前就否决、保持不变」）。详见 设计_检索工具化_search_rerun_2026-08-16.md。
    VerbSpec(
...
# route.request（2026-08-17 M1 逃生口；2026-08-17 nl1 转正常驻）：decide 环内元动词——
...
# 环内「结果处理」四工具（2026-08-18 四工具批；蓝本 rank/rerank 的登记方式）：
...
#: 前四动词 2026-08-01 cur1 接线，`curate.restore` 同日随统一对话窗口（uni1）接线毕业，
#: `curate.check_updates`（2026-08-03 agent1 新增）随同批前端改动接线毕业
...
#: search.rerun（2026-08-16）：独立执行入口 = langgraph 图内 LOOP_TOOLS 工具 +
...
#: 环内专属动词，前端无任何独立触发路径。
#: rank / rerank（2026-08-17 M2；nl1 转正常驻）：同为环内专属检索工具——上屏
...
#: compare.datasets / compat.find / fair.check（2026-08-18 四工具批）：环内专属「结果
...
#: plan_action 保底通道的排除表（2026-08-17 nl2 codex 对抗评审 高1）：闭环动词表里的
...
#: ============ 下一步行动 suggested_recipe allowlist（2026-08-22 engagement P6，设计 §5.2）============
#:
...
#: 不出现在铁律里（2026-08-17 nl2 codex 对抗评审 高2）。
_CONSTRAINT_RULE_BODIES_ZH: tuple[str, ...] = (
    ...
    "none 和执行类一律不填。",
    # 2026-08-07 探针 v8 遗留 k08：多事项句的首步排序——模型偶发跳过原话第一件事直接做后面的。
    "一句话里说了**好几件事**（「搜X数据入库，然后检查Y更新，再告诉我库里多少条」）时，"
    "先做原话里**最前面**的那件事；后面的事会有后续步骤接着做，不用一次做完。",
    # 2026-08-07 第三批任务6：a18/a21  repeat-10 实测坐实「缺现场」边界双向抖动——
...
#: 收窄面变体（nl2 高2）：面里没有 ROUTE_QUERY 动词时，铁律 5/7/10 换不引用退役动词的
...
def _constraints_zh(verbs: Any = None) -> str:
    ...
    （2026-08-17 nl2 高2：scoped 的 JSON 兜底壳曾原样带全表铁律，铁律 5/7/8/10 指着
...
def _verb_table_zh(verbs: Any = None) -> str:
    ...
    `verbs`（2026-08-17 M1 scoped 路由）：显式给 VerbSpec 子集时按子集出表
...
def build_action_prompt(utterance: str, *, has_results: bool, result_total: int,
    ...
    **逐位一致**（离线钉与既有断言零漂移）。
    ...
    `verbs`（2026-08-17 M1 scoped 路由）：显式给 VerbSpec 子集时「可选动作」段按子集出表
    （understand 的套件收窄面）；缺省 None = 全表，与历史输出**逐位一致**。
    2026-08-17 nl2 高2 起铁律段与 JSON 模板的 effective_query 行也随 `verbs` 按面生成
    ...
    if current_query:
        ...
        # 2026-08-07 第三批任务6：a18/a21 的 none 边界抖动在「规则+示例」后仍 ~7-8/10——
    ...
    # nl2 高2：effective_query 的模板行同按面生成——面里没有 ROUTE_QUERY 动词时
...
#: **动作词不是否定语素**（2026-08-03 agent1 模拟剧本当场抓到的真 bug）：`NEGATION_GUARDS_CN`
...
#: 疑问/陈述用法的「没」**不是**否定语素（2026-08-09 病灶猎杀批实锤：j03/b12b/k02a/k09a
#: 族马拉松句——「检查下ArrayExpress更新没，有新增就搜来入库」里的疑问「没」落在续步
#: quoted 的紧邻左窗（≤4 字），被极性门误判 cancelled=true 把多步链当场掐死；cr4final
...
#: 顺承/条件句的「没」（2026-08-15 触发点审计 C-4）：「没找到就联网搜」「没有就导出来」里
...
def _limit_was_said(utterance: str, value: int) -> bool:
    ...
    判据是**数字边界**而不是裸子串（2026-08-15 审计 C-5）：裸子串会把「2025」里的「25」、
...
def raw_shape_violations(raw: dict[str, Any], utterance: str) -> list[str]:
    """公共 raw 形状校验（2026-08-10 codex 架构评审裁决落地：此前 `agent_exec._validate_raw`
    ...
    产品拍板，见 agent_exec._validate_raw 文档串）。"""
    if not raw:
        ...
        if not plan.get("quoted"):
            return _finalize(
                _blank_plan(
                    ...
                    # 2026-08-15 审计 C-7：这是「读懂了但缺原文依据」的**降级 none**，不是真 none——
...
def rule_fallback_plan(
    ...
    （`_action_verb_noun_usage`，2026-08-15 审计 ta2 收敛，与 C-1 同根）：「下载量大的
...
#: 动作词的**名词用法**语境尾随字（2026-08-15 触发点审计 C-1）：ACTION_VERBS 是裸子串，
...
def _action_verb_noun_usage(low: str, marker: str) -> bool:
    """公共匹配助手（2026-08-15 审计 ta2 收敛）：小写动作词 `marker` 在 `low`（小写原话）里
...
def rule_operation_marker(utterance: str) -> str:
    ...
    操作指令，不算命中（2026-08-15 审计 C-1；判定本体在 `_action_verb_noun_usage`）。
    用在 turn.route_turn 的两处：agent_off 分支检出的句子回「这是操作指令，需开启 AI 执行」
    降级气泡（2026-08-03 agent2 降级语义）；LLM 缺席兜底用它拦住管护句，不许 fail-open 成检索
    （审计 C-2）。
    """
...
def rule_curate_op_marker(utterance: str) -> str:
    ...
    用在 turn.route_turn 的编号快速道闸（2026-08-15 审计 C-3）：「编号 + 管护操作」
...
def build_plan_from_raw(
    ...
    `allowed_verbs`（2026-08-17 nl2 高1）：调用方给的允许动词表（plan_action 保底通道
    ...
    for slot in spec.slots:
        ...
        if slot == "display":
            # display 布尔槽（2026-08-17 M2 rank/rerank）：认 JSON 布尔 true 与字符串
            ...
            continue
        if slot == "uids":
            # uids 数组槽（2026-08-20 pbt1 批，设计 §4）：cite.export 的编号清单——保留
    ...
    if spec.kind == EXEC and spec.requires_results:
        ...
        if raw_scope and raw_scope != "primary":
            # 2026-08-09 codex P0-6：旧文案「还不支持取全部文件」是假话（Web/MCP/provision
...
def plan_action(
    ...
    *,
    ...
    `allowed_verbs`（2026-08-22 engagement P6，设计 §5.2）：suggested_recipe 收窄的动词
    ...
    prompt = build_action_prompt(
        ...
        verbs=narrow_specs,   # nl2 高1：保底通道不出环内专属动词（提示层）；P6：suggested_recipe 再收窄
    )
    ...
    return build_plan_from_raw(
        ...
        allowed_verbs=narrow_allowed,   # nl2 高1：提示不是围栏，表外动词机械拒（闸层）；P6：suggested_recipe 再收窄
    )
