...
设计蓝本：`设计_执行侧Agent化_langgraph_2026-08-03.md` + `设计_长程多步执行_2026-08-04.md`
+ `设计_langgraph换装_2026-08-07.md`（本版 = 换装后形态）。图拓扑与全部机械护栏自 2026-08-04
...
4. **decide 迁 tool-calling 主通道**：绑 5 个 loop 工具 + `finish` + `unsupported_next_step`
   ...
   逐位一致）、幻觉工具名/散文=非法=停环；**多 tool_call 取第一个**（2026-08-09 病灶
   猎杀批：DeepSeek 不遵守 parallel_tool_calls=False，多调用是常态且第一个调用实证
   17/17 合法，循环会再判断后续，不吞事）——**2026-08-14 mcl1 起追加同批只读消费**：
...
- **decide**（仅 execute 真跑过工具后进入）：LLM 看「原话 + 已完成步骤紧凑投影」决定
  finish/续步/婉拒。机械校验双保险 + MAX_STEPS=8 硬上界（2026-08-15 gr2 由 6 放宽，
  ...
  **一次**重问（共享同一份预算，每次 decide 调用至多一次）：主通道非法应答（散文拒答/
  ...
  带检查意见重问一次；**机械闸拒收同样回灌一次**（2026-08-09 codex 对抗评审 P0-1：
...
1. **图内执行仅限 `LOOP_TOOLS` 注册表**：db_status（只读）、check_updates（只读在线比对）、
   search_online（联网搜 + 入库——写操作，依据是产品所有者 2026-08-03 的全自动化预先授权：
...
# M4 波2（2026-08-17 可追溯性，设计 §6）：trace 挂钩——recorder 经 contextvars 传递
...
class _AgentState(TypedDict, total=False):
    """langgraph 图状态。六个累积型 key 带 reducer（2026-08-07 换装；2026-08-07 B 方案 +1；
    2026-08-08 任务3 +usage_ledger）：
    ...
    raw_batch: list[dict]     #: 多调用的同批只读续步 raw（2026-08-14 mcl1 起 decide 产出；2026-08-22 ov1-wf1 起 understand 首步同口径产出 → validate 消费后即清）
    ...
    truncated_settled: bool   #: 到顶结算闸（2026-08-15 gr2）：到顶时可机械核验事项已全部结清——narrate 改缀「预算用完但事已做完」，不谎报
    ...
    usage_ledger: Annotated[list[dict], operator.add]  #: LLM 缓存用量台账（任务3）：{node, input, cache_read, output}
    checklist: list[dict]         #: 任务4 清单（覆写语义；complex+EXEC 首步由 understand 产出，immutable）
    ...
    entry_mode: str               #: 入口模式（2026-08-16）：""=常规对话链 / "rescue"=检索救回（工具面收敛到 search.rerun/none）
    ...
    search_date_to: str
    route_scope: str              #: scoped 路由（2026-08-17 M1）：route_consensus 的共识结果 ""/"search"/"action"/"general"；route.request 步放行后由 execute 改写（覆写语义）
    required_capabilities: list[dict]  #: 混合诉求能力账（2026-08-22 ov1-fix1c，覆写语义）：机械闸命中时由 route_consensus 产出（{capability, verbs, label_zh, anchor}），finish 机械核销逐项对账，缺项拒收回灌
    artifact_context: str        #: 课题上下文卡（2026-08-22 engagement B4，设计 §3.3）：独立字段，只进 route_consensus/understand 的 prompt 作结构化上下文块；缺省空串 = 与旧版逐位一致
...
class _AgentContext:
    ...
    decide_model/decide_model_name/decide_lane（2026-08-07 第三批任务1，复杂度路由）：
    decide 节点专用档——仅当 utterance 评分进 complex 车道且配置了 LLM_MODEL_COMPLEX 时
    非 None；understand/validate/repair/narrate 恒走 chat_model（病灶数据见任务记录 §1）。"""
    ...
    # P0-1（2026-08-10 codex 二轮评审）：成功经验库分区键——principal=会话账户 id
    ...
    # cr1（并发分流与确定性 RAG 策略，设计 §4.4）三个可选注入缝；缺省 = 今天逐位不变
    ...
    #   回调（**只做 abandoned/lazy 标记，不发射**，r3 P1-4）；rescue 短路不调。
    # - route_extra_zh：有标记分支的机械标记事实行（共识上下文尾部拼接，缺省空串=今天）。
    retrieval_provider: Any = None
    on_route_verdict: Any = None
    route_extra_zh: str = ""
    # eng-b4（2026-08-22，设计 §3.3）：课题上下文卡——understand/route_consensus 的
...
def _build_chat_model(config: LLMConfig) -> Any:
    ...
    DeepSeek V4 思考旋钮（2026-08-08 任务1）：config.thinking / reasoning_effort 非 None
...
#: 它是入参契约的一部分，与生成逻辑同模块才不会再长成两份拷贝（病灶史见该模块注释）。
...
def _artifact_context_block_zh(artifact_context: Any) -> str:
    """课题上下文的结构化注入块（2026-08-22 engagement B4，设计 §3.3）：独立字段、
...
def _context_zh(utterance: str, *, has_results: bool, result_total: int,
    ...
    上下文、不是用户的话，位置必须如实分开。
    `artifact_context`（2026-08-22 engagement B4）与 examples 同位的背景上下文块
...
def _route_retrieval_zh(retrieval: Any) -> str:
    """route_consensus 专用的规则匹配概览（2026-08-17 M1 诚实不变量，codex 对抗评审 高3）：
...
def _route_context_zh(utterance: str, *, has_results: bool, result_total: int,
    ...
    """route_consensus 的现场段（2026-08-17 M1）：与 `_context_zh` 同上下文面，两个刻意
    差别——检索概览用 `_route_retrieval_zh`（不含结果集标题，诚实不变量）；原话由调用方
    自拼「用户原话」尾段，本函数不附带（避免 `_context_zh` 自带的「用户这一句」造成
    原话进 prompt 两次）。nl1 转正后 route_consensus 常驻环首，本函数是其唯一现场段。"""
    if has_results:
        ctx = f"当前屏幕上已经有一批检索结果（共 {int(result_total)} 条命中）。"
    else:
        ctx = "当前屏幕上**还没有**检索结果。"
...
def _raw_from_message(message: Any, name_to_verb: dict[str, str]) -> dict[str, Any]:
    ...
    **多 tool_call 取第一个**（2026-08-09 病灶猎杀批，取代 2026-08-07 换装的「多调用
    机械拒绝」codex P1-3 旧策）：DeepSeek 实测**不遵守** `parallel_tool_calls=False`
    ——decide 不可读 17/17、understand「no_tool_calls」跌兜底 8/8 全是模型一次回了
    ≥2 个调用，且微探针实证**第一个调用 17/17 是合法续步**（模型的批量调用是「规划
...
def _usage_record(answer: Any, node: str) -> dict | None:
    """从 AIMessage 抠 DeepSeek 磁盘缓存用量（2026-08-08 任务3 埋点）。
...
def _invoke_text_with_continuation(
    ...
    *,
    ...
    """文本应答调用 + **截断两段式续写**（2026-08-08 任务2 有界自愈重试器的唯一增量）。
...
def _invoke_tool_channel_impl(
    ...
    *,
    ...
    """`_invoke_tool_channel` 的本体（2026-08-17 M4 波2 拆壳）：返回五元组
...
def _invoke_tool_channel(
    ...
    *,
    ...
    """工具调用通道的**统一三级实现**（2026-08-07 换装收口：understand/repair/decide 共用）。
    ...
    2026-08-09 病灶猎杀批，证据见两函数 docstring）。
    ...
    M4 波2（2026-08-17，设计 §6-3）：本壳在 impl 返回后统一发一次 trace llm_call
...
#: 点名源一致性护栏覆盖的动词（2026-08-03 agent1b）：这两个动词的 source 槽语义都是
#: 「用户点名的那个来源」，填错来源 = 查错对象（真机病灶：「检查10x更新」被填成 source=ArrayExpress）。
_NAMED_SOURCE_VERBS: tuple[str, ...] = ("curate.check_updates", "curate.search_online")

#: 点名源**半闸**覆盖的动词（2026-08-15 gr2，codex 对抗评审 Critical-3）：sync 的
...
#              的显式契约，工具不自己回头摸全局。
#
# 返回形状闸（2026-08-06 schema 加固第一刀）：各动词的返回契约模型登记在
...
#
# 写操作只有 search_online / sync_updates 两个，依据是产品所有者 2026-08-03 的全自动化预先
...
#: 2026-08-15 gr2 批 6→8（codex 对抗评审后采纳）：① 多调用整批消费（mcl1/dsv）让单轮
#: decide 产步更多、预算顶得更早；② 5 步链 + 一次网络重试恰好烧穿 6 步余量——但使
#: 边界**诚实**的修复不是数字本身，是到顶结算闸（见 decide 到顶分支注释：可机械核验
#: 事项全部结清时不谎报「没做完」）。codex 建议的「按复杂度分档 6/8」未采纳：复杂度
#: 路由宁窄勿宽，长链误入 simple 档会被 6 重新截断——同一病灶换门复现，不如统一 8
#: + 结算闸。recursion_limit=50 仍有近两倍余量（8 步最深路径 ≈26-30 节点次）。
MAX_STEPS = 8

#: 写步独立预算（2026-08-08 codex 裁决 R4 修正件）：search_online / sync_updates 是写工具
#: （联网搜回即入库 / 比对即入库，记账+回收站可撤销），总步数翻倍不该把单请求潜在写入
#: 次数也翻倍——每请求至多这么多次写步，超出由 `_adjudicate_decide_obj` 机械拒绝并如实
#: 点名（提示不是围栏，与联网暂停同哲学）。
MAX_WRITE_STEPS = 2

#: 写条数独立预算（2026-08-09 codex P2-2）：「写步」计的是调用次数——一次 sync 调用
...
#: 逃生口预算（2026-08-17 scoped 路由批 M1 钉死点 2）：每轮请求至多这么多次
...
#: 2026-08-08 探针 v4 病灶 k08 起二分：联网二连败 → **联网暂停**（整族禁提联网工具）。
#: 2026-08-08 约束放松批（codex 裁决 K3）再二分：非网络二连败不再硬停环——旧实现是
...
#: 联网暂停的 prompt 机械注入段（2026-08-08 探针 v4 病灶 k08）：联网二连败后 decide 照常
...
#: 「先新后旧」注入段（2026-08-08 探针 v5 病灶4：k03/k04 里重试挤占 MAX_STEPS 预算，
#: 没做过的新事反被截断）——存在失败步时提示优先推进新事项。**纯 prompt 劝导，无机械闸**：
#: 失败重试的放行是病灶 k03 的刻意设计，本段只排先后、不禁止。
_FAILED_STEP_BLOCK_ZH = (
    "\n----- 有步骤失败时的次序 -----\n"
    "先把没做过的**新事**做完；重试失败的事最多一次，放在最后。"
)

#: 写步预算耗尽的 prompt 机械注入段（2026-08-08 约束放松批 B）：提示不是围栏，
...
#: rank / rerank 预算耗尽的 prompt 机械注入段（2026-08-17 M2，同 search.rerun 预算哲学）：
...
#: 环内结果处理四工具的预算注入段（2026-08-18 四工具批，同预算哲学；未用满恒空段）。
...
#: 逃生口机会用完的 prompt 机械注入段（2026-08-17 M1，同预算哲学）：decide 面同时把
...
def _rescue_block_zh(unresolved_terms: Any) -> str:
    ...
    放宽依据（rescue-ev1 取证，用户拍板）：5 次真触发全被择优闸「改空拒」挡下——
...
def _write_steps_used(steps: list[dict]) -> int:
    ...
    2026-08-15 gr2 豁免（codex 对抗评审 High-5）：`search_online` 的 **network_error**
...
def _write_records_used(steps: list[dict]) -> int:
    """已写入条数（**从 steps 真实结果现算**，codex P2-2）：search 取 record_count、
...
def _failed_tool_ban(steps: list[dict]) -> frozenset[str]:
    ...
    （codex 裁决 K3）；联网二连败不在此列——网络在抖时整族都该停，走
...
def _failed_tool_ban_block_zh(banned: frozenset[str]) -> str:
    ...
    文案如实（2026-08-09 codex P0-5）：两个**不同**动作各败一次不许说成「连续失败两次」——
    那是同一动作连败两次的专属表述。"""
    names = sorted(
...
def _is_network_call(verb: str, source: Any) -> bool:
    """该次调用是否触网（2026-08-09 codex P1-5）：search/sync 恒触网；check_updates 按
...
def _agent_project_root() -> Path:
    """agent 图内工具的项目根：**复用 corpus_status 的默认根真源**（W1 起 = runtime_paths
...
def _loop_search_online(slots: dict, root: Path) -> dict:
    ...
    return {
        ...
        # P1-4 实体级去重回显（2026-08-10）：apply 侧重检为准（TOCTOU），plan 侧兜底
...
def _loop_search_rerun(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    ...
    rerank_audit 关——防「审核改写→重搜→再审核」循环）。采纳与否由机械闸裁定，不靠 LLM 自评：
      ...
      ③ 否则采纳——**命中 0 条同样采纳**（2026-08-23 用户拍板：条件变更重检的空结果就是
    ...
    rescue2（2026-08-17）：ctx.unresolved_terms（原检索投影的未收录词，rescue 端点链下入；
...
#: scoped 路由提示词的内置最小降级版（2026-08-17 M1）：prompts/ 下的 .md 文件即真源，
...
def _rewrite_query(model: Any, query: str,
    ...
    rewritten=False 如实标注，不靠 LLM 自评。`usage_sink`（2026-08-17 codex 评审 中4）：
    给了就把本次改写调用的缓存用量经 `_usage_record` 追加进去（读不到用量自然跳过）。"""
    if model is None:
        return query, False
    t0 = time.monotonic()  # M4 波2（设计 §6-3）：rerank 独立改写调用的 llm_call 留痕
    try:
...
def _top_digest(rows: Any, n: int = 3) -> list[dict[str, Any]]:
    ...
    dataset_uid（2026-08-20 pbt1 批补：同批依赖占位的解析源，`$<N>.top[<i>].dataset_uid`）/
    dataset_name / species / tissue / disease / source / rank（1 起序号，pbt1 批补——
...
def _loop_rank(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """裸新检索（2026-08-17 scoped 路由批 M2）：以 query 跑**标准 RAG 管线**（与
    ...
    display=true 时构造载荷（recommend 同形）并附 M3 批次原料（batch 键）推往前端，
    ...
    sources = ctx.get("search_sources")
    # 2026-08-23 ku1-覆盖策略修复（设计 §10.1）：rank 与 search.rerun 同口径——必须吃同一份
    ...
    if display:
        ...
        batch = {
            "kind": "rank",
            "label": query[:20],
            # query_raw = 本轮用户原话（M3 契约；2026-08-17 codex 评审 中6：曾张冠李戴
...
def _loop_rerank(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """坏 query 优化重检（2026-08-17 M2）：**独立 LLM 改写调用**（独立上下文 +
    ...
    display=true 产 M3 批次原料（label = 生效的 rewritten_query）。
    ...
    display = slots.get("display") is True
    sources = ctx.get("search_sources")
    # 2026-08-23 ku1-覆盖策略修复（设计 §10.1，与 _loop_rank 同纪律）：坏 query 改写只换检索句、
    ...
    if display:
        ...
        batch = {
            ...
            # query_raw = 本轮用户原话（M3 契约，同 _loop_rank 的中6 修复口径）；
...
def _loop_compare_datasets(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """对比两个数据集（2026-08-18 四工具批）：确定性字段 diff（`compare.diff_items`，
...
def _sanitize_uids_slot(value: Any, limit: int = 20) -> list[str]:
    """cite.export 的 uids 槽清洗（2026-08-20 pbt1 批，设计 §4）：列表 / 单字符串 →
...
def _loop_cite_export(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    ...
    （2026-08-18 四工具批；2026-08-20 pbt1 批扩 uids 槽，设计 §4）：产物落
...
def _loop_compat_find(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """给一个数据集找**元数据兼容**同伴（2026-08-18 四工具批）：包
...
def _loop_fair_check(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """对单个数据集做 13 项 FAIR 元数据自检（2026-08-18 四工具批）：包
...
LOOP_TOOLS: dict[str, dict[str, Any]] = {
    "curate.db_status": {
        ...
        # B1a（2026-08-08 调研六候选批）：「看看库里多少条」被并入检查步的病灶（k03）——描述层锚定独立事项。
    ...
    "curate.check_updates": {
        ...
        "readonly": True,
        # B1a（2026-08-08）：多来源只检一个（k09）与主题问句误选（j05 型）两处病灶的描述层锚。
    ...
    "curate.search_online": {
        ...
        "readonly": False,
        # B1a（2026-08-08）：条件成立后该步被放弃（j03/l07/b04 型）——锚定「下一步就是我」。
    ...
    "curate.sync_updates": {
        ...
        "readonly": False,
        # B1a（2026-08-08）：「联网搜 X 入库」被磁吸（k08）——抽象边界补一个具体反例。
...
# rank / rerank（2026-08-17 scoped 路由批 M2；2026-08-17 nl1 转正常驻）：环内检索工具组，
...
def _loop_route_request(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """逃生口元动词（2026-08-17 M1）：如实记录换线请求——真正的路线切换由 execute
...
# 逃生口注册（2026-08-17 M1；nl1 转正常驻）：所有套件的 decide 面都含它；readonly 但
...
def _execute_detail_zh(spec: dict, result: dict) -> str:
    ...
    if kind == "search_online":
        ...
        if not filename:
            # 零写入是合法结果（候选全部已在库中，fix-exec H1）：直接引用 apply 写实的
    ...
    # 环内结果处理四工具（2026-08-18 四工具批）：note_zh / comparison_zh 由工具按真实
...
#: 观测设施自身故障的留痕纪律（2026-08-15 ta2 exec2，审计 cross-trace D1-D4；与
...
def _audit_loop_tool(root: Path, verb: str, slots: dict, ok: bool, note: str,
    ...
    「写入条数」可记，字段整个省略——恒 0 会让账本统计口径失真（2026-08-04 评审坐实）。"""
    try:
        ...
        # cross-trace D1：账本写失败不再零痕迹——观测设施自身故障必须可观测。
...
def _audit_fallback(root: Path, node: str, reason: str, utterance: str, model: str) -> None:
    ...
    try:
        ...
        # cross-trace D2：降级审计写失败同样留一行（调用方 turn 侧的外层兜底属别家批次）。
...
def _read_example_rows(path: Path) -> list[dict]:
    ...
    坏行不再零痕迹（cross-trace D4）：跳过前 `_warn_once` 留一行——否则「账本损坏」
    与「本来就是空的」事后不可区分。"""
    rows: list[dict] = []
    if path.is_file():
...
def _endpoint_fp_from_config(config: Any) -> str:
    """端点指纹（P0-1 成功经验库分区键）：sha256(base_url|model) 前 12 位。
...
def _maybe_record_success(root: Path, utterance: str, plan: dict, steps: list,
    ...
    **机械判据**（不问 LLM）：有真跑的工具步、每步都 ok、至少一步是 curate.* 管护动词、
    ...
    P0-1（2026-08-10）：每行打**分区标**（principal 会话账户 + endpoint_fp 端点指纹）——
    原话是隐私面，候选展示与注入都只取同分区行。
    收录质量闸（2026-08-13）：**「跑通」不等于「干得漂亮」**——下列信号任一命中即不录：
    ...
    try:
        ...
        # cross-trace D3：成功经验候选池写失败不再静默——用户得到的是"已收录"语义，
...
def _example_partition_of(row: dict) -> tuple[str, str]:
    """分区标（P0-1）：缺字段的存量行按 ("anonymous","") 计——与 `_load_success_examples` 同口径。"""
...
def list_example_candidates(root: Path, *, principal: "str | None" = None,
    ...
    try:
        ...
        # cross-trace D4：读盘失败（含账本损坏）不再静默当空——返回形状不变，stderr 留痕。
...
def approve_example_candidates(root: Path, ids: Any, *, principal: "str | None" = None,
    ...
    try:
        ...
        # cross-trace D4：账本异常 → 零计数的口径不变，但失败本身必须可观测。
...
def dismiss_example_candidates(root: Path, ids: Any, *, principal: "str | None" = None,
    ...
    try:
        ...
        # cross-trace D4：同 approve——零计数口径不变，失败留痕。
...
def _load_success_examples(root: Path, utterance: str,
    ...
    P0-1 分区（2026-08-10 codex 二轮评审）：principal 非 None 时只取**同分区**行
    ...
    try:
        ...
        for line in path.read_text(encoding="utf-8").splitlines()[-500:]:
            ...
            try:
                ...
                # cross-trace D4：与 `_read_example_rows` 同纪律——坏行跳过但留一行痕迹。
                _warn_once(f"example_ledger_bad_line::{type(exc).__name__}",
                           f"经验库账本 {path.name} 有无法解析的行（{type(exc).__name__}），"
                           "已跳过——若跳光等价于账本损坏，请检查该文件。")
                continue
        ...
        # cross-trace D4：注入侧静默降级回纯静态 few-shot 的口径不变，但「账本坏了所以
...
def _examples_prompt_zh(root: Path, utterance: str, *,
    ...
    P0-1：principal 非 None 时只检索同分区样例（跨账户/跨端点原话不进 prompt）。"""
...
def _step_projection(step: dict, *, reasked_write: bool = False) -> dict:
    ...
    if kind == "db_status":
        ...
        # 检索事实投影（2026-08-17 M2）：裁决/计数/生效条件/top digest 给 decide/narrate
        ...
        # 逃生口投影（2026-08-17 M1）：换线结果给 decide/narrate 当事实。
        out["result"] = {
            k: r.get(k)
        ...
        # 对比投影（2026-08-18 四工具批）：结论句 + 计数 + 降级态给 decide/narrate 当事实
...
# ==========================================================================================
# decide：多步循环的「判断下一步」（2026-08-04 散文 JSON 版；2026-08-07 换装迁 tool-calling）
#
...
#: 2026-08-17 nl1 转正：rank / rerank / route.request 常驻（原两开关分支合并为单表）。
...
def _decide_tool_table_zh(verbs: Any = None) -> str:
    ...
    带 source 槽的动词行尾程序拼候选清单（2026-08-08 codex 裁决 N1：与 schema 枚举
    同出 `agent_schemas.source_candidates_zh`——prompt 名单与 schema 枚举不再有第二份拷贝）。
    `verbs`（2026-08-17 M1 scoped 路由）：显式给顺序子集时按子集出表（套件收窄面）；
...
#: decide prompt 的共享段落拆分（2026-08-07 换装：规则本体一份真源，两种输出格式壳）。
...
_DECIDE_INTRO_ZH = (
    ...
    # 2026-08-08 探针 v3 病灶 b12/g04：多件事的马拉松指令做两件就 finish——判断前先核销。
...
_DECIDE_TOOLS_BULLETS_ZH = (
    ...
    "有先后依赖或会写库的动作仍一次只发一个\n"
    # 2026-08-20 pbt1 批（设计 §6 + 施工修正）：教**唯一占位引用形状** + 两个示例 + 禁令。
    # N 的编号口径 = **本轮执行序号**（1 起；真机探针实测模型自然形态是「understand 先 rank、
    # decide 单发 compare($1)」——批内局域编号会把它误杀，见设计文档「施工修正」节）。
...
_DECIDE_RULES_TAIL_ZH = (
    ...
    # 2026-08-08 探针 v4 病灶 l07：rule 6 补「检出疑似新增 ≠ 已经入库」——模型把检查发现的
    ...
    "不许拿「新增里已经有了」替系统放弃。\n"
    # 2026-08-08 探针 v4 病灶 k06：rule 7 补反向条件——「看看A有没有新数据，没有的话就检查B和C」
    ...
    # 2026-08-08 codex 裁决 N3：旧句「不要搜，回 done」会把原话里**其余独立事项**一起判死刑——
...
#: 2026-08-07 换装时与旧手抄版逐字一致；2026-08-08 起随 INTRO/rule 10 的**刻意修订**漂移
#: （探针 v3 病灶 b12/g04 提前收工——tests/test_agent_schemas.py 的钉字同步刻意更新）。
_DECIDE_RULES_ZH = (
...
def _build_decide_tool_specs() -> tuple[list[dict], dict[str, str]]:
    """decide 的 tool-calling 工具面（2026-08-07 换装，codex P1-5 改案）：
    ...
    **5 个 loop 工具**（2026-08-16 检索工具化起含 search.rerun；schema 由
    ...
    工具，显著提高误选率、还吹大 prompt（codex P1-5）。
    ...
    **模块加载期构建一次**（与 `_DECIDE_RULES_ZH` 同纪律）：读的是真实 LOOP_TOOLS——
    ...
    tools.append({
        ...
        "function": {
            ...
            # B1a（2026-08-08）：负向锚——还有没核销的事项时不许 finish（与核销硬闸互文）。
            "description": ("判断用户要求的事已经全部完成，或剩下的事做不到/条件不成立时，"
                          "调用它收尾（必须附 completion_report 核销报告）。"
                          "还有没核销的事项时不许调用——做完一件就收尾与没做同罪。"),
            # 必填 completion_report（2026-08-08 探针 v3 病灶 b12/g04 第二刀：可选 checklist
            ...
            "parameters": {
                ...
                "properties": {
                    "completion_report": {
                        "type": "string",
                        # 2026-08-08 探针 v5 病灶2：豁免行也要举证（据第几步的结果得出）——
...
# ---------------------------------------------------------------- scoped 路由套件面（2026-08-17 M1）
#
# 设计钉死点 3：提示词共享核心单源化（prompts/loop_core.md 唯一一份）+ 路线差异段；
# decide 工具表继续由 LOOP_TOOLS 注册表**程序生成**（按套件过滤），禁手抄。
# 2026-08-17 nl1 转正：本节为**唯一**装配路径（原 OFF 既有面分支已摘除归档）。

#: 套件 = LOOP_TOOLS 注册表子集（设计 §M1：套件只能装注册表项 + 本波新登记项）。
...
#: scoped understand 的系统提示（nl1 转正后为非 rescue 首步的**唯一**系统提示；rescue
...
def _is_duplicate_step(verb: str, raw: dict, steps: list[dict]) -> bool:
    ...
    比对集 = 成功步 + **非 network_error** 失败的步（2026-08-08 探针 v5 病灶 f04 收窄；
    W2a4 曾一律豁免失败步，把 bad_result_shape 这类确定性失败也放去重试白烧一步）：
    - network_error 是唯一真·可重试码——失败步什么都没做成，同指纹重试放行
      （check 只有 source 槽，同源重查恒同指纹，探针 v4 病灶 k03）；重试上界由连续失败
      处置二分天然兜底（再败即联网暂停/硬停）；
...
def _search_coverage_violation(obj: dict, steps: list[dict]) -> str | None:
    """搜索覆盖闸（2026-08-08，探针 v3 病灶 a06/d01：首搜成功后 decide 换措辞/加过滤再搜
...
#: finish 核销报告（completion_report）机械否决的判定词表（2026-08-08 探针 v3 病灶 b12/g04
#: 第二刀）：prompt 劝导（INTRO 核销句 + rule 10 马拉松实例 + 可选 checklist）没拦住马拉松
...
#: 形态 B 依赖借口词（2026-08-08 探针 v4 病灶 k03/k08）：豁免词命中行里若同时夹带这类
...
#: 形态 A 已做声称（探针 v4 病灶 b08/k01）：报告把没跑过的事标成「已做」（没跑 db_status
...
def _completion_report_veto(report: str, n_steps: int) -> tuple[str | None, str]:
    ...
    - "exempt_without_step"（2026-08-08 探针 v5 病灶2：k02/k09/k10/k11/l07 提前收工残余）：
...
def _reask_write_veto(report: str, reask_writes: list[dict]) -> tuple[str | None, str]:
    """重问写步的**强制核销复核**（2026-08-07 产品拍板 B 方案配套——重问后的写动词从
...
#: 未决事项机械提示的判定词表（2026-08-08 探针 v6 病灶族：7 条失败同出一辙——模型在
...
#: 这是提示不是闸，拿不准绝不报。「几条」带左侧边界（2026-08-15 fix-exec M3）：
...
#: curate 侧补充点名表（`_named_sources_in` 第三趟，2026-08-08 任务4/codex 二审 P1-5）：
...
def _named_sources_in(utterance: str) -> list[str]:
    ...
    （逐字趟与 `_named_source_violation` 的 w2a2 豁免同口径：词表刻意不收裸「encode」
    这类普通英文词，但用户原样写出全大写 ENCODE 就是点名）。
    2026-08-07 第三批任务1 从 `_pending_hints_block_zh` 规则 1 抽出的共享真源——
...
def _pending_hints_block_zh(utterance: str, steps: list[dict]) -> str:
    """decide prompt 的「未决事项机械提示」段（探针 v6；**提示不是闸**——不拦截任何东西）。
    ...
    return ("\n----- 机械提示：以下事项可能还没做（逐项核对，确实做了就忽略）-----\n"
            ...
            # 拉回工具通道（2026-08-08 w2a6 真机 A/B：无此句时散文首答率显著升高）。
            + "\n核对完仍按上面的规则回答：调用恰好一个工具，或调用 finish 并填好核销报告。")


def _import_hint_request(utterance: str) -> bool:
    """提示层规则 3 的入库诉求**极性判定**（2026-08-15 ta2 exec2，与已修硬闸
...
# ---------------------------------------------------------------- 任务清单核销（2026-08-08 任务4）
#
# codex 两轮评审（task-msjb0dxp NO-GO 3P0+4P1；task-msjbpzw3 复审意见）修订定稿 v2.1：
...
def _agent_status_block_zh(state: "_AgentState", *, steps: list[dict], moratorium: bool,
    ...
    """decide 双壳恒注入的**执行状态栏**（2026-08-08 B2 调研六候选批；epub 第2章状态栏思想）。

    三条铁律（书 2.6）：①**代码维护**——全部字段从图状态确定性现算，模型不许统计；
    ②有损投影要谨慎选维度——只放真实机械状态（codex #6：步数/失败/联网暂停/已搜主题/
...
def _parse_checklist(payload: Any, utterance: str) -> tuple[list[dict], int]:
    ...
    - anchor 全半角归一后必须是 utterance 的**子串**且 ≥4 字（codex P1：anchor 机械可验，
    ...
    for raw in items[: _CHECKLIST_MAX_ITEMS * 2]:   # 输入本身也可能超长，先看一批再截
        ...
        out.append({"task_id": f"t{len(out) + 1}", "text": text[:60], "anchor": anchor,
                    ...
                    # 来源从 text+anchor **合并**提取（codex 二审 P0-1：模型 text 可能省略
                    # 来源；anchor 是已验证的原话子串，两处同挖取并集——漏来源 = 对账
                    # 退化成「任何来源都算数」，比多收来源危险得多）。
                    "sources": _named_sources_in(text + " " + anchor)})
    # N4（2026-08-08 codex 裁决）：合法但超 `_CHECKLIST_MAX_ITEMS` 上限被截断的条目也必须
...
def _step_covered_sources(step: dict) -> set[str]:
    ...
    - 空 source 的 check/sync 步 → 从 result.sources 的条目还原（codex 二审 P1-4：
...
def _task_settled_by(task: dict, steps: list[dict]) -> bool:
    ...
    codex 二审 P0-1 修复：**按来源覆盖核销**——条目点了 N 个来源，就要 N 个来源
    ...
    expect=rank（ov1-fix1c）：混合句的检索半——任一 ok 的 rank/rerank/search.rerun 步
...
def _checklist_unsettled(checklist: list[dict], steps: list[dict],
    ...
    unsettled: list[dict] = []
    # 逐项豁免（codex 二审 P1-3 修订版）：declined_zh 非空只豁免**第一条** unsupported——
...
def _capability_item_states(caps: list[dict], steps: list[dict],
                            declined_zh: str = "") -> list[dict]:
    """混合诉求能力账的逐项**代码现算**状态（ov1-fix1c；与 `_checklist_item_states`
...
def _task_checklist_call(chat_model: Any, utterance: str,
    ...
    """清单轻量产出（任务4）：一次 chat 调用拆事项 → (合法条目, 剔除数, 失败原因)。
    ...
    # 解析器，对数组会静默返回首元素，2026-08-08 spy 实锤）。
    text = _message_text(answer).strip()
    payload: Any = None
    try:
...
def _finish_veto_all(report: str, n_steps: int, reask_writes: list[dict],
    ...
    """finish 否决的**聚合口**（任务4；codex P1-1：两闸分次否决会耗尽重试额度）——
    ...
    - 混合诉求能力账 `_capabilities_unsettled`（ov1-fix1c，机械闸产出 capabilities 时；
    ...
    # 去重（保序）+ 限幅（codex P1：回灌 ≤2KB——按条数与单条长度双控）。
    seen: set[str] = set()
...
def _pending_violations(utterance: str, steps: list[dict]) -> list[tuple[str, str]]:
    """pending 机械判定**升硬闸**（任务4；原 `_pending_hints_block_zh` 的三条提示）——
    ...
    规则 3 另有否定极性豁免（`_import_hard_request`，2026-08-15 fix-exec H1）：
    ...
    # 规则 3：检出未入库（按**来源差集**——codex 二审 P0-1 附带修复：「任一来源搜过
...
#: 规则 3 的**否定极性**豁免（2026-08-15 fix-exec H1，与 `_DENIAL_MORPH_RE` 同思路）：
...
def _import_hard_request(utterance: str) -> bool:
    """原话是否含**非否定**的入库诉求（pending 硬闸规则 3 的极性判定，fix-exec H1）：
...
#: 词表与阈值（2026-08-07 第三批任务1）：45 例探针全量定标（docs/工作记录/任务记录_2026-08-07_第三批.md §1）——
#: score≥2 命中全部 K/L 断链族且 H/C/E/I 克制类全在 simple（reasoner 的过动回落零暴露）。
#: 宁窄勿宽：误进 simple 只是没治到病（退化为现状），误进 complex 才付 4-6 倍时延与过动风险。
#: codex P1-2 增补并列/追加语素（另外/同时/一并/以及/并且/分别——真实两步任务的常见说法，
...
#: 克制语素一票留 simple（codex P1-3）：带叫停/否定语义的句子即便词面分高也不进 complex——
...
#: 库容 + 任何另一事项信号 = 多事项链（b08 实锤：「检查…顺便看看库里多少条」chat 3/3 断链、
...
def decide_lane(utterance: str) -> str:
    ...
    病灶实锤（v11 chat vs v10r reasoner 逐例对照）：chat 的 K/L 失败以 **decide 断链**为主、
    understand/repair 首步误判为辅（k04 r2，codex P1-1 实锤）；reasoner 的克制类回落全在
...
def _adjudicate_decide_obj(obj: dict, state: "_AgentState", *,
    ...
    `allow_placeholders`（2026-08-20 pbt1 批）：True = 占位接地续步（`_batch_readonly_extras`
    ...
    # scoped 路由套件闸（2026-08-17 M1；nl1 转正后常驻；提示不是围栏——decide 的面收窄是
    ...
    # 联网暂停（2026-08-08 探针 v4 病灶 k08）：联网二连败（network_error）状态下，联网工具
    # 的提议机械拒绝——按 done 收尾、note 如实写「联网暂停中」；离线工具（db_status）照常放行。
    # 2026-08-09 codex P1-5：联网性按 (verb, 解析源) 判定——离线快照源的检查只读本地快照，不连坐。
    if (verb in _NETWORK_LOOP_TOOLS and _is_network_call(verb, obj.get("source"))
        ...
        return None, (f"联网已连续失败两次，联网暂停中——「{shown}」是联网工具，"
                      "这一步本回合不再尝试，按「已完成」收尾。"), "", ""
    # 回滚独立预算（2026-08-18）：不占、不受正向写预算；每轮至多 2 次防 ping-pong。
    if verb == _ROLLBACK_LOOP_TOOL and _rollback_used(list(state.get("steps") or [])) >= MAX_ROLLBACK:
        return None, (f"本次请求的回滚预算已用完（每轮最多 {MAX_ROLLBACK} 次），这一步没有再执行，"
                      ...
                      "回滚预算已用完。"), ""
    # 写步预算闸（2026-08-08 约束放松批 B）：正向写步已用满 MAX_WRITE_STEPS 次后，写工具提议
    # 机械拒绝——按 done 收尾、note 如实写「写步预算已用完」；只读工具照常放行。
    # 2026-08-09 codex P2-2 增第二维度：写**条数**用满 MAX_WRITE_RECORDS 同样拒（步数
    # 管循环、条数管写入量）。
    ...
    # rank / rerank 预算闸（2026-08-17 M2，同 search.rerun 预算哲学）：已用满后再提议 →
    # 机械拒绝，按 done 收尾、note 如实点名。nl1 转正后二动词常驻 LOOP_TOOLS，闸恒可达。
    if verb == "rank" and \
    ...
    # 环内结果处理四工具的独立预算闸（2026-08-18 四工具批，同 rank 预算哲学）：
    ...
    if verb == "fair.check" and \
        ...
        return None, (f"本次请求已 FAIR 自检 {MAX_FAIR} 次（一次请求最多 {MAX_FAIR} 次），"
                      ...
                      f"{MAX_FAIR} 次，预算已用完。"), ""
    # 失败动作禁提（2026-08-08 约束放松批 G，codex 裁决 K3）：非网络二连败后，刚失败的
    ...
    # 点名源缺槽位的确定解补位（2026-08-08 任务2：与 validate 节点同一助手——decide 续步
    ...
    if not allow_placeholders:
        # 主步占位校验（2026-08-20 pbt1 批，设计 §1 + 施工修正）：占位只能引用**本轮次
    ...
    # 搜索覆盖闸（2026-08-08 探针 v3 病灶 a06/d01）：同主题换措辞/加过滤重搜——槽位变了
...
def _readonly_loop_verbs() -> frozenset:
    """只读白名单（2026-08-14 mcl1 批）：LOOP_TOOLS 注册表 `readonly=True` 的动词，**减去**
    下方显式排除项——代码口径（2026-08-22 ov1-wf1 修正 docstring，与代码逐位对齐）现况
    ...
    （search_online / sync_updates）**永不**进白名单：mcf1 探针实测预发写调用逐位保真
    仅 45%（参数要等前序真实结果接地、条件要等前步判定）。
    route.request（2026-08-17 M1）虽只读也**永不**进批：它是控制面元动词——同批多调用
    里混进换线会让面切换时序不可判，换线必须独占一轮。
    compare.datasets / compat.find / fair.check（2026-08-18 四工具批）同样只读但**永不**
    ...
    2026-08-20 pbt1 批（设计 §1）：四工具的**缺省依赖**由「同批**占位引用**」显式接地后
...
# ----------------------------------------------------------------- 依赖占位批量计划 v2（2026-08-20 pbt1 批）
# 设计：`docs/工作记录/设计_依赖占位批量计划_2026-08-20.md`。decide 一轮可输出带依赖占位
# 的一串调用，execute 顺序解析执行——砍掉「搜→对比/兼容/FAIR/引文」依赖链的 LLM 往返。
# 不做通用路径（v1 评审 P0），只做一条白名单受控链；两阶段校验；失败四分；解析源局部化。

#: 依赖占位**唯一合法形状**（设计 §1）：`"$<N>.top[<i>].dataset_uid"`——N = 前序调用在
...
#: 占位允许的**生产者**（设计 §1）：只有 rank / rerank 的 top digest 可被引用。
_PLACEHOLDER_PRODUCERS: frozenset[str] = frozenset({"rank", "rerank"})

#: 占位允许的**消费槽位矩阵**（设计 §1，显式矩阵，此外一律截断回炉）：
...
#: 元槽位（禁占位，设计 §1）：quoted/confidence/reason/cancelled 等。
_PLACEHOLDER_META_SLOTS: frozenset[str] = frozenset(
    {"quoted", "confidence", "reason", "cancelled"})

#: 永不进批清单（设计 §1，硬编码）：route.request（控制面元动词，换线必须独占一轮）/
...
def _placeholder_ref(value: Any) -> tuple[int, int] | tuple[str, ...] | None:
    """单值占位识别（设计 §1）：返回 (N, i) = 合法占位；`_PH_BAD` = 形似占位但不合规
...
def _placeholder_static_violations(verb: str, raw: dict, position_verbs: list[str],
                                   pos: int) -> list[str]:
    """**静态阶段**占位校验（设计 §2；2026-08-20 pbt1 施工修正：`pos` 与 `N` 的编号口径
...
def _resolve_placeholder_slots(raw: dict, resolved: dict[int, dict],
    ...
    """execute **解析阶段**（设计 §3）：把 raw 里占位槽替换为真实 dataset_uid。
        ...
        if slot == "uids" and isinstance(value, list):
            ...
            for item in value:
                ...
                if isinstance(ref, tuple):
                    ...
                    resolved_uids.append(item)  # 字面量与占位混用（设计 §1）
            if resolved_uids:
...
def _resolve_placeholder_plan(raw0: dict, resolved: dict[int, dict], batch_id: str,
    ...
    """pbt1 **解析后全闸**（设计 §2/§3）：占位替换为真实值 → `_validate_raw` →
    ...
    # pbt1（2026-08-20 真机探针实测）：build_plan_from_raw 的字符串槽 80 字符截断会把
...
def _batch_readonly_extras(calls: list, first_raw: dict,
    ...
    """多调用的**同批消费**过滤（2026-08-14 mcl1 批；2026-08-20 pbt1 批升级为支持
    **占位接地**批量，设计 §1-§2。实测依据见 `_readonly_loop_verbs` 注释与
    ...
    - **占位静态校验**（设计 §2）：占位引用只过正则 + 序号越界 + 流向矩阵——违规即
    ...
    - 参数键 ⊆ 该动词 decide 工具 schema 的声明键（mcl1 探针实锤模型偶把 keywords/species
    ...
    seen_steps = steps + [{"verb": str(first_raw.get("verb") or ""),
                           ...
                           "ok": True}]
    # 占位静态校验的**执行序号**现场（pbt1 施工修正：序号 = 本轮次执行步序号，1 起）：
    ...
    for i in range(1, len(calls)):
        ...
        if not isinstance(args, dict) or not set(args) <= props_by_name.get(str(name), set()):
            ...
            continue
        raw = {"verb": verb, **args}
        # 占位静态校验（设计 §2）：正则 + 序号越界 + 流向矩阵——违规即截断整尾。
        ...
        if _is_duplicate_step(verb, raw, seen_steps):
            dropped += 1
            continue
        # 预算闸增量裁决（2026-08-17 codex 对抗评审 高1）：对原始 state 裁决时，同批已采纳
...
def _decide_answer_kind(answer: Any, name_to_verb: dict[str, str]) -> tuple[str, Any]:
    ...
    **多 tool_call 取第一个**（2026-08-09 病灶猎杀批：DeepSeek 不遵守
    `parallel_tool_calls=False`，decide 不可读 17/17 全是多调用且第一个 17/17 合法；
    循环带着新状态会再判断后续，吃第一个不吞任何事）——多调用留痕由 decide 节点
    拼进 trace detail（`_classify`）；2026-08-14 mcl1 起第 2..N 个里的只读独立续步
...
_STEPS_REPORT_RULES_ZH = (
    ...
    "既遂含义说；\n"
    # 2026-08-07 探针 v8 误伤/谎称双归因：旧版没说清动词的既遂含义，模型把 search_online
    ...
    # 2026-08-23 用户拍板：命中 0 条也采纳上屏（空结果集就是诚实答案）——adopted=false
    ...
    # 2026-08-07 探针 v9 回归修复：旧措辞诱导模型把来源名和数字拆进两句话
    ...
    "8. 不要建议、不要评论、不要客套。\n"
    # 2026-08-23 用户拍板：用户关心的是「用什么查的、查到什么」，不是内部机制。
...
#: **为什么不能再沿用穷举整句词表**（旧 _WRITE_CLAIMS_ZH 六个字面量的教训，2026-08-04
#: 对抗评审坐实）：「已完成下载」「下载完成」「已为你下载并保存」恰是 LLM 最自然的措辞，
...
#: 「好啦/搞定」族是 R2-1 P0-1 坐实的词表外漏网语素（「下载好啦」「下载搞定」「搞定了下载」）。
...
#: 否认侧的判定素材说明（2026-08-06 批A D1 P0）：旧实现是穷举整句词表 `_WRITE_DENIALS_ZH`
#: （「未执行入库/未入库/…」十个字面量）——与 claim 侧 R2-1 的教训同型：「导入」恰在
...
def _denies_done_read(report: str, steps: list[dict]) -> bool:
    """汇报里是否有「读动作否认」声称（第五路机械后检，2026-08-08 探针 v3 病灶 d07：
...
#: sync 主题闸的维度面（2026-08-08 探针 v5 病灶1）：只认「具体主题」四维的值别名——
...
#: cancelled=true 的**否定语素**机械镜像（2026-08-08 任务2，j03 r1/r2 实锤：reasoner 在毫无
...
def _utterance_topic_alias(utterance: str) -> str | None:
    ...
    2026-08-09 codex P1-1：先**等长遮蔽来源专名**再扫描——「Human Cell Atlas」自带 human、
...
#: sync 主题闸的分句边界（2026-08-08 任务2）：硬标点切句。
...
def _sync_gate_scope_zh(utterance: str, quoted: str) -> str:
    """sync 主题闸的**分句作用域**（2026-08-08 任务2，k11 实锤）：主题词只有落在 sync 所引
...
def _sync_topic_elsewhere_exempt(utterance: str, raw: dict[str, Any]) -> bool:
    """sync 主题闸的**别支豁免**（2026-08-08 任务2，k11 实锤：「检查CELLxGENE更新，有新增就
...
def _report_names_untouched_source(report: str, steps: list[dict]) -> bool:
    """只读侧的**假性声称**（2026-08-06 批B P2-1 真机抓到：「检查了10x和ArrayExpress的更新」
...
# ==========================================================================================
# 数字交叉核验（2026-08-06 schema 加固第一刀 · 第三路机械后检）
#
...
#      **以及 online_recent / local_count**（2026-08-07 探针 v8 误伤修复：「在线发现 12 条
...
def _step_true_counts(steps: list[dict]) -> set[int]:
    ...
    for s in steps:
        ...
        if kind == "search_online":
            ...
            # 环内结果处理四工具（2026-08-18 四工具批）：如实汇报数字的登记——
...
def _report_contradiction_reason(report: str, steps: list[dict]) -> str | None:
    ...
    - `denied_read`：明明有成功的读步（联网搜索/检查更新），汇报却出现「未搜索 X」式
      ...
      → 谎称没做做了的（只读侧，2026-08-08 探针 v3 病灶 d07）；
    ...
    if _denies_done_read(report, steps):
        return "denied_read"
    # wrote 的口径（2026-08-07 探针 v8 修复）：search_online 必须 record_count>0 才算
...
def _steps_report_fallback_zh(steps: list[dict]) -> str:
    ...
    for s in steps:
        ...
        if kind == "db_status":
            ...
            if not filename:
                # 零写入（候选全部已在库中，fix-exec H1 连带 L7）：引用 apply 写实的
...
def _autofill_named_source(verb: str, raw: dict[str, Any], utterance: str) -> str | None:
    """点名源缺槽位的**确定性补位**（2026-08-08 任务2，b08/k11 实锤）：understand 掉 source
...
def _sync_all_online_named(verb: str, raw: dict[str, Any], utterance: str) -> str:
    """点名单源场景下 sync 空槽的**如实留痕**（2026-08-15 ta2 exec2，审计 exec-gates M5）：
...
def _named_source_violation(verb: str, raw: dict[str, Any], utterance: str) -> str | None:
    ...
    规范名逐字（2026-08-08 探针 w2a2 豁免、2026-08-08 任务2 b08 升格为点名判定）：词表
    刻意不收裸「encode」（普通英文动词，见 search_request.SOURCE_ALIASES 注释），但用户
    **原样写出受控规范名**（全大写 ENCODE）就是点名——b08 实锤：understand 槽位落空时
    ...
    if not filled:
        # 半闸动词（sync）：不填 = 同步全部，覆盖点名源，合法放行（2026-08-15 gr2）。
        if verb in _NAMED_SOURCE_OPTIONAL_FILL_VERBS:
            return None
        # 「动词本身不用换」（2026-08-08 任务2 k11 实锤）：repair 收到「source 没填」后
...
def _step_grounding_texts(steps: list[dict] | None) -> list[str]:
    """已完成步骤**真实结果**里可供 keywords 接地的文本（decide 续步的出处之二，2026-08-06 批C）。
...
def _validate_raw(raw: dict[str, Any], utterance: str,
    ...
    （2026-08-10 codex 架构评审裁决落地——此前此处私拷一份还自称与 `_finalize`「口径
    一一对应」，评审实锤两路径已漂移）。本函数只叠加多步执行专属闸：点名源 /
    幻觉取消 / sync 主题 / keywords 接地。

    与 fallback 的 cancelled 语义分流是**刻意的产品拍板**（不是漂移）：单步 fallback
    错收取消的最坏结果是用户再说一次（`action_plan._finalize` 照收 LLM 自报）；多步链
    里幻觉取消会当场杀链（j03 实锤），故本路径把「原话无否定语素的 cancelled=true」
    ...
    if verb and verb in _ap.VERB_BY_NAME:
        ...
        if (spec.kind == _ap.EXEC and raw.get("cancelled") is True
            ...
            # 幻觉取消镜像闸（2026-08-08 任务2，j03 实锤）：cancelled=true 以原话有否定
        ...
        if verb == "curate.sync_updates":
            # sync 主题闸（2026-08-08 探针 v5 病灶1：b12/k08/j03/d05 族混合句被 understand
            # 磁吸到 sync——when_zh 文案层收益已尽，A/B 两版文案真机均 6/6 磁吸，上机械
            # 镜像闸）：sync 不过滤主题，会把所有疑似新增都入库。与点名源校验同属机械护栏；
            # validate→repair 链给模型一次改判机会。
            # 2026-08-08 任务2 起按**分句作用域**分流（k11 实锤：「检查CELLxGENE更新，有新增
        ...
        if verb == "curate.search_online":
            # 出处之二（2026-08-06 批C，产品所有者决策；依据是三模型 A/B 实测：干净链路上
...
# ==========================================================================================
# 图节点（2026-08-07 换装：模块级函数 + Context 注入 + reducer 增量返回；不再是闭包）
# ==========================================================================================
...
# ---------------------------------------------------------------- 混合诉求机械意图闸（2026-08-22 ov1-wf1 批）
#
...
#: 既有 live 用例实锤：b04/b12/k02/l07 族全部因此豁免）。
#: v2（ov1-fix1c）：否决从全句级收窄为**子句级**——调用方逐子句调用，跨子句的独立
...
#: v1-2026-08-21 = ov1-wf1 初版，**全句级**判定（入库链否决也是全句级）；
#: v2-2026-08-22 = ov1-fix1c 子句级判定：动作/检索信号须落在**不同子句**才闸，
...
def _hybrid_intent_gate(text: str) -> bool:
    ...
    v2（ov1-fix1c）：全句级 → 子句级。同子句双信号不闸（「帮我找可下载数据」是检索句）；
...
def _hybrid_required_capabilities(text: str) -> list[dict[str, Any]]:
    """混合句的**能力账**（ov1-fix1c 修复1）：闸命中时产出本句要求的能力清单，
...
def _run_route_consensus(model: Any, prompt: str,
    ...
    """分流共识（2026-08-17 M1 钉死点 1：共识帽 + 机械兜底）：**并行** 2 次独立调用
    ...
    ——votes 是**全部原始投票**（温度/原文/解析结果），由节点落 trace 附加字段（M4 错误
    分析的第一现场）。`usage_sink`（2026-08-17 codex 评审 中4）：给了就把每票的缓存
    ...
    def _vote(temp: float) -> dict:
        ...
        t0 = time.monotonic()  # M4 波2：票级延迟进 vote 实录（节点代发 llm_call 用）
        try:
    ...
    # 的唯一最高」都不是共识（2026-08-17 codex 对抗评审 高2：[invalid, invalid, action]
...
def _notify_route_verdict(ctx: Any, route: str) -> None:
    """cr1（设计 §4.2）：verdict hook **只做 abandoned/lazy 标记，不发射**（r3 P1-4：
...
def route_consensus(state: _AgentState, *, runtime: Any) -> dict:
    """环首分流共识节点（2026-08-17 M1；nl1 转正后常驻入图）：无条件进环后的第一次 LLM
    ...
    2026-08-22 ov1-wf1：**混合诉求机械预闸**——`_hybrid_intent_gate` 检出同句同时含
    ...
    # 混合诉求机械预闸（2026-08-22 ov1-wf1）：动作信号 ∧ 检索信号同句 → 跳过 LLM 投票
    # 直接 general（全能地板，两半都能办）。投票留痕如实记空（一票未发）。
    # ov1-fix1c：闸命中同时产出**能力账**（required_capabilities）写 state/trace——
    ...
    # 现场段用分流专用构造器（2026-08-17 codex 评审 高3）：与 understand 同上下文面，
    ...
    # M4 波2（设计 §6-3/6-4）：全部原始投票 stash 进 turn 暂存袋（route_decision 的
    ...
    # cr1（设计 §4.2）：verdict hook 经 `_notify_route_verdict` 调用（只做标记不发射；
...
def _scoped_understand_face(state: "_AgentState"
    ...
    """非 rescue 回合的 understand/repair 套件收窄面（nl1 转正后常驻）：返回
...
def understand(state: _AgentState, *, runtime: Any) -> dict:
    ...
    started = time.monotonic()
    ctx: _AgentContext = runtime.context
    # prelim1（2026-08-16）：长 LLM 节点的「即将开始」即时事件——verb="node"，
    ...
    # scoped 路由（2026-08-17 M1；nl1 转正后常驻）：非 rescue 时按 state.route_scope 收窄
    ...
    if (resolved is None and getattr(ctx, "retrieval_provider", None) is not None
        ...
        try:
            ...
            # 形状——rule_match_summary 本身 fail-open，双保险（设计 §4.4）。
    ...
    # P0-1：只注入同分区（同账户 + 同端点指纹）的成功样例——跨账户/跨端点的原话不进 prompt。
    ...
    # eng-b4（设计 §3.3）：课题上下文卡只进 agent prompt——主通道的 context 已带
    ...
    if rescue:
        ...
        # rescue2（2026-08-17）：未收录词清单取自原检索投影（端点注入 state.retrieval
        ...
        json_prompt += block
    usage_local: list = []
    # scoped 路由（2026-08-17 M1；nl1 转正后常驻）：套件路径的系统提示为退役后口径
    ...
    if not fb_reason:
        ...
        ok = bool(raw)
        mode = "json"
    # understand 首步同批只读消费（2026-08-22 ov1-wf1 批，复制 decide 的 mcl1 raw_batch
    ...
    # 清单可观测（codex P2-2）：原始清单/不可用原因/剔除数落 trace 附加字段
    ...
    # M4 波2（设计 §6-4）：understand 原始投票 stash 进 turn 暂存袋——
    ...
    # 禁止原地改写传入 state（图 reducer 语义，设计 §4.4）。
    return {
        "raw": raw, "mode": mode,
        "retrieval": resolved,
...
def validate(state: _AgentState, *, runtime: Any) -> dict:
    ...
    # scoped 路由的套件首步闸（2026-08-17 M1；nl1 转正后常驻；提示不是围栏——
    ...
    if violations:
        return {
            ...
            # 首步/续步没过闸时同批续步一并作回炉（下一轮 decide 带新状态重判，mcl1）
    ...
    # 同批只读续步（2026-08-14 mcl1）：decide/understand 已初筛的 raw_batch 逐个过**同一套**
    # 机械校验 + build_plan_from_raw（与首步完全同口径的双检镜像）；违规即剔除回炉（不走 repair
    # ——批量项从严，回炉的下一轮 decide 会带新状态重判），产出挂 loop_batch。
    # pbt1（2026-08-20 依赖占位批量计划 v2，设计 §2）：**占位接地**续步走两阶段——静态
...
def repair(state: _AgentState, *, runtime: Any) -> dict:
    ...
    调用，「结构化通道治槽位落空」在首次校验失败后又退回散文，正是槽位落空的病灶温床。
    2026-08-07 第三批任务1（codex P1-1，实锤 k04 r2）：complex 车道的 repair 走
    ...
    tools, name_to_verb = _get_tool_specs()
    # scoped 路由（2026-08-17 M1；nl1 转正后常驻）：与 understand 同一收窄面——套件闸打回
...
def execute(state: _AgentState, *, runtime: Any) -> dict:
    ...
    **同批只读续步**（2026-08-14 mcl1）：decide 多调用采纳、validate 过检的 `loop_batch`
    紧随本步**逐个同口径执行**（同一道形状闸/审计/实录，一步不少、顺序不变）——只读
    白名单由 `_batch_readonly_extras` 把守，写库动词/回滚/换线永不进批（cite.export 带
    占位接地可进批，2026-08-20 pbt1）；某一步失败不连坐其余独立只读步（各自如实记
    ok=False，decide 带全量结果再判）。**占位接地续步**（pbt1）：主步与续步的占位在
    本节点解析（`_resolve_placeholder_plan` 全闸助手），解析失败（resolver_error /
    dependency_unavailable）不执行、不记步、留 trace。"""
    # prelim1（2026-08-16）：tool_start 即时事件回调（非流式/rescue 为 None，自然静默）。
    ...
    # 重问写步落账（2026-08-07 B 方案）：本步若是 decide 放行的重问写动词，无论成败都
    ...
    any_failure = False
    # 任务3 埋点同口径（2026-08-17 codex 评审 中4）：环内工具发起的 LLM 调用（rerank 的
    # 独立改写）经 ctx 里的 usage_sink 回本节点增量——不进 plan.steps（模型对象/用量
    # 都不落实录），只随 state.usage_ledger 进末端 llm_usage 聚合。
    usage_local: list = []
    # pbt1（2026-08-20 依赖占位批量计划 v2，设计 §3 + 施工修正）：**解析源局部化**——
    ...
    if has_ph and _slots_has_placeholder(active.get("slots")):
        # 主步占位解析（pbt1 施工修正：decide 单发续步（如 compare($1)）是真实模型自然形态
    ...
    def _trace_step(step: dict) -> None:
        """M4 波2（设计 §6-5）：**全部**动词的工具步留痕——step 实录的机器可读持久化
    ...
    def _trace_snapshot_finalize(sid: str | None, v: str) -> None:
        """M4 波2（设计 §6-5）：写动词操作后 finalize（diff 出 created/modified/deleted）
    ...
    def _run_one(act: dict) -> dict | None:
        ...
        供 pbt1 批的解析源 `resolved_results` 现取 result——不进 state reducer）。"""
        ...
        t0 = time.monotonic()
        # M4 波2（设计 §6-5）：写动词进 try 前 capture（inventory + preimage 字节；
        ...
        try:
            if sp.get("needs_context"):
                ...
                # rescue2（2026-08-17）：原检索投影的未收录词一并下入——择优闸机械比对
                # dropped_terms（改写句里消失的未收录词）供如实披露，不采信 LLM 自报。
                # M2（2026-08-17）：chat_model 下入供 rerank 的独立改写调用——ctx 只被
                # run 消费、不落 steps（模型对象不可 JSON 序列化），注入安全。
                # M3 溯源（2026-08-17 codex 评审 中6）：utterance 下入供批次 query_raw
                # （契约 = 本轮用户原话）；中4：usage_sink 下入收改写调用的缓存用量。
                # rb1（2026-08-17）：steps 实录下入供 curate.rollback 的机械闸现定回退
                ...
                result = sp["run"](slots, root)
            # run() 出口的**形状闸**（2026-08-06 schema 加固第一刀）：返回契约模型
            ...
            any_failure = True
            # M4 波2：失败也 finalize（半写现场如实 diff）+ 工具步留痕（ok=False）。
        ...
        new_steps.append(step)
        # M4 波2：成功步 finalize 快照（写动词；sid=None 时零操作）+ 工具步留痕。
        _trace_snapshot_finalize(sid, v)
        _trace_step(step)
        if sp.get("observation"):
            # db_status 的既有契约（2026-08-03 agent2）：产出同时挂 plan.observation 与
    ...
    # 解析后再过全闸）：紧随主步逐个执行。
    # 2026-08-15 gr2 **批内熔断**（codex 对抗评审 Critical-2）：初筛对的是**批前**状态，
    ...
    for loop_idx, extra in enumerate(list(state.get("loop_batch") or [])):
        ...
        if extra.get("deferred"):
            # pbt1 延迟通道（设计 §2 解析后阶段，与主步共用 `_resolve_placeholder_plan`）：
            ...
            if skip_reason:
                # 失败四分（设计 §3）：resolver_error / dependency_unavailable / cancelled /
            ...
            act = plan2
            if resolve_note:
                # 原始占位串进 trace（设计 §3）：解析成功也留一条「占位 → 真实 uid」的
            ...
            if espec is None:
                continue
        # fix-exec M4：熔断剔步**留痕不留步**——不执行、不记步的纪律不变（行为钉在案），
        # 但 trace 必须如实交代「这一步为什么没跑」：decide/validate 已宣称「同批采纳
        # 执行/一并过检」，执行侧零留痕会让三层 trace 互相矛盾、事后无从定位。
        # deferred 项已解析（act 是解析后 plan），闸对**最终实参**重过（设计 §2 同口径）。
        ...
        if ev in _NETWORK_LOOP_TOOLS \
            ...
            new_trace.extend(_trace_entry(
                ...
                False, time.monotonic()))
            continue
        # rank/rerank 预算批内复查（2026-08-17 codex 对抗评审 高1，gr2 批内熔连同哲学）：
        ...
        if ev == "rerank" and _rerank_used(live_steps) >= MAX_RERANK:
            new_trace.extend(_trace_entry(
                ...
                False, time.monotonic()))
            continue
        # pbt1：环内四工具预算批内复查（仅占位接地续步可达——无占位的四工具进不了批；
        # 设计 §2「解析后实参过全部政策闸」的预算维度，与 rank/rerank 复查同哲学）。
    ...
    # 逃生口换线（2026-08-17 M1；nl1 转正后常驻）：本批真跑成的 route.request 步把
    # state.route_scope 改写为目标路线——下一轮 decide 按新套件装面。以**实录结果**为准
    # （不采信计划），同批多步换线取最后一个（mcl1 已把 route.request 挡在同批消费外，
    # 正常只会有一个）。
    for s in new_steps:
...
def decide(state: _AgentState, *, runtime: Any) -> dict:
    ...
    幻觉工具名 / 散文 / 全通道没拿到 = 非法；**多 tool_call 取第一个**（2026-08-09
    病灶猎杀批：DeepSeek 不遵守 parallel_tool_calls=False，微探针实证 decide 不可读
    17/17 全是多调用、第一个调用 17/17 合法续步——批量调用是「规划先行」，后续动作
    循环会带新状态再判断，吃第一个不吞事；留痕「一次给了 N 个」拼进 trace）。
    **2026-08-14 mcl1 起同批只读消费**：被采纳应答的第 2..N 个调用里，只读白名单
    ...
    **仅调用本身抛异常**才跌散文 JSON 兜底。主通道拿到非法应答 → **重问一次**
    （2026-08-08 探针 v4/w2a3 病灶 b08/l07/k10：非法一刀切停环是 K/L 长链断裂的最大
    单因）。重问后的写动词处置：2026-08-08 初版是只读闸（写动词机械拒绝，codex P0-2
    「不多问」本意的细化）；**2026-08-07 产品拍板改 B 方案——放行 + 强制核销复核**
    （只读闸稳定截断 g04/l07 型「首答散文→重问答对但首选写动词」的长链：误杀代价每次
    ...
    2026-08-08 核销硬闸（探针 v3 病灶 b12/g04 第二刀）：finish 必附 completion_report，
    ...
    2026-08-08 探针 v4 核销闸升级（`_completion_report_veto` 三形态，反馈文案按形态区分）：
    ...
    started = time.monotonic()
    ctx: _AgentContext = runtime.context
    # 复杂度路由（2026-08-07 第三批任务1）：complex 车道的 decide 走专用档——首答/非法重问/
    # 否决回灌是**同一次决策会话**，三个调用点统一这一个 model，不中途换脑。
    model = ctx.decide_model or ctx.chat_model
    model_name = ctx.decide_model_name if ctx.decide_model is not None else ctx.model_name
    lane_note = "长链档｜" if ctx.decide_model is not None else ""
    usage_local: list = []   # 任务3 埋点：本节点全部 LLM 调用的缓存用量（return 增量带出）
    steps = list(state.get("steps") or [])
    if len(steps) >= MAX_STEPS:
        # 2026-08-15 gr2 **到顶结算闸**（codex 对抗评审 Critical-1）：旧实现到顶一律
        ...
        # fix-exec M1：再追加**失败污染**检查——pending 规则 1 把失败步「碰过」点名源
        ...
        try:
            ...
            settled = False
        # exec-gates M2（2026-08-15 ta2 exec2）：清单没建成（checklist_unavailable）时
    ...
    # 连续失败处置二分（2026-08-06 失败语义批；2026-08-08 探针 v4 病灶 k08 改）：
    ...
    # ② 其余二连败（形状闸等非网络码）：2026-08-08 约束放松批（codex 裁决 K3）起不再硬停，
    ...
    # scoped 路由（2026-08-17 M1；nl1 转正后非 rescue 恒走套件面）：按 state.route_scope
    ...
    # rank / rerank 预算耗尽注入段（2026-08-17 M2，同口径；未用满恒空段）。
    ...
    # 环内结果处理四工具预算耗尽注入段（2026-08-18 四工具批，同口径；未用满恒空段）。
    ...
    # 逃生口机会用完注入段（2026-08-17 M1，同口径；rescue/未用完恒空段）。
    ...
    # 未决事项机械提示段（探针 v6；提示不是闸，三条规则都无命中时整段不出现）。
    ...
    batch_holder: list[list] = []  # 被采纳应答的全部 tool_calls（同批只读消费的原料，mcl1）
    ncalls_holder: list[int] = []  # M4 波2：被采纳应答的调用个数（batch_emission 留痕用）

    def _classify(ans: Any) -> tuple[str, Any]:
        """应答分诊 + 多调用留痕（2026-08-09 病灶猎杀批：DeepSeek 不遵守
    ...
    if kind == "invalid" and answer is not None and not fb_reason:
        # 非法应答重问一次（2026-08-08 探针 v4/w2a3 病灶 b08/l07/k10：散文拒答/幻觉
        # 工具名一刀切停环，是 K/L 长链断裂的最大单因）。answer=None（全通道异常）或
        # fb_reason 非空（已跌 JSON 兜底）不再问——兜底档是通道异常时的保命档，不多问。
        # 重问后的写动词：2026-08-07 产品拍板 B 方案——放行 + 强制核销（下方落账），
    ...
    # 处置对称化——l07 r3 实锤：可修的校验违规（quoted 非逐字、keywords 无出处、sync 主题
    ...
    # P0-1（2026-08-09 codex 对抗评审头条）：「拒绝当前提议」绝不等于「整条请求完成」。
    ...
    if kind == "finish":
        while kind == "finish":
            ...
            # 任务4 聚合否决：文本闸/重问写步闸/清单对账/pending 硬闸一次算全（首条=旧口径
            ...
            if vetoes == 2:
                ...
                if _forced is not None:
                    ...
                    # ov1-fix1c：清单无可指项后，查混合诉求能力账——首个核账动词在环面
    ...
    if kind == "finish":
        ...
        # 重问写动词**放行 + 强制核销**（2026-08-07 产品拍板 B 方案，取代只读闸）：
    ...
    # 多调用同批只读消费（2026-08-14 mcl1 批）：被采纳应答若一次给了 ≥2 个调用，
    ...
    if batch_extras:
        ...
        if any(_raw_has_placeholder(e) for e in batch_extras):
            # pbt1（2026-08-20）：含占位接地的批量——写动词（cite.export）也随批，措辞
            ...
            multi_note = (f"模型一次给了 {n_calls} 个调用，其中 {len(batch_extras)} 个只读且"
                          "互相独立，同批采纳执行"
                          + (f"，其余 {rest} 个回炉再判" if rest > 0 else "") + "；")
    # M4 波2（设计 §6-6）：多调用同批消费留痕——模型一次给了几个/同批采纳几个/回炉几个
    # （非 loop 的多调用：第 2..N 个不消费，如实记 dropped）。pbt1 批 additive：
...
def narrate(state: _AgentState, *, runtime: Any) -> dict:
    ...
    usage_local: list = []   # 任务3 埋点：narrate 的 LLM 汇报调用缓存用量
    ...
    if (len(steps) <= 1 and spec and spec.get("report") and plan.get("observation")
        ...
        if report and discard_reason:
            ...
            # 措辞保持中性（R2-1 P2-1）：后检是模式匹配，分不清「真谎称」与「措辞像
        ...
        # 用户面对空白，连「没做」都不知道（2026-08-06 批B P2-2）。
    ...
    if state.get("truncated") and plan.get("report_zh"):
        if state.get("truncated_settled"):
            # 2026-08-15 gr2 到顶结算闸：可机械核验事项全结清时不缀「没做完」——
    ...
    # exec-gates M5（2026-08-15 ta2 exec2）：点名单源场景下 sync 空槽 = 按全部在线源同步
    ...
    # 任务4：清单未决的如实标注（finish 正常过闸时对账必为空、不附注；早收/fail-safe
    ...
    if discarded_report is not None:
        # 批B P2-3：被机械后检弃用的 LLM 汇报原文留痕（trace 附加字段，前端不渲染），
...
def _get_graph() -> Any:
    """模块级编译单例（2026-08-07 换装刀 1）：首次调用构建 + compile 一次，之后复用；
    ...
    if _COMPILED_GRAPH is None:
        with _GRAPH_LOCK:
            if _COMPILED_GRAPH is None:
                ...
                # scoped 路由（2026-08-17 M1；nl1 转正后常驻）：环首分流共识节点——
...
def plan_with_agent_events(
    ...
    *,
    ...
    search_date_from: str = "",
    search_date_to: str = "",
    # cr1（并发分流与确定性 RAG 策略，设计 §4.4）三缝透传——缺省 None/"" = 今天逐位
    # 不变（既有测试的 monkeypatch seam 全保）。
    retrieval_provider: Any = None,
    on_route_verdict: Any = None,
    route_extra_zh: str = "",
    # eng-b4（2026-08-22，设计 §3.3）：课题上下文卡——独立字段透传进图状态，
    ...
    `decide_model`（2026-08-07 复杂度路由，codex P2-5）是 decide/repair 档的**显式注入缝**：
    给了就用它（不再按 env 自建），不给且车道为 complex 且配置了 LLM_MODEL_COMPLEX 才自建。
    `principal`（2026-08-10 P0-1）：成功经验库分区主体（会话账户 id；空 → anonymous）。
    ...
    if not agent_available():
        # M4 波2（设计 §6-7）：收尾留痕——agent 不可用（fail-soft，OFF 零操作）。
    ...
    if chat_model is None:
        ...
        if not ok:
            # M4 波2（设计 §6-7）：收尾留痕——大模型不可用。
            _te.emit_finish_reason(kind="unavailable")
            raise AgentUnavailable(f"大模型不可用（{reason}）——agent 路径与单次分类同闸。")
        chat_model = _build_chat_model(config)

    # 复杂度路由（2026-08-07 第三批任务1；定标与病灶数据见 docs/工作记录/任务记录_2026-08-07_第三批.md §1）：
    ...
    # 2026-08-08 任务1（V4 迁移）：新增 `LLM_COMPLEX_THINKING`/`LLM_COMPLEX_EFFORT` 旋钮——
    ...
    graph = _get_graph()
    # P0-1：成功经验库分区键——principal（空 → anonymous）+ endpoint_fp（config 派生，key 不参与）。
    ...
    if final.get("violations"):
        # M4 波2（设计 §6-7）：收尾留痕——护栏修复预算耗尽，plan 未产出。
    ...
    if not plan:
        ...
        _te.emit_finish_reason(kind="unavailable")  # M4 波2（设计 §6-7）
    ...
    # 任务3 缓存埋点汇总（2026-08-08）：usage_ledger 非空才写 plan（FakeModel/老 provider
    ...
    # M4 波2（设计 §6-7）：正常收尾留痕——completed / truncated / truncated_settled
...
def plan_with_agent(
    ...
    *,
    ...
    on_route_verdict: Any = None,
    route_extra_zh: str = "",
    # eng-b4（2026-08-22，设计 §3.3）：课题上下文卡，与 plan_with_agent_events 同义。
    artifact_context: str = "",
) -> tuple[dict, list[dict]]:
    """`plan_with_agent_events` 的无回调薄封装（2026-08-03 agent1b 起）——
