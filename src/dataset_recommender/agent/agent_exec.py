...
4. **decide 迁 tool-calling 主通道**：绑 5 个 loop 工具 + `finish` + `unsupported_next_step`
   ...
   逐位一致）、幻觉工具名/散文=非法=停环；**多 tool_call 取第一个**（问题
   猎杀批：DeepSeek 不遵守 parallel_tool_calls=False，多调用是常态且第一个调用实证
   17/17 合法，循环会再判断后续，不吞事）——** 起追加同批只读消费**：
   第 2..N 个调用里只读白名单（check_updates/db_status）且互相独立、逐个过全套机械闸的
   续步随首步同批执行（验证 A 类批量 2..N 合法率 100%，n=371）；写动词/未知工具名
...
# M4 波2（可追溯性，设计 §6）：trace 挂钩——recorder 经 contextvars 传递
...
class _AgentState(TypedDict, total=False):
    """langgraph 图状态。六个累积型 key 带 reducer（换装； B 方案 +1；
     任务3 +usage_ledger）：
    ...
    raw_batch: list[dict]     #: 多调用的同批只读续步 raw（起 decide 产出； 起 understand 首步同口径产出 → validate 消费后即清）
    ...
    search_date_to: str
    route_scope: str              #: scoped 路由（M1）：route_consensus 的共识结果 ""/"search"/"action"/"general"；route.request 步放行后由 execute 改写（覆写语义）
    ...
    artifact_context: str        #: 课题上下文卡（engagement B4，设计 §3.3）：独立字段，只进 route_consensus/understand 的 prompt 作结构化上下文块；缺省空串 = 与旧版逐位一致
...
class _AgentContext:
    ...
    decide_model/decide_model_name/decide_lane（第三批任务1，复杂度路由）：
    ...
    route_extra_zh: str = ""
    # （设计 §3.3）：课题上下文卡——understand/route_consensus 的
...
def _build_chat_model(config: LLMConfig) -> Any:
    ...
    DeepSeek V4 思考旋钮（任务1）：config.thinking / reasoning_effort 非 None
...
#: `LLM_COMPLEX_EFFORT` 合法枚举（对 DeepSeek V4 验证 400 报文列举）。
...
def _artifact_context_block_zh(artifact_context: Any) -> str:
    """课题上下文的结构化注入块（engagement B4，设计 §3.3）：独立字段、
...
def _context_zh(utterance: str, *, has_results: bool, result_total: int,
    ...
    上下文、不是用户的话，位置必须如实分开。
    `artifact_context`（engagement B4）与 examples 同位的背景上下文块
...
def _route_retrieval_zh(retrieval: Any) -> str:
    """route_consensus 专用的规则匹配概览（M1 诚实不变量，验证 高3）：
...
def _route_context_zh(utterance: str, *, has_results: bool, result_total: int,
    ...
    """route_consensus 的现场段（M1）：与 `_context_zh` 同上下文面，两个刻意
    差别——检索概览用 `_route_retrieval_zh`（不含结果集标题，诚实不变量）；原话由调用方
    自拼「用户原话」尾段，本函数不附带（避免 `_context_zh` 自带的「用户这一句」造成
    原话进 prompt 两次）。 转正后 route_consensus 常驻环首，本函数是其唯一现场段。"""
    if has_results:
        ctx = f"当前屏幕上已经有一批检索结果（共 {int(result_total)} 条命中）。"
    else:
        ctx = "当前屏幕上**还没有**检索结果。"
...
def _raw_from_message(message: Any, name_to_verb: dict[str, str]) -> dict[str, Any]:
    ...
    **多 tool_call 取第一个**（问题猎杀批，取代 换装的「多调用
    机械拒绝」验证 旧策）：DeepSeek 验证**不遵守** `parallel_tool_calls=False`
    ——decide 不可读 17/17、understand「no_tool_calls」跌兜底 8/8 全是模型一次回了
    ≥2 个调用，且微验证实证**第一个调用 17/17 是合法续步**（模型的批量调用是「规划
...
def _usage_record(answer: Any, node: str) -> dict | None:
    """从 AIMessage 抠 DeepSeek 磁盘缓存用量（任务3 埋点）。
...
def _invoke_text_with_continuation(
    ...
    *,
    ...
    """文本应答调用 + **截断两段式续写**（任务2 有界自愈重试器的唯一增量）。
...
def _tool_choice_rejected(exc: Exception) -> bool:
    ...
    deepseek-reasoner， 验证）对 `tool_choice="required"` 回 400，报文点名
...
def _invoke_tool_channel_impl(
    ...
    *,
    ...
    """`_invoke_tool_channel` 的本体（M4 波2 拆壳）：返回五元组
...
def _invoke_tool_channel(
    ...
    *,
    ...
    """工具调用通道的**统一三级实现**（换装收口：understand/repair/decide 共用）。
    ...
    M4 波2（设计 §6-3）：本壳在 impl 返回后统一发一次 trace llm_call
...
#              的显式契约，工具不自己回头摸全局。
#
# 返回形状闸（schema 加固第一刀）：各动词的返回契约模型登记在
...
#
# 写操作只有 search_online / sync_updates 两个，依据是产品侧 的全自动化预先
...
#: 但 起改走独立 MAX_ROLLBACK 小额预算，防 ping-pong 而不牺牲恢复出口。
...
#: 换词重检独立预算（检索工具化 Phase 1）：search.rerun 只读、不写库，但每跑一次
...
#: rank / rerank 独立预算（scoped 路由批 M2，与 search.rerun 预算同哲学：
...
#: 环内结果处理四工具的独立预算（四工具批，与 rank 预算同哲学：提示不是
#: 围栏，代码是）：
...
#: 逃生口预算（scoped 路由批 M1 钉死点 2）：每轮请求至多这么多次
...
#: 验证 v4 问题 k08 起二分：联网二连败 → **联网暂停**（整族禁提联网工具）。
#: 约束放松批（验证决定 K3）再二分：非网络二连败不再硬停环——旧实现是
...
#: 联网暂停的 prompt 机械注入段（验证 v4 问题 k08）：联网二连败后 decide 照常
...
#: 「先新后旧」注入段（验证 v5 问题4：k03/k04 里重试挤占 MAX_STEPS 预算，
#: 没做过的新事反被截断）——存在失败步时提示优先推进新事项。**纯 prompt 劝导，无机械闸**：
#: 失败重试的放行是问题 k03 的刻意设计，本段只排先后、不禁止。
_FAILED_STEP_BLOCK_ZH = (
    "\n----- 有步骤失败时的次序 -----\n"
    "先把没做过的**新事**做完；重试失败的事最多一次，放在最后。"
)

#: 写步预算耗尽的 prompt 机械注入段（约束放松批 B）：提示不是围栏，
...
#: 换词重检预算耗尽的 prompt 机械注入段（检索工具化 Phase 1，同写步预算哲学）：
...
#: rank / rerank 预算耗尽的 prompt 机械注入段（M2，同 search.rerun 预算哲学）：
...
#: 环内结果处理四工具的预算注入段（四工具批，同预算哲学；未用满恒空段）。
...
#: 逃生口机会用完的 prompt 机械注入段（M1，同预算哲学）：decide 面同时把
...
#: rescue（检索救回）回合的 understand 注入段（检索工具化 Phase 1 初版；
#: rescue2 放宽为动态段）：双壳 prompt 尾部注入——面收敛不是纯提示：
...
def _write_steps_used(steps: list[dict]) -> int:
    ...
    每一步都可能已产生副作用（sync 中途失败前可能已写过部分来源的批次文件）。
     豁免（验证-5）：`search_online` 的 **network_error**
...
def _write_records_used(steps: list[dict]) -> int:
    """已写入条数（**从 steps 真实结果现算**，验证）：search 取 record_count、
...
def _failed_tool_ban(steps: list[dict]) -> frozenset[str]:
    ...
    （验证决定 K3）；联网二连败不在此列——网络在抖时整族都该停，走
...
def _failed_tool_ban_block_zh(banned: frozenset[str]) -> str:
    ...
    文案如实（验证）：两个**不同**动作各败一次不许说成「连续失败两次」——
    那是同一动作连败两次的专属表述。"""
    names = sorted(
...
def _is_network_call(verb: str, source: Any) -> bool:
    """该次调用是否触网（验证）：search/sync 恒触网；check_updates 按
...
def _rescue_disclosure_zh(rewrite: str, n_after: int | None, dropped_terms: list) -> str:
    """rescue 采纳的**确定性披露句**（rescue2）：丢弃词/改写词/命中数全部
...
def _loop_search_rerun(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """换一组查询词把本地库重新检索一遍（检索工具化 Phase 1，机械择优闸）。
    ...
    rerank_audit 关——防「审核改写→重搜→再审核」循环）。采纳与否由机械闸裁定，不靠 LLM 自评：
      ...
      ③ 否则采纳——**命中 0 条同样采纳**（用户拍板：条件变更重检的空结果就是
    ...
    **消失**的未收录词（子串比对，双向都逐字来自真实串），附 `disclosure_zh` 确定性
    ...
    nl-A（挂账，择优闸与屏口径一致）：n_before/n_after 保持**择优闸口径**
...
#: scoped 路由提示词的内置最小降级版（M1）：prompts/ 下的 .md 文件即真源，
...
def _rewrite_query(model: Any, query: str,
    ...
    rewritten=False 如实标注，不靠 LLM 自评。`usage_sink`（验证 中4）：
...
def _top_digest(rows: Any, n: int = 3) -> list[dict[str, Any]]:
    ...
    dataset_uid（批补：同批依赖占位的解析源，`$<N>.top[<i>].dataset_uid`）/
...
def _loop_rank(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """裸新检索（scoped 路由批 M2）：以 query 跑**标准 RAG 管线**（与
    ...
    sources = ctx.get("search_sources")
    # （设计 §10.1）：rank 与 search.rerun 同口径——必须吃同一份
    ...
    if display:
        ...
        batch = {
            "kind": "rank",
            "label": query[:20],
            # query_raw = 本轮用户原话（M3 契约； 验证 中6：曾张冠李戴
    ...
    return {
        ...
        # 原样透传（曾被误当 mapping dict() 强转——有约束的查询必炸， run2 复盘修）。
...
def _loop_rerank(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """坏 query 优化重检（M2）：**独立 LLM 改写调用**（独立上下文 +
    ...
    display = slots.get("display") is True
    sources = ctx.get("search_sources")
    # （设计 §10.1，与 _loop_rank 同纪律）：坏 query 改写只换检索句、
...
def _loop_compare_datasets(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """对比两个数据集（四工具批）：确定性字段 diff（`compare.diff_items`，
...
def _sanitize_uids_slot(value: Any, limit: int = 20) -> list[str]:
    """cite.export 的 uids 槽清洗（批，设计 §4）：列表 / 单字符串 →
...
def _loop_fair_check(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """对单个数据集做 13 项 FAIR 元数据自检（四工具批）：包
...
LOOP_TOOLS: dict[str, dict[str, Any]] = {
    "curate.db_status": {
        ...
        # B1a（调研六候选批）：「看看库里多少条」被并入检查步的问题（k03）——描述层锚定独立事项。
    ...
    # search.rerun（检索工具化 Phase 1）：「换词重检」工具化——本地管线（不触网，
...
def _loop_route_request(slots: dict, root: Path, ctx: dict | None = None) -> dict:
    """逃生口元动词（M1）：如实记录换线请求——真正的路线切换由 execute
...
# 逃生口注册（M1； 转正常驻）：所有套件的 decide 面都含它；readonly 但
...
def _execute_detail_zh(spec: dict, result: dict) -> str:
    ...
    # 环内结果处理四工具（四工具批）：note_zh / comparison_zh 由工具按真实
...
#: 观测设施自身故障的留痕纪律（exec2，审计 cross-trace D1-D4；与
...
def _audit_loop_tool(root: Path, verb: str, slots: dict, ok: bool, note: str,
    ...
    「写入条数」可记，字段整个省略——恒 0 会让账本统计口径失真（验证坐实）。"""
    try:
...
# ==================== 成功经验 few-shot 库（五机制批；Vanna auto_train 式自学习回路） ====
...
def _maybe_record_success(root: Path, utterance: str, plan: dict, steps: list,
    ...
    """成功收尾机械追加一行进**候选池**（起不直接进正式库——用户在记忆模块
...
def _load_success_examples(root: Path, utterance: str,
    ...
    """按关键词重叠（共享二元字组数）从账本检索 top-N 相似成功样例。任何异常 → 空表。
     ...
     分区（验证）：principal 非 None 时只取**同分区**行
...
def _step_projection(step: dict, *, reasked_write: bool = False) -> dict:
    ...
    `reasked_write=True` 时打标（B 方案：重问后放行的写步——模型据此知道
    ...
    if kind == "db_status":
        ...
        # 检索事实投影（M2）：裁决/计数/生效条件/top digest 给 decide/narrate
        ...
        # 逃生口投影（M1）：换线结果给 decide/narrate 当事实。
        out["result"] = {
            k: r.get(k)
        ...
        # 对比投影（四工具批）：结论句 + 计数 + 降级态给 decide/narrate 当事实
...
# ==========================================================================================
# decide：多步循环的「判断下一步」（散文 JSON 版； 换装迁 tool-calling）
#
...
#: 四工具批：compare.datasets / cite.export / compat.find / fair.check 常驻。
...
#: decide prompt 的共享段落拆分（换装：规则本体一份真源，两种输出格式壳）。
...
_DECIDE_INTRO_ZH = (
    ...
    # 验证 v3 问题 b12/g04：多件事的马拉松指令做两件就 finish——判断前先核销。
...
_DECIDE_TOOLS_BULLETS_ZH = (
    ...
    "有先后依赖或会写库的动作仍一次只发一个\n"
    # 批（设计 §6 + 施工修正）：教**唯一占位引用形状** + 两个示例 + 禁令。
    # N 的编号口径 = **本轮执行序号**（1 起；集成验证 模型自然形态是「understand 先 rank、
...
_DECIDE_RULES_TAIL_ZH = (
    ...
    # 验证 v4 问题 l07：rule 6 补「检出疑似新增 ≠ 已经入库」——模型把检查发现的
    ...
    "不许拿「新增里已经有了」替系统放弃。\n"
    # 验证 v4 问题 k06：rule 7 补反向条件——「看看A有没有新数据，没有的话就检查B和C」
    ...
    # 验证决定 N3：旧句「不要搜，回 done」会把原话里**其余独立事项**一起判死刑——
...
#: 换装时与旧手抄版逐字一致； 起随 INTRO/rule 10 的**刻意修订**漂移
#: （验证 v3 问题 b12/g04 提前收工——tests/test_agent_schemas.py 的钉字同步刻意更新）。
_DECIDE_RULES_ZH = (
...
def _build_decide_tool_specs() -> tuple[list[dict], dict[str, str]]:
    """decide 的 tool-calling 工具面（换装，验证 改案）：
    ...
    **5 个 loop 工具**（检索工具化起含 search.rerun；schema 由
    ...
    - `finish`：结构化 done——「要做的事已全部完成，或剩下的做不到/条件不成立」；
      **必填** `completion_report` 参数承载逐件核销报告（起机械层消费：
    ...
    **模块加载期构建一次**（与 `_DECIDE_RULES_ZH` 同纪律）：读的是真实 LOOP_TOOLS——
    ...
    tools.append({
        ...
        "function": {
            ...
            # 必填 completion_report（验证 v3 问题 b12/g04 第二刀：可选 checklist
...
#: rescue 档 decide 工具面（检索工具化 Phase 1）：检索救回回合只给
...
# 转正：本节为**唯一**装配路径（原 OFF 既有面分支已摘除归档）。

#: 套件 = LOOP_TOOLS 注册表子集（设计 §M1：套件只能装注册表项 + 本波新登记项）。
#: 四工具批：环内结果处理四工具（compare/cite/compat/fair）同时入 search 与
...
def _understand_suite_verbs(suite: str) -> tuple[str, ...]:
    ...
    不进任何首步面：首步没有「发现路错」可言）。none 恒在（诚实的「不是执行诉求」出口）。
     四工具批：search/action 两线都装结果处理四工具（检索后追问
...
def _is_duplicate_step(verb: str, raw: dict, steps: list[dict]) -> bool:
    ...
    比对集 = 成功步 + **非 network_error** 失败的步（验证 v5 问题 f04 收窄；
    W2a4 曾一律豁免失败步，把 bad_result_shape 这类确定性失败也放去重试白烧一步）：
    - network_error 是唯一真·可重试码——失败步什么都没做成，同指纹重试放行
      （check 只有 source 槽，同源重查恒同指纹，验证 v4 问题 k03）；重试上界由连续失败
      处置二分天然兜底（再败即联网暂停/硬停）；
...
def _search_coverage_violation(obj: dict, steps: list[dict]) -> str | None:
    """搜索覆盖闸（验证 v3 问题 a06/d01：首搜成功后 decide 换措辞/加过滤再搜
...
#: finish 核销报告（completion_report）机械否决的判定词表（验证 v3 问题 b12/g04
...
#: 形态 B 依赖借口词（验证 v4 问题 k03/k08）：豁免词命中行里若同时夹带这类
...
#: 形态 A 已做声称（验证 v4 问题 b08/k01）：报告把没跑过的事标成「已做」（没跑 db_status
...
def _completion_report_veto(report: str, n_steps: int) -> tuple[str | None, str]:
    ...
    - "exempt_without_step"（验证 v5 问题2：k02/k09/k10/k11/l07 提前收工残余）：
...
def _reask_write_veto(report: str, reask_writes: list[dict]) -> tuple[str | None, str]:
    """重问写步的**强制核销复核**（策略 B 方案配套——重问后的写动词从
...
#: 未决事项机械提示的判定词表（验证 v6 问题族：7 条失败同出一辙——模型在
...
#: curate 侧补充点名表（`_named_sources_in` 第三趟， 任务4/验证）：
...
def _named_sources_in(utterance: str) -> list[str]:
    ...
    这类普通英文词，但用户原样写出全大写 ENCODE 就是点名）。
     第三批任务1 从 `_pending_hints_block_zh` 规则 1 抽出的共享真源——
...
def _pending_hints_block_zh(utterance: str, steps: list[dict]) -> str:
    """decide prompt 的「未决事项机械提示」段（验证 v6；**提示不是闸**——不拦截任何东西）。
...
_CHECKLIST_MAX_ITEMS = 8      #: 聚合回灌限幅（验证：反馈 ≤2KB，每条只带编号+截断文本）
...
def _agent_status_block_zh(state: "_AgentState", *, steps: list[dict], moratorium: bool,
    ...
    """decide 双壳恒注入的**执行状态栏**（B2 调研六候选批；epub 第2章状态栏思想）。

    三条铁律（书 2.6）：①**代码维护**——全部字段从图状态确定性现算，模型不许统计；
    ②有损投影要谨慎选维度——只放真实机械状态（验证：步数/失败/联网暂停/已搜主题/
...
def _parse_checklist(payload: Any, utterance: str) -> tuple[list[dict], int]:
    ...
    - anchor 全半角归一后必须是 utterance 的**子串**且 ≥4 字（验证：anchor 机械可验，
...
def _step_covered_sources(step: dict) -> set[str]:
    ...
    - 空 source 的 check/sync 步 → 从 result.sources 的条目还原（验证：
...
def _task_settled_by(task: dict, steps: list[dict]) -> bool:
    ...
    验证 修复：**按来源覆盖核销**——条目点了 N 个来源，就要 N 个来源
...
def _checklist_item_states(checklist: list[dict], steps: list[dict],
    ...
    """清单逐项的**代码现算**状态（调研-长程agent批 候选4：核销状态栏逐项化——
...
def _checklist_unsettled(checklist: list[dict], steps: list[dict],
    ...
    unsettled: list[dict] = []
    # 逐项豁免（验证 修订版）：declined_zh 非空只豁免**第一条** unsupported——
...
def _task_checklist_call(chat_model: Any, utterance: str,
    ...
    if not tasks:
        # 空清单一律视为失败（验证：全剔除也走这里——「dropped>0 但 err 空」
...
#: 第二次否决时 pending 缺口码 → 必做动作映射（调研-长程agent批 候选1 的
...
def _finish_veto_all(report: str, n_steps: int, reask_writes: list[dict],
    ...
    """finish 否决的**聚合口**（任务4；验证：两闸分次否决会耗尽重试额度）——
...
def _pending_violations(utterance: str, steps: list[dict]) -> list[tuple[str, str]]:
    ...
    规则 3 另有否定极性豁免（`_import_hard_request`， H1）：
    ...
    # 规则 3：检出未入库（按**来源差集**——验证 附带修复：「任一来源搜过
...
#: 规则 3 的**否定极性**豁免（H1，与 `_DENIAL_MORPH_RE` 同思路）：
...
#: 验证定标零翻转）；英文连接词（and/then/if any）暂不收——中文优先产品的已知豁口，记录在案。
...
#: 克制语素一票留 simple（验证）：带叫停/否定语义的句子即便词面分高也不进 complex——
...
def decide_lane(utterance: str) -> str:
    ...
    问题实锤（v11 chat vs v10r reasoner 逐例对照）：chat 的 K/L 失败以 **decide 断链**为主、
    understand/repair 首步误判为辅（k04 r2，验证 实锤）；reasoner 的克制类回落全在
...
def _adjudicate_decide_obj(obj: dict, state: "_AgentState", *,
    ...
    `allow_placeholders`（批）：True = 占位接地续步（`_batch_readonly_extras`
    ...
    if verb == "none":
        # 换装：none = 干净的 done。旧散文版会落入下方的婉拒路径，说出
    ...
    # rescue 面收敛闸（检索工具化 Phase 1）：检索救回回合只允许 search.rerun——
    ...
    # scoped 路由套件闸（M1； 转正后常驻；提示不是围栏——decide 的面收窄是
    ...
    # 联网暂停（验证 v4 问题 k08）：联网二连败（network_error）状态下，联网工具
    ...
    if verb == _ROLLBACK_LOOP_TOOL and _rollback_used(list(state.get("steps") or [])) >= MAX_ROLLBACK:
        return None, (f"本次请求的回滚预算已用完（每轮最多 {MAX_ROLLBACK} 次），这一步没有再执行，"
                      ...
                      "回滚预算已用完。"), ""
    # 写步预算闸（约束放松批 B）：正向写步已用满 MAX_WRITE_STEPS 次后，写工具提议
    # 机械拒绝——按 done 收尾、note 如实写「写步预算已用完」；只读工具照常放行。
    # 验证 增第二维度：写**条数**用满 MAX_WRITE_RECORDS 同样拒（步数
    # 管循环、条数管写入量）。
    ...
    # 换词重检预算闸（检索工具化 Phase 1，与写步预算同哲学）：search.rerun 已用满
    ...
    # rank / rerank 预算闸（M2，同 search.rerun 预算哲学）：已用满后再提议 →
    # 机械拒绝，按 done 收尾、note 如实点名。 转正后二动词常驻 LOOP_TOOLS，闸恒可达。
    if verb == "rank" and \
    ...
    # 环内结果处理四工具的独立预算闸（四工具批，同 rank 预算哲学）：
    ...
    if verb == "fair.check" and \
        ...
        return None, (f"本次请求已 FAIR 自检 {MAX_FAIR} 次（一次请求最多 {MAX_FAIR} 次），"
                      ...
                      f"{MAX_FAIR} 次，预算已用完。"), ""
    # 失败动作禁提（约束放松批 G，验证决定 K3）：非网络二连败后，刚失败的
    ...
    # 点名源缺槽位的确定解补位（任务2：与 validate 节点同一助手——decide 续步
    ...
    # 搜索覆盖闸（验证 v3 问题 a06/d01）：同主题换措辞/加过滤重搜——槽位变了
...
def _readonly_loop_verbs() -> frozenset:
    """只读白名单（批）：LOOP_TOOLS 注册表 `readonly=True` 的动词，**减去**
    ...
    /17 检索工具化与 scoped 路由批入注册表后即自然落进本白名单——本 docstring
    此前滞后写作「现况=check_updates/db_status」，以代码为准）。写动词
    （search_online / sync_updates）**永不**进白名单：mcf1 验证 预发写调用逐位保真
    仅 45%（参数要等前序真实结果接地、条件要等前步判定）。
    route.request（M1）虽只读也**永不**进批：它是控制面元动词——同批多调用
    里混进换线会让面切换时序不可判，换线必须独占一轮。
    compare.datasets / compat.find / fair.check（四工具批）同样只读但**永不**
    ...
    （readonly=False），天然不进白名单。
     批（设计 §1）：四工具的**缺省依赖**由「同批**占位引用**」显式接地后
...
# 不做通用路径（v1 验证），只做一条白名单受控链；两阶段校验；失败四分；解析源局部化。
...
def _placeholder_static_violations(verb: str, raw: dict, position_verbs: list[str],
                                   pos: int) -> list[str]:
    """**静态阶段**占位校验（设计 §2； 施工修正：`pos` 与 `N` 的编号口径
...
def _resolve_placeholder_plan(raw0: dict, resolved: dict[int, dict], batch_id: str,
    ...
    """ **解析后全闸**（设计 §2/§3）：占位替换为真实值 → `_validate_raw` →
...
def _batch_readonly_extras(calls: list, first_raw: dict,
    ...
    **占位接地**批量，设计 §1-§2。实测依据见 `_readonly_loop_verbs` 注释与
    ...
    - 参数键 ⊆ 该动词 decide 工具 schema 的声明键（验证实锤模型偶把 keywords/species
    ...
    for i in range(1, len(calls)):
        ...
        if _is_duplicate_step(verb, raw, seen_steps):
            dropped += 1
            continue
        # 预算闸增量裁决（验证 高1）：对原始 state 裁决时，同批已采纳
...
_STEPS_REPORT_RULES_ZH = (
    ...
    "既遂含义说；\n"
    # 验证 v8 误伤/谎称双归因：旧版没说清动词的既遂含义，模型把 search_online
    ...
    # 检索工具化 Phase 1：换词重检的既遂含义（只读重跑检索，不写库）。
    # 用户拍板：命中 0 条也采纳上屏（空结果集就是诚实答案）——adopted=false
    ...
    # 验证 v9 回归修复：旧措辞诱导模型把来源名和数字拆进两句话
...
#: 验证坐实）：「已完成下载」「下载完成」「已为你下载并保存」恰是 LLM 最自然的措辞，
...
#: 「好啦/搞定」族是 R2-1  坐实的词表外漏网语素（「下载好啦」「下载搞定」「搞定了下载」）。
...
#: 「被取消了」「完成了吗」的否定/疑问落在完成态语素**之后**（R2-1  函数级误伤 6/19
...
#: 零数量语素（验证 v8 误伤修复）：「导入0条 / 导入数量为 0 / imported_total=0」
...
#: 否认侧的判定素材说明（批A D1）：旧实现是穷举整句词表 `_WRITE_DENIALS_ZH`
...
def _denies_done_read(report: str, steps: list[dict]) -> bool:
    """汇报里是否有「读动作否认」声称（第五路机械后检， 验证 v3 问题 d07：
...
#: sync 主题闸的维度面（验证 v5 问题1）：只认「具体主题」四维的值别名——
...
#: cancelled=true 的**否定语素**机械镜像（任务2，j03 r1/r2 实锤：reasoner 在毫无
...
def _sync_gate_scope_zh(utterance: str, quoted: str) -> str:
    """sync 主题闸的**分句作用域**（任务2，k11 实锤）：主题词只有落在 sync 所引
...
def _sync_topic_elsewhere_exempt(utterance: str, raw: dict[str, Any]) -> bool:
    """sync 主题闸的**别支豁免**（任务2，k11 实锤：「检查CELLxGENE更新，有新增就
...
# **以及 online_recent / local_count**（验证 v8 误伤修复：「在线发现 12 条
...
def _report_contradiction_reason(report: str, steps: list[dict]) -> str | None:
    ...
    if _denies_done_read(report, steps):
        return "denied_read"
    # wrote 的口径（验证 v8 修复）：search_online 必须 record_count>0 才算
...
def _autofill_named_source(verb: str, raw: dict[str, Any], utterance: str) -> str | None:
    """点名源缺槽位的**确定性补位**（任务2，b08/k11 实锤）：understand 掉 source
...
def _sync_all_online_named(verb: str, raw: dict[str, Any], utterance: str) -> str:
    """点名单源场景下 sync 空槽的**如实留痕**（exec2，审计 exec-gates M5）：
...
def _named_source_violation(verb: str, raw: dict[str, Any], utterance: str) -> str | None:
    ...
    规范名逐字（验证 w2a2 豁免、 任务2 b08 升格为点名判定）：词表
    ...
    **原样写出受控规范名**（全大写 ENCODE）就是点名——b08 实锤：understand 槽位落空时
    ...
    if not filled:
        ...
        if verb in _NAMED_SOURCE_OPTIONAL_FILL_VERBS:
            return None
        # 「动词本身不用换」（任务2 k11 实锤）：repair 收到「source 没填」后
...
def _ungrounded_keyword_tokens(keywords: Any, utterance: str) -> list[str]:
    """keywords 里**无法在接地语料里找到出处**的 token 清单（全接地 → 空表）。
     ...
     批A D5（集成）：原话「看看有没有什么新数据，有的话拿回来」零主题词，
...
def _step_grounding_texts(steps: list[dict] | None) -> list[str]:
    """已完成步骤**真实结果**里可供 keywords 接地的文本（decide 续步的出处之二， 批C）。
...
def _validate_raw(raw: dict[str, Any], utterance: str,
    ...
    （验证裁决落地——此前此处私拷一份还自称与 `_finalize`「口径
    一一对应」，验证实锤两路径已漂移）。本函数只叠加多步执行专属闸：点名源 /
    ...
    if verb and verb in _ap.VERB_BY_NAME:
        ...
        if (spec.kind == _ap.EXEC and raw.get("cancelled") is True
            ...
            # 幻觉取消镜像闸（任务2，j03 实锤）：cancelled=true 以原话有否定
        ...
        if verb == "curate.search_online":
            # 出处之二（批C，策略；依据是三模型 A/B 验证：干净链路上
...
# ==========================================================================================
# 图节点（换装：模块级函数 + Context 注入 + reducer 增量返回；不再是闭包）
# ==========================================================================================
...
def _run_route_consensus(model: Any, prompt: str,
    ...
    """分流共识（M1 钉死点 1：共识帽 + 机械兜底）：**并行** 2 次独立调用
    ...
    分析的第一现场）。`usage_sink`（验证 中4）：给了就把每票的缓存
    ...
    # 的唯一最高」都不是共识（验证 高2：[invalid, invalid, action]
...
def _notify_route_verdict(ctx: Any, route: str) -> None:
    """（设计 §4.2）：verdict hook **只做 abandoned/lazy 标记，不发射**（r3 ：
...
def route_consensus(state: _AgentState, *, runtime: Any) -> dict:
    """环首分流共识节点（M1； 转正后常驻入图）：无条件进环后的第一次 LLM
    ...
    # 现场段用分流专用构造器（验证 高3）：与 understand 同上下文面，
    ...
    # （设计 §4.2）：verdict hook 经 `_notify_route_verdict` 调用（只做标记不发射；
...
def understand(state: _AgentState, *, runtime: Any) -> dict:
    ...
    tools, name_to_verb = _get_tool_specs()
    # rescue 面收敛（检索工具化 Phase 1）：检索救回回合首步只允许
    ...
    # scoped 路由（M1； 转正后常驻）：非 rescue 时按 state.route_scope 收窄
    ...
    # 成功经验 few-shot（五机制批）：从 `.userdata/curate_examples.jsonl` 按
    ...
    # （设计 §3.3）：课题上下文卡只进 agent prompt——主通道的 context 已带
    ...
    if rescue:
        ...
        json_prompt += block
    usage_local: list = []
    # scoped 路由（M1； 转正后常驻）：套件路径的系统提示为退役后口径
    ...
    # 清单可观测（验证）：原始清单/不可用原因/剔除数落 trace 附加字段
...
def validate(state: _AgentState, *, runtime: Any) -> dict:
    ...
    # rescue 面收敛的机械闸（检索工具化 Phase 1；提示不是围栏）：检索救回回合
    ...
    # scoped 路由的套件首步闸（M1； 转正后常驻；提示不是围栏——
    ...
    # （依赖占位批量计划 v2，设计 §2）：**占位接地**续步走两阶段——静态
...
def repair(state: _AgentState, *, runtime: Any) -> dict:
    """自我修正（≤1 次）：把 violations 原样喂回，让 LLM 在知情的前提下重答。
     换装起同样走结构化工具通道（共享助手 auto 档）——旧版是无 bind 的散文
    调用，「结构化通道治槽位落空」在首次校验失败后又退回散文，正是槽位落空的问题温床。
     第三批任务1（验证，实锤 k04 r2）：complex 车道的 repair 走
    ...
    tools, name_to_verb = _get_tool_specs()
    # scoped 路由（M1； 转正后常驻）：与 understand 同一收窄面——套件闸打回
...
def execute(state: _AgentState, *, runtime: Any) -> dict:
    ...
    **同批只读续步**：decide 多调用采纳、validate 过检的 `loop_batch`
    ...
    # 重问写步落账（B 方案）：本步若是 decide 放行的重问写动词，无论成败都
    ...
    def _run_one(act: dict) -> dict | None:
        ...
        供 批的解析源 `resolved_results` 现取 result——不进 state reducer）。"""
        ...
        try:
            if sp.get("needs_context"):
                ...
                # M3 溯源（验证 中6）：utterance 下入供批次 query_raw
                ...
                result = sp["run"](slots, root)
            # run() 出口的**形状闸**（schema 加固第一刀）：返回契约模型
    ...
    for loop_idx, extra in enumerate(list(state.get("loop_batch") or [])):
        ...
        if extra.get("deferred"):
            # 延迟通道（设计 §2 解析后阶段，与主步共用 `_resolve_placeholder_plan`）：
        ...
        if ev in _NETWORK_LOOP_TOOLS \
            ...
            new_trace.extend(_trace_entry(
                ...
                False, time.monotonic()))
            continue
        # rank/rerank 预算批内复查（验证 高1， 批内熔连同哲学）：
    ...
    # 逃生口换线（M1； 转正后常驻）：本批真跑成的 route.request 步把
...
def decide(state: _AgentState, *, runtime: Any) -> dict:
    """判断下一步（仅 execute 真跑过工具后进入）：**主通道 tool-calling**（换装；
    取代 的散文 JSON 主通道）——LLM 看「原话 + 已完成步骤紧凑投影」，绑
    5 个 loop 工具 + finish + unsupported_next_step（`_DECIDE_TOOL_SPECS`）：
    回 loop 工具 = 续步提议；finish = done；unsupported = 婉拒表外动作；
    幻觉工具名 / 散文 / 全通道没拿到 = 非法；**多 tool_call 取第一个**（问题猎杀批：DeepSeek 不遵守 parallel_tool_calls=False，微验证实证 decide 不可读
    ...
    **仅调用本身抛异常**才跌散文 JSON 兜底。主通道拿到非法应答 → **重问一次**
    （验证 v4/w2a3 问题 b08/l07/k10：非法一刀切停环是 K/L 长链断裂的最大
    单因）。重问后的写动词处置： 初版是只读闸（写动词机械拒绝，验证
    「不多问」本意的细化）；** 策略改 B 方案——放行 + 强制核销复核**
    ...
    「若有则…」的条件语义由 LLM 看真实结果自然处理（如 new_count=0 → done）。
     核销硬闸（验证 v3 问题 b12/g04 第二刀）：finish 必附 completion_report，
    ...
    不再加门槛，见 `_unfinished_business` docstring）。
     验证 v4 核销闸升级（`_completion_report_veto` 三形态，反馈文案按形态区分）：
    ...
    started = time.monotonic()
    ctx: _AgentContext = runtime.context
    # 复杂度路由（第三批任务1）：complex 车道的 decide 走专用档——首答/非法重问/
    ...
    if len(steps) >= MAX_STEPS:
        ...
        try:
            ...
            settled = False
        # exec-gates M2（exec2）：清单没建成（checklist_unavailable）时
        ...
        return {
            "loop_next": False,
            "usage_ledger": usage_local,
            # B5：强制停环必须留旗标——narrate 据此如实标注「还有事没做完」。
            "truncated": True,
    ...
    # ② 其余二连败（形状闸等非网络码）： 约束放松批（验证决定 K3）起不再硬停，
    ...
    # rank / rerank 预算耗尽注入段（M2，同口径；未用满恒空段）。
    ...
    # 环内结果处理四工具预算耗尽注入段（四工具批，同口径；未用满恒空段）。
    ...
    # 逃生口机会用完注入段（M1，同口径；rescue/未用完恒空段）。
    ...
    # 未决事项机械提示段（验证 v6；提示不是闸，三条规则都无命中时整段不出现）。
    ...
    if fb_reason:
        # decide 也跌兜底了——抓现场账（node 如实标 decide； 前 decide 没有兜底档，
    ...
    if kind == "invalid" and answer is not None and not fb_reason:
        ...
        reasked = True
    # violation 重问一次（借鉴批：decide 站「提议非法」与「应答不可解析」的
    ...
    if kind == "finish":
        while kind == "finish":
            ...
            # 不收尾，把缺口回灌重问（每请求至多 2 次—— 调研-长程agent批 候选2：
...
def narrate(state: _AgentState, *, runtime: Any) -> dict:
    ...
    if (len(steps) <= 1 and spec and spec.get("report") and plan.get("observation")
        ...
        if report and discard_reason:
            ...
            # 措辞保持中性（R2-1）：后检是模式匹配，分不清「真谎称」与「措辞像
    ...
    if discarded_report is not None:
        ...
        # schema 加固：矛盾原因码一并留痕（untouched_source / count_mismatch /
...
def _get_graph() -> Any:
    """模块级编译单例（换装刀 1）：首次调用构建 + compile 一次，之后复用；
...
def plan_with_agent_events(
    ...
    *,
    ...
    search_date_from: str = "",
    search_date_to: str = "",
    # （并发分流与确定性 RAG 策略，设计 §4.4）三缝透传——缺省 None/"" = 今天逐位
    # 不变（既有测试的 monkeypatch seam 全保）。
    retrieval_provider: Any = None,
    on_route_verdict: Any = None,
    route_extra_zh: str = "",
    # （设计 §3.3）：课题上下文卡——独立字段透传进图状态，
    ...
    `decide_model`（复杂度路由，验证）是 decide/repair 档的**显式注入缝**：
    ...
    `entry_mode`（检索工具化 Phase 1）："rescue" = 检索救回回合——understand/
    ...
    # 任务1（V4 迁移）：新增 `LLM_COMPLEX_THINKING`/`LLM_COMPLEX_EFFORT` 旋钮——
    # 不配模型名、只开 thinking 也建第二 client（同模型名 + 思考参数），官方旋钮替代
    # 「deepseek-reasoner 别名」的双模型路由（别名=V4-flash+thinking 开， 验证）。
    ...
    context = _AgentContext(
        ...
        # （设计 §3.3）：课题上下文卡进 context，供 prompt 注入消费。
    ...
    plan["trace"] = trace
    # 成功经验库（五机制批）：成功收尾的 curate 会话机械追加进**候选池**
    # `.userdata/curate_example_candidates.jsonl`（失败/被闸/取消不录，防毒化；账本失败静默）——
    # 起用户在记忆模块预览勾选后才迁入正式库 `curate_examples.jsonl`，注入侧只读正式库。
    ...
    # 任务3 缓存埋点汇总：usage_ledger 非空才写 plan（FakeModel/老 provider
...
def plan_with_agent(
    ...
    *,
    ...
    on_route_verdict: Any = None,
    route_extra_zh: str = "",
    # （设计 §3.3）：课题上下文卡，与 plan_with_agent_events 同义。
    artifact_context: str = "",
) -> tuple[dict, list[dict]]:
    """`plan_with_agent_events` 的无回调薄封装（起）——
