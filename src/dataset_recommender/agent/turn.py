...
def rule_match_summary(text: str, *, sources: Any = None,
    ...
    `search_params`（prelim1 初步结果先行）：给了就改用**真实检索参数**跑
...
#: 「没听懂」死胡同的可点选候选（五机制批 · 婉拒候选 chips；借鉴 WrenAI
...
# ---- （并发分流与确定性 RAG 策略，设计 §4.1）：RAG flight 与全局准入 ------------
...
def _emit_preliminary(flight: "_RagFlight | None", *, agent_path: bool,
    ...
    """ 发射（verdict-gated，设计 §4.2）：闸**全与**才发，至多一次，盒子持锁原子。
...
def _make_route_verdict_hook(holder: dict) -> Callable[[str], None]:
    """route_consensus verdict hook（设计 §4.2：**只做 abandoned/lazy 标记，不发射**）。
...
def _make_retrieval_provider(holder: dict, *, agent_path: bool,
    ...
    """understand 入口的 retrieval provider（设计 §4.4）：join（deferred 在此同步
...
def _recipe_narrow_plan(plan: dict | None, recipe_verbs: "frozenset[str] | None") -> dict | None:
    """suggested_recipe 机械收窄（engagement P6，设计 §5.2；只缩小、不扩权）。
...
def _route_turn_serial(
    ...
    **逐位复刻 HEAD 版 `_route_turn_impl`**（master@1d634de， 并发施工前）：同步
    ...
    # （设计 §5.2）：suggested_recipe 收窄面——allowlist 校验在 action_plan 单一真源；
        ...
        if agent_cfg is not None and _ap.should_use_llm(agent_cfg)[0]:
            ...
            try:
                ...
                try:
                    ...
                    # cross-trace D2 移交（→ 本批）：审计落盘失败的调用方外层
    ...
    if plan is None:
        ...
        if hybrid_abstain:
            ...
            plan = _ap.plan_action(
                ...
                allowed_verbs=recipe_verbs,   # （设计 §5.2）：suggested_recipe 收窄动词面
            )
        if agent_fell_back:
            # 审计 C-6：agent 路径失败过这件事本身在 plan 上留痕（additive 字段，
    ...
    # （设计 §5.2）：agent 路径产出的 plan 同样机械收窄——EXEC 动词不在 recipe
    ...
    # M2（RAG 工具组）：rank/rerank 的 display 批次 payload 同样汇入环内上屏
    # 哨兵 loop_payload（display=true 语义即上屏，**不受**批次机制限制）。
    # M3（设计 §M3； 转正常驻）：轮内累积批次（preliminary 在前、环内批次在后），
    # 补 batch_id/seq/created_at/turn_id 后挂响应；无批时两键不出现。
    result_payload: dict[str, Any] | None = None
    # 环内上屏批哨兵（验证 中5）：b 档判定的「无环内采纳/上屏」改用它，
    ...
    if prelim_payload is not None:
        batches.append({
            "kind": "preliminary",
            # 溯源（验证 中6）：pre-loop 管线实际跑的是本轮原话 text
    ...
    # 兼容）」， 验证 中5）；preliminary_final 的 b 档判定改用独立哨兵
    ...
    # 审计 C-2：search_shaped 的反向闸不查管护词表——「联网搜一下有没有新的
    ...
    # 审计 C-7：EXEC 缺 quoted 被机械降成的 none（downgraded_from 非空）不是
...
def _route_turn_impl(
    ...
    *,
    ...
    （M4 波2：本体从 `route_turn` 改名而来——公开入口是文件底部的同名薄壳，
    ...
    - route == "none"：`echo_zh` 是给用户的如实回音；`plan` 一并带回（含规则兜底信息）。
      ...
      前端据它渲染成带设置指路的美观气泡。
      `suggestions`（五机制批）只在 **LLM 真判的 none**（「没听懂」死胡同）
    ...
    `search_params`（prelim1 初步结果先行，设计 §2.2-2.4； 并发分流 v3.1
    ...
    None = 旧轻量概览逐位不变。（设计 §3/§4.1-4.2）preliminary 改为 **verdict-gated**：
    ...
    M3（设计 §M3； 转正常驻）：轮内有批时再加 `result_batches`
    ...
    # 「AI 执行」开：规则匹配概览 + 原始查询一起进 LLM 分流（100%，无短路）。
    # （并发分流，设计 §4.1）：agent 分支条件**先求值定型 agent_path**（起跑与发射
    ...
    if agent_path:
        ...
        if plan is None:
            try:
                ...
                # 三缝（设计 §4.4）：retrieval_provider（understand 入口 join + 发射）、
    ...
    hybrid_abstain = False
    if plan is None:
        # 保底分支（设计 §4.1/§4.2 ②）：先 join flight（未起则就地起）构造分类
    ...
    # return 装配（设计 §4.1 join 点）：search/general/none 路线必 join（图内已
...
def route_turn(
    ...
    *,
    ...
    `suggested_recipe`（engagement P6，设计 §5.2，additive）：结果页阶梯 chip /
    ...
    # 任务 3（基线+补丁包）：webapp 中间件绑定的补丁作用域是 contextvar，SSE
    ...
    with _bind_recorder(rec), bind_patch_scope(
        ...
        # （设计 §5.2）：suggested_recipe 处理结论如实附注（非法 → 忽略说明；
