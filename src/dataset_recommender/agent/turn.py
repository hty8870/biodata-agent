...
## 管线（2026-08-03 agent2 定稿：「AI 执行」开关 = LLM 分流器的总闸）
...
0. **编号快速道**：贴数据集编号/直链（GSE… / E-MTAB… / DOI）→ 直接 search（按原话），
   ...
   **唯一例外**（2026-08-15 审计 C-3）：「编号 + 管护操作」（删掉/检查更新类，
...
#: dl-auto-1 任务A/C：前端直派 exec 动词面。这些动词有前端 ACT_RUNNERS runner（act.js）、
...
def _batch_scope_fingerprint(query_effective: str, sources: Any, search_params: dict | None) -> str:
    """批次检索范围指纹（2026-08-23 ku1-覆盖策略修复，设计 §10.2）。
...
#: ov1-fix1c 修复2:混合句(操作+检索串联)在 plan_action 保底通道的弃权回音。
...
# ---- cr1（并发分流与确定性 RAG 策略，设计 §4.1）：RAG flight 与全局准入 ------------
...
# r3 P1-3：内联使全局并发与延迟无上界）；deferred 在 join 点由调用线程同步补跑
# （=今天串行时序，正确性保底，不会比今天差，设计 §7 残留风险④）。
...
class _RagFlight:
    ...
    def join(self) -> dict | None:
        ...
        if self.abandoned and self._deferred:
            # abandoned 的 deferred 直接弃（不补跑）——设计 §4.6。
            self._joined = True
            return None
        if self._deferred:
    ...
    def ensure_payload(self) -> dict | None:
        """recommend_payload 建一次存 flight（M3 原则不重跑）。"""
...
def _warmup_rag_environment(search_params: dict | None) -> None:
    """cr1 预热闭合（r3 P1-5）：主线程幂等 ensure vector/env 初始化——flight 线程从此
...
def _emit_preliminary(flight: "_RagFlight | None", *, agent_path: bool,
    ...
    """cr1 发射（verdict-gated，设计 §4.2）：闸**全与**才发，至多一次，盒子持锁原子。
...
def _make_route_verdict_hook(holder: dict) -> Callable[[str], None]:
    """route_consensus verdict hook（cr1 设计 §4.2：**只做 abandoned/lazy 标记，不发射**）。
...
def _make_retrieval_provider(holder: dict, *, agent_path: bool,
    ...
    """understand 入口的 retrieval provider（cr1 设计 §4.4）：join（deferred 在此同步
...
def _consensus_extra_zh(text: str, *, sources: Any = None,
    ...
    """route_extra_zh 内容生成（v3.2 修正，设计 §0 盲跑实测处置 + §5 复验裁定）。
...
def _recipe_narrow_plan(plan: dict | None, recipe_verbs: "frozenset[str] | None") -> dict | None:
    """suggested_recipe 机械收窄（2026-08-22 engagement P6，设计 §5.2；只缩小、不扩权）。
...
def _route_turn_serial(
    ...
    **逐位复刻 HEAD 版 `_route_turn_impl`**（master@1d634de，cr1 并发施工前）：同步
    ...
    # eng-p6（设计 §5.2）：suggested_recipe 收窄面——allowlist 校验在 action_plan 单一真源；
    ...
    # M3（设计 §M3）：preliminary 载荷复用——发屏与批次组卷用同一份，不重跑 recommend_payload。
    ...
    if use_agent and llm_call is None and _agent.agent_available():
        ...
        if agent_cfg is not None and _ap.should_use_llm(agent_cfg)[0]:
            # prelim1 初步结果先行（设计 §2.2 机械闸，**全与**才发；单调用点天然最多一次）：
            ...
            try:
                ...
                if on_event is not None:
                    ...
                    # 无回调时走 plan_with_agent 薄封装：行为与 agent1 逐位一致，
                ...
                # 2026-08-10 codex 架构评审：预期外异常（导入错误/代码 bug）不该和模型故障
                ...
                try:
                    ...
                    # cross-trace D2 移交（ta2-exec2 → 本批）：审计落盘失败的调用方外层
    ...
    if plan is None:
        # ov1-fix1c 修复2:混合句(操作+检索串联)在单次单动词保底通道上必只做一半,
        ...
        if hybrid_abstain:
            ...
            plan = _ap.plan_action(
                ...
                allowed_verbs=recipe_verbs,   # eng-p6（设计 §5.2）：suggested_recipe 收窄动词面
            )
        if agent_fell_back:
            # 2026-08-15 审计 C-6：agent 路径失败过这件事本身在 plan 上留痕（additive 字段，
            # 不改任何既有键）——否则一次请求付了 2~3 次 LLM 调用，via="llm" 却看不出
            # agent 刚才失败过，线上排查「为什么这么慢/这么贵」无据可查。
            plan["agent_fallback"] = True
            # M4 波2（设计 §6-8）：agent 跌保底这一事实进 trace 收尾账（fail-soft）。
            _te.emit_finish_reason(kind="agent_fallback")

    # eng-p6（设计 §5.2）：agent 路径产出的 plan 同样机械收窄——EXEC 动词不在 recipe
    # 允许集 → 降 none 如实回音（hint 只缩小不扩权，绝不执行用户没点的能力）。
    plan = _recipe_narrow_plan(plan, recipe_verbs)

    # prelim1（设计 §2.3）：环内 search.rerun 采纳档的 recommend_payload（多个采纳步取
    # 最后一个）——数据本就在 plan.steps 实录里，扫出来挂 final 即可，不碰 state、不改图。
    # M2（2026-08-17 RAG 工具组）：rank/rerank 的 display 批次 payload 同样汇入环内上屏
    # 哨兵 loop_payload（display=true 语义即上屏，**不受**批次机制限制）。
    # M3（设计 §M3；nl1 转正常驻）：轮内累积批次（preliminary 在前、环内批次在后），
    # 补 batch_id/seq/created_at/turn_id 后挂响应；无批时两键不出现。
    result_payload: dict[str, Any] | None = None
    # 环内上屏批哨兵（2026-08-17 codex 评审 中5）：b 档判定的「无环内采纳/上屏」改用它，
    ...
    if prelim_payload is not None:
        batches.append({
            "kind": "preliminary",
            # 溯源（2026-08-17 codex 评审 中6）：pre-loop 管线实际跑的是本轮原话 text
    ...
    for _step in list(plan.get("steps") or []):
        ...
        if (isinstance(_res, dict) and _step.get("ok")
            ...
            _rerun_batch = {
                ...
                # query_raw = 本轮用户原话（M3 契约；中6：曾填 current_query 旧查询）。
                "query_raw": text,
                "query_effective": str(_res.get("query") or ""),
                "payload": _res["payload"],
            }
            # r2p（2026-08-18）：采纳档的确定性披露句随批下发（additive，无披露句不出现）
            ...
            # rank/rerank 的 kind/label/query_raw/query_effective 由环内工具按 §M2 规则
    ...
    # 一批，仅 preliminary 批也镜像——设计 §M3「既有字段镜像 active batch（过渡期回退
    # 兼容）」，2026-08-17 codex 评审 中5）；preliminary_final 的 b 档判定改用独立哨兵
    ...
    if batches:
        ...
        for _i, _b in enumerate(batches):
            ...
            _b["created_at"] = _now
            _b["turn_id"] = _turn_id
            # ku1-覆盖策略修复（设计 §10.2）：每批补规范化检索范围指纹（契约级身份键）。
    ...
    # ov1-fix1c 修复2:混合句弃权分支——必须先于「规则 none 看检索脸」分支,
    ...
    # 2026-08-15 审计 C-2：search_shaped 的反向闸不查管护词表——「联网搜一下有没有新的
    ...
    # 2026-08-15 审计 C-7：EXEC 缺 quoted 被机械降成的 none（downgraded_from 非空）不是
...
def _route_turn_impl(
    ...
    *,
    ...
    （2026-08-17 M4 波2：本体从 `route_turn` 改名而来——公开入口是文件底部的同名薄壳，
    ...
    `on_event`（2026-08-03 agent1b 流式）只在 agent 路径透传给 `plan_with_agent_events`
    （每节点落定时回调 trace 条目）；保底路径**不回调**——它是一次性 LLM 调用，没有节点可播。

    `search_params`（2026-08-16 prelim1 初步结果先行，设计 §2.2-2.4；cr1 并发分流 v3.1
    ...
    None = 旧轻量概览逐位不变。cr1（设计 §3/§4.1-4.2）preliminary 改为 **verdict-gated**：
    ...
    上屏批时镜像 active 批——仅 preliminary 批也镜像，设计 §M3「legacy 镜像 active
    batch」；无批则 None）与 `preliminary_final`
    （保守判定，见 search 分支注释——任一条件不明即 False，宁可重检不跳检）。
    M3（2026-08-17，设计 §M3；nl1 转正常驻）：轮内有批时再加 `result_batches`
    ...
    回归的根因补偿；盲跑裁定追加式机械行格式有偏置，§5 记录）。off 串行路径不注入。
    """
    text = _ap.normalize_utterance(utterance)

    # 编号快速道：贴编号/直链 → 直接按原话检索（编号优先于执行词，见模块 docstring）。
    # 审计 C-3：「编号 + 管护操作」句（「把 GSE123456 从我上传的里删掉」）不走快速道——
    ...
    if agent_path:
        if not markers:
            # 预热闭合（r3 P1-5）：主线程先 ensure vector/env 初始化，flight 线程只读。
    ...
    agent_fell_back = False
    if agent_path:
        # dl-auto-1 任务A：含动作标记的句子先探测一次 plan_action 单次分类——agent 图的
        ...
        if plan is None:
            try:
                ...
                # cr1 三缝（设计 §4.4）：retrieval_provider（understand 入口 join + 发射）、
                ...
                if on_event is not None:
                    plan, _trace = _agent.plan_with_agent_events(
                        ...
                        # eng-b4（设计 §3.3）：课题上下文只进 agent prompt。
                        artifact_context=artifact_context,
                        **_agent_search_kwargs,
                    )
                else:
    ...
    hybrid_abstain = False
    if plan is None:
        # cr1 保底分支（设计 §4.1/§4.2 ②）：先 join flight（未起则就地起）构造分类
        # 上下文；plan_action 判出非 EXEC（search/general 向）∧ 闸过 ∧ 未发射 → 发射；
        # EXEC/none 一律抑制（r3 P0-2：plan_action 本身才判 EXEC，判前发射 = 无 verdict
        ...
        # ov1-fix1c 修复2:混合句弃权(与 serial 路径同口径)——单次单动词通道做不全
    ...
    # cr1 return 装配（设计 §4.1 join 点）：search/general/none 路线必 join（图内已
    # 汇合或保底已 join，瞬时）；tool 路线不 join——retrieval 恒 None（breaking 契约，
    # §4.1 r3 P1-5）+ additive retrieval_note（"skipped_action_marker"=marker 分支未起 /
    ...
    # ov1-fix1c 修复2:混合句弃权分支(与 serial 同口径)——先于「规则 none 看检索脸」,
...
def route_turn(
    ...
    *,
    ...
    `suggested_recipe`（2026-08-22 engagement P6，设计 §5.2，additive）：结果页阶梯 chip /
    ...
    M4 波2（设计 `docs/工作记录/设计_可追溯性_2026-08-17.md` §6-1/6-2/6-9）：
    ...
    with _bind_recorder(rec), bind_patch_scope(
        ...
        if rec.enabled:
            # additive 回显（设计 §6-9）：webapp 透传进响应 meta，用户报障给号用。
            result["trace_turn_id"] = rec.turn_id
            _te.emit_route_decision(result)
        # eng-p6（设计 §5.2）：suggested_recipe 处理结论如实附注（非法 → 忽略说明；
