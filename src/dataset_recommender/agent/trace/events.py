# -*- coding: utf-8 -*-
"""trace 事件词表与载荷构造（2026-08-17 可追溯性波1）。
...
def route_decision_payload(*, route: str, via: str, plan: dict | None = None,
    ...
    cr1（设计 §4.6，additive）：`preliminary`（"emitted"/"suppressed_action"/
...
def batch_emission_payload(*, n_calls: int, adopted: int, dropped: int,
    ...
    """decide 多调用同批消费（mcl1；2026-08-20 pbt1 批 additive `n_placeholder` = 采纳的
    **占位接地**续步数——依赖占位批量计划的机械信号，探针/复盘据此统计占位使用率）：
...
def emit_route_decision(result: dict, *, votes: dict | None = None) -> bool:
    ...
    cr1：additive 字段取自 result 的 `_preliminary_trace`（内部键，响应层不透传）与
