...
def load_env_candidates(project_root: Path) -> Path | None:
    ...
    # The project-external file is loaded before project-local fallbacks, so Codex and
    # Claude Code can share one secret file without storing the key in either client config.
    # 2026-08-15 触发点审计 E-01：所有存在的候选**都加载**（setdefault 语义下排前者优先，
...
# （历史上曾有一套死字段，未脱敏 placeholder key，已删除，见开发日志「四十九」）。
...
def _default_enable_llm() -> bool:
    """ENABLE_LLM 的有效开关（**P1 · 产品所有者 2026-07-19 决策**：有 key 默认开 / 无 key 默认关；显式设置优先）。
...
def get_settings() -> Settings:
    ...
    # 2026-08-08 约束放松批：默认返回条数 5→10（与前端 cfgTopK 默认值、launch_web.ps1 同源联动；
