# -*- coding: utf-8 -*-
"""p10 · LLM 执行结果总结层（**默认随 key（P1 已决策）· fail-open · 只在 `/api/act/summary` 端点**）。
...
- **默认随 key（P1，产品所有者 2026-07-19 已决策：「如果填了apikey就默认开启，否则默认关闭」）**：
...
#: 一句话模式（2026-08-03 agent1b，设计 §5.3）：执行收尾用**一句**话原位替换总结泡正文。
...
def _summarize_with_prompt(
    ...
    *,
    ...
    """长/一句话两档**共用的调用核心**（2026-08-03 agent1b 抽出）：同一把 should_use_llm 闸、
...
def summarize_brief_with_llm(
    ...
    *,
    ...
    """一句话档（2026-08-03 agent1b，设计 §5.3）：执行收尾用**一句 ≤35 字**原位替换总结泡正文。
