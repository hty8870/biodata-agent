...
def is_auth_error(error: object) -> bool:
    ...
    超时/5xx/返回空是临时故障，稍后重试即可。两句话必须分开（C3）。
    """
...
def resolve_enable_llm(provider: str | None = None) -> bool:
    """ENABLE_LLM 的有效开关（** · 产品侧 决策**：「如果填了apikey就默认开启，否则默认关闭」）。
...
def load_llm_config(
    ...
    return LLMConfig(
        ...
        # thinking={"type":"disabled"}（验证 400），且思考开启下
...
def complex_model_name() -> str:
    """复杂度路由的 decide 档模型名（`LLM_MODEL_COMPLEX` 环境变量， 第三批任务1）。
...
def _call_chat_completions(
    ...
    # 思考旋钮（与 agent_exec langchain 路径同语义， T3 补齐 plain 路径）：
...
def call_llm(prompt: str, config: LLMConfig, retrieved_records: list[dict[str, Any]] | None = None) -> LLMResult:
    ...
    if provider == TRIAL_PROVIDER:
        # 试用通道走 OpenAI 兼容协议（bigmodel 官方端点， 起），但归因如实标
