...
#: 限量试用通道（T3 网页版护栏，2026-08-25；2026-08-27 换型 GLM-5.3-Flash）：部署方在
...
class LLMConfig:
    ...
    max_tokens: int = 8000
    # DeepSeek V4 思考模式旋钮（2026-08-08 任务1，实测契约见 docs/工作记录/任务记录_2026-08-08_过夜任务群.md）：
...
def resolve_enable_llm(provider: str | None = None) -> bool:
    """ENABLE_LLM 的有效开关（**P1 · 产品所有者 2026-07-19 决策**：「如果填了apikey就默认开启，否则默认关闭」）。
...
def complex_model_name() -> str:
    """复杂度路由的 decide 档模型名（`LLM_MODEL_COMPLEX` 环境变量，2026-08-07 第三批任务1）。
...
def _call_chat_completions(
    ...
    # 思考旋钮（与 agent_exec langchain 路径同语义，2026-08-25 T3 补齐 plain 路径）：
