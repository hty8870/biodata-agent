...
/* prelim1（设计 §2.5.4，实现 code ≥2s 规则）：行动流头部实时秒表——<2s 不亮（防闪烁），
...
/* 两个谓语分开（R2-2  僵尸行动流）：arxFinish 后 _run 还留 420ms 做折叠动效，
...
/* 推进一格：之前的 running 条目收成 done，新增一条 running。
   ...
   opts.state / opts.detail（R2-2）：plan.trace 的回放步与 SSE step 事件都是**已落定的真实节点
...
/* 决策条目：需要用户给东西/拍板时调用（后只剩浏览器安全边界一种：import 的系统
...
/* 完成：步骤块做折叠动效（CSS grid-rows 过渡，REDUCE_MOTION 时直接消失），
   ...
   幂等（R2-2）：已在 collapsing 的流快照早就交出去了，再 finish 一次会重启折叠计时器、
   还把同一份快照交第二回（总结泡 details 双份）——直接返回 []。 */
export function arxFinish() {
    if (!_run || _run.collapsing) return [];
    _stopElapsed();
    //  prelim1（设计 §2.5.5，实现 code "Done in Ns" 的等价物）：收尾头部弱信息行——
