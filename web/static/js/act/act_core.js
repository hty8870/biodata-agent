 ...
 * ----------------------------------------------------------------------------
 * 从 act.js 抽出（Phase C · C0）：回执/注记的构造全是**纯函数**，
...
export function actWhatHappened(plan, outcome) {
    ...
    if (a.restored_name) bits.push("文件 " + a.restored_name);   //  移回回执显示库里恢复原名的叶子（夜班 B 路 P3-2：此前显示回收站时间戳全名，前缀叠加冗长）
