...
/* C4 起本文件是 ES Module：core 的工具、shell 的 getConfig、interactions 的 queryForRetrieval/
   ...
   dl2（2026-08-19）：本文件同时承载「真实数据下载」UX——/api/download/plan 分级 →
...
/* 一句话任务包的界面层：先看清单，确认后再产包。
   ...
   诚实约束与 ux1 同款：行动流只放关键行、长明细收 <details>；同一时刻只允许一个下载任务
...
/* ---- 真实下载状态（dl2）。状态单一真源在这里，renderTaskPackPlan 每次重渲染都照它画下载区。---- */
let _dl = {
...
export function resetTaskPack() {
    /* dl2 守卫（真机抓到）：下载进行中，任何「结果重渲/换查询/检索失败」触发的 resetTaskPack
...
export async function previewTaskPack(opts) {
    ...
    const myGen = ++_tpSeq;
    /* 2026-08-16 pack1：预览**不再自动打开面板**（原 panel.hidden=false 在此 + renderTaskPackPlan 末尾，
       连「下载top5」这类自动打包都会把面板弹到用户脸上；且用户手动关面板后迟到的 fetch 会把它重开
       ——vs3 起在册的 race，随本处一并消掉）。显式开面板的调用方（toggleTaskPackPanel /
    ...
    if (myGen !== _tpSeq) return { ok: false, stale: true, error: "已被更晚的一次预览取代" };
    await tpIdentityLookup();   // P2-2：同名行身份锚点（发表日期）用的全库索引；本地服务亚秒级，失败静默降级
    if (myGen !== _tpSeq) return { ok: false, stale: true, error: "已被更晚的一次预览取代" };
    if (!payload.plan) {
        _tpPlan = null; _tpChosen = new Set(); _tpCount = { said: 0, used: 0 };
...
/* 面板行的身份锚点（普查 P2-2）：同名数据集（uid …-ff-ultima 与 …-ff-ultima-4，同名同来源）
...
function renderTaskPackPlan(plan, payload) {
    ...
    html += '<div class="tp-rows">' + (plan.items || []).map(function (it) {
        ...
        // P2-2：名称之外补一个最紧凑的身份锚点（发表日期），同名两行由此可区分；
    ...
    // dl2 主/次双按钮：次按钮（任务包 zip 兜底）恒在，label 明确「真实数据共约 X」——
...
export async function tpDownloadPlan() {
    // 公网护栏硬化（2026-08-26）：护栏模式后端 download 系列端点一律 403（服务端不代下），
...
function renderTaskPackDownloadZone() {
    const zone = $("tpDlZone");
    if (!zone) return;
    // 公网护栏硬化（2026-08-26）：护栏模式隐藏整个真实下载区（含「直接下载真实数据」主按钮）。
...
export async function tpDownloadStart(uids) {
    ...
    try {
        ...
        renderTaskPackDownloadZone();
        // dl what:"script"（ov1-tel1）：UI 里没有独立的「下载脚本」动作——脚本在任务包 zip 内
        // （pack 已记）；真实下载由服务端下载器执行，正是原「下载脚本自己跑」能力的在线形态，
        // 故在这个语义等价点接线（协调者提示词的死标签，见包报告说明）。只在真启动成功后记。
...
/* tpBytes 已归入纯核 act_core.js（2026-08-02 Phase C · C0，task_pack / act 共用单一真源）——
   C4 起经 import 取（见文件头）。 */
...
function toggleTaskPackItem(uid) {
    ...
    // dl2：勾选变了 → 下载区状态作废（plan 要按新 uids 重查；确认条/终态重置回 idle）
...
/* downloadBlobAs（sm2 批）上移 #core 共用——本文件 757 行的调用点不变。 */
