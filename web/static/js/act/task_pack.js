...
let _dl = {
    ...
    timer: null,       // 轮询定时器句柄
    /* （运行中可增删）：在途下载队列里的数据集编号集合（「更新下载」差量的基准）。
...
function toggleTaskPackPanel() {
    ...
    panel.hidden = false;
    // 运行中重新打开面板 = 放弃未提交的改勾（差量没落地），勾选恢复成当前在途集。
...
export async function previewTaskPack(opts) {
    ...
    const myGen = ++_tpSeq;
    /* 预览**不再自动打开面板**（原 panel.hidden=false 在此 + renderTaskPackPlan 末尾，
...
/* 在途队列的权威 uid 集合（_dl.runningUids，start/update 维护）。 */
...
/* 真实下载启动内核（抽出供 act.js runner 复用）：调 /api/download/start → stage 置
...
/* 更新提示：把服务端确实处理过的增删如实报给用户；拒绝/不在队列的也点名。 */
...
/* downloadBlobAs 上移 #core 共用——本文件 757 行的调用点不变。 */
