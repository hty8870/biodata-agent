...
/*  · 一句话执行层的界面侧：**派发既有能力 + 行动流播报 + 由真实产物构造事实句**。
   ...
   `previewTaskPack` / `buildTaskPack` 在  里被改成有返回值，就是为了这一刻；在那之前它们
...
async function actRunPackPreview(plan) {
    ...
    /* pack1 分流（previewTaskPack 起不再自动开面板）：
...
/* ---------------- 管护（curate.*）：全自动化直推 ----------------
   ...
   产品侧预先授权（推翻「写操作必须人批准」）：AI 执行开启时一切动作**直接执行**，
...
/* ---- curate.sync_updates：检查更新 → 有新增则自动入库（复合流，「工作流即工具」批） ----
...
/* 关键词提取优先级（起； 全自动化后它直接决定执行参数，不再是问卷初值）：
...
async function actCuratePrefill(plan, said) {
    ...
    // ① 规划侧槽位优先：agent / action_plan 已经解析出关键词或物种时直接用（起即执行参数）。
    const slots = (plan && plan.slots) || {};
...
function actMatchFile(target, files, keyOf) {
    ...
    //  让「没有这个名字的文件。现有：…」分支（!m.one）变死代码（触发点审计 H-01）。
...
/* ---- search.rerun（检索工具化 sr1，设计 §4）：换词重检步骤卡 ----
...
function actFixChips(plan, said, outcome) {
    ...
    //  给一颗「列出外部库的文件」候选 chip（五机制批 · 婉拒候选；点击即把这句重新入环，
    ...
    //  pack1：「按原话重新检索」「以后别自动执行」两颗 chip 退役——
...
function actSummaryHtml(opts) {
    ...
    //  折叠条全部撤下；工具调用计数摘要自 起归信息流压缩行（flow_trace.compressFlow，
...
function actFetchLlmSummary(plan, outcome, said, factual, entry) {
    ...
    fetch(API.actSummary, {
        ...
        //  原位替换**正文**：「AI 总结」标照挂（起 entry.html 只剩功能钮，无折叠区可留）。
...
function actFixClick(event) {
    //  婉拒候选 chips（五机制批）：data-act-say = 把这句当用户的话重新入环
    ...
    //  pack1：research（按原话重新检索）/off（以后别自动执行）两分支随 chip 一并退役。
    if (what === "undo") {
        actUndoRun(btn);
        return;
    }
...
export async function actDispatchPlan(plan, said) {
    ...
    const renderPlanTrace = (skipArx) => {
        ...
        trace.forEach(function (t, i) {
            ...
            // R2-2 ：state/detail 一并进行动流——空闲通道（!skipArx）此前只推文案，
    ...
    if (Array.isArray(plan.steps) && plan.steps.length) {
        ...
        //  总结照出（起不再有执行过程快照进 details——折叠条已撤）。
