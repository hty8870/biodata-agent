...
/* C1 起本文件是 ES Module：core 的存储/工具经 import 取，排序/淘汰/归一由纯核
...
export function userMemoryEnabled() {
    ...
    // 与 usage 采集的默认口径相反是产品既定口径；两处都已在设置页文案写明（2026-08-15 ta2 M-06）。
...
export function initUserMemory() {
    ...
    // 「记住这次需求」按钮已从结果头移除（ux3）；绑定加 null 守卫（否则 initUserMemory 里 $()==null 抛错、整个前端启动崩）。
    ...
    // UX1 焦点陷阱：模态开着时 Tab / Shift+Tab 在模态内循环，不逃到背景控件（对齐 #settings 的隔离语义）。
