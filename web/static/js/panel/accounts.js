...
/* 本地账户前端：whoami / 登录 / 注册 / 登出 / 一键切换 + 账户态 UI。
   ...
   - 账号 chip 常驻「设置 → 账户」（起；此前在导航卡底部，用户判定不必占导航位）。
...
export function onAccountChanged() {
    ...
    //  追踪（engagement F1 +）：数据本体按 scope 隔离在 IndexedDB——切换只断引用不删数据。
...
function renderAccountState() {
    ...
    if (!sub) return;
    //  网页版设置卡精简（产品侧裁决）：文字只保留用户名（chip 上）+
...
function toggleAccountMenu() {
    ...
    menu.innerHTML = html;
    menu.hidden = false;
    /* R2-9 ：chip 在设置抽屉**顶部**（账户块是 drawer-body 第一块），一味上弹会压住抽屉头、
...
export function initAccounts() {
    ...
    //  账号 chip / 菜单（起在设置·账户块内，登录/注册/登出/切换全走这颗 chip 的菜单）
