...
/* C1 起本文件是 ES Module：core / usage_log / memory 同批转换、经 import 取绑定；
...
/* ---------- 账户切换反转钩子（2026-08-22 engagement F1） ----------
...
async function accountAuth(kind, username, password, remember, inviteCode) {
    ...
    // 公网护栏硬化（2026-08-26）：护栏模式服务端本就不下发 session_token（data.session_token
...
async function accountLogout() {
    ...
    // 下次加载 whoami 又把该账户解析回来、暴露其命名空间给共用机器的下一个人（安全评审）。
...
export function onAccountChanged() {
    ...
    // usageOnAccountChanged 由 #usage_log import（C1 起 usage_log 已转 ESM）——拼错名字模块加载期就炸。
    usageOnAccountChanged();
    benchfbOnAccountChanged();   // benchmark 采集记录同样 per-account 命名空间（同 usage 纪律）
    // 追踪（engagement F1 + ku1）：数据本体按 scope 隔离在 IndexedDB——切换只断引用不删数据。
    // artifactsOnProfileSwitched 清内存缓存与活动课题句柄（B1 钩子）；课题 UI 态（上下文卡/活动
    // 详情/首页「继续课题」条）由 projects.js 经 setAccountChangedHook 注册的回调清（设计 §3.2）。
    ...
    // ku1：收藏迁入「我的库」浮窗收藏页签，不再是独立视图——开着且停在 favs 页签时经 shell 重渲
...
function renderAccountState() {
    ...
    if (!sub) return;
    // 2026-08-26 网页版设置卡精简（产品所有者裁决）：文字只保留用户名（chip 上）+
...
function toggleAccountMenu() {
    ...
    // 公网护栏硬化（2026-08-26）：护栏模式隐藏一键切换账号项（后端 /api/account/switch 已 403，
    // 本地也不再持有任何账号 token）。
    ...
    menu.innerHTML = html;
    menu.hidden = false;
    /* R2-9 P2-8：chip 在设置抽屉**顶部**（账户块是 drawer-body 第一块），一味上弹会压住抽屉头、
...
export function initAccounts() {
    ...
    ACCOUNTS_READY = fetch(API.health)
        ...
        .then(function (h) {
            ...
            // 公网护栏硬化（2026-08-26）：护栏模式下若发现历史遗留的一键切换 token 记录
