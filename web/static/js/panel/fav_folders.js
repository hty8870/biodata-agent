 ...
 *
 * C3 起本文件是 ES Module：core/cards 经 import 取。cards/dataset_page（openFavPopover）、
 * shell/accounts（resetFavFolderState）、browse（renderFavFolderBar/renderFavFolderGroups/setFavRerender）、
 * interactions（toggleFavFolderManage）经 import 取本文件导出（C5 起绞杀桥全退役）。
 *
 * 2026-08-10 评审 P1-8：原「import { renderFavorites } from "#browse"」与 browse→fav_folders
...
/* ku1：收藏「在对话中使用」→ 设置数据集上下文 chip。projects.js 经 setCtxDataSetHandler 注册
...
/* ku1：收藏从一级视图迁进「我的库」浮窗收藏页签——「是否在收藏视图」改判本机状态（libWin 开 + favs 页签）。 */
...
/* 目录三态访问器（browse.js 模块求值期经 setCatalogLookup 注册——regist 反转防 browse↔fav_folders 环回退，
   与 P1-8 同范式）。未注册时（dataset.html 等不加载 browse 的页面）空值守卫安全空转。 */
...
/* ku1-w4：目录加载器同范式注册（browse.js ensureDatasetsLoaded）——「更新」遇 load_error 先拉目录再重查。 */
...
function _favPopoverAdd(anchorBtn, it, folderId) {
    toggleFav(it, folderId, anchorBtn);   // anchorBtn 供遥测算名次（ov1-tel1 fav 带 uid/pos）
...
/* 收藏卡片运维条（ku1 视觉 spec §4.1：卡片级动作用文字 .btn——更新 / 在对话中使用 / 删除）：
...
/* 收藏「更新」（设计 §4 独立指纹）：{uid, title, sample_size, raw_data_status}。found 且指纹变 →
   ...
   ku1-w4：load_error 先经注册的加载器拉一次目录再重查（没进过浏览视图时目录本就没加载，
