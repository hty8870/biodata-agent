...
/* 本文件是 ES Module：core 的工具、board_core 纯核（cbRowsFrom/cbSummary/cbPushFrame）、
...
/* ubRouteBody 构造完主体后附加 artifact_context（请求体**独立字段**、不拼进用户原话；
   只进 agent prompt，不进 identifier/query parser 快速道——那是后端的事）。
   无活动卡 → 不加键（请求体与现状逐位一致）。 */
...
/* 条件板 + 统一对话窗口的界面层。
   ...
   2. 统一对话窗口（微信式输入行）：首屏 `#queryInput`（起始对话）与
      ...
      不过「AI 执行」开关永远直派—— 全自动化，plan→apply 链式无人工停点）、
      none 在对话流里如实回音
   ...
   3. 侧栏「对话记录」页签承载历史/帧列表：对话记录（#cbHistory，含每条的 查看历史回复气泡按钮）、
      ...
      收进统一输入条，由 placeScopeControls 与主控制台之间整块搬家。
      #cbHistory 的归宿：**有检索结果**时住侧栏 #sideBoardScroll；**无结果但有对话**
      （首句是工具调用 / clarify 回音 / 检索还在路上）时住主区 hero 内的 #chatMain
      （实现 式中央列，输入框就是它下方的 hero console）——见 cbChatInMain/placeChatLog。
...
export function renderCondBoard(data) {
    ...
    if (_st === "abstained" || _st === "clarification_required") {
        ...
        //  直到下一次切页签/缩放才自愈（隐藏主检索框把这个旧的早退问题从「1px 分隔线」放大成可见死态·验证）。
        swSync();
        return;
    }
    _cbCoverageCounts = {};
...
export function swSync() {
    ...
    // chat-in-main（无结果、对话在主区）时整卡收起——对话界面在主区，这里不重复摆一份。
    //  视图交换无需额外分支：它生效的前提是 has-results，此时 cbChatInMain() 恒假、
    // swAvailable().board 恒真，本卡自然显示（承载搬进侧栏的结果网格）。
    card.hidden = !(onQuery && inSidebar && (has.facets || has.board)) || cbChatInMain();
    //  双窗口（导航卡 + 本工作卡并立）→ 导航卡折叠成两列布局（凡是双窗口，
...
function swApplyMode(mode, has, opts) {
    ...
    if (cardEl) cardEl.dataset.swMode = mode;   // 当前页签标识——交换态「检索结果」页签纯 CSS 隐藏 #swHits 用（只读显示态，不落盘）
    if (opts && opts.picked) {
        _swPicked = true;
    ...
    placeChatSuite();       //  视图交换：对话套件 ↔ 结果网格 换位搬家，同一条链收口
}
...
/* ============ 聊天 + 细化记录 ============
   ...
   对话记录会随**历史记录**一起留存 —— 但写盘的是 core.js 的 `pushHist`，
...
const CB_LOG_MAX = 40;
let _cbLog = [];
/* ---------- 信息流· 过程轨迹（用户重申定稿：上工具行/中唯一气泡/下 pill）----------
...
/* （视觉层）：压缩过渡动画的两帧余量计数——flowFinish 置 2、渲染每帧减一。
   ...
   同步连发，浏览器只绘制最后一帧——动画类必须活到回执那一帧才能被真正看见（验证核验发现
...
export function cbLogPush(kind, text, opts) {
    ...
    // needsAgent：仅 sys 用（降级气泡）——「AI 执行」关时规则检出操作指令，
    ...
    // html：仅 sys 用（执行总结），存可信 HTML（没做到的紧凑行/纠错 chips/结果卡/过程 details；
...
/* LLM 总结落地：原位改写一条还在对话流里的 sys 条目（被剪枝/清空则 no-op，静默 fail-open）。 */
...
/* 纯工具对话（一句检索都没跑过）从不触发 pushHist——被丢弃前先补一条
...
export function cbProgressDone(text) {
    if (!_cbProg) return false;   //  无进度泡 → no-op（防御；改条件重跑自 起也开泡——用户动作上屏就必须有回复）
...
/* 查看历史回复（原「查看结果」）：跳到该消息对应的那一帧、把当时的结果画回来，但**不截断**——
...
function cbHistoryClick(event) {
    ...
    if (pill) {
        ...
        // 零命中 pill —— 不再切换批次。仅当它是最新结果（最后一个回执 entry 的活跃批）
    ...
    //  直达兑现：无 key（llmCapable 假）→ 展开「AI / API 配置」并滚到；
...
export function cbRenderHistory() {
    ...
    //  一帧上可能挂着「搜索/细化」+若干「已执行」消息，取该 frameId 最后出现的那条（验证 的口径）。
    ...
    //  每帧的「查看历史回复」入口**并入该帧的系统回复泡**（footer 内联链接）——
    ...
    box.innerHTML = _cbLog.map(function (e, i) {
        if (e.kind === "sys") {
            ...
            const pillsBlock = (Array.isArray(e.pills) && e.pills.length)
                ...
                : "";
            // 本轮系统回复的评分卡挂载点（rec.id 由 benchfb 收尾时经 benchfbOnChatEntry
            ...
            return '<div class="cbh-turn cbh-sys' + (grpOf("sys") ? " cbh-grp" : "") + '">'
                ...
                + (e.needsAgent
                    ? '<div class="cbh-reply-row"><button type="button" class="cbh-agent-cta" data-cbh-settings>'
                        // 无 key 时「去开启 AI 执行」会被门控弹回（死路），如实改指「去配置 API」。
        ...
        return '<div class="cbh-turn ' + kindClass + (grpOf("user") ? " cbh-grp" : "") + '"' + (isCurrent ? ' aria-current="true"' : "") + '>'
            ...
            + reply
            + "</div>";
    }).join("") + (arxVisible() ? arxTailHtml(grpOf("sys")) : "")   // 行动流：恒在对话流尾部（系统侧；渲染侧含 collapsing 余韵，见 arxVisible）
    ...
    //  滚最近的滚动祖先，否则 scrollTop 写在不可滚元素上是空操作（验证）。
    const scroller = box.closest(".sw-board-scroll") || box;
    scroller.scrollTop = scroller.scrollHeight;
    // benchmark 采集：评分卡是采集层的投影、不进 _cbLog——innerHTML 重画会把它抹掉，
...
/* 弹层「右坞双圆钮 + 单面板」：数据来源/发表时间两枚圆钮竖排在右，
...
/* ============ 对话记录（#cbHistory）落位 ============
   ...
   - **有检索结果**（has-results，含零结果/弃权——结果区有东西要显示）：
     对话记录住侧栏 #sideBoardScroll（微信式窄列）。
   - **无结果但有对话**（cbChatInMain：首句是工具调用 / clarify 回音 / 检索还在路上）：
     对话记录住主区 hero 内的 #chatMain——实现 式中央列，输入框就是它下方的 hero console
...
/* ============ 视图交换：主区结果网格 ↔ 侧栏对话窗 ============
   ...
   侧栏 #sideBoardScroll 以 grid-mini 紧凑卡呈现；结果区头部件（#resultsHead：
...
/* 交换搬家的唯一收口（swApplyMode 链尾、placeChatLog 之后调用；对话记录本体的落位
   ...
   #sideBoardScroll（加 grid-mini 紧凑化），页签文案「继续对话」→「检索结果」。
    起 #resultsHead（标题行/摘要卡/各诚实回显条）随 grid 一起搬进侧栏顶部（卡片列表之前），
...
function placeChatLog() {
    ...
    const inSidebar = !inMain
        ...
        && window.innerWidth > 780
        && document.body.classList.contains("on-query");
    /* 视图交换生效时，对话记录的家是结果区 #chatStage 的上槽（自滚动宽列）——
    ...
    /* 过渡（缺陷修复，与 results.js 同一「淡入微升」语言）：
       ...
       (b) 对话视图 → 左下侧边栏（fromParent===main && inSidebar，结果落地）：工作卡入场**已有**
           CSS swIn（app.css .side-work:not([hidden)]，整卡滑入+淡入，hidden→显示自动重播）
    ...
    if (swapHome) {
        //  视图交换：对话记录住进 #chatStage 上槽；搬家动效/焦点/贴底由链尾 placeChatSuite 统一播。
    ...
    if (heroFlip) {
        ...
        // （顶部对话列）；时长 0.48s ≤ 500ms 上限；对话容器同步淡入微升。
    ...
    //  （输入栏统一）：chat-in-main 态主区不再挂 hero 大框——统一输入条
...
function initSideWork() {
    //  视图交换钮：只切旗标、走 swSync 链整体重排（搬家统一在 placeChatSuite 收口）。
    ...
    _swTabIds.forEach(function (id, i) {
        ...
        //  WAI-ARIA tab 模式的方向键：左右/上下在两个页签间移动焦点并激活（验证）。
    ...
    //  否则 #facetActive(常驻栏,swSync 管) 与 #facetBar(分面条,placeFacetBar 管) 会落到不同容器/被搁在隐藏区（验证）。
    window.addEventListener("resize", function () { placeFacetBar(false); });
    // 「范围」chip：点开合并弹层；点外面 / Esc 收起。摘要跟随两个既有摘要 span
...
/* 推一帧。**只被 search.js 的检索落地共享入口 landRecommendResult 调用**（runRecommend 两个
   落地点 + 零命中救回换屏同走它），绝不放进 applyRecommendResult：
...
export function cbPushCurrent(data, query, popts) {
    ...
    // 零结果文案按结果区**真实给了什么**分口径——有放宽 chips 才许说「给了放宽方式」。
...
function cbReplay(frame) {
    ...
    // 不复位按钮，检索按钮会永久转圈。
    bumpRecSeq();   // 属主是 search.js（ESM）：原 `_recSeq += 1` 裸写在 getter 桥上会 TypeError，必经其写口
    resetSubmitButton();
    _cbReplaying = true;
    try {
    ...
    //  让系统回复气泡的呈现始终跟着游标走（验证：此前 undo/redo 漏了这步重画）。
    cbRenderHistory();
}
...
export function cbRestoreConversation(rows) {
    ...
    const hint = $("cbStepHint");
    if (hint) {
        // 仅对话回看：一行帧都建不起来（当时没有检索）时照实说，不说成「一次结果」。
...
export function cbClear(opts) {
    // 丢弃前归档「仅对话」历史行。账户切换除外（accounts.js 传 archive:false）——
...
/* 幂等请求号（修复）：**每次 ubSubmit 提交生成一个新号**，流式请求与
...
function ubNewReqId() {
    try {
        // 静态门禁止 typeof 守卫（会把打错的函数名静默短路）——crypto 是浏览器内建，
...
function ubRouteBody(text, opts) {
    ...
    const body = Object.assign({
        ...
        current_filters: data.query_constraints || [],
        sources: cfg.sources,
        // LLM 配置覆盖（设置三维度化）：use_llm 由 API 可用性门控
        ...
        model: cfg.model,
        //  AI 执行开关（cfgAgentExec， 合并旧「说了就直接做」+「Agent 规划执行」）：
...
export async function ubSubmit(source, opts) {
    ...
    //  否则两次 runRecommend 并发，先落地的会被后落地的 cbPushCurrent 一并吞并（验证）。
    const _sb = $("submitBtn");
    ...
    //  时序要点：「发送前主区是否已经在进行一段对话」（chat-in-main）必须在 say 上屏**之前**
    ...
    //  发送即清空（微信语义）：说过的话由对话记录与「当前检索句」状态真源保管，
    ...
    box.value = "";
    autoGrow(box);
    // 零反应治理（双来源统一）：气泡**先上屏**再走路由——此前要等 /api/utterance 回来
    ...
    usageBeginTurn();
    // benchmark 采集（默认关）：一轮的开头——用户原话 + 现场。后续 benchfbTurnRoute/
    ...
    try {
        ...
        if (!reply) {
            ...
            if (!res.ok) {
                //  验证：503（服务忙/在途请求过多）与 409（req_id 撞号）
        ...
        // 但绝不静默：操作句被当检索词是最伤的静默错——亮一条 sys 错误泡
        ...
        // 路由层失败也进 usage 事件流（stage:"route" 与检索失败 stage:"search" 分开算）。
...
/* 流式路由请求：POST /api/utterance 带 additive stream:true → text/event-stream。
   ...
   逐条喂 onStep 实时上屏）→ preliminary / tool_start（即时帧，同走 onStep 第二参
   分种类）→ final（与非流式**同形**的
    ...
    for (;;) {
        ...
        while ((cut = buf.indexOf("\n\n")) >= 0) {
            ...
            frame.split("\n").forEach(function (line) {
                ...
                // 其余事件种类一律忽略（additive 演进——旧前端对新流完全兼容，不留硬耦合）
            });
        }
    }
...
function _applyBatchDecision(text, reply, decision, opts) {
    ...
    /*唯一气泡规则——纯检索计划：批次回执是唯一气泡（actFinish 由 _execReceiptCovered 抑制）；
    ...
    if (decision && decision.mode === "display" && decision.view) {
        ...
        // 换屏留痕优先用采纳批随行的确定性披露句（即「哪些词没被理解/被丢弃」）；批上无
    ...
    // 混合计划时唯一气泡归 actFinish，批次回执不推（否则两颗气泡）。
...
/* （supersede 即丢弃）：剔除被胜者（activeBatchId 那批）覆盖的批次。胜者保留；其余若被
...
function ubDispatch(text, reply, fromChat, wasChat) {
    ...
    //  rescue 去重（评估迁移步骤 1）：final 帧 plan.steps 沿 runRecommend →
    ...
    if (route === "search") {
        ...
        //  补充（同缺陷的 polish-on 面）：preliminary_final 的 b 档判定含「润色不会跑」
        ...
        // 该批镜像、不是环内采纳，a 档不得截胡——落 c 档照旧 runRecommend（旧行为：
        ...
        if (fromChat || wasChat) {
            ...
            // 改写回显不再提前 push 一颗独立 sys（那会挂到 preliminary 先行帧、
            // 渲染时 ≠ 最终帧误显示「查看历史回复」），改作 _cbProgHint 前缀——runRecommend 落地时
            // 由 cbPushCurrent 的 cbProgressDone 拼进完成句（「我把这句按…检索；检索完成：…」），
            // 挂最终帧、语序照旧。
            // 回归钉：wasChat 漏掉时，chat-in-main 主框发检索句会把前段工具对话清掉（keepConv 只看 fromChat）。
    ...
    if (route === "tool") {
        /* 三 flag（scoped 路由/RAG 工具/多批）ON 时检索改由
           ...
           pre-loop 先行批（「初步结果」徽标也摘不掉），环内采纳批与批次切换器全丢（验证核实：换条件
           一轮后屏上仍是上一轮结果）。消费口径（与 search 档 三档同一纪律）：
        ...
        // tool 档「活跃批落地」是**异步**推帧（runRecommend 内部 await
    ...
    // echo_zh 缺失 ≠ 没听懂——后端契约里 none 路由恒带 echo_zh，拿不到就是
    ...
    //  LLM 缺席的规则兜底回音（via=rule）末尾那句「可以再发一次试试」
    ...
    //  婉拒候选 chips：LLM 真判 none 的死胡同回音下方给 2~3 颗可点候选
...
/* 行动流收尾保险（基本是 no-op）：流式期间 arx 不再开播（过程展示归信息流工具行），
...
function ubDispatchAction(said, plan, fromChat, wasChat) {
    ...
    if (!hasResults && plan.requires_results) {
        ...
        // keepConv 与 search 档同判据（fromChat || wasChat）——chat-in-main 主框来的
        // 工具句也不许清掉前段对话。say 的存续分两态：keepConv 时 ubSubmit
        // 那句 say 还在（sayPushed 防双泡）；新时间线（hero 首句）会被 runRecommend 清场，
        // 必须给 sayText 让它清完重推——否则原话从对话流里消失。
        // 与 search 档同一口径：原话不在在途期间写回输入框——经 queryOverride
    ...
    // **不需要结果的执行动词直接派发**（curate.* 五动词，作用对象是本地语料库而非屏上结果）。
    // 管护句关键词往往零命中——turn pipeline 的 LLM 分流带原始查询，这类句子不再被关键词阶段毙掉。
    //  动作句不是检索句，不该占住「当前检索句」真源——发送即清空后，
    ...
    setLastRecommendData(data);   // 属主是 search.js（ESM）：写必经 setter
    usageLog(USAGE_KINDS.conv, { mode: "chat" });
    //  curate.* 不过「AI 执行」开关、永远直派（全自动化：plan→apply 链式直推，
    ...
    // 执行过程与总结都长在对话流里（行动流 + 总结 sys），结果区不再需要为面板提前露头
    // （enterResultsLayout 块随 #curatePanel/#actReceipt 一起退役）。
...
/* 「继续对话」与「初次对话」对齐：这句话不是改条件时，别只回一句听不懂。
   ...
   ACTION_MARKERS 与标识符判据两个既有单一真源，前端不另抄一份词表）：

     action     → 在**当前这批结果**上打开任务包（不重搜；原因见函数体注释）
     identifier → 整句送回主搜索框重搜，天然带上标识符直链反查
...
/* 统一框「先检索后派发」的回注（act.js actAfterSearch 的 actPlan 档用）：把执行注记挂到那句**原话**
   上。目标必须按 kind 找：actDispatchPlan 的总结 sys（actFinish）先落地，
...
function cbRouteAsFirstBox(text, route, actionMarkers) {
    if (route === "action") {
        ...
        // 空查询命中全库（实测 8022 条），于是「打包当前结果」悄悄变成「打包全库前 10 条」——
...
/* hadSay：本 turn 是否已经 push 过一条「说」（统一框打字发送路径 ubDispatch 才有）。显式传参，
   不再靠「_cbLog 末尾 frameId 是否为 null」这种隐式尾状态判断——那在并发提交下会被别的在途消息干扰（验证）。
...
function cbApplyPlan(plan, hadSay) {
    ...
    if (plan.status === "needs_confirm") {
        ...
        // 应用后的这一步会作为一条「细化」消息进记录；走错了也能随时 分支/回退（双键），撤销成本极低。
    ...
    if (plan.route === "action" || plan.route === "identifier") {
        ...
        if (done) {
            // action 只开下载面板、不检索 → 不产新帧，得手动把这条用户消息挂到**当前**帧上（供「查看历史回复」按钮定位），
            //  否则它 frameId 恒 null：永远没按钮、且被帧剪枝的 frameId!=null 过滤静默吃掉（验证）。
...
function cbCommit(plan, hadSay) {
    ...
    if (queryInput && String(queryInput.value || "") !== request.query) {
        ...
        setFacetState({ queryHits: [] });   // 换了句子，原始命中的旧快照必须失效，否则会与新条件同屏矛盾（重赋值经属主 setter）
    }
    setFacetState({   // 四个分面状态的重赋值必经 results.js（属主）setter
    ...
    if (!(request.facet_filters || []).length) setFacetState({ facetFilters: [] });   // 重赋值经属主 setter
    ...
    // （条件改了、也重搜了，聊天记录里却什么都没有），故补一条（验证）。用显式 hadSay 而非「_cbLog 末尾 frameId
    //  是否为 null」的隐式尾状态：后者在两次提交并发在途时会被对方的未回填消息干扰、把该补的这条误跳过（验证）。
...
async function cbChoose(choiceId) {
    ...
    //  否则两次 cbCommit 并发在途，且 cbPushCurrent 会把两批改动吞并进同一帧（验证）。
    const _sb = $("submitBtn");
...
function cbResetAllConditions() {
    resetFacetState();   // 四个分面状态一起归零，经 results.js（属主）setter
    cbCancel();
...
function cbRemoveFacet(dim) {
    setFacetState({ facetFilters: (_facetFilters || []).filter(function (f) { return f.dim !== dim; }) });   // 重赋值经属主 setter（读取走 import 活绑定）
    runRecommend({ keepFacets: true });
}

export function initCondBoard() {
    // benchfb 收尾通知的绑定回调注册（chat 轮收尾 → 评分卡绑到本轮系统回复 entry）。
    benchfbOnChatEntry(_bfOnChatClosed);
    // 把输入框自动伸展工具注入 benchfb（断环——benchfb 不 import interactions，
    // 那会把它拉进 SCC 环；评语输入框的动态 textarea 经它随行数伸展）。
    benchfbSetAutoGrow(autoGrow);
    //  历史打标钩子注册进 core（core→board 反向边切断，改注册反转；
