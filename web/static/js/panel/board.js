...
/* 条件板 + 统一对话窗口的界面层。
   ...
   2. 统一对话窗口（uni1 起； cur3 微信式输入行）：首屏 `#queryInput`（起始对话）与
...
function swAvailable() {
    ...
    return {
        ...
        //  反例（图4）：首次检索**进行中**只有 say——此时落地页上侧栏工作卡
...
export function swSync() {
    ...
    //  双窗口（导航卡 + 本工作卡并立）→ 导航卡折叠成两列布局（用户点图：凡是双窗口，
...
/* ============ 聊天 + 细化记录 ============
   ...
   撤销栈里是完整的后端响应，落盘会撑爆配额（test_board_only_persists 钉死本文件只许存 UI 偏好）。
    ...
    起，对话记录会随**历史记录**一起留存 —— 但写盘的是 core.js 的 `pushHist`，
...
export function cbLogPush(kind, text, opts) {
    ...
    // isError：仅 sys 用（R2-9）——传输/服务异常回音（非「没听懂」）渲染成错误样式气泡，
...
/* ---------- 进度泡（点8）：发送后立即出现的那颗系统回复气泡 ----------
   继续对话发一句 → 用户气泡下方**立刻**浮出一颗系统气泡：三点跳动（起数字百分比撤下）+ 流式档
...
function _ftSummaryBlock(flow) {
    ...
    /* /w3：ft-compressed 两帧余量（计数在 flowFinish 置 2），让摘要原地淡入活到
...
export function cbRenderHistory() {
    ...
    //  点9：每帧的「查看历史回复」入口**并入该帧的系统回复泡**（footer 内联链接）——
    ...
    //  检索在途且进度泡已蜕变成文字：实时三点挂在最后一颗系统泡的**右端**继续滚（起不再滚百分比）。
    ...
    box.innerHTML = _cbLog.map(function (e, i) {
        ...
        //  思考指示（点8， 重定）：**恒在流式输出最底部**（turns → 行动流 → 轨迹 → 本块）——
...
/* ============ 检索范围控件（来源 / 时间）============
   ...
   点5：「范围」触发器从带摘要的 chip 改成**与发送键同形的圆钮**——
...
function scopePopOpen(open) {
    ...
    if (open) {
        ...
        //  右坞（用户点图）：当前选中维是自动、另一维被自定义过 → 跳到自定义那维。
        scopeDockWire();
        const srcAuto = !!($("srcPanel") && $("srcPanel").classList.contains("mode-auto"));
        const timeAuto = !!($("timePanel") && $("timePanel").classList.contains("mode-auto"));
...
function placeScopeControls() {
    ...
    if (inSide && host && pop) {
        ...
        if ($("srcPanel")) $("srcPanel").hidden = false;
        if ($("timePanel")) $("timePanel").hidden = false;
        //  用户点图：右坞双圆钮 + 单面板——按钮只绑一次；开面板时若当前维是自动、
...
/* 弹层「右坞双圆钮 + 单面板」（用户点图）：数据来源/发表时间两枚圆钮竖排在右，
...
/* ============ 对话记录（#cbHistory）落位 ============
   p10（用户图2 指令）重做主区/侧栏分工，取代 cur3「主区永不再有对话界面」：
...
/* ============ 视图交换：主区结果网格 ↔ 侧栏对话窗 ============
   ...
   侧栏 #sideBoardScroll 以 grid-mini 紧凑卡呈现； 起结果区头部件（#resultsHead：
...
function placeChatLog() {
    ...
    /* 过渡（缺陷修复，与 results.js 同一「淡入微升」语言； R2-10 复活 hero 位移）：
...
/* 推一帧。**只被 search.js 的检索落地共享入口 landRecommendResult 调用**（runRecommend 两个
   落地点 + 零命中救回换屏同走它， sr1），绝不放进 applyRecommendResult：
...
export function cbPushCurrent(data, query, popts) {
    ...
    // R2-2 ：零结果文案按结果区**真实给了什么**分口径——有放宽 chips 才许说「给了放宽方式」。
...
export function cbRestoreConversation(rows) {
    ...
    //  恢复路径与推帧同一截断闸（验证决定 R9 修正件）：历史行数 > CB_MAX_FRAMES 时
...
function ubRouteBody(text, opts) {
    ...
    //  （设计 §5.2）：建议动作 hint——任务卡模板**未经编辑**时由 ladder.js 显式传入
...
export async function ubSubmit(source, opts) {
    ...
    benchfbTurnBegin(text, {
        ...
        //  （设计 §5.5）：任务卡/chip 生成文本提交时携带 true/false（普通手打无此键）。
        templateOriginated: opts.templateOriginated,
    });
    const _bfT0 = Date.now();
    //  流式规划（设计 §5.2）：AI 执行开且扩展可用时走 SSE——规划步骤随流实时上屏
    //  （claudecode 式），行动流是主反馈；AI 执行关 / 扩展缺失时维持不确定态加载（起无数字）。
    ...
    //  R2-9 × R2-10 （产品侧拍板）：流式档**同样起跑**； 起数字里程表退役，
    ...
    try {
        ...
        //  但绝不静默（触发点审计 H-02）：操作句被当检索词是最伤的静默错——亮一条 sys 错误泡
...
/* -覆盖策略修复（设计 §10.3）· 统一应用 selectDisplayBatch 的决定。search a 档与 route=tool 档
...
/* 任务3c：空态卡放宽 chips（results.js applyRelaxation）与选择条需要状态同步——
...
function ubDispatch(text, reply, fromChat, wasChat) {
    ...
    if (route === "search") {
        ubStreamRunSettle();          //  防御性收尾（起流式不开 arx，恒 no-op）
        ...
        //  （M3 波2，验证 复核中5 跨契约缺陷）：preliminary_final 分支必须**先于**
        ...
        if (fromChat || wasChat) {
            ...
            //  （任务4 修复）：改写回显不再提前 push 一颗独立 sys（那会挂到 preliminary 先行帧、
            ...
            //  handSubmit:true（夜班 B 路）：统一框是用户亲手提交的主检索路径，
    ...
    if (route === "tool") {
        /* （M3 波2 真链路核实）：三 flag（scoped 路由/RAG 工具/多批）ON 时检索改由
        ...
        //  （任务4 修复）：tool 档「活跃批落地」是**异步**推帧（runRecommend 内部 await
        ...
        if (_tpay && _tpay.ok === true) {
            //  -覆盖策略修复（设计 §10.3）：与 search a 档共用同一选择函数——严格更高级才
    ...
    // R2-9 ：echo_zh 缺失 ≠ 没听懂——后端契约里 none 路由恒带 echo_zh，拿不到就是
    ...
    //  婉拒候选 chips（五机制批）：LLM 真判 none 的死胡同回音下方给 2~3 颗可点候选
...
/* 行动流收尾保险（起基本是 no-op）：流式期间 arx 不再开播（过程展示归信息流工具行），
