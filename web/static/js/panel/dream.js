...
/* ============================================================================
 * dream.js —— dream 记忆界面层（**ES Modules 试点**：Phase C 模块化重写的第一个模块）
 * ----------------------------------------------------------------------------
 * 加载：index.html 以 `<script type="module">` 引入（ deferred，先于 DOMContentLoaded 执行完）。
 * 依赖全部经 import：#memory / #dream_core（C1 同批）、#core 与 #shell（C5 起绞杀桥全退役、
...
/* 预览清单：每条一个勾选（默认全选），底部「写入所选 / 放弃」。
   B6（2026-08-08）：每条附出处摘录（服务端出处核验放行的 evidence 原文片段）；
