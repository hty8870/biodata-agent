...
/* C2 起本文件是 ES Module：core 的工具/常量、usage 层、reuse_pack 的 downloadTextBlob 与
   fav_folders 的 openFavPopover 经 import 取（cards↔fav_folders 成环，绑定只在函数体内使用）。
   dataset_page/browse/fav_folders/interactions/act 经 import 取卡片与介绍渲染函数（C5 起绞杀桥全退役）。 */
...
/* opts 一览：rank=结果序号角标；compact=精简卡（介绍弹窗下半窗的兼容网格/对比用——
   隐藏「数据集详情」「查看全部文件」按钮、rank 角标与 F10 承诺行，保留心形/徽章/CTA 直链）。 */
export function buildCard(rawItem, opts) {
    opts = opts || {};
    const it = normalizeItem(rawItem);
    const card = document.createElement("article");
    const library = opts.variant === "library";   // ku1：我的库卡片化（候选/收藏/更新 delta）——保留「打开数据集页」「数据集详情」，省下载与文件展开
    ...
    const tags = [["物种", it.species], ["组织", it.tissue], ["疾病", it.disease]]
        .filter(([, v]) => v)
        // ux2c：tags 容器限高截断（CSS max-height 2 行），每个 tag 的 title 兜住完整值——悬浮可见全文，不丢信息、不靠 JS 测量
    ...
    // ux3b：「查看介绍」改为打开**独立浏览器标签页** /dataset（不再模态弹窗）。链接携 uid/url/name/source；
    ...
    // ux2：改名「数据集详情 ↗」并从 card-foot 文字链升格为 .card-cta 之下的独立全宽按钮行（.btn-detail）。
    ...
    // N11 国内可达性徽章：按下载 host 推断（非实测速度）。未识别的 host 不显示，不臆造。
    ...
    const filesBtnHtml = (!opts.compact && !library && nFiles > 1)
        ...
        : "";
    // R2-9 P2-1：样本量/基因数过千分位（与首页覆盖声明、可行性面板同一记数法；年份不过——
    // 4 位年份会被加成 2,025）。R2-9 P2-2：每段包 .sample-bit nowrap，折行只许发生在分隔符上——
    // 此前「发表」与年份被拆到两行（年份孤挂）。
    ...
    // ux2c 底部钉齐：无文件按钮的完整卡补同盒尺寸占位槽（visibility:hidden、aria-hidden，结构与真实按钮一致 → 高度恒等），
    ...
    const filesBtn = card.querySelector("button.files-open");   // ux2c：占位槽是 span.files-open-slot，只给真按钮绑事件
    ...
    if (detailLink) detailLink.addEventListener("click", () => {
        // 2026-08-15 ta2 M-05：handoff 双写——按 uid 分键是主通道（多标签连开不同数据集互不覆盖）；
        ...
        // v2（ov1-tel1）：uid/pos 归因到具体条目与名次（r 保留——聚合器 usageRankBuckets 读它）。
        // v3（ov1-fix1a）：卡级动作经 usageLogCardAction——结果页的卡带展示快照 tid/iid，
...
export function renderIntroduction(it, opts) {
    ...
    // N6 样本量语义提醒：只在服务端返回（确定性单一真源 units.sample_size_note）时展示，
    ...
    // N5 · LLM 中文导读（实验性、opt-in）。`llm_status` 键**只在用户点过「AI 导读」**（llm=1）后才出现，
...
export function bindIntroductionBody(body, it, opts) {
    ...
    // ux3b：介绍改独立浏览器标签页后不再有模态 tab 栏，本函数只渲染正文；子标签（全部文件/兼容/FAIR/引文/对比）
...
// N5：用户显式点「AI 中文导读」→ 带 llm=1 重取介绍（双层门的**客户端**那半：管理员开 ENABLE_LLM +
...
/* ux3b：openIntroModal / closeIntroModal（介绍模态弹窗）已退役——「查看介绍」改为打开独立浏览器标签页
...
// N13 元数据兼容分组：同物种 + 兼容 chemistry/platform 的其它数据集。
...
/* ux3b：兼容数据集卡**完全对齐真正的检索结果卡**——直接用缺省 buildCard（心形 / 徽章 / 标题 / tags /
...
/* ---------- 单条引文导出（N10 可发现性）：tab 栏「导出引文 RIS/BibTeX」推挤下拉 ----------
...
export function closeFilesModal() {
    ...
    if (MOTION) {
        ...
        setTimeout(done, 420);   // F19 同类兜底：rAF 冻结时 tween 不完成 → 强制收尾（滚动锁不残留）
    } else { done(); }
}
