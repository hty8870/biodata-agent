...
/* 检索落地共享入口（检索工具化 sr1，设计 §4）：runRecommend 的两个落地点与
   ...
   prelim1（初步结果先行，设计 §2.5.1）新增三档 opts：
...
export async function runRecommend(opts) {
    ...
    //  keepConv（图5）：分面/抑制照旧清空，但**对话记录保留**——
    // 「接着这句往下走」的重跑不是「另起一段对话」，整段对话被清掉正是用户报的异常。
    //  接线方在 board 路径（原回执 chip「按原话重新检索」已于 pack1 退役，机制保留）。
    if (!keep) {
        resetFacetState();   // 四个分面状态的属主是 results.js（C2 起 ESM）：重赋值必经属主 setter
        //  A2：清对话前先归档「仅对话」——纯工具对话从未走过 pushHist，
    ...
    if (cached) {   // 命中：秒出、不走网络
        if (myGen === _recSeq) {
            try {
                ...
                // R2-10 ：收尾用**完成语义**（finishProgress 同步摘 loading），不用 resetSubmitButton
    ...
    //  加载态由 .loading 类接管：按钮保留静态 loading（图标/文案不动， 起数字里程表已退役），
