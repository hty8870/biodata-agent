...
/* C3 起本文件是 ES Module：core/shell/results/facets/progress/usage 层经 import 取
...
/* prelim1（设计 §2.5.1）：「初步结果」徽标的唯一写口——任何结果重渲（落地/历史回看/回退）
...
/* 当前检索参数快照（prelim1，设计 §2.4）：runRecommend 发 /api/recommend 与 board.js
   ubRouteBody 发 /api/utterance（后端 pre-loop 检索用）**同源构造**——加字段时两处一起变，
   不许两处口径漂移。polish（2026-08-16 pl1b 收尾）：后端 preliminary_final 判定要看
...
export function applyRecommendResult(data, query, opts) {
    opts = opts || {};
    setLastInterpretation((data && data.interpretation) || null);   // 属主是 interactions.js（C3 起 ESM）：写必经 setter
    LAST_RECOMMEND_DATA = data || null;
    _prelimBadge(false);   // prelim1：任何结果重渲先摘「初步结果」徽标（fromPrelim 落地由共享入口随后亮回）
    renderStatus(data);
    // ov1-fix1a（schema v3 imp）：展示归因上下文随渲染下发——policy 优先用后端响应里的
...
/* 检索落地共享入口（2026-08-16 检索工具化 sr1，设计 §4）：runRecommend 的两个落地点与
   ...
   prelim1（2026-08-16 初步结果先行，设计 §2.5.1）新增三档 opts：
   ...
   ku3-w5：零命中落地不再自动跑救回门禁——救回选项由 board.js 选择条呈现（零命中 pill
...
export function landRecommendResult(data, query, opts) {
    ...
    // ux1（2026-08-18）：planSteps（utterance final 帧的环内工具记录）的 verb 随帧落地——
    ...
    // ku3-w5：零命中救回不再自动跑——零命中时的放宽/换词选项改由 board.js 的救回选择条
...
/* 请求代号：分面芯片在请求进行中仍可点（有意保持响应），故可能并发多个 runRecommend。
   ...
   丢弃晚到的旧响应，杜绝「结果/计数属于旧筛选、面包屑却是新筛选」的错配（评审 medium）。 */
let _recSeq = 0;
...
/* prelim1：ubSubmit 发送时快照检索代（preliminary 帧落地前的代际闸——用户在环跑期间
...
export async function runRecommend(opts) {
    ...
    // 接线方在 board 路径（原回执 chip「按原话重新检索」已于 2026-08-16 pack1 退役，机制保留）。
    if (!keep) {
        resetFacetState();   // 四个分面状态的属主是 results.js（C2 起 ESM）：重赋值必经属主 setter
        // A2（2026-08-04）：清对话前先归档「仅对话」——纯工具对话从未走过 pushHist，
        ...
        // 点8 进度泡：新检索（hero 首查 / chat 判成新检索）也有「发送即回一颗进度泡、落地渐变成完成摘要」。
    ...
    // prelim1 final a 档（设计 §2.1）：环内采纳的更优结果随 utterance final 抵达
        ...
        if (myGen === _recSeq) {
            try {
                ...
                usageLogSearch(cached, query, Object.assign({}, opts, { cached: true, telemetryScope: telemetryScope,
                    policyParts: { strategy: reqBody.strategy, rerank: reqBody.rerank, recall: reqBody.recall } }));
                // benchmark 采集（b1）：缓存命中同样是「问了一句并看到结果」——请求体在这里脱敏
                ...
                // R2-10 P2-2：收尾用**完成语义**（finishProgress 同步摘 loading），不用 resetSubmitButton
                // 的取消语义——统一框发重复查询时是同一在途请求的继续，秒出＝这次检索瞬间完成。
                // 放在 myGen 守卫**内**：只有当前代有权收尾；若本代已被更晚的请求接管，
                // 那一代可能正在合法加载，收尾会抹掉它的 loading 态。
                // dl2（2026-08-19）补 finally 兜底：上面渲染/打点任何一步抛错（典型：新旧 JS 混合缓存
    ...
    // 加载态由 .loading 类接管：按钮保留静态 loading（图标/文案不动，ux2 起数字里程表已退役），
    ...
    try {
        ...
        if (prefetched) {
            ...
            // 否则这次退化结果会粘住整会话、稍后重试也被缓存拦截、永远打不到后端（评审确认的 med 级坑）。
        ...
        usageLogSearch(data, query, Object.assign({}, opts, { telemetryScope: telemetryScope,
            policyParts: { strategy: reqBody.strategy, rerank: reqBody.rerank, recall: reqBody.recall } }));
        // benchmark 采集（b1）：完整请求（脱敏）+ 完整响应（results 全字段/search_trace 全步）入档。
        ...
        cbProgressDone("检索失败：" + emsg);   // 点8 进度泡：失败也要如实收尾成文字（无进度泡时 no-op）
        _prelimBadge(false);   // prelim1：失败屏不是先行结果，徽标顺手摘掉
        ...
        // 「检索失败」屏上就挂着旧一次的查询条件（含可点的 忽略/× 按钮）——正是本分支要防的陈旧背书（对抗评审 major）。
...
/* allDatasets/bs 两个浏览态已归位 browse.js（C3：它们从来只服务浏览/收藏/历史，本文件从不读写——
