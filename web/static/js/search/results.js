...
/* C2 起本文件是 ES Module：core 的工具、board_core 的 CB_DIM_LABEL、#cards 的 buildCard、
   ...
   search/board/browse/interactions/act/accounts 经 import 取本文件导出（C5 起绞杀桥全退役）。 */
...
import { benchfbAfterSearchRender } from "#benchfb";   // fb1：结果区重建后重新挂 hero 评分卡槽位

/* ---------- 结果曝光追踪（2026-08-22 ov1-tel1；ov1-fix1a 重写为 500ms 状态机，schema v3）----------
...
/* ---------- imp 展示事件 + 卡级归因（2026-08-22 ov1-fix1a，schema v3）----------
...
/* 回退附注：**每一句都由后端出**（`step.fallback_note`，单一真源 workflow._fallback_note）。
   ...
   谁都看不出它坏了。`shell.js` 的开发者面板早就守着「尝试过但失败才算故障」（F5 轮），
   唯独这句用户天天看见的话没守。

   所以措辞的产地只有一个：后端。前端只负责把层名和后端给的那半句话拼起来。
   老后端没有这个字段时退到「没能完成」——**宁可把「没启用」说重，也绝不把故障说轻**。 */
/* 后端 fallback_note 里的内部工程术语 → 人话（R2-9 P2-6）。后端措辞是诚实真源（未启用/没能完成
...
function fallbackLayerNotes(steps, wanted) {
    ...
    (steps || []).forEach(function (s) {
        ...
        // 2026-08-10 评审 P2-9：note 是后端任意字符串（可含 provider 原始报错），返回值最终经
        // innerHTML 上屏（renderResultSummary）——拼接前必须转义，XSS 面封死在产地。
        notes.push(wanted[s.id] + (note ? "（" + escapeHtml(note) + "）" : "没能完成"));
    });
    return notes;
}
/* ux3/ux3c 结果摘要：把旧「N 条 · 方法 · 库中匹配 N 条」计数行、「本次检索用了什么」trace 折叠、覆盖缺口
   三处合并成**一段自然语言**。ux3c：逐步 trace 明细「查看每一步」折叠已删（用户判冗余——方法句已含回退附注，
...
function renderResultSummary(data) {
    ...
    if (!trace || !Array.isArray(trace.steps)) {
        ...
        if (total > 0) {
            ...
            // sum1（2026-08-16 用户）：方法层关键词上行内高光，阅读时一眼定位用了哪几层——
    ...
    // vs4：交换态侧栏摘要压 2 行截断，title 兜全文（正常态不截断，title 无害）
    txt.title = txt.textContent || "";
    box.hidden = false;
}

// ux3b/ux3d 对比池：把本次结果集写进 localStorage，供「数据集详情」独立标签页的「数据集对比」子页取用（跨同源标签共享）。
// ux3d 改存**完整归一化记录**（原只存轻量五字段）：全屏并排对比要把选中数据集当作 handoff 交给一个完整
...
// setItem 失败（配额）时**删除旧 key**（2026-08-09 codex P1-4：旧实现空 catch——写失败会留着上一查询的
...
/* fp1（2026-08-17）：pill 文案撞车时的批次来源前缀——初步批 label=本轮原话、rank 批
...
/* ku1-覆盖策略修复（设计 §10.3）：非活动备选批的排序层标注——「新检索 · 规则排序」这类
...
export function renderBatchSwitcher(data) {
    const bar = $("batchBar");
    if (!bar) return;
    /* ku3-w3b（用户 2026-08-24 手写定稿「检索结果页不再展示该 pill」）：结果头部的批次 pill
...
export function switchBatch(batchId) {
    ...
    // ku2-w1：用归一 id 比「已在这批」——批缺 batch_id 时 data.active_batch 可为非空合成键，
    ...
    // keepTurn（ov1-fix1a）：批次切换是**同一轮检索**内的换屏，imp 归因沿用当前轮 tid，
...
/* 渲染钩子（注册式反转，2026-08-22 engagement F1）：结果区每次重建（新检索/分面重跑/放宽/
...
/* 下一步行动阶梯（2026-08-22 engagement P6，设计 §5）：同一注册式反转模式——ladder.js 经
...
export function renderResults(items, data, imp) {
    _seenBegin();   // ov1-tel1：每次结果区重渲 = 一次新展示（上一屏曝光在 begin 内先收尾）
    // R2-9 P1-2：可行性面板的数据是按「点开那一刻的查询」聚合的——任何一次新结果渲染
    ...
    if (_ladderRenderHook) _ladderRenderHook(data);   // eng-p6：结果区阶梯 chips / 过宽收窄建议
    const wrap = $("resultsWrap");
    const firstReveal = wrap.style.display !== "block";
    wrap.style.display = "block";
    // P2（2026-08-03 agent2）：首页 → 结果区的容器级过渡——整块淡入 + 微升（0.45s power2.out，
    ...
    renderResultSummary(data);     // ux3：方法句 + 计数 + 逐步 trace 合并成一段摘要（替换旧 resultsCount/resultsSource/renderSearchTrace）
    renderBatchSwitcher(data);     // mb1：多批切换器（result_batches>1 才现身；缺失/≤1 批恒 hidden=回退）
    writeComparePool(items);       // ux3b：把本次结果写进对比池 localStorage，供「数据集详情」新标签页的「数据集对比」子页取用
    ...
    renderUnusedQueryTerms(data);  // N1 静默丢词：无对应筛选维度、被静默丢弃的实义描述词显式回显
    renderOrHandling(data);        // 「A 或 B」实际怎么执行的（同维度多值＝或；跨维度只能同时满足）
    renderActionHint(data);        // 执行类说法（打包/下载脚本/导出引文）：只指路、不代劳
    renderIdentifierLookup(data);  // N8 标识符精确反查：贴 DOI/编号直达记录 / GEO/SRA 诚实 fail-closed
    renderFeasibilityBar(data);    // N12 可行性概览入口：有结果时露出「这个方向有多少可复用数据」按钮
    ...
    if (!items.length) {
        ...
        if (data && data.resolution_status === "clarification_required" && data.clarification) {
            ...
            // L8：空态卡与有结果卡同一入场（MOTION 门控）——否则首次检索的 hero FLIP 位移途中，
            // slim 控制台会扫过已瞬时可见的空态卡（评审实测「压住 32px」；有结果态靠同一 reveal 天然遮蔽）。
            revealCards(grid.children, false);
            return;
        }
        // 分面细化把结果收窄到 0：不提「放宽 query」（空是筛选所致），提示移除一个筛选。
        // 文案不写死"上方/左侧"——面包屑落位随状态在结果区上方或左侧栏「数据细化」面板间搬家（评审 low）。
        ...
        if (_facetFilters.length && !(data && data.error)) {
            ...
            revealCards(grid.children, false);   // L8：同上空态入场对齐
            return;
        }
        // error:true（网络/后端出错）：空**不是**筛选/语义所致，如实报「检索失败」，绝不套用「安全弃权」话术甩锅用户查询。
        // 正文与详情分工（2026-08-04 P2-2）：data.markdown 已是人话（网络层错误在 search.js 翻好），
        ...
        if (data && data.resolution_status === "abstained") {
            ...
            // 2026-08-03 agent2：后端文案已收敛成一句话（哪个词没收录、去掉可能有结果），
    ...
    // fb1：结果区重建（新检索 / 分面重跑 / 放宽 / 历史回看）后把 hero 轮评分卡挂回顶部槽位——
    // 空态/失败/弃权/澄清各分支已在上面 return，那张卡只对「有结果」的屏出现。
    benchfbAfterSearchRender();
    _seenObserve(grid);   // ov1-tel1：hero/横幅挂完再观察——非卡片节点不影响 .card 名次口径
    _emitImpression(items, imp);   // ov1-fix1a：imp 展示事件 + 每张卡绑不可变归因快照
}
...
function applyClarification(optionId) {
    ...
    if (optionId === "exclude_raw") {
        // 2026-08-15 ta2 M-07：只改写锚定在 fastq/原始数据 短语上的那一处否定词——
...
/* 放宽预览**实际生效**的条件集（普查 P2-1 的真源，不是旧解析快照）：
...
function applyRelaxation(data, i) {
    // ku3-w6 任务3c：点空态卡放宽 chip 进入就地预览（不重渲对话流）——若琥珀色选择条还开着同一
    ...
    // v2（ov1-tel1）：补维度/键与降级档的被忽略词（截 5 个），分析侧才知放宽的是哪一类条件。
    ...
    const _uqt = $("unusedQueryTerms"); if (_uqt) _uqt.hidden = true;   // N1：同理隐去陈旧的「未作为筛选维度」框
    const _idlk = $("identifierLookup"); if (_idlk) _idlk.hidden = true;   // N8：放宽预览时隐去陈旧的标识符反查框
    if (_ladderRenderHook) _ladderRenderHook(null);   // eng-p6：放宽预览不是完整结果屏，阶梯 chips/收窄建议一并隐去
    // 只收起条件板里的「原严格条件」展示层（摘要 + 经典条件行），**不隐藏整块 #condBoard**——
    // telegram（对话记录）态下整块条件板就是聊天窗（#cbHistory 在其内、由 placeCondBoard 搬进侧栏），
    // 整块 hidden 会把用户整段对话记录一起抹掉、直到下条消息才回来（用户点1 反馈的真 bug）。
    ...
    // 普查 P2-1：「查询条件」chips 同步换成预览**实际生效**的条件——旧代码只藏了 condBoard
    ...
    // chips/分面组也按原数据复原（renderFacets 重跑 = 回到放宽预览前的实时命中镜像，P2-1）。
    ...
    revealCards(grid.querySelectorAll(".card"), false);
    _seenObserve(grid);   // ov1-tel1：放宽预览的卡片同样进曝光追踪
    // ov1-fix1a：放宽预览也是一次真实展示，同样发 imp + 绑卡级快照。tid 沿用当前轮
...
function renderCoverageCaveats(data) {
    ...
    if (caveats.length) {
        const rows = caveats.map((c, i) => {
            ...
            // 2026-08-01 ux6③（修正 ux5 误读）：用户要的是**多档放宽策略选择**，不是「看看是哪些」记录预览。
...
/* 「不按「X」筛选」（ux6③ 放宽方式第二档 drop）：把该维条件**整个放开**——已标注为其它取值的也进来。
...
// N1 静默丢词诚实层回显：用户输入了**结构上无对应筛选维度**的实义描述词（性别/年龄/受试者/功能类，如
...
// N8 标识符精确反查回显：query 本身是标识符时。indexed 且命中 → 直达该数据集卡片；
...
function renderIdentifierLookup(data) {
    ...
    // 共享标识符多命中（A6）：message 只列编号，候选的名称/来源被丢弃用户无从挑选——
...
// N12 可行性概览：有结果时露出入口；点开对当前查询做全命中集聚合（候选数/总细胞量下限/分布/缺口）。
// ux2c：按钮收进结果头行（.results-head-acts，独立横带已拆），显隐直接钉按钮本身；展开面板 #feasibilityPanel 仍在结果区原位。
