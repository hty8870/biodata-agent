...
/* 后端 fallback_note 里的内部工程术语 → 人话（R2-9）。后端措辞是诚实真源（未启用/没能完成
...
/* -覆盖策略修复（设计 §10.3）：非活动备选批的排序层标注——「新检索 · 规则排序」这类
...
/* 渲染钩子（注册式反转， engagement F1）：结果区每次重建（新检索/分面重跑/放宽/
...
/* 下一步行动阶梯（engagement P6，设计 §5）：同一注册式反转模式——ladder.js 经
...
export function renderResults(items, data, imp) {
    ...
    // R2-9 ：可行性面板的数据是按「点开那一刻的查询」聚合的——任何一次新结果渲染
    ...
    if (!items.length) {
        ...
        if (data && data.resolution_status === "clarification_required" && data.clarification) {
            ...
            //  slim 控制台会扫过已瞬时可见的空态卡（验证 「压住 32px」；有结果态靠同一 reveal 天然遮蔽）。
...
function applyClarification(optionId) {
    ...
    if (optionId === "exclude_raw") {
        //  M-07：只改写锚定在 fastq/原始数据 短语上的那一处否定词——
...
function applyRelaxation(data, i) {
    //  任务3c：点空态卡放宽 chip 进入就地预览（不重渲对话流）——若琥珀色选择条还开着同一
