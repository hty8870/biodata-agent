...
/* ============ 数据细化「提交」暂存（用户 2026-07-24·点4）============
   ...
   **单一 choke point**：_facetStage 的清空只发生在 renderFacets 顶部——任何真检索/回放都会走到 renderFacets，
   ...
   不会散在各按钮里各自漏一处（对抗评审 F2）。renderFacets 只读 _facetStage 渲染 grid 高亮 + 提交条，不写。 */
...
export function facetStageSubmit() {
    ...
    const _sb = $("submitBtn");
    if (_sb && _sb.disabled) { toast("上一步还在检索，稍等一下再提交"); return; }   // 在途互斥（对抗评审 F5）
...
/* 草稿**将被丢弃**时的统一出口：点4 的核心承诺是「未提交的改动不静默丢失」。对抗评审确认了几条会绕过提交、
...
/* 就地切换一条**原始命中**（查询语句解析出的硬约束）的生效状态：
   ...
   被忽略的命中不再渲染进「查询条件」排（ux5④：那里只展示当前真正在筛的条件），
...
/* 「查询条件」chips 行（#facetActive）的唯一渲染口（普查 P2-1）。两处调用：
   ...
   「忽略」按钮行为不漂移。hits 为空且无细化筛选时整条收起（ux5④：栏里不再放恢复入口；
   还原走 condBoard「已忽略」分区 / 对话「回退至此」/ 撤销步进条「恢复全部条件」）。
   2026-08-04（圈2）：后加的细化筛选与原始命中**同款待遇**——同一 accent 底、同一「忽略」按钮，
...
export function renderFacets(data) {
    const bar = $("facetBar"), groups = $("facetGroups");   // chips 行渲染已收口进 renderActiveChips（P2-1），它自取 #facetActive
    // 单一 choke point（点4·对抗评审 F2）：一旦有结果被（重新）应用——真检索 / 缓存命中 / 撤销重做 / 回退查看，
    // 都会走到这里——就丢弃「数据细化」里还没提交的草稿，grid 重建成干净态。不散在各按钮里各自清一处。
    // 用 facetStageDiscardIfDirty 而非裸 `=null`：脏草稿被丢时 toast 一声（提交在途又攒新草稿这类并发下有真损失，评审 confirmed）。
    ...
    // ux3：「库中匹配 N 条」计数已并入结果摘要卡（renderResultSummary 的方法句），此处不再单独渲染 #resultsTotal。
    ...
    // （被忽略维度不在其中），刷新后 _queryHits 天然=当前生效集。还原路径（ux5④ 用户 2026-08-01：
    ...
    // 「查询条件」＝**实时正在被命中的关键词镜像**（用户 2026-07-24；ux5④ 2026-08-01 收敛为「只展示
...
export function placeFacetBar(animateNav = true, animateFade = animateNav) {
    ...
    // 否则（无分面 / 侧栏收起 / 移动端）→ 回到结果区上方原位。移动端排除（innerWidth>780）：透明抽屉两卡间缝会透出深色遮罩成暗带（评审 low）。
    ...
    //（对抗评审 confirmed）。renderFacets 末尾也会调 placeFacetBar，但那时草稿已被 choke point 清空，这里不误报。
    ...
    if (useSidebar) {
        ...
        // 回到 grid 之前的原位。2026-08-16 vs1：视图交换态 grid 整节点搬在侧栏 #sideBoardScroll，
...
function setFacetsActive(active, animate = true) {
    ...
    gsap.killTweensOf(card); card.style.height = "";                // 杀掉上一段未收尾的卡高 tween/inline，量准新自然高（评审 low：否则收尾跳变）
...
/* 数据细化卡的「可见性」：仅在查询视图(on-query)且有分面(facets-active)时可见；离开查询页淡出、回到查询页淡入。
   与导航卡布局解耦——导航卡两列只看 facets-active（跨视图保持）；fx1 起非查询视图由 app.css 的
...
function syncFacetsCard(animate) {
    ...
    // gsap.set 也带 overwrite（2026-08-03 agent2）：否则上一次 show 分支留下的 delay:0.1 延迟
