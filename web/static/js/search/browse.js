...
export async function ensureDatasetsLoaded(force) {
    ...
    // R2-10 ：首载在途窗口就把计数区复位成「—」——否则它整窗显示 HTML 静态默认的
...
export function renderBrowse() {
    ...
    /* E3网格重建幂等：卡片集没变就不动 DOM；只收藏态变（账户切换）就地翻心形。
    ...
    /* E3b点心形不许触发浏览器默认聚焦滚动（-③ 根因之二）。心形半露出视口边缘时，
       ...
       验证插桩实锤：closeFavPopover ← _onFavPopoverPassiveClose ← #document scroll）。
...
/* 历史按**对话**分组（用户 点6）：一段对话会一轮一行地写进历史，
...
function viewHistorySnapshot(group) {
    ...
    //  A2仅对话行：当时一句检索都没跑过，没有结果快照可落——如实回到
...
/* ---------- 历史记录（独立浮窗 #histWin； 拆回单页签）----------
   ...
   行交互（用户点4）：**点行本体**＝在本标签页找回这条对话（viewHistorySnapshot：
