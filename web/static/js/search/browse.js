...
/* C3 起本文件是 ES Module：core/cards/progress/results/shell 经 import 取。
   ...
   2026-08-10 评审 P1-8：browse↔fav_folders 环已切——renderFavorites 由本文件在模块求值期
...
/* P1-8 注册反转：把 renderFavorites 注册给 fav_folders（它不再 import 本文件，沿 core.js
...
/* ku1：把 catalogLookup 注册给 fav_folders（收藏操作条「更新」需要目录三态访问器；浏览→收藏
   反向边保持切断，fav_folders 不 import 本文件）。 */
if (typeof setCatalogLookup === "function") setCatalogLookup(catalogLookup);
/* ku1-w4：连同目录加载器一起注册——收藏页签「更新」在目录未加载时先拉目录再重查一次
...
/* ---------- browse 状态（所有来源并列展示，可按来源筛选；C3 自 search.js 末尾归位） ---------- */
...
let browseUnknownYearsLive = 0;  // 当前筛选上下文（不含年份筛选）下的未标日期数（C3 实时口径）
let browseLoadFailed = false;    // 上次全量加载是否失败（区分「加载中」与「加载失败」两种 null 态）
let datasetsGen = 0;             // 数据代际：每次成功拉取全量后 +1（E3 内容签名的一部分——重拉后同名同址记录字段可能已变，必须判旧重建）
...
// 单一真源：后端每条已解析好 published_year(int|null)（A2）；老响应无此字段时回退解析 published_date。
function yearOf(it) {
...
// 按该筛选上下文显示真实条数、可继续切换年份，与 fSource/fSpecies 等其它分面语义一致（C3）。
...
function syncBrowseTimelineState() {
    ...
    // 未标日期计数跟随筛选上下文（C3）：有其它筛选时用实时口径，避免 aria/文案断言与当前视图不符的数字。
...
export async function ensureDatasetsLoaded(force) {
    ...
    // R2-10 P2-4：首载在途窗口就把计数区复位成「—」——否则它整窗显示 HTML 静态默认的
    // 「符合条件 0 条 / 第 1 / 1 页」（还没加载就报 0 条，正是 F12 要消灭的伪精确；二次进入
    ...
    try {
        ...
        datasetsGen++;   // E3：内容可能变了——让 renderBrowse 的内容签名失效、判旧重建
...
/* 目录三态访问器（ku1 设计 §4）：区分「真不在目录」与「/api/datasets 加载失败/未完成」。**无 DOM 副作用**，
...
function filteredDatasets(opts) {
    const ignoreYear = !!(opts && opts.ignoreYear);   // C3：算年份分面时忽略年份筛选自身
...
export function renderBrowse() {
    ...
    if (!allDatasets) {
        ...
        delete grid.dataset.items; delete grid.dataset.favs;   // 占位/失败块覆盖了网格：E3 内容签名作废，下次成功渲染必重建
        if (browseLoadFailed) {
            grid.innerHTML = `<div class="muted-block">数据集加载失败，请稍后重试。 <button class="btn browse-retry" type="button">重试</button></div>`;   // F12：用户口吻，去「请检查服务」管理员腔
        ...
        // F12：加载中/失败是「还没有数」，不是「0 条」——不显示伪精确的 0 与 0/0 页
        $("browseCount").textContent = "—";
        $("browsePageInfo").textContent = "—";
        $("browsePrev").disabled = true; $("browseNext").disabled = true;
    ...
    /* E3（2026-08-04）网格重建幂等：卡片集没变就不动 DOM；只收藏态变（账户切换）就地翻心形。
       根因之一（普查 P2-③「加载后 ~1s 内首次点心形偶发无反应」）：启动期 whoami 落定 →
    ...
    /* E3b（2026-08-04）点心形不许触发浏览器默认聚焦滚动（P2-③ 根因之二）。心形半露出视口边缘时，
    ...
    // C3：每次渲染都按当前筛选态重算时间线（柱高/计数/aria-label 跟随筛选，不再是全库静态数）。
...
/* 历史按**对话**分组（用户 2026-07-29 点6）：一段对话会一轮一行地写进历史，
...
function viewHistorySnapshot(group) {
    ...
    // A2（2026-08-04）仅对话行：当时一句检索都没跑过，没有结果快照可落——如实回到
    // 「只有对话没有结果」的态：作废在途、退出结果态（屏上旧结果/分面/条件板随之收起；
    // 其 cbClear 会先归档屏上未归档的对话，同一 A2 口径），再把整条对话搬回主区。
    if (h && h.chatOnly) {
        bumpRecSeq();
        resetSubmitButton();
        showView("query");
    ...
    if (!h || !h.snap || !h.snap.results) {   // 迁移兜底：老格式无快照 → 重跑（runRecommend 自带 _recSeq 递增与收尾）
        if (h && h.snap_evicted) {
            // P1-4：快照是配额不足被剥掉的——接下来是**重新检索**，不是「当时的结果」，如实先说。
    ...
    // 把刚渲染的快照顶掉、且用当前（快照的）_facetFilters 回灌一条错配历史（评审 medium）。
    bumpRecSeq();   // 属主是 search.js（C3 起 ESM）：作废在途 runRecommend 必经其写口（原 ++_recSeq 裸写在 getter 桥上会 TypeError）
    resetSubmitButton();   // 取消在途请求的进度 rAF + 同步复位（其 finally 因 myGen 失配会跳过收尾）
    showView("query");
    $("queryInput").value = h.query || "";
    autoGrow($("queryInput"));
    setFacetState({   // C2：四个分面状态的属主是 results.js（ESM），重赋值必经属主 setter（live binding 只读）
...
export function renderHistory() {
    ...
    // F3：空历史禁用「清空」；每次渲染复位二段确认态（resetHistClear 在 interactions.js 定义，运行时必已加载）
    ...
    histGroups(hist).forEach((g) => {
        ...
        // A2 仅对话行：没有「N 条结果」可报——照实报「仅对话 · N 条消息」；也没有可重跑的检索句
...
export function initHistWin() {
    /* ku1（2026-08-23）：历史拆回独立浮窗 #histWin（骨架在 shell.js initHistWinSkeleton），
...
export async function runDiagnose() {
    ...
    try {
        ...
        // 诊断语境下连接失败就是真故障（F5：显式 tone:"no" 染红；日常状态 false 保持中性）
