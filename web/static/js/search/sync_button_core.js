...
/* sync_busy 冲突时的固定上屏文案（agent 可能在对话里发起了同步） */
...
let _syncNow = Date.now;   // 测试可注入（与 projects_core 的 projectsSetClock 同款；不注入时恒真实时钟）
...
/* 空闲态「上次同步」文案：从未同步如实写「从未同步」，当天写「今天」，否则「X 天前」。
   「上次同步」是**实例级事实**，只从后端取，不落 per-profile localStorage。 */
...
/* 从 sync-updates 的 operation receipt 提取三计数（摘要口径）：
   ...
   sync 无「更新既有记录」语义，这里**没有** updated 字段——「更新 Y」永远不可能出现。 */
...
/* 结果摘要一行：新增 X / 已存在 Z / 失败 W（原文格式；不写「更新 Y」）。
...
/* 失败项明细（「失败项如实列出原因」）：failed_sources[] → [{label, reason}]。
...
/* 从 recall 回执提取三计数（「撤掉 N 个文件/失败 M」）：
...
/* 把一次 POST /api/curate/sync-updates 的失败分类成可上屏的状态：
   ...
   message = 固定上屏文案（sync_busy 场景）；detail = 后端原文，作次要行如实展示。 */
