...
/* 下一步行动 · 结果页阶梯 UI 壳（2026-08-22 engagement P6，设计 §5；纯规则在 ladder_core.js）
 *
 * ## 挂点
 ...
 *
 * ## 三类行为（设计 §5.1，判据全在 ladder_core，这里只接线）
 * ① 直接执行（raw_only / 收窄建议）→ 套既有 facet 筛选 + 重跑（确定性、零 LLM）；
 * ② 确定性导出（P5 导出中心两颗）→ **探测式依赖 P5**：动态 import("#export_center")
 *    解析成功（P5 落地后提供 openExport(kind)）才渲染；失败/未落地 → 整颗隐藏不报错；
...
/* P5 导出中心探测（设计 §5.1 ②）：动态 import 一个**尚未存在**的模块 specifier 会在
   解析期 reject（try/catch 兜住）；P5 落地后把 #export_center 键加进 importmap 并提供
   具名导出 `openExport(kind)`（kind 取 LADDER_EXPORT_CHIPS 的 id），本行零改动自动接上。
   这是跨并行包耦合的标准探测式降级（目标不存在 → 隐藏入口，不报错不虚构）。 */
...
/* 渲染代次哨兵：P5 探测是异步的，期间结果区可能已被新一轮检索重建（hook 换 data）——
...
export function ladderAfterRender(data) {
    ...
    // ② 确定性导出（P5 探测通过才渲染）：异步探测回来后核对代次，避免旧屏 chip 挂到新屏。
    _probeExportCenter().then(function (m) {
