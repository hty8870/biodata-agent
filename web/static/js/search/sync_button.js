...
/* 数据集页一键同步 · 薄 DOM 壳（engagement F2 = 设计 v2 §7「F4 数据集页一键同步」）
 *
 ...
 * - corpus-sync 批：网页形态（guard on）下 sync-updates 改异步——返回
...
function _renderResult(result) {
    ...
    //  corpus-sync 批：「并检查 N 个追踪的更新」联动已撤
...
async function _startSync() {
    ...
    try {
        ...
        //  corpus-sync 批：网页形态异步响应（{async:true, job}）→ 轮询 job 到终态再呈现
