...
/* 数据集页一键同步 · 薄 DOM 壳（engagement Wave 1B F2 = 设计 v2 §7「F4 数据集页一键同步」）
 *
 ...
 *   - 2026-08-26 corpus-sync 批：网页形态（guard on）下 sync-updates 改异步——返回
 *     {async:true, job}，本壳轮询 GET /api/curate/sync-updates/status 到终态再呈现回执；
 *     本机形态响应逐字节不变（同步返回 result）。ENG-P4-MOUNT「并检查 N 个追踪的更新」
 *     联动已随全体批量按钮一并退役。
 *
 * 本页加载序与 dataset_page.js 同源（dataset.html 只加载自身模块子集）：自挂 DOMContentLoaded，
 * 不经 boot（boot 是 index.html 入口）。后端三端点契约见 B2（webapp.py:2883-2926）。
 */
...
/* 结果摘要（设计 §7）：新增 X / 已存在 Z / 失败 W；失败项逐条列原因；created_files 非空才给
   「撤销本次同步」（没有可撤的东西就不给假按钮）；ENG-P4-MOUNT 联动挂点（探测式降级）。 */
function _renderResult(result) {
    ...
    // 2026-08-26 corpus-sync 批：ENG-P4-MOUNT「并检查 N 个追踪的更新」联动已撤
...
async function _startSync() {
    ...
    try {
        const res = await fetch(API.curateSyncUpdates, {
            ...
            body: JSON.stringify({}),   // sources 缺省 = 全部已注册来源（B2 契约）
        });
        const j = await res.json().catch(function () { return null; });
        if (res.ok && j && j.ok && j.result) { _renderResult(j.result); return; }
        // 2026-08-26 corpus-sync 批：网页形态异步响应（{async:true, job}）→ 轮询 job 到终态再呈现
...
/* 异步 job 轮询（网页形态；corpus-sync 批）：GET /api/curate/sync-updates/status 到
