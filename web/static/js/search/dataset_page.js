...
const DS_COMPARE_POOL_KEY = "biodata_compare_pool_v1";  //  主页每次检索写入的对比池（起存完整归一化记录）
...
/* 取完整记录：优先 split handoff（分屏 embed 页按 uid）→ 按 uid 分键 handoff（M-05，
...
function dsRenderHeader() {
    ...
    const subBits = [];
    //  handoff 完整记录未取到（M-05）：如实标注，不再静默展示残缺页头
