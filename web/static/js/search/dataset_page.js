...
/* ==================== 数据集介绍详情页（ux3b）====================
   ...
   C3 起本文件是 ES Module：core/cards/fav_folders 经 import 取（同页三模块全 type=module，
...
const DS_COMPARE_POOL_KEY = "biodata_compare_pool_v1";  // 主页每次检索写入的对比池（ux3d 起存完整归一化记录）
const DS_SPLIT_KEY = "biodata_split_handoff_v1";        // ux3d 全屏并排：开分屏前按 uid 写入两侧完整记录，供 embed iframe 各自取回

// ux3d 嵌入态：分屏浮层里的两侧各是一个 `?embed=1` 的完整 /dataset iframe。嵌入态隐藏自己的返回主页 topbar，
...
/* ux3d split handoff：按 uid 取回开分屏时写入的完整记录（两个 iframe 读同一 localStorage，故需按 uid 各取各的）。 */
...
/* 取完整记录：优先 split handoff（分屏 embed 页按 uid）→ 按 uid 分键 handoff（2026-08-15 ta2 M-05，
...
function dsTabDefs(it) {
    ...
    // ux3d 嵌入态（分屏浮层里的 iframe）不给「数据集对比」——不在对比里再开对比（防嵌套分屏）。
...
function dsRenderHeader() {
    ...
    const subBits = [];
    // handoff 完整记录未取到（ta2 M-05）：如实标注，不再静默展示残缺页头
...
/* ux3d 全屏并排对比：对比子标签是**启动器**——从最近检索池选一条 → dsOpenSplit 打开全屏分屏浮层，
   左右各是一个完整的 /dataset 页（iframe，embed 态），可各自完整操作、可关掉一侧保留另一侧。
   （替代 ux3c 在子标签内塞两个 bare 介绍正文的窄栏；用户澄清「分屏」是全屏意义上的分屏。）

   2026-07-29 点5：候选从 `<select>` 下拉改为**铺开的结果卡网格**。理由是选下拉要先点开、再逐行读
...
/* ---------- 初始化 ---------- */
function dsInit() {
    if (DS_EMBED) document.body.classList.add("ds-embedded");   // ux3d 分屏 iframe：隐藏自己的返回主页 topbar
