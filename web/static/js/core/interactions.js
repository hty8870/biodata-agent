...
/* C3 起本文件是 ES Module：core/cards/memory/progress/results/shell 经 import 取。
   ...
   boot 与 search/browse/shell/results 经 import 取本文件导出（C5 起绞杀桥全退役）。 */
import { API, LS, MOTION, REDUCE_MOTION, $, clampInt, escapeHtml, nsKey, prettySource, readJSON, toast, writeJSON } from "#core";
...
import { applyPreset, closeSettings, openSettings, saveConfig, setSidebar, showView, aiGateChange, syncAiGates, syncStrategyNode,
    toggleLibWin, libWinOpen, closeLibWin, toggleHistWin, histWinOpen, closeHistWin } from "#shell";
...
let INTERPRET_PREVIEW_FAILS = 0;   // 连续失败计数（2026-08-15 ta2 M-08）：连败 3 次起 pill 摘要如实标注「实时识别暂不可用」
...
let browseSearchTimer = null;   // 浏览页搜索框（fSearch）250ms trailing debounce 挂钟（bind 内接线，P-12）
/* 输入框自动伸展（2026-08-20 T1 任务8）：随行数自动增高、最大高度上限（超出上限内部滚动）。
...
function scheduleInterpretationPreview() {
    ...
    if (!q) { LAST_INTERPRETATION = null; updateSrcSummary(); updateTimeSummary(); return; }
    // 停顿后识别（2026-08-03 agent2 P1-6，500ms trailing debounce）：180ms 时代在正常打字的
...
function activeFocusContainer() {
    // ux3b：介绍弹窗退役（改独立标签页 /dataset），焦点环只覆盖仍是弹窗的记忆/文件/账户三个
    // （问卷弹窗 surveyModal 2026-08-03 agent2 随执行侧全自动化退役）。
...
/* F3：历史「清空」二段确认态的复位——按钮回原文案、摘 armed、清超时。renderHistory 每次渲染也会调它。 */
...
/* 「复制接入提示词」单一真源（2026-08-22 ux1 帮助页首落地；fx2 提为 export 供教程内
   onboarding.js 委托复用同一实现）：fetch /api/guide/agent-prompt 拿全文 → 剪贴板。
   localhost 是安全上下文，navigator.clipboard 可用；老环境退 textarea + execCommand 兜底。
...
export function copyAgentPrompt(btn) {
    ...
    fetch("/api/guide/agent-prompt").then((r) => {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text).then(done).catch(() => { if (!legacyCopy(text)) toast("复制失败，请改用「下载技能包」"); });
        } else if (!legacyCopy(text)) {
            toast("复制失败，请改用「下载技能包」");
        } else done();
...
export function bind() {
    ...
    // 我的库 / 历史记录（ku1：拆回两独立浮窗，均是浮窗不是视图）：导航点击切换**浮窗**，不切视图；
    ...
    // 帮助页「接入 AI 助手」（2026-08-22 ux1）：接线走 copyAgentPrompt 单一真源
    ...
    // （2026-08-03 缺陷修复·防跳三件套①）：打字期间 pill 文本与框宽完全静止——摘要在两处更新：
    ...
    // 例外（2026-08-04 任务2）：chat-in-main 时主框是**对话起草器**（发送即清空），在里面删字
    ...
    // maxlength 上限提示（2026-08-04 P2-1）：500 字截断是无声的——贴长文时用户以为整段都进来了。
    ...
    // 统一对话窗口（uni1）：一个框，提交先问后端统一路由 /api/utterance 再分发（board.js ubSubmit）。
    ...
    // 微信式统一输入行（cur3）：侧栏工作卡最下方的「继续说」框，结果态唯一输入入口。
    ...
    // 设置三维度（2026-08-03 agent2）：LLM 依赖开关统一过禁点闸 aiGateChange（未配 key 弹回并指路）；
    ...
    // 记住设置 / 记住api key 联动（2026-08-25 夜 ux 精简批）：key 落盘的前提是设置落盘，
    ...
    // 浏览搜索防抖（P-12）：参照本文件识别预览 500ms debounce 的先例（scheduleInterpretationPreview，
    ...
    // F2：一键复位全部筛选维度（关键词/来源/物种/平台/FASTQ/年份范围）并同步控件值；
    ...
    // 写裸 LS.hist 会既清不掉登录用户自己的历史、又误清匿名命名空间（账户隔离漏洞，安全评审 CONFIRMED）。
    // F3：二段确认——第一次点进入确认态（3 秒内再点才执行，超时自动复位），空历史时禁用（renderHistory 同步）。
    ...
    // ux3b：介绍弹窗退役——「查看介绍」是 <a target=_blank> 直接开新标签页 /dataset，首页不再有 introModal 接线。
    document.addEventListener("keydown", (e) => {
        ...
        // 上面三处弹窗/抽屉的 Esc 永远走不到（普查 P1-1）。故只在面板**不**寄生于 scopePop 时
        // 才把「未 hidden」当作「有浮面板开着」。
        const pop = $("scopePop");
        const hosted = (p) => !!(p && pop && pop.contains(p));
        ...
        // 我的库 / 历史浮窗也进 Esc 逐层链（R2-2 P2-1；ku1 拆回两窗后各自吃 Esc——非模态浮层里
...
/* 首屏问候（用户 2026-08-18）：h1 从「用一句话，找到对的数据集」换成按时段的情景问候
   ...
   不会出现中午说晚上好的错位；hc2 起整句统一为同一文字样式（不再给后半句挂 .grad
...
/* ku1 首页 hero 轮播（视觉 spec §8.3）：三条文案（时段问候/标语一/标语二）同网格位叠放交叉淡化；
...
export function playHero() {
    ...
    // autoAlpha 系的 tween 一律 clearProps:"all"（2026-08-03 agent2 侧边栏顽疾根因）：
    ...
    // ku1：hero 三条问候/标语合一容器轮播，出场只以 .hero-rot 为整体入场（内部 slide 由 hero-rot CSS
...
export function openTimePanel(open) {
    ...
    if (!t || !p) return;
    // 弹层托管态（board.js placeScopeControls 把节点零复制搬进了 #scopePop）：面板在弹层里**恒展开**——
    // document 级「点面板外收起」与 Esc 都不得把它合上；否则弹层只剩两行标签、控件全灭（点8：无法改变筛选/切自动）。
    if (!open) { const pop = $("scopePop"); if (pop && pop.contains(p)) { p.hidden = false; return; } }
...
export function updateSrcSummary() {
    ...
    if (!el) return;
    animateConsoleWidth();   // 摘要将变 → 给 .console 宽度变化补过渡（rAF 延后到文字更新之后量新宽度）
    // title 恒存全文（P1-6：.sf-cur 省略号截断后，悬停仍读得到完整摘要）。
