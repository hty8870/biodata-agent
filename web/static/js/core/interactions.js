...
let INTERPRET_PREVIEW_FAILS = 0;   //  连续失败计数（M-08）：连败 3 次起 pill 摘要如实标注「实时识别暂不可用」
...
/* 输入框自动伸展（T1 任务8）：随行数自动增高、最大高度上限（超出上限内部滚动）。
...
function scheduleInterpretationPreview() {
    ...
    // 停顿后识别（500ms trailing debounce）：180ms 时代在正常打字的
...
/* 「复制接入提示词」单一真源（帮助页首落地； 提为 export 供教程内
...
export function bind() {
    ...
    document.querySelectorAll(".side-nav .nav-item[data-view]").forEach((n) => n.addEventListener("click", () => {
        ...
        //  已在智能查询页再点一次「智能查询」＝**在新标签页另开一段新对话**（用户点图：
        //  取代 的「清空当前窗口」——原对话在原窗口里原样保留，检索/对话不被打断；
    ...
    //  帮助页「接入 AI 助手」：接线走 copyAgentPrompt 单一真源
    ...
    //  （缺陷修复·防跳三件套①）：打字期间 pill 文本与框宽完全静止——摘要在两处更新：
    ...
    //  例外（任务2）：chat-in-main 时主框是**对话起草器**（发送即清空），在里面删字
    ...
    //  maxlength 上限提示：500 字截断是无声的——贴长文时用户以为整段都进来了。
    ...
    //  设置三维度：LLM 依赖开关统一过禁点闸 aiGateChange（未配 key 弹回并指路）；
    ...
    //  记住设置 / 记住api key 联动（夜 ux 精简批）：key 落盘的前提是设置落盘，
    ...
    //  写裸 LS.hist 会既清不掉登录用户自己的历史、又误清匿名命名空间（账户隔离漏洞，安全验证）。
    ...
    document.addEventListener("keydown", (e) => {
        ...
        // 上面三处弹窗/抽屉的 Esc 永远走不到（普查）。故只在面板**不**寄生于 scopePop 时
        ...
        //  我的库 / 历史浮窗也进 Esc 逐层链（R2-2； 拆回两窗后各自吃 Esc——非模态浮层里
...
/* 首屏问候（用户）：h1 从「用一句话，找到对的数据集」换成按时段的情景问候
   （实现 首页式），「能做什么」交给副标题讲。按本地小时分桶、全天 24h 无缝覆盖，
...
export function playHero() {
    ...
    // autoAlpha 系的 tween 一律 clearProps:"all"（侧边栏顽疾根因）：
...
/* 发表时间范围：年份可**手动输入**（用户点3：下拉 2016 地板太死），datalist 给
...
/* 弹层自适应（用户点图）：控制台落在视口底部时（chat-in-main 态），向下展开的
