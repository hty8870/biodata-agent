...
/* 本文件是 ES Module：core/cards/memory/progress/results/shell 经 import 取。
   ...
   boot 与 search/browse/shell/results 经 import 取本文件导出。 */
...
let INTERPRET_PREVIEW_FAILS = 0;   //  连续失败计数：连败 3 次起 pill 摘要如实标注「实时识别暂不可用」
...
/* 输入框自动伸展：随行数自动增高、最大高度上限（超出上限内部滚动）。
...
export function bind() {
    ...
    document.querySelectorAll(".side-nav .nav-item[data-view]").forEach((n) => n.addEventListener("click", () => {
        ...
        //  已在智能查询页再点一次「智能查询」＝**在新标签页另开一段新对话**（
        //  取代「清空当前窗口」——原对话在原窗口里原样保留，检索/对话不被打断；
    ...
    // 例外：chat-in-main 时主框是**对话起草器**（发送即清空），在里面删字
    ...
    //  记住设置 / 记住api key 联动（ux 精简）：key 落盘的前提是设置落盘，
...
/* 首页 hero 轮播：三条文案（时段问候/标语一/标语二）同网格位叠放交叉淡化；
...
/* 发表时间范围：年份可**手动输入**（下拉 2016 地板太死），datalist 给
...
export function initTimeFilter() {
    ...
    const onChange = (el) => {
        ...
        // 面板类/激活态/摘要/重检一处同步（此前只写 LS 不调 applyTimeMode，面板停在 mode-auto——
...
/* 弹层自适应：控制台落在视口底部时（chat-in-main 态），向下展开的
...
function renderSourceChips() {
    ...
    box.querySelectorAll(".src-chip").forEach((chip) => {
        chip.addEventListener("click", () => {
            ...
            persistSources();
            // 面板类/激活态/摘要/重检一处同步（此前只写 LS 不调 applySourceMode，弹层里「自动识别」假激活）。
