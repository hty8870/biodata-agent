...
/* 追踪更新检查 · 纯逻辑核（engagement Wave 2 P4 = 设计 v2 §4「F3 追踪更新检查闭环」）
 *
 * ## 本文件是什么 / 不是什么
 *
 * - 是：P4 更新检查的**纯逻辑**——diff（相对保存的 baseline 比 `/api/watch/check` 确定性重跑
 *   结果）、material change 判定（真实新增/消失/指纹变化；排序/score/文案不算）、
 *   截断语义（>200 不得声称「某条已从全部结果消失」）、「检索规则已更新」单列、
 *   上游同步编排与语料代哨兵的纯推导（2026-08-26 corpus-sync 批；全体批量按钮已撤）。
 *   设计见 `docs/工作记录/设计_engagement落地包_2026-08-22.md` §4.1/§4.2/§4.3/§4.4。
...
/* ---------- 2026-08-26 corpus-sync 批：上游同步编排 + 语料代哨兵 ----------
...
/* 截断文案（设计 §4.1 原文口径）：本次结果 >200 时消失不可判定 */
...
/* ---------- 上游同步编排（corpus-sync 批：单追踪检查先追溯语料同步） ---------- */
...
/* 语料代比对：current 不可得（null/空串）→ false（B5 降级：跳过自动刷新，不报错）；
...
export function watchDiff(r, baseline, sentSpecVersion) {
    ...
    /* 真实消失 = 保存的 baseline 里有、本次全量结果里没有。仅当本次结果全量
       ...
       「消失」是错误断言（设计 §4.1 原文「不得声称某条已从全部结果消失」）。
...
/* 待查看更新逐条结构（UI 逐条渲染/处理用）：[{uid, kind}]。
   ...
   （设计 §4.4：逐条比较后决定纳入/排除；纳入只对存在的数据集有意义）。 */
...
/* 单追踪检查回执（ku2-w1，设计 §4.4「结果汇总如实呈现」+ 用户口径「检查了 N 个，X 个有更新，
