...
/* C2 起本文件是 ES Module：core 的 $/REDUCE_MOTION 经 import 取；search/board/browse/interactions
   ...
   （animateConsoleWidth 在  曾随「.console-bar width:0 排除法」退役；
...
/* 检索进度（起改为不确定态）：后端是单次阻塞调用、拿不到真实进度，此前用「恒速爬升到 96% 再补满」
...
/* 期望时长：纯前端拿不到后端真实进度，据「是否走 LLM」估一个量级。 起数字里程表退役，
...
/* 把「检索」按钮从 loading/disabled 态**同步**复位。
   ...
   **不借道 finishProgress**（验证抓到的竞态）：那是「**完成**」语义；这里是「**取消**」语义——
    起数字机器已退役，两者都同步收干净，区别只在语义标签与调用点约定。 */
