...
/* C2 起本文件是 ES Module：core 的 $/REDUCE_MOTION 经 import 取；search/board/browse/interactions
   经 import 取进度函数与查询缓存（C5 起绞杀桥全退役）。
   （animateConsoleWidth 在 2026-08-03 agent2 P1-6 曾随「.console-bar width:0 排除法」退役；
...
/* 检索进度（ux2 起改为不确定态）：后端是单次阻塞调用、拿不到真实进度，此前用「恒速爬升到 96% 再补满」
...
/* 期望时长：纯前端拿不到后端真实进度，据「是否走 LLM」估一个量级。ux2 起数字里程表退役，
...
export function startProgress(expectedMs) {
    const btn = $("submitBtn");
    if (!btn) return;
    // 幂等（点7）：统一框在路由问句前就已把按钮置 loading 并起跑；runRecommend 接手时不重启。
    // ux2 起不再逐帧翻数（数字已撤），动画交给 CSS 三点跳动，这里只管 loading 态与在途旗标。
...
/* 把「检索」按钮从 loading/disabled 态**同步**复位。
   ...
   **不借道 finishProgress**（评审抓到的竞态）：那是「**完成**」语义；这里是「**取消**」语义——
   ux2 起数字机器已退役，两者都同步收干净，区别只在语义标签与调用点约定。 */
