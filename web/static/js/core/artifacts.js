 ...
 * ## 本文件是什么 / 不是什么
 *
 * - 是：追踪（project）的**纯数据层**。设计见 `docs/工作记录/设计_engagement落地包_2026-08-22.md`
 *   §2（概念模型）/ §3.1（数据模型）/ §3.2（存储裁决）。P4（更新检查 diff 闭环）与 P5（导出中心）
 *   将直接建在本文件的 schema 之上，字段语义注释按「P4/P5 直接消费」的标准写。
 * - 不是：UI。零 DOM、零窗口、零 localStorage、零网络——界面、上下文卡、活动 tab 持久化全在
 *   后续包（Wave 1B UI 层），本文件不碰。设计 §3.2 明确「localStorage 只存活动 tab/活动追踪 id
...
export function artifactsIsoNow(ts) {
    ...
    if (t instanceof Date) {
        ...
        /* ISO 字符串（如服务端响应的 checked_at）：Number()=NaN，走数值路径会被 `|| 0`
           错成 epoch 0（2026-08-22 eng-close 修复：追踪「上次检查于 20687 天前」事故）。 */
        d = new Date(t);
    } else {
...
/* 确定性检索规格（check_condition.spec，设计 §3.1）。字段名与 B2 `/api/watch/check`
   入参**逐一对齐**（= /api/recommend 入参子集，协调者收口时统一）：query（解析后的
...
/* 读时修复（2026-08-22 eng-close）：旧版 artifactsIsoNow 不接受 ISO 字符串，曾把
