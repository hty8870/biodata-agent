 ...
 * 生产通道为明文 HTTP，属 tc1 已知风险裁决，consent 弹窗如实告知）、
 ...
 *
 * ## 评分卡为什么不进 _cbLog（fb1 起各绑各的 rec.id；ux1 起会话降频）
 *
 ...
 * **2026-08-22 ux1 起加会话降频**（派发口径「就你最关注的地方出现两次就行」）：
 ...
 *   T1 任务7（2026-08-20）：none/error 轮（纯埋点、S5 起不出完整卡）在挂载点渲染一颗
...
/* eng-b3：意见反馈队列（相对 import，不进 importmap/静态图——同 usage_upload 的哲学）；
...
/* 结果区内容指纹（2026-08-15 ta2 M-04）：逐卡取「数据集详情」链接 href（含 uid/url/名称/来源），
...
/* 用户发出一句话（ubSubmit 第一落点）。env 由调用方从 getConfig() 摘好传入——
   ...
   opts.templateOriginated（2026-08-22 engagement P6，设计 §5.5）：任务卡/chip 生成文本
...
/* 并段白名单（2026-08-15 ta2 M-03）：只有「检索→接打包」系动作段允许并回上一轮记录——
...
/* 在途轮次挂错误注记但**不收尾**（2026-08-15 ta2 fix-frontend 补漏）：路由 fail-open 时
...
/* ---------- 评分标签事件（2026-08-22 ov1-fix1a，schema v3 label）----------
   每次评分写入**无论 benchfb 记录还在不在本地**都发一条 usage label 事件：记录可能被
   上传 ACK 精确删除，但「这轮评了什么」必须仍可达接收端（ov1-fix2 起事件带 recId，导出侧按
...
function _emitLabel(recId, updated, patch, scope) {
    try {
        ...
        usageLog(USAGE_KINDS.label, {
            ...
            // ov1-fix2：带 recId（记录 id）——导出侧按 (tid, recId) 与 benchfb 记录双源去重，
...
/* ---------- 评分卡（fb1：各绑各的 rec.id；ux1：完成度三选 + 原因 chips 取代星级）----------
...
/* 折叠「评价」按钮（低视觉权重小字）：none/error 纯埋点轮（T1 任务7）与 ux1 起被会话
...
/* 对话轮挂载点（[data-bf-mount]）：按挂载点上的 rec.id 填卡；已收起/记录被清 → 留空
   （.bf-mount:empty 不占位）。折叠档（_collapsedRate：none/error 轮，或 ux1 降频闸拦下的
...
function _renderHeroMount() {
    ...
    // ux1：会话降频闸同样约束 hero 槽位——被拦下的轮次只给折叠「评价」按钮。
...
function _enterMarkMode() {
    ...
    // 2026-08-15 ta2 M-04：判据从「只比卡片数量」升级为「数量 + 内容指纹」——
...
function _buildPackage() {
    ...
    // eng-b3：本地导出反馈包包含本 profile 已发送/待发送的意见记录（明文本地账本，
...
/* 设置区导出按钮态（2026-08-21 a5：卡片状态小字随文案收敛移除，这里只剩按钮态）。
   ux1：顺带刷新「本机编号」行——只读既有键，**绝不为了展示而生成**（未产生数据时如实显示）。 */
...
export function initBenchfb() {
    ...
    document.addEventListener("click", function (e) {
        ...
        if (!recId) return;
        // 折叠档（none/error 轮、或被 ux1 会话降频闸拦下的 search/tool 轮）点「评价」→
        ...
        // 完成度三选（ux1 取代星级）：点选入库；再点同一项 = 取消选择。
