"use strict";
/* 意见反馈 · 核心模块（2026-08-22 eng-b3，F5 后端/核心层；UI 对话框在 Wave 1B 的 F3）
 *
 * 纯逻辑、零 DOM、零网络、零墙钟（id/时间由调用方注入）：只做
 * - 开发者公钥配置点（ENG-B3-PUBKEY）：2026-08-22 收口已填生产公钥；
 ...
 * usage_core.telemetryMaskString（含 eng-b3 追加的 API Key 形态——加密前第一层，
 * 接收端解密后还有第二层，双层同规则）。
 *
...
/* ============================================================================
 * ENG-B3-PUBKEY —— 开发者公钥配置点（2026-08-22 收口已填生产公钥）
 * ----------------------------------------------------------------------------
...
/* 公钥为空或 WebCrypto 不可用 → false。UI 据此走「复制到剪贴板」兜底；
   ...
   pubkeyB64 参数仅供测试/未来可配置覆盖（生产默认取 ENG-B3-PUBKEY 常量）。 */
...
/* 加密一条意见 → 密文载荷（含 feedback_id/identity 元数据；identity 由调用方按
   ...
   pubkeyB64 缺省取 ENG-B3-PUBKEY 常量（生产路径）；测试传入自备公钥。 */
