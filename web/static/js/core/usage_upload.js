...
/* 网页版护栏判定（公网护栏硬化，）：唯一真源 = /api/health 的 account.required，
...
/* sendFeedback：只发送本 profile feedback_pending 中**已授权**（status="pending"）的意见
   ...
   返回 boolean 只供内部/调试用；调用方（F3 UI）fire-and-forget。 */
...
document.addEventListener("click", function (e) {
    ...
    if (t.closest("#consentAgreeBtn")) {
        ...
        usageSetTrainingConsent(!!(training && training.checked), scope); //  独立授权；默认勾选（起默认开启，可关）
