...
/* 遥测自动上传 · 唯一出网通道（2026-08-20 tl1 S2）
 *
 ...
 *   ov1-fix2：中继空页但 next_offset 已推进（since_ts 过滤墙——墙内行被滤掉、服务端 offset
 *   照常前进）时回执推进本地 offset，过滤墙不再冻结游标。
 * - **激活 ping**（tl1 追加）：consent 同意即发一次性 hello 包（`sendActivationPing()`）——
...
/* eng-b3：意见反馈核心（纯逻辑模块；相对 import，不进 importmap/静态图——与本模块
...
/* 接收端由交付/部署在 HTML meta 中注入。安全默认是空（仅本地采集+手动导出）；
   ...
   明文公网 HTTP 属已知风险（tc1 合并裁决，用户知情授权）：若部署方接受明文传输，
...
/* 触发阈值（设计 §2，契约门钉「常量存在且 ≤ 设计值」）。设计意图：工具使用量小，
   阈值定低，轻度使用也能上传。2026-08-22 ov1-tel1 激进上传：benchfb 1 轮 / usage 10 条
   ——benchfb 单条即一轮完整检索现场，轮次收尾后尽快上传。
   ov1-adapt1 起 usage 默认降到 2 条（无 hint 时几乎实时、绝不积压），且阈值/最小间隔
...
/* 自适应上传阈值（2026-08-22 ov1-adapt1，server-driven）：
...
/* 配额双闸（2026-08-22 ov1-fix1a，接收端防刷）：常规上传距上次成功不足 UPLOAD_MIN_INTERVAL_MS
   （3 分钟默认）不再发——ov1-adapt1 起实际间隔取当前动态值（meta.hintIntervalMs 或默认）；
...
if (window.addEventListener) {
    ...
    // 页面关闭/隐藏前尽力补一发（ov1-tel1）：keepalive 档只带 usage 事件。
...
/* 周期兜底定时器（ov1-tel1）：页面可见时每 5 分钟评估一次；node 规格里 unref 防挂住进程。 */
...
export function setUsageConsent(v, scope) {
    ...
    // v2（2026-08-22 ov1-fix1a）：同意落的是**同意时刻 ISO 串**（MCP 中继 since_ts 下界要用）；
...
/* 缓存代读法 2026-08-22 ov1-tel1 起统一在 core.js 的 cacheGeneration()（本模块从 #core 委托），
...
async function _packetId(profileId, usageIds, benchIds, kind, mcpIds, contractIds) {
    // ov1-fix1a：附了 MCP 中继记录的包把 call_id 一并拼进幂等材料——同一批记录重传
...
/* 单条 benchfb 自身超预算时的降级（2026-08-22 ov1-tel1）：把 search.res.results 截到
   ...
   ov1-fix1a：用户标注的「有用条目」若在截断点之外（名次 >20 且 ≤ 原数组长度），把对应条目
...
/* UTF-8 字节数（低-6，审核）：接收端按**字节**计 body 上限（2MB），与 fetch 实际
...
/* ---------- MCP 调用记录中继（2026-08-22 ov1-tel1，跨包契约；ov1-fix1a 改相对同源） ----------
   ...
   ov1-fix1a：改**相对同源** fetch（webapp 与 API 同源；旧代码误取 ingest 端点的 origin，
...
async function _mcpFetchPage(scope) {
    try {
        ...
        try {
            ...
            // ov1-fix2：空页但 next_offset 已推进（since_ts 过滤墙——墙内行被滤掉、服务端 offset 照常
...
async function _mcpAck(scope, offset) {
    try {
        ...
        const config = _ingestConfig();
        if (!config) return;
        // 只前进（ov1-fix1a）：跨标签页/重试并发时，旧页拿到的小 offset 不得把本地已推进的
...
async function _maybeUploadLocked(scope, startup, opts) {
    ...
    try {
        ...
        // 配额双闸（ov1-fix1a）：常规上传距上次成功不足当前动态间隔不再发（ov1-adapt1 起间隔
        ...
        // ov1-fix1a：提前到空队列判断**之前**——usage/benchfb 都空但有中继记录时也要发（MCP-only 包）。
        const mcpPage = keepalive ? null : await _mcpFetchPage(scope);
        const mcpCount = mcpPage ? mcpPage.records.length : 0;
        // ov1-fix2：中继空页但 next_offset 已推进（since_ts 过滤墙）→ 立即回执推进本地 offset——
        ...
        // 激进上传（2026-08-22 ov1-tel1）：启动/尽力档有待发即传；常规触发只看低阈值，
        // 原先的「启动距上次成功 > 6 小时」与 2 分钟评估节流已退役（防抖改由 maybeUploadUsage 承担）。
        // ov1-fix1a：MCP-only 包不受阈值拦（否则中继记录永远凑不够阈值才出门）。
        // ov1-adapt1：usage 阈值取当前动态值（meta.hintThreshold，无 hint 回落默认 2）。
        ...
        if (!sent.ok) {
            // ov1-adapt1：429 = 接收端限流/过载 → 临时升到高档（20 条 / 5 分钟）并持久化，
            ...
            const m = throttled
                ...
                : meta;
            // keepalive 尽力档失败只记尝试时刻、**不进退避**（ov1-fix1a）：页面正在卸载/隐藏的
        ...
        // ov1-adapt1：200 且响应带 server_hint → 采用并持久化（跨刷新生效）；老接收端无该字段 → 维持当前动态值。
...
/* 同标签页 single-flight + 跨标签页 Web Lock。没有 Web Locks 时仍依赖稳定 packet/event id
   让服务端去重，不能退回数组长度截断。
   防抖（ov1-tel1）：非启动/非强制的调用距上次评估不足 30s 时只挂一个尾随定时器收尾，
...
/* ---------- 意见反馈发送（2026-08-22 eng-b3，F5；独立入口，绝不捎带其它队列） ---------- */
...
/* sendFeedback：只发送本 profile feedback_pending 中**已授权**（status="pending"）的意见
   ...
   返回 boolean 只供内部/调试用；调用方（Wave 1B F3 UI）fire-and-forget。 */
