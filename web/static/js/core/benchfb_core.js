 ...
 * 用户评分（2026-08-22 ux1 起：完成度三选 + 可选原因 + 逐条有用标注 + 自由评语；
 * 此前的星级已退役——三选对 benchmark 的判定价值高于 1-5 连续分）。产品所有者原话：
 * 「信息量充足，宁多勿少」。
 *
 * ## 脱敏是结构性的，不靠记性
 *
...
/* 完成度三选（2026-08-22 ux1 评分结构改版，取代星级）：记录里存机器可读 token，
...
/* 会话级评分降频（2026-08-22 ux1，派发口径「就你最关注的地方出现两次就行」）：
...
/* 评分合并：返回**新记录对象**（旧记录不被改——导出与提示卡各拿各的快照，不漂移）。
   2026-08-22 ux1 起记录形状（additive，不再写 stars）：
...
/* 单条记录的导出形态：原记录 + rating.useful_resolved（标注名次 → 数据集）
   + rating.useful_uids（同一解析的纯 uid 清单，2026-08-22 ux1 起随新评分形状带出）。
...
/* 包内容统计（导出弹窗的「大白话清单」数据源）。
   ...
   2026-08-22 ux1：完成度计数取代星级均值（星级已退役）；旧形状记录（带 stars 无 completion）
