...
/* C2 起本文件是 ES Module：core 的工具经 import 取；LS_SIDE 自 #cards、placeFacetBar 自 #facets、
   ...
   绑定只在函数体内使用）；benchfbSyncSettings 自 #benchfb import（ux1：设置打开时刷新
...
export function setSidebar(open) {
    ...
    // 移动端抽屉与浮窗互斥（2026-08-04 A7；ku1：我的库 + 历史两窗都关）：浮窗曾靠抬 z 压回抽屉，
...
function closeSidebarOnMobile() { if (window.innerWidth <= 780) { document.body.classList.add("side-closed"); if (typeof placeFacetBar === "function") placeFacetBar(); } }   // 抽屉关闭 → 分面条搬回结果区，别遗落在离屏抽屉里（评审 medium）
/* R2-9 P1-3：≤780px 固钉 logo（side-fab）恒显会压住从它下面滚过的正文（放宽横幅/控制台顶边被盖）。
...
export function initSidebarResize() {
    ...
    resizer.addEventListener("keydown", (event) => {
        ...
        else if (event.key === "Home") next = SIDEBAR_WIDTH.min;   // UX1：Home=最窄（惯例），End=最宽；重置默认宽用双击分隔条
...
export function showView(name) {
    ...
    // ku1：我的库（追踪+收藏）与历史记录都收进浮窗（#libWin / #histWin），不占视图位——
...
/* ---------- 通用浮窗工厂（ku1：骨架 开合/拖动/缩放/落位/重钳 每窗一份） ----------
...
export function initFloatingWin(winEl, opts) {
    ...
    function open() {
        ...
        if (w.hidden) {
            ...
            if (!w.dataset.placed) {   // 首次落位：主区中央偏上（侧栏占左，稍右偏）；之后记住拖动位置
                ...
                // D2（2026-08-04）：+110 的桌面右偏在 ≤780px 视口豁免（实测移动宽度右缘溢出 98px、
    ...
    /* R2-2 P2-1：落位钳制此前只在首开（dataset.placed）算一次——浮窗开着时缩窗，窗口留在
    ...
    // 缩放（右下角把手）：指针捕获 + 按下时记偏移的真实拖拽几何（D1，2026-08-04）——
    ...
    // 缩窗重钳（R2-2 P2-1）：浮窗开着时视口变小，把它钳回屏内（关闭按钮恒可达）。
...
// trial（T3 限量试用，2026-08-25）：key/地址/模型全部由服务端托管并锁定（BIODATA_TRIAL_API_KEY，
...
export function openSettings(opts) {
    ...
    // ux1：刷新使用反馈卡片的「本机编号」行——编号可能在首次上传/导出后才生成，
...
/* C1（2026-08-04）指路「直达」兑现：打开设置后把目标滚进视野并短暂高亮——
...
/* ---------- AI 能力门控与设置三维度（2026-08-03 agent2 设置体系重写） ----------
...
/* health 快照到达钩子（2026-08-26 corpus-sync 批；注册式反转——project_updates 的登录后
...
/* 网页版公网护栏（2026-08-26 护栏硬化批）前端统一判定口：唯一真源 = /api/health 的
...
export async function syncAgentAvailability() {
    try {
        ...
        // 零配置默认（2026-08-03 agent2）：用户从没存过设置、当前还是本地演示，而服务端已配好
        // 真实 key → 接入方式默认到与服务端一致的那个预设（服务端 key 直接适用，AI 开箱可用）。
        // 用户一旦自己选过/存过（LS.cfg 存在）就绝不替他改——存档优先。
        // T3（2026-08-25）：服务端没配正式 key 但开了限量试用通道 → 默认到「限量试用」，
        ...
        if (!readJSON(LS.cfg, null) && _healthLlm && _llmWire() === "mock") {
            ...
            if (hit) {
                ...
                // 替用户改配置必须可见（2026-08-15 ta2 M-02）：「服务端有 key」这个条件用户看不到，
        ...
        // fail-open：开关维持原状。但必须留痕（2026-08-15 ta2 M-02）——health 探测失败时
...
function _renderLocalModelStatus(state) {
    ...
    // 2026-08-26 方案A 放量：服务端开了智谱 API 向量召回/重排（health.recall_api）时，
...
// trial（T3）：地址/密钥两行**整行隐藏**（2026-08-25 夜 ux 裁决：锁定项不占地），模型框只读
...
/* 试用额度卡（2026-08-25 夜）：trial 预设 + 服务端通道可用 → 显示并拉取「今日剩余」。
...
export function getConfig() {
    ...
    // 2026-08-03 agent2：大模型总开关退役。use_llm = mock 演示恒 true（演示输出靠它），
