...
/* C1 起本文件是 ES Module。toggleFav 用到的 usageLogCardAction / USAGE_KINDS 来自同批转换的
   usage_log / usage_core（ESM live binding）。
   2026-08-10（codex 架构评审裁决落地）：**切断 core→board 反向边**——对话 id/对话日志
...
/* 缓存代（2026-08-22 ov1-tel1 提到 core 共用）：任意 /static/js/ 脚本 src 的 ?v= 令牌，
...
/* Blob 下载共用样板（sm2 批）：task_pack/reuse_pack/benchfb/project_exports 四处同款手抄的
...
/* 与后端 units.format_sample_size（全项目规范单一真源）逐位对齐（F8）：
...
/* F7 分面值展示归一（只动展示层；原值经 data-val / title 保留）：
...
/* 千分位分组（R2-9 P2-1）：结果卡上的大数字与首页覆盖声明「5,712」/ 可行性面板「3,086,388」
...
export function normalizeItem(it) {
    // F8：不再把「未说明 {unit}」私删成「未说明」——与后端 format_sample_size / 介绍关键事实同一格式
...
export function toggleFav(it, folder, anchorEl) {
    ...
    setFavs(a);
    // schema v2（2026-08-22 ov1-tel1）：记 uid + 名次（anchorEl 是点中的卡片内按钮，
    // 经它找回卡片算名次），与 open/dl 同一套归因口径；仍不记数据集名等研究内容。
    // v3（ov1-fix1a）：经 usageLogCardAction——结果页的卡带展示快照 tid/iid，
...
/* 仅对话历史行（2026-08-04 A2）：纯工具对话（一句检索都没跑过）从不经过 pushHist——
...
/* 历史记录：不再只存「查询 + 条数」（点一下重跑），而是**存下当次的具体结果快照**——
   ...
   2026-07-29 起每行还带 `convId` + `chat`（用户点6）：一段对话会产出多行历史，
...
export function pushHist(q, data, facetFilters, suppressed, queryHits, lenientDims, replaceLast) {
    ...
    for (;;) {
        try { writeJSON(nsKey(LS.hist), arr); break; }
            ...
            // snap_evicted 打标（2026-08-09 codex P1-4）：点开被剥的行会回退重跑——
