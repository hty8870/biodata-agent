...
/* C1 起本文件是 ES Module。toggleFav 用到的 usageLogCardAction / USAGE_KINDS 来自同批转换的
   ...
   （验证裁决落地）：**切断 core→board 反向边**——对话 id/对话日志
...
/* Blob 下载共用样板（批）：task_pack/reuse_pack/benchfb/project_exports 四处同款手抄的
...
/* 千分位分组（R2-9）：结果卡上的大数字与首页覆盖声明「5,712」/ 可行性面板「3,086,388」
...
/* 仅对话历史行（A2）：纯工具对话（一句检索都没跑过）从不经过 pushHist——
