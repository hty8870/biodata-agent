...
/* 追踪 UI 壳（engagement Wave 1B F1 + ku1：一级导航「我的库」+ 我的库浮窗追踪页签 + 追踪详情 +
 * 「存为追踪」入口 + 上下文 chip + 首次保存 coachmark）。
 *
 * 设计：`docs/工作记录/设计_engagement落地包_2026-08-22.md` §2（概念模型）/ §3（F1 追踪）/
 * §3.3（上下文 chip）/ §3.4（首次保存就地引导）/ §9（教程：coachmark 承担就地引导，不加步骤）。
 * 数据层全部走 B1 `artifacts.js`（IndexedDB）；纯逻辑在 `projects_core.js`；本文件只管 DOM。
 *
 * ## 交互契约
 * - 追踪面板是「我的库」浮窗（ku1：追踪 + 收藏双页签）的 tracks 页签：浮窗骨架（开合/拖动/缩放/
 ...
 *   `artifact_context` 请求体字段（设计 §3.3：独立字段、不拼进用户原话）。后端 B4 合并前
 *   extra=forbid 会 422 → 发送失败如实降级（chip 上标注「未随上一条消息发出」，不崩不装成功）。
 * - accounts.js 在 profile 切换重置点调 artifactsOnProfileSwitched()（B1 钩子）；本模块经
 *   setAccountChangedHook（注册反转，防 accounts→projects 成环）再清上下文 chip/活动追踪 UI 态。
 *
 * ## 埋点（计数型无文本，usage_log 既有通道；追踪名/query/uid 不进遥测——设计 §1.3）
...
import { setCtxDataSetHandler } from "#fav_folders";   // ku1：注册数据集上下文 chip 设置接口（注册反转防 fav_folders→projects 环）
...
import { p4DetailMount, runProjectCheck, pendingDeltaCount, checkFailed,
    setWatchesRefreshedHook } from "#project_updates";   // ENG-P4-MOUNT：P4 更新检查交互（探测式降级，见挂载点）
...
/* 行级操作三按钮的内联 SVG（ku1 视觉 spec §3.2：lucide 同族描边语言；行级重复操作用图标钮）。 */
...
/* 候选卡片化（ku1 视觉 spec §6）：每条候选 → .prj-cand-wrap（buildCard(variant:"library")
   + 操作条 + 理由折叠；ku2-w2 起状态徽标收进卡片 .badges 行首，不再浮贴出界）。
...
function _buildCandNodes(p, ui) {
    ...
    (p.candidates || []).forEach((c) => {
        ...
        /* ku2-w2（任务 4a）：状态徽标不再绝对定位浮贴在卡片外（top:-8px 探出卡片上沿被截），
...
/* ============================================================================
 * 浮窗开合（fx2：骨架属主在 shell.js——这里是薄封装，只负责追踪页签语义与渲染触发）
 * ========================================================================== */
...
async function projectsRenderList() {
    ...
    body.innerHTML = projects.map((p) => {
        ...
        // ku1（视觉 spec §3.1）：行拆 .prj-card-main（button 语义承载点击进详情）+ .prj-row-acts（sibling 三按钮），
    ...
    body.querySelectorAll("[data-prj]").forEach((el) => {
        ...
        /* ku1 设计 §3.1：行按钮检查完要重渲列表行——检查时间 / 「待查看 N」徽章 / 行内失败标记
           都靠这次重渲上屏（ku1-w4 补：此前不调 outcome、不重渲，时间戳要重开窗才刷新）。
...
function renderProjectDetail(p) {
    ...
    /* 检查条件展示区（设计 §3.1/§4.1）：展示实际保存的规范化检索规格 + display_query。
       本节内容为空时不渲染（不伪造条件）；P4 更新检查交互挂点 ENG-P4-MOUNT。 */
    let checkHtml = "";
    if (hasCheck) {
        const rows = [];
    ...
    /* 导出区（设计 §6）：P5 挂点 ENG-P5-MOUNT。区域默认整段隐藏——P5 模块（project_exports.js）
       经 MutationObserver 发现 `[data-p5-mount-export]` 后渲染并展开本节；无 P5 时不渲染
       （保持 F1「空时不渲染」语义，不出现空标题）。挂点带追踪 id 供 P5 读库。 */
    ...
    /* 候选列表（ku1 卡片化，视觉 spec §6）：目录能解析到 → buildCard(variant:"library") 包 .prj-cand-wrap
    ...
    body.innerHTML = '<div class="prj-detail">'
        ...
        + "</div></div>"
        + exportHtml   // ku2-w2（任务 4b）：导出区从详情底部挪到页面顶部（头卡之下第一区）——批量导出不再要翻过长候选列表
        + checkHtml   // ku1：检查条件卡靠前（追踪的可检查规格是复查/更新的入口，先于研究目标）
    ...
    /* ENG-P4-MOUNT（P4 更新检查交互）：检查条件区挂点 div 由 P4 壳填充——「检查更新/
       重试生成基线」按钮 + 双时间戳 + 待查看更新逐条处理；P4 未落地（#project_updates
       未加载，p4DetailMount 不存在）探测式降级：挂点保持 hidden，不渲染不报错。 */
...
/* ============================================================================
 * 上下文 chip（ku1：注入文本 + kind 追踪/数据集；ku2-w2 全面返工：双挂点 + 发送即清）
...
/* 数据集上下文（ku1：fav_folders 收藏操作条「在对话中使用」→ 设置 ctx chip，kind="dataset"）。
...
function _ctxPrivacy() {
    const cfg = getConfig();
    // 隐私口径（设计 §1.3）：远端模型明示「会发往你配置的 AI 服务商」；本地模型/本地演示「不出本机」。
...
/* 主框变体：完整胶囊 chip（图标 + 名称截断 + ✕），点击主体展开 popover（向下）。
   ...
   单行 textarea 裁出半截灰字（ku2-w4 视觉核验），chip 移除后原文恢复。 */
...
async function projectsSaveFromSearch() {
    ...
    try {
        ...
        /* 基线（设计 §4.1）：确定性重跑（strategy=fixed 等由后端强制）；失败不掀翻追踪——
           追踪仍保存（spec 留着供 P4 重试基线），如实提示。 */
        try {
...
/* ---------- coachmark（设计 §3.4：首次保存就地引导；localStorage 只记「已看过」） ---------- */
 ...
 * 清内存缓存（B1 钩子）+ 清上下文卡/活动追踪 UI 态（设计 §3.2）
 * ========================================================================== */
...
export function initProjects() {
    /* ku1：追踪面板注册为「我的库」浮窗的 tracks 页签渲染器（骨架在 shell.js initLibWin；
    ...
    // 语料代哨兵自动刷新完成钩子（2026-08-26 corpus-sync 批；注册式反转防环）：重渲列表，
