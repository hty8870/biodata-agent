...
/* 追踪 UI 壳（engagement F1 +：一级导航「我的库」+ 我的库浮窗追踪页签 + 追踪详情 +
 * 「存为追踪」入口 + 上下文 chip + 首次保存 coachmark）。
 *
 * 设计：engagement 落地包 §2（概念模型）/ §3（F1 追踪）/
 * §3.3（上下文 chip）/ §3.4（首次保存就地引导）。
...
import { p4DetailMount, runProjectCheck, pendingDeltaCount, checkFailed,
    setWatchesRefreshedHook } from "#project_updates";   // ：P4 更新检查交互（探测式降级，见挂载点）
...
/* 候选卡片化（视觉 spec §6）：每条候选 → .prj-cand-wrap（buildCard(variant:"library")
   + 操作条 + 理由折叠； 起状态徽标收进卡片 .badges 行首，不再浮贴出界）。
...
function _buildCandNodes(p, ui) {
    ...
    (p.candidates || []).forEach((c) => {
        ...
        /* （任务 4a）：状态徽标不再绝对定位浮贴在卡片外（top:-8px 探出卡片上沿被截），
...
function renderProjectDetail(p) {
    ...
    if (hasCheck) {
        ...
        checkHtml = '<section class="prj-sec"><div class="prj-sec-title">检查条件'
            ...
            + '<div class="prj-check-mount" data-p4-mount-check hidden></div><!-- PROJECT_UPDATES_MOUNT -->'
            + "</section>";
    }

    /* 导出区（设计 §6）：P5 挂点。区域默认整段隐藏——P5 模块（project_exports.js）
    ...
    const exportHtml = '<section class="prj-sec" data-p5-mount-section hidden>'
        ...
        + '<!--：P5 在此挂导出按钮与导出记录台账；无 P5 时不渲染 --></section>';
    ...
    body.innerHTML = '<div class="prj-detail">'
        ...
        + "</div></div>"
        + exportHtml   //  （任务 4b）：导出区从详情底部挪到页面顶部（头卡之下第一区）——批量导出不再要翻过长候选列表
    ...
    /* （P4 更新检查交互）：检查条件区挂点 div 由 P4 壳填充——「检查更新/
...
export function initProjects() {
    ...
    //  语料代哨兵自动刷新完成钩子（corpus-sync 批；注册式反转防环）：重渲列表，
