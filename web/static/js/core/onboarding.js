...
/* C3 起本文件是 ES Module：core/shell/usage_log 经 import 取。boot 经 import 取
   initOnboarding（C5 起绞杀桥全退役）。fx2 起 import #interactions 的 copyAgentPrompt
   （教程「接进你自己的 AI 助手」步复制按钮，与帮助页同一实现；ku2-w3 曾前移至第 3 屏，
   2026-08-23 任务C 起按产品方向后移并精简——走 MCP/技能包会绕开 react agent 侧的数据采集、
...
/* 版本分叉（2026-08-13 ob1）：反馈强化版默认开、主线版默认关——同一份 onboarding.js 服务两个版本，
   文案各自如实。构建判定收口在 usage_log.isBenchfbBuild（2026-08-15 ta2 M-01：显式 meta 标记优先，
   不再各文件自探 DOM）。 */
const BENCHFB_BUILD = isBenchfbBuild();

const ONBOARDING_STEPS = [
    {
        // 2026-08-19 tu1 新增「第0步」：开场先讲数据采集承诺 + 引导发高质量 query。
        // 2026-08-19 ob2 终稿：只改反馈强化版（true 分支）——标题「你的使用，在帮它变好」
        // 渲染成主题绿色、正文整体加粗；用 additive 字段 titleAccent/boldText 按 BENCHFB_BUILD
        // 绑定（false 分支字段为 false，主线版视觉与文案一律不动），渲染端据此挂样式类。
        // 2026-08-21 tc1：默认本地采集、per-profile consent、安全通道配置后上传（旧「导出反馈包手动
        // 发给开发者」口径清除，导出保留作兜底），与设置区使用反馈 tooltip 及末步指路一致。
        // 2026-08-22 ux1 如实化：明文 HTTP + 90 天保留 + 结构性去标识口径写进正文（与 consent 弹窗、
        // 设置页采集卡片同一份事实）；评分频率改口「最关注的轮次、每会话最多两次」。
        // 2026-08-22 ov1-fix2：结构性去标识 → 尽力过滤（手机号/证件号/邮箱自动遮蔽、API Key 等
    ...
    },
    {
        ...
        // 原 ob3 的两步在第 11/12 屏，用户容易错过；本步吸收其能力例与诚实边界并前移到第 2 屏。
        target: "#queryInput",
    ...
    },
    {
        target: ".console-bar",
        title: "来源和时间默认自动识别",
        text: "查询里的来源名称和时间会自动进入检索范围。需要固定范围时，也可以展开来源和时间，改成手动选择。检索框下方的条件标签和结果上方的摘要，都会如实告诉你系统把你的话理解成了什么。",
        visual: "scope",
        size: "compact",
    },
    {
    ...
    },
    {
        ...
        // 2026-08-22 eng-docs（engagement Wave 3）：本步吸收结果页「下一步行动」与过宽收窄建议
        // （设计 §5）——风险分层如实写：确定性动作直接执行、多步任务先进可编辑任务卡确认。
    ...
    },
    {
        ...
        // ku1（设计 §11）：本步吸收「存为追踪 / 我的库双页签 / 上下文 chip / 检查更新」——
    ...
    {
        // 任务C（2026-08-23）：新增「我的库」介绍。与第 11 屏「出结果后可以接着改条件」衔接——
    ...
    {
        // 2026-08-22 fx2 可选步「接进你自己的 AI 助手」；ku2-w3（2026-08-23）曾按用户要求前移至
        // 教程第 3 屏。2026-08-23 任务C 起按产品方向后移至结尾并精简——走 MCP/技能包会绕开
        ...
        target: "#queryInput",
        title: "接进你自己的 AI 助手",
        text: "BioData 的检索与数据管护能力能接进你常用的 AI 助手（Claude Code / Kimi Code / Codex）：点「复制接入提示词」发给你的助手，按提示接入，或下载技能包自己看。这一步完全可选，跳过不影响任何功能。",
        visual: "agent",
        size: "medium",
    },
    {
        // 2026-08-13 ob1 起：当场开关按钮行退役，这一步只负责「指路」——打开设置抽屉、
        // 高亮「使用反馈」那一行，让用户知道以后去哪管。
        // 2026-08-21 tc1：默认本地采集、安全通道配置后上传（单版本化，无主线版分叉），
        // 教程里如实告知；导出反馈包保留作手动兜底。
        // 2026-08-22 ux1 如实化：明文 HTTP/90 天/结构性去标识口径与 consent 弹窗一致；
        // 「本机编号」行（删除已上传数据用）也在这一行下面，一并指路。
        // 2026-08-22 ov1-fix2：结构性去标识 → 尽力过滤（并非匿名化），与 consent v2 口径一致。
...
function initOnboardingDrag() {
    ...
    // UX1：手柄可键盘聚焦 + 方向键微移（复用 placeDraggedOnboarding 的 12px 视口钳制），
...
/* ux1：教程卡里的服务商按钮接通右侧真实表单。按钮 data 值与 #cfgProvider 的 option value 严格同源；
...
/* ux1：详情页实拍图加载失败时优雅兜底——一次性 error 监听隐藏图、显示文字说明（如实列六个子标签）。
...
/* 2026-08-13 ob1：教程里的「当场选择」机器（choice 按钮行）已退役——使用反馈的默认态
   按版本分叉（强化版默认开、主线版默认关），第 9 步改为指路设置里的管理入口。
   DOM 行与样式一并移除；若未来复活当场决策，从这里补回即可。 */

// 实时读「减弱动效」偏好（UX1）：REDUCE_MOTION 是加载时取一次的常量，会话中途在系统里开启对本模块的
...
function showOnboardingStep(index) {
    ...
    // ob2：第 0 步 true 分支经 additive 字段 titleAccent/boldText 挂样式类（标题主题色 / 正文加粗），
...
export function initOnboarding() {
    ...
    // ux1：事件委托——点教程卡里的服务商按钮，等于在右侧真实设置里切换 provider：
    ...
    $("onboardingVisuals").addEventListener("click", (event) => {
        // fx2 教程「接进你自己的 AI 助手」步：教程内「复制接入提示词」与帮助页同一实现
