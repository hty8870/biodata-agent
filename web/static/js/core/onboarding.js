...
/* 本文件是 ES Module：core/shell/usage_log 经 import 取。boot 经 import 取
   initOnboarding。另 import #interactions 的 copyAgentPrompt
   （教程「接进你自己的 AI 助手」步复制按钮，与帮助页同一实现；此前曾前移至第 3 屏，
    后按产品方向后移并精简——走 MCP/技能包会绕开 react agent 侧的数据采集、
...
/* 单一构建：同一份 onboarding.js，采集默认开；
   构建判定收口在 usage_log.isBenchfbBuild（显式 meta 标记优先，
   不再各文件自探 DOM）。 */
const BENCHFB_BUILD = isBenchfbBuild();

const ONBOARDING_STEPS = [
    {
        // 「第0步」：开场先讲数据采集承诺 + 引导发高质量 query。
        // 标题「你的使用，在帮它变好」
        // 渲染成主题绿色、正文整体加粗；用 additive 字段 titleAccent/boldText 按 BENCHFB_BUILD
        // 绑定（false 分支字段为 false，标准构建视觉与文案一律不动），渲染端据此挂样式类。
        // 默认本地采集、per-profile consent、部署方配置上传通道后才上传（旧「导出反馈包手动
        // 发给开发者」口径清除，导出保留作兜底），与设置区使用反馈 tooltip 及末步指路一致。
        // 如实口径写进正文（与 consent 弹窗、设置页采集卡片同一份事实）：本包出厂 meta 是
        // 占位符、上传通道未配置 → 记录只存本机；部署方把 meta 换成真实端点后才明文 HTTP
        // 上传（90 天保留）；尽力过滤（手机号/证件号/邮箱自动遮蔽、API Key 等直接标识删除，
        // 并非匿名化）。评分频率「最关注的轮次、每会话最多两次」。
        target: "#queryInput",
        title: BENCHFB_BUILD ? "你的使用，在帮它变好" : "尽管用真需求考验它",
        titleAccent: BENCHFB_BUILD,
        text: BENCHFB_BUILD
            ? "这个版本默认开启本地数据采集，每个账户首次发送前独立确认；记录只保存在本机，不会上传——除非部署方另外配置了上传通道：配置后，记录经尽力过滤（手机号/证件号/邮箱自动遮蔽、API Key 等直接标识删除，并非匿名化）经明文 HTTP 上传到部署方的服务器（保留 90 天），用于改进推荐排序与制作评测基准。不采集 API Key、密码和账户名。请尽管用真需求考验它——评分只在你最关注的检索/执行轮主动出现（每个会话最多两次），不满意就写一句哪里不对；请勿输入患者隐私、样本信息或未公开的研究内容。"
    ...
    },
    {
        ...
        // 原教程尾部的两步在第 11/12 屏，用户容易错过；本步吸收其能力例与诚实边界并前移到第 2 屏。
        target: "#queryInput",
    ...
    },
    {
        ...
        //  本步吸收「存为追踪 / 我的库双页签 / 上下文 chip / 检查更新」——
        // 一级导航、更新检查绝不自动纳入、导出中心一句话带过；上下文 chip 如实写
    ...
    {
        //  新增「我的库」介绍。与第 11 屏「出结果后可以接着改条件」衔接——
    ...
    {
        //  可选步「接进你自己的 AI 助手」；曾按用户要求前移至
        //  教程第 3 屏。后按产品方向后移至结尾并精简——走 MCP/技能包会绕开
    ...
    {
        //  当场开关按钮行退役，这一步只负责「指路」——打开设置抽屉、
        // 高亮「使用反馈」那一行，让用户知道以后去哪管。
        // 默认本地采集、部署方配置上传通道后才上传（无构建分叉），
        // 教程里如实告知；导出反馈包保留作手动兜底。
        //  如实化：通道未配置只存本机 / 配置后明文 HTTP 上传（90 天保留）/
        // 尽力过滤口径与 consent 弹窗一致；
        // 「本机编号」行（发生过上传时请求删除用）也在这一行下面，一并指路。
        target: ".usage-setting",
        title: "使用反馈，在这里管理",
        text: BENCHFB_BUILD
            ? "这个版本默认开启本地采集，每个账户独立确认；记录只存在本机——若部署方配置了上传通道，记录经尽力过滤（手机号/证件号/邮箱自动遮蔽、API Key 等直接标识删除，并非匿名化）后经明文 HTTP 上传到部署方服务器（保留 90 天），用于改进推荐排序与制作评测基准。想关、清空本地记录、手动导出，或查本机编号（发生过上传时请求删除用），都在高亮这一行管理。"
...
/* 教程里的「当场选择」机器（choice 按钮行）已退役——使用反馈的默认态
   按构建分叉（benchfb 采集构建默认开、标准构建默认关），第 9 步改为指路设置里的管理入口。
