...
/* C3 起本文件是 ES Module：core/shell/usage_log 经 import 取。boot 经 import 取
   initOnboarding（C5 起绞杀桥全退役）。 起 import #interactions 的 copyAgentPrompt
   （教程「接进你自己的 AI 助手」步复制按钮，与帮助页同一实现； 曾前移至第 3 屏，
    任务C 起按产品方向后移并精简——走 MCP/技能包会绕开 react agent 侧的数据采集、
...
/* 版本分叉（ob1）：反馈强化版默认开、主线版默认关——同一份 onboarding.js 服务两个版本，
   文案各自如实。构建判定收口在 usage_log.isBenchfbBuild（M-01：显式 meta 标记优先，
   不再各文件自探 DOM）。 */
const BENCHFB_BUILD = isBenchfbBuild();

const ONBOARDING_STEPS = [
    {
        //  tu1 新增「第0步」：开场先讲数据采集承诺 + 引导发高质量 query。
        //  ob2 终稿：只改反馈强化版（true 分支）——标题「你的使用，在帮它变好」
        // 渲染成主题绿色、正文整体加粗；用 additive 字段 titleAccent/boldText 按 BENCHFB_BUILD
        // 绑定（false 分支字段为 false，主线版视觉与文案一律不动），渲染端据此挂样式类。
        // ：默认本地采集、per-profile consent、安全通道配置后上传（旧「导出反馈包手动
        // 发给开发者」口径清除，导出保留作兜底），与设置区使用反馈 tooltip 及末步指路一致。
        //  如实化：明文 HTTP + 90 天保留 + 结构性去标识口径写进正文（与 consent 弹窗、
        // 设置页采集卡片同一份事实）；评分频率改口「最关注的轮次、每会话最多两次」。
        // ：结构性去标识 → 尽力过滤（手机号/证件号/邮箱自动遮蔽、API Key 等
    ...
    },
    {
        ...
        //  （engagement 教程批）：本步吸收结果页「下一步行动」与过宽收窄建议
    ...
    {
        //  任务C：新增「我的库」介绍。与第 11 屏「出结果后可以接着改条件」衔接——
    ...
    },
    {
        ...
        //  教程第 3 屏。 任务C 起按产品方向后移至结尾并精简——走 MCP/技能包会绕开
    ...
    {
        //  ob1 起：当场开关按钮行退役，这一步只负责「指路」——打开设置抽屉、
        // 高亮「使用反馈」那一行，让用户知道以后去哪管。
        // ：默认本地采集、安全通道配置后上传（单版本化，无主线版分叉），
        // 教程里如实告知；导出反馈包保留作手动兜底。
        //  如实化：明文 HTTP/90 天/结构性去标识口径与 consent 弹窗一致；
        // 「本机编号」行（删除已上传数据用）也在这一行下面，一并指路。
        // ：结构性去标识 → 尽力过滤（并非匿名化），与 consent v2 口径一致。
...
/* ob1：教程里的「当场选择」机器（choice 按钮行）已退役——使用反馈的默认态
