 ...
 *   projects.js 经 `#project_updates` import 取 updateDetailMount / runProjectCheck /
 *   setWatchesRefreshedHook，boot 不加键；dataset.html 不再加载本模块（联动钩子已撤）。
 * - 详情挂点 [data-update-mount-check] 是 projects.js renderProjectDetail 动态渲染的骨架
 *   div（预留，初始 hidden）——本模块每次收到 updateDetailMount(p) 调用时重建其内容
...
import { WATCH_CHECK_COPY, WATCH_KIND_ADDED, WATCH_KIND_FP, WATCH_KIND_REMOVED,
    ...
    watchCheckableProjects, watchDeltaEntries, watchDiff, watchGenChanged, watchReceiptText,
...
/* 会话内「待查看更新」：projectId → [{uid, kind}]（kind ∈ added/fp/removed）。
   不落库（artifacts.js schema 白名单无此字段，「追踪内待查看更新」只保留在会话内）；
...
/* delta_review_completed：用户把某追踪的「待查看更新」逐条处理到空时计一次
...
/* ============================================================================
 * 追踪详情检查条件区（projects.js renderProjectDetail 调 updateDetailMount(p)）
 * ========================================================================== */

/* 挂点定位：详情 body 内预留的骨架 div（projects.js 每次重渲详情都会重建它）。 */
function _detailMountEl() {
    const body = $("artifactsWinBody");
    return body ? body.querySelector(".prj-check-mount[data-update-mount-check]") : null;
}
...
export function updateDetailMount(p) {
    try {
        ...
        /* 双时间戳行（分开显示）——本地目录同步（实例级事实，异步补；失败如实） */
        ...
        runBtn.addEventListener("click", async () => {
            ...
            try { const fresh = await artifactsGetProject(_scope(), projectId); updateDetailMount(fresh || p); } catch (_e) { updateDetailMount(p); }
        });
...
/* 待查看更新列表（卡片化）：每条 uid → .prj-cand-wrap 包 compact 卡；目录能解析
...
function _renderDeltaList(projectId, p, deltas) {
    ...
    if (deltas.some((d) => { let l = null; try { l = (typeof catalogLookup === "function") ? catalogLookup(d.uid) : null; } catch (_e) { l = null; } return l && l.status === "load_error"; }) && typeof ensureDatasetsLoaded === "function") {
        ensureDatasetsLoaded().then(() => { try { const fresh = artifactsGetProject(_scope(), projectId).then((np) => updateDetailMount(np || p)); } catch (_e) {} }).catch(() => {});
    }
    return box;
}
...
async function _ignoreDelta(projectId, uid, btn) {
    ...
    if (p) updateDetailMount(p);
    else updateDetailMount({ project_id: projectId });
}

/* 单追踪「检查更新」（行级入口）：POST /api/watch/check（确定性重跑，零 LLM）→ 相对保存的
   baseline diff → 回填 baseline + last_checked_at（「检查完回填」）→ 更新会话待查看列表。
   导出 `runProjectCheck(projectId, btnEl)`：**进入立刻占 _checking + 禁按钮**（修 验证 竞态——
...
export async function runProjectCheck(projectId, btnEl) {
    ...
    _checking.add(id);   // 先占位，再 await（防竞态）
    ...
    try {
        ...
        /* 网页形态（guard on）先向上追溯——按追踪 spec 的
        ...
        if (out && out.error) {
            ...
            //   ——否则用户看到「候选 10 · 待核验 10」不动误以为坏了（绝不自动改纳入表）。
...
/* diff 三态计数（新增/信息变化/消失）——供 runProjectCheck 结构化 outcome 与列表徽章用。 */
function watchDiffCounts(diff) {
...
async function _watchCheckOnce(p) {
    ...
    try {
        ...
        if (!res.ok || !j || !j.ok || !j.result) {
            ...
            /* spec_version 与当前端点支持版本不一致 → 「检索规则已更新」单列，
        ...
        /* 检查完回填 baseline + last_checked_at（明文；baseline=null 时即
           「重试生成基线」的回填路径）。 */
        try {
        ...
        /* 埋点（计数型无文本）：watch_checked{changed} */
