...
/* 追踪更新检查 · UI 壳（engagement Wave 2 P4 = 设计 v2 §4「F3 追踪更新检查闭环」；
 * 2026-08-26 corpus-sync 批改版）
 *
 * 纯逻辑在 project_updates_core.js（零 DOM，node 可单测）；本文件只管 DOM 与网络：
 *   - 追踪详情检查条件区（ENG-P4-MOUNT）：规范化检索规格 + display_query + 双时间戳
 ...
 *   - 单追踪「检查更新」向上追溯编排（corpus-sync 批 B2，仅网页形态 guard on）：先按
 ...
 *     watch/check。全体「检查 N 个追踪的更新」按钮与 F2 联动钩子已撤（用户：点一下
 *     太耗费资源）——批量诉求由下方语料代哨兵自动刷新承接。
 *   - 登录后自动刷新（corpus-sync 批 B3，仅 guard on）：health 到达/账户切换时比对
 ...
 *   setWatchesRefreshedHook，boot 不加键；dataset.html 不再加载本模块（F2 联动钩子已撤）。
 * - 详情挂点 [data-p4-mount-check] 是 projects.js renderProjectDetail 动态渲染的骨架
 *   div（F1 预留，初始 hidden）——本模块每次收到 p4DetailMount(p) 调用时重建其内容
...
/* 会话内「待查看更新」：projectId → [{uid, kind}]（kind ∈ added/fp/removed）。
   不落库（artifacts.js schema 白名单无此字段，评审②裁决「追踪内待查看更新」由 P4 持有）；
...
/* ============================================================================
 * 追踪详情检查条件区（ENG-P4-MOUNT：projects.js renderProjectDetail 调 p4DetailMount(p)）
 * ========================================================================== */

/* 挂点定位：详情 body 内 F1 预留的骨架 div（projects.js 每次重渲详情都会重建它）。 */
...
/* 待查看更新列表（ku1 卡片化，视觉 spec §6.2）：每条 uid → .prj-cand-wrap 包 compact 卡；目录能解析
...
/* 单追踪「检查更新」（ku1 行级入口）：POST /api/watch/check（确定性重跑，零 LLM）→ 相对保存的
   baseline diff → 回填 baseline + last_checked_at（设计 §4.4「检查完回填」）→ 更新会话待查看列表。
   导出 `runProjectCheck(projectId, btnEl)`：**进入立刻占 _checking + 禁按钮**（修 codex F8 竞态——
...
export async function runProjectCheck(projectId, btnEl) {
    ...
    _checking.add(id);   // 先占位，再 await（修 F8 竞态）
    ...
    try {
        ...
        /* 2026-08-26 corpus-sync 批（B2）：网页形态（guard on）先向上追溯——按追踪 spec 的
        ...
        if (typeof webGuardOn === "function" && webGuardOn()) {
            ...
            if (up && up.error) {
                ...
                const gen = await _fetchCorpusGen();   // B4：手动同步完成写回新语料代，防自动刷新紧接重跑
        ...
        if (out && out.error) {
            ...
            // ku2-w1：回执如实说清「检查了 N 条，X 条有更新，Y 条已是最新」，并点明「检查不自动改候选表」
...
/* ============================================================================
 * 上游同步编排（2026-08-26 corpus-sync 批 B2）：启动/附着语料同步 job 并轮询到终态。
...
/* ============================================================================
 * 登录后自动刷新（2026-08-26 corpus-sync 批 B3，仅 guard on）：health 到达/账户切换时比对
...
/* 当前语料代哨兵：health 快照取（钩子触发时快照刚写入）。不可得 → null（B5 降级跳过）。 */
...
/* 重新拉一次 health 取语料代（B4：手动同步完成后写回；快照可能还是旧的）。 */
...
function initProjectUpdates() {
    /* 全体「检查 N 个追踪的更新」按钮（浮窗头 + F2 联动钩子）已撤（2026-08-26 corpus-sync 批，
