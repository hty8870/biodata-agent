...
/* 追踪更新检查 · UI 壳（engagement P4 = 设计 v2 §4「F3 追踪更新检查闭环」；
 * corpus-sync 批改版）
 *
...
/* 会话内「待查看更新」：projectId → [{uid, kind}]（kind ∈ added/fp/removed）。
   不落库（artifacts.js schema 白名单无此字段，验证裁决「追踪内待查看更新」由 P4 持有）；
...
export async function runProjectCheck(projectId, btnEl) {
    ...
    try {
        ...
        /* corpus-sync 批（B2）：网页形态（guard on）先向上追溯——按追踪 spec 的
...
/* ============================================================================
 * 上游同步编排（corpus-sync 批 B2）：启动/附着语料同步 job 并轮询到终态。
...
function initProjectUpdates() {
    /* 全体「检查 N 个追踪的更新」按钮（浮窗头 + F2 联动钩子）已撤（corpus-sync 批，
