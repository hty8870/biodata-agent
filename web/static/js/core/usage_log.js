 ...
 *
 * b1 起：回传通道 = benchfb 的「导出反馈包」（全量交互记录 + 评分 → 单个 JSON 文件）。
 ...
 *
 * - **默认态（tl1 单版本化）**：主线版分叉废弃，行为恒取原反馈强化版——**默认开**；
 *   显式选择（开/关）永远优先于默认。关闭 = 零采集、零上传、零评分 UI；**关闭不删已有数据**。
 * - **consent**：per-profile `nsKey(biodata_consent_v2)`（起 v2：值为同意时刻
...
/* 构建版本判定（tl1 单版本化）：主线版分叉废弃，**恒为 true**（原反馈强化版）。
...
export function usageLog(kind, payload, scopeOverride) {
    ...
    //  sid/tid/iid 中央注入（schema v2）：全事件带 sid，适用事件带
...
/* 一次检索的打点。**「从后端响应里读哪些字段」只写在这一个地方** ——
   ...
   opts.handSubmit（夜班；前身 userSubmit 已随统一路由退役）沿用 actAfterSearch 的同一判据：
...
export function usageLogSearch(data, query, opts) {
    ...
    //  AI 各层的成败单独记。**「没启用」和「试过但没成」必须分开** —— 这正是
...
export function usageCardRank(card) {
    ...
    if (!grid) return 0;
    //  只数 .card 兄弟（修正）：grid.children 会被 benchfb hero mount、
...
/* ---------- 卡级展示归因（schema v3 ImpressionContext）----------
...
export function initUsage() {
    ...
    //  启动时也尝试一次上传：有待发数据即传（起取消「距上次成功 > 6 小时」
    //  的等待——激进上传：阈值 benchfb 1 轮 / usage 默认 2 条， 起 usage 阈值与最小
