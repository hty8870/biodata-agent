...
/* C1 起本文件是 ES Module：core 的存储/工具与 usage_core 的聚合函数都经 import
...
import { USAGE_KINDS, USAGE_AI_LABELS, usageActiveImpressionId, usageActiveTurnId,
    telemetryExperimentAssign, usageBeginTurn, usagePolicyRef, usageSessionId } from "#usage_core";
 ...
 *
 * 2026-08-13 b1 起：回传通道 = benchfb 的「导出反馈包」（全量交互记录 + 评分 → 单个 JSON 文件）。
 ...
 *
 * - **默认态（2026-08-20 tl1 单版本化）**：主线版分叉废弃，行为恒取原反馈强化版——**默认开**；
 *   显式选择（开/关）永远优先于默认。关闭 = 零采集、零上传、零评分 UI；**关闭不删已有数据**。
 * - **consent**：per-profile `nsKey(biodata_consent_v2)`（2026-08-22 ov1-fix1a 起 v2：值为同意时刻
...
export function usageDropSnapshotForScope(scope) {
    ...
    const revision = rows.length ? (parseInt(_fnv(rows.map(function (r) { return r.id; }).join("|")), 36) % 2147483646) + 1 : 0;
    return total ? {
        revision: revision,
        dropped_count: total,
        by_queue: by,
...
function _legacyEventId(event, index) {
    ...
    return "legacy-u-" + String(Number(event && event.t) || 0) + "-" + index + "-" + _fnv(body);
}
...
/* 构建版本判定（2026-08-20 tl1 单版本化）：主线版分叉废弃，**恒为 true**（原反馈强化版）。
...
export function usageEnabledForScope(scope) {
    ...
    // 单版本化（2026-08-20 tl1）：恒取原强化版默认——开。
    return true;
}
...
export function usageClientId() {
    ...
    if (!id) { id = _randomId("client"); try { localStorage.setItem(LS.usageClient, id); } catch (_e) {} }
    return id;
}
...
export function usageProfileIdForScope(scope) {
    ...
    if (!id) { id = _randomId("profile"); try { localStorage.setItem(key, id); } catch (_e) {} }
    return id;
}
...
export function usagePersist(list, scope) {
    ...
    try {
        (Array.isArray(list) ? list : []).forEach(function (raw) {
            ...
            event.event_id = String(event.event_id || _randomId("u"));
...
/* 同步读当前 profile 是否同意：发送拦截用。键属主是 usage_upload.js；这里只读同一 nsKey。
   v2（2026-08-22 ov1-fix1a）：值为同意时刻 ISO 串（"0"=显式拒绝）；任何非 "0" 非空值都算同意，
   兼容旧写法的 "1"。 */
...
/* 训练授权默认开启（2026-08-25 夜，产品所有者决策）：键不存在 = 从未表态 → 按同意计；
...
export function usageExperimentContext(scope) {
    scope = scope === undefined ? usageScope() : scope;
    const experimentId = _experimentMeta("biodata-experiment-id");
    const armsText = _experimentMeta("biodata-experiment-arms");
    ...
    const arm = _experimentMeta("biodata-experiment-arm");
    const propensity = Number(_experimentMeta("biodata-experiment-propensity"));
...
/* 首次发送拦截（设计 §3，S5）：打开 consent 告知弹窗并等待用户决定。
...
export function usageLog(kind, payload, scopeOverride) {
    ...
    // sid/tid/iid 中央注入（2026-08-22 ov1-tel1 schema v2）：全事件带 sid，适用事件带
    // 当前轮 tid 与当前展示 iid；payload 里显式给的值优先（view 事件用 begin 时的快照，
    // 防止新一轮 turn 串号；v3 起 imp/卡动作/label 也靠这条显式通道传快照或显式 null）。
    const entry = Object.assign({ event_id: _randomId("u"), t: Date.now(), k: kind,
...
/* 一次检索的打点。**「从后端响应里读哪些字段」只写在这一个地方** ——
   ...
   opts.handSubmit（2026-08-05 夜班；前身 userSubmit 已随统一路由退役）沿用 actAfterSearch 的同一判据：
...
export function usageLogSearch(data, query, opts) {
    ...
    // v2（ov1-tel1）：tid 兜底——正常由 board.js ubSubmit 开局；历史重跑等不过 ubSubmit
    // 的路径在这里补开一轮，保证 search 事件永远有轮次可归。
    const tid = usageActiveTurnId() || usageBeginTurn();
    // v3（ov1-fix1a）：策略串**优先用后端响应里的 policy_id**（缓存命中时就是缓存响应里那份，
...
export function usageCardRank(card) {
    ...
    if (!grid) return 0;
    // 只数 .card 兄弟（2026-08-22 ov1-tel1 修正）：grid.children 会被 benchfb hero mount、
...
/* ---------- 卡级展示归因（2026-08-22 ov1-fix1a，schema v3 ImpressionContext）----------
...
export function initUsage() {
    ...
    // 启动时也尝试一次上传：有待发数据即传（2026-08-22 ov1-tel1 起取消「距上次成功 > 6 小时」
    // 的等待——激进上传：阈值 benchfb 1 轮 / usage 默认 2 条，ov1-adapt1 起 usage 阈值与最小
