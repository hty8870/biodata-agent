 ...
 *
 * 回传通道 = benchfb 的「导出反馈包」（全量交互记录 + 评分 → 单个 JSON 文件）。
 ...
 *
 * - **默认态（单版本化）**：构建分叉废弃，行为恒取单一版本——**默认开**；
 *   显式选择（开/关）永远优先于默认。关闭 = 零采集、零上传、零评分 UI；**关闭不删已有数据**。
 * - **consent**：per-profile `nsKey(biodata_consent_v2)`（v2：值为同意时刻
...
/* 构建版本判定（单版本化）：**恒为 true**。
   ...
   直接调用本函数定默认分支，恒 true 使其永远走采集构建文案。 */
...
/* 一次检索的打点。**「从后端响应里读哪些字段」只写在这一个地方** ——
   ...
   opts.handSubmit（前身 userSubmit 已随统一路由退役）沿用 actAfterSearch 的同一判据：
...
export function initUsage() {
    ...
    //  启动时也尝试一次上传：有待发数据即传（取消「距上次成功 > 6 小时」
