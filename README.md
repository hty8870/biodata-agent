...
BioData Agent 是一个面向公开单细胞与空间组学数据集目录的**对话式数据发现 Agent**。架构上以 **ReAct 为主环、RAG 为检索核**（ReAct+RAG hybrid）：用户用自然语言对话，系统调用大模型进行意图观察后按需加载对应工具与系统提示词；每次工具调用都附着完整的执行信息与执行时的背景信息，全程可审计、可回退。
...
首次进入页面会显示 14 步新手教程，可拖动、可跳过，之后能在“帮助”里重新打开。教程第 5 步会展开真实的 API 配置表单：先选服务商，再填自己的 API Key（默认只在本次会话使用，除非主动勾选记住设置）；不想现在配置可直接跳过，基础检索仍然可用。桌面端左侧栏可拖动调整宽度，也支持键盘方向键微调。
...
“使用反馈”默认开启本地采集：软件会记录你发送的消息、系统展示的结果、点开或下载的条目、耗时与报错、评分和评语。不采集 API Key、密码和账户名；**每个本机账户首次发送前独立确认**。记录只存在本机——出厂未配置上传通道，不会上传；只有部署方另外配置了上传通道（可能为明文 HTTP，机制与风险见 SECURITY.md），尽力过滤后的记录才会自动上传。可随时关闭，或清空当前账户尚未上传的 usage/benchmark 记录；本地清空不会远程删除已经上传的数据。
...
> 目录布局：交付包里 MCP 入口在包根 `mcp_server.py`；**GitHub 仓库克隆**里在 `src/dataset_recommender/app/mcp_server.py`（skills 在 `.agents/skills/`）。教程按交付包布局书写，克隆用户请对应替换路径；`setup_mcp.ps1` 对两种布局都能自动定位。
...
├─ docs/                          使用说明书、验收用的 测试说明.txt，以及 CHANGELOG.md、QUALITY_GATES.md、AUTOMATION_AND_RELEASE.md、WINDOWS_ENVIRONMENT.md
