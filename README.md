...
面向公开单细胞与空间组学数据集目录的**对话式数据发现 Agent**：用一句中文描述实验需求，系统在 8,022 条公开数据集元数据上完成硬过滤、可解释排序、引文与下载整理。架构上以 **ReAct 为主环、基于LLM与向量模型的 RAG 为检索核**；离线与联网均可使用。每次工具调用都附着完整的执行信息与执行背景，全程**高度可审计、可回退**。另外拥有一些业界主流Agent的优秀特性，如梦境记忆、PTC模式、plan模式、工具选择性装载等等。
...
- 纪律本身就是产品：冻结语料 SHA-256 指纹锁（CRLF/LF 免疫）；阈值只能收紧、调高视同放宽门须记录授权；holdout 单次使用、不公开，禁止据其回改实现后邀功；全部评测离线可复现。

### 工程可信度

- 4,558 条 pytest + 19 条前端 JS spec 全绿（2026-09-01 口径；政策上钉「全部通过」，不钉数量）。
- 13 门清单驱动质量门：依赖一致性、治理校验、全量测试、项目/Web/MCP 三端冒烟、三套冻结评测、安装包契约。
...
面向开发者的环境、架构、HTTP 端点、扩展方法和验证矩阵见[开发指南](DEVELOPMENT.md)。当前 Web API 版本为 `2.9.0`。开发和交付优先使用清单驱动的统一质量门：
