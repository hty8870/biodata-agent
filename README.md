...
面向公开单细胞与空间组学数据集目录的**对话式数据发现 Agent**：用一句中文描述实验需求，系统在 8,022 条公开数据集元数据上完成硬过滤、可解释排序、引文与下载整理。架构上以 **ReAct 为主环、RAG 为检索核**——默认离线、默认不烧 token，每次工具调用都附着完整的执行信息与执行背景，全程**可审计、可回退**。
