...
- **Failure**: user asks "is there data on rare tissue X?", tool returns 0, agent says "no such data exists / this is unstudied." The user writes that into a grant. It was false — the catalog only covers 6 sources, and scRNA-seq raw data mostly sits in GEO/SRA (which the tool does **not** index).
- **Do**: "No match in this catalog's 6 sources (10x / CELLxGENE / HCA / EBI SCEA / ArrayExpress / ENCODE). This does not mean it is unstudied — much scRNA-seq data is in GEO/SRA, which this tool does not index."
...
## 5. Never upload without explicit per-session authorization

- `upload_dataset` writes to the external library (`database/external/`). It is the only mutating tool.
