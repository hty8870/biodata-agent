...
def _check_venv(python: Path) -> None:
    ...
    if probe.returncode != 0:
        ...
        # 2026-08-24 wvfix 验收缺口：构建 venv 没有 pywebview，modulegraph 扫不到
...
def run_web_shell_probe(exe: Path) -> dict:
    ...
    `ok` = 退出码 0 **且** 探针文件内容含 `SHELL_PROBE_OK`（两条同时满足才算过，fail-closed）。
    """
...
def verify_web_shell_probe(app_dir: Path) -> None:
    """frozen Web 桌面壳真依赖探针（fail-closed）：缺 exe / 超时 / 非零退出 / 无
    SHELL_PROBE_OK 全挡——2026-08-24 的 2.5.0 正是「frozen 无壳但构建全程无告警」漏出。"""
