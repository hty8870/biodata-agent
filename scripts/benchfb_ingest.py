#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""benchmark 采集反馈包 · 接收侧 ingest（2026-08-13 b1 支线版）。

用户在支线版本里「设置 → 使用反馈 → 导出反馈包」得到单个 JSON 文件（schema
...
def _candidate(rec: dict) -> dict[str, Any]:
    ...
    rating 容忍新旧两种形状（2026-08-22 ux1 起评分结构改版）：新形状
    ...
    return {
        ...
        # eng-p6（2026-08-22，设计 §5.5）：模板轮次标记随候选携带；写 candidates 前默认已排除
...
# 完成度三选（2026-08-22 ux1 新评分形状）的人读标签；旧形状记录的星级在 _rate_txt 里兜底。
...
def main(argv: list[str] | None = None) -> int:
    ...
    # eng-p6（设计 §5.5）：benchmark 候选默认排除「未经编辑直接发送」的模板轮次
