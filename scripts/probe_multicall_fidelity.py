#!/usr/bin/env python3
"""多 tool_call **保真度微验证**（乐观执行架构决策批，只读实验，不改生产代码）。

与 `probe_decide_format.py`（问题猎杀批：量「多调用取第一个」的合法性——
第一个 17/17 合法续步）互补：本验证量的是**第 2 个及以后调用**的保真度——模型在批量
...
混在一起）：
  A 臂（现状语义）：正常循环，包装 LOOP_TOOLS 的 run 记录**实际执行**的调用序列
      （verb + slots，含失败步）。
  B 臂（批量采集）：spy `_invoke_tool_channel`（沿用 08-09 验证同一挂钩点），把
...
#: 跨模型对照批：`--out-dir` 把采集/分析定向到子目录（如 模型/、k3/），
#: 与 DeepSeek 主臂产物隔离；`PROBE_THINKING_OFF=1` 时给主模型关思考档（模型 默认思考开，
#: deepseek-chat 默认无思考——对照口径需要同档）。两者都只作用于验证，不动生产代码。
