#!/usr/bin/env python3
"""多 tool_call **保真度微探针**（2026-08-14 乐观执行架构决策批，只读实验，不改生产代码）。

与 `probe_decide_format.py`（2026-08-09 病灶猎杀批：量「多调用取第一个」的合法性——
第一个 17/17 合法续步）互补：本探针量的是**第 2 个及以后调用**的保真度——模型在批量
...
混在一起）：
  A 臂（现状语义）：正常循环，包装 LOOP_TOOLS 的 run 记录**实际执行**的调用序列
      （verb + slots，含失败步）。
  B 臂（批量采集）：spy `_invoke_tool_channel`（沿用 08-09 探针同一挂钩点），把
...
用法：
  采集（真 API，每用例独立沙箱，与 live 探针同纪律——绝不碰真库）：
...
产物（`research/reports/multicall-fidelity-probe/`，不入库、不碰冻结基准）：
...
#: 2026-08-14 跨模型对照批：`--out-dir` 把采集/分析定向到子目录（如 glm46/、k3/），
#: 与 DeepSeek 主臂产物隔离；`PROBE_THINKING_OFF=1` 时给主模型关思考档（GLM 默认思考开，
#: deepseek-chat 默认无思考——对照口径需要同档）。两者都只作用于探针，不动生产代码。

#: 探针集（36 条多步维护链）：hard22 全量 22 条（多事项马拉松对抗集）+ aug 扩增集 14 条
...
_LOOP_VERBS = frozenset(_TOOL_META)  # 图内可执行的四个 loop 动词（真源与 live 探针同一份）
...
def capture(ids: list[str], rounds: int, sleep_s: float, resume: bool,
    ...
    def spy(chat_model, *, tools, messages, choice, json_prompt=None,
        ...
        # 2026-08-14 GLM-4.6 臂实测（仅此探针内生效，不动 src/）：zhipu 端点对
        # tool_choice="required" 不报 400 而是把请求挂住到超时（60s×重试≈180s），
        # 挂住的请求还占服务端并发槽、随后整账户 429(code 1302)；同一请求换 "auto"
        # 2s 内正常返回 tool_calls。故 GLM 臂把 required 降为 auto——understand 的
        # auto 车道本就在 _invoke_tool_channel 的 refallback 设计内（DeepSeek 臂的
