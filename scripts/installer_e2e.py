...
2. **安装器模式**（真实 Inno 产物，W4 联调后启用）：安装器 exe 存在时跑
   ...
   隔离钩子（见 `协同/交接/installer-w5-e2e-w4.md`）。
...
def _matrix_handlers(installer: Path, workdir: Path) -> list[dict]:
    ...
    W7 联调收口（对齐 协同/交接/installer-w5-e2e-w4.md 验收）：
    ...
    # 注意：真机「首装到自定义目录」由 GUI 真人实测覆盖（见开发日志），此处矩阵语义
    ...
    if downgrade is None:
        ...
        try:
            ...
            # （W4 交接「旧版覆盖新版被拦」实测：提示后确定退出）——超时同样视为被拦。
            rc, timed_out = -1, True
    ...
    if _webview2_installed():
        ...
        # ② 窗口模式启动 → launcher.log 无「已回退」行（仅探针通过才有判别意义）
        if m13_ok:
    ...
    if not authorized:
        return [{"id": iid, "name": "", "status": "skip",
                 "detail": f"安装器 {installer.name} 已就位但矩阵未授权真跑（需显式 "
                           ...
                           "且与 W4 并行施工/真实环境存在冲突面——待 W4 联调窗口由协调者授权运行"}
                for iid in MATRIX_IDS]
    if _installer_in_use(installer):
        return [{"id": iid, "name": "", "status": "skip",
                 "detail": f"安装器 {installer.name} 正被其他进程使用（疑似 W4 并行实测），"
