...
#
# database/external 官方快照口径（与安装器一致，2026-08-24 对齐）：`packaging/pyinstaller/
...
PUBLIC_DOC_FILES = {
    ...
    "docs/CHANGELOG.md",
    "docs/CICD_NATURE_SKILLS_RESEARCH_2026-07-14.md",
    "docs/research/sources/CICD_NATURE_SKILLS_SOURCE_LEDGER_2026-07-14.md",
}
EVAL_INPUT_FILES = {
...
# 仓库一级目录整理（2026-08-27）：requirements 群迁入 requirements/、启动脚本迁入
...
#      runtime/review item; `.deliveryignore` likewise excludes it from the customer artifact).
...
def _reject_release_forbidden_tokens_bytes(payload: bytes, relative: str) -> None:
    ...
    docs/agent/*, scripts/*, …) that embeds a真名/内部专名/本机路径 would otherwise ship
