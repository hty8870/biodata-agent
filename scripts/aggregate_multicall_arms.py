#!/usr/bin/env python3
"""跨模型对照聚合（2026-08-14 跨模型对照批）：把多条模型臂的保真度探针产物聚成一张对照表。

每条臂 = `research/reports/multicall-fidelity-probe/` 下的一个目录（主目录本身是
DeepSeek 臂；glm46/、k3/、codex/ 等是子目录臂）。直接从 capture.jsonl / pairs.jsonl /
...
#: 与探针分析同口径的只读/写动词划分（report.md 的"分 verb"表用的就是这套名字前缀）。
...
def arm_metrics(name: str, cases: set[str] | None = None,
    ...
    # 多调用率与探针 report.md 同口径：分母是"c1 为 loop 动词且被实际采纳"的事件。
...
def main() -> int:
    ...
    cli.add_argument("arms", nargs="*", help="臂名（. = DeepSeek 主目录）；缺省自动发现")
    cli.add_argument("--cases", default=None,
                     help="逗号分隔的用例 id 白名单——k3/codex 臂是 20 例子集，"
                          "跨臂对比时用同一子集重算各臂，消 selection bias")
    cli.add_argument("--round", default=None,
                     help="只统计某几轮的记录，逗号分隔（k3/codex/glm52 臂只跑 r1，"
                          "跨臂对比时各臂锁同轮次）")
    args = cli.parse_args()
