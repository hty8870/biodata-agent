...
def _search(page, query: str = SEARCH_Q) -> None:
    ...
    # 120s：本机检索含本地重排/向量召回模型推理，单次 utterance 可达 ~50s（2026-08-19 bl1 实测）；
...
def setup_history_pop(page) -> None:
    ...
    page.click('#histNav')               # ku1 起：历史记录是独立浮窗 #histWin（不再走 #archiveWin）
...
def setup_settings(page) -> None:
    ...
    # 把它们滚进视口（2026-08-21 a5 起卡片无动态数字行，无 mask 需求）。
...
def setup_favorites(page) -> None:
    # ku1 起：「我的收藏」视图已删，收藏收进「我的库」浮窗 #libWin 的收藏页签（浮窗非视图）。
    page.click('#libNav')
...
def _health(base_url: str) -> str:
    # 30s：本机检索路径含本地重排/向量召回模型推理，忙时 health 也可能排队（2026-08-19 bl1 实测）
