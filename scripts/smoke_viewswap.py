# -*- coding: utf-8 -*-
"""视图交换（2026-08-16 vs1 + vs2）Playwright 视觉/操作回归。
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        # ---- j1) vs2：头部件随结果进侧栏（#sideBoardScroll 顶部、卡片列表之前），主区无头部件 ----
        j1 = page.evaluate(
            """() => {
        ...
        # ---- j2) vs2：主区纯聊天——白卡框已去（无背景/边框/阴影），列宽 880px ----
        j2 = page.evaluate(
            """() => {
        ...
        # ---- j3) vs2：紧凑卡恢复「数据集详情」小钮且可点（新标签 /dataset） ----
        ...
        # ---- j5) vs2：交换态点侧栏两按钮，面板在主区对话列上方展开/可关 ----
        for btn_id, panel_id, shot in (("feasibilityBtn", "feasibilityPanel", "j5a-交换态-可行性面板.png"),
            ...
            try:
                ...
                if panel_id == "taskPackPanel":
                    ...
                    # （vs3 首轮回归真踩到：点关 → fetch 落地 → renderTaskPackPlan 又 hidden=false）。
        ...
        # ---- k1) vs3①：交换态「检索结果」页签下 #swHits（常驻查询条件栏）不可见；节点仍在 ----
        k1 = page.evaluate(
            """() => {
        ...
        # ---- l1) vs4：头部件紧凑化——头部区总高 ≤150px、标题行单行、短文案钮 ----
        def head_span():
        ...
        # ---- l2) vs4：放宽提示折叠单行、「放宽方式」钮折叠态可点、点击展开回流完整来源计数 ----
        ...
        # ---- k2) vs3②：输入条结构性钉底——960/1080 视口 + 20+ 条长对话（DOM 注入压力） ----
        ...
        # l3) vs4 正常态防误伤：还原后按钮显完整文案、摘要无截断盒（紧凑规则只在交换态命中）
        n_vs4 = page.evaluate(
            """() => {
                const fb = document.getElementById('feasibilityBtn');
        ...
        # ---- l4) vs4 同内容往返测高 + 无放宽提示态：「10x Visium 数据」（API 探测零 caveats）。
