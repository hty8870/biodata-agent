# -*- coding: utf-8 -*-
"""promo1：upload 运行时产物的**晋升机制**——把 curate sync 自动入库的 `upload_*.json`
...
- 归档即留痕：原 upload 文件搬入 `research/promotions/`（tracked；2026-08-27 随
...
#: 归档目录（顶层 research/promotions，tracked：归档即留痕；2026-08-27 随迁移批
...
def promote_uploads(
    ...
    *,
    ...
    for path, raw_bytes, records in accepted:
        ...
        note = (f"promo1 晋升：{path.name} → {BASE_SYNCED_NAME}"
