...
- `make_delivery.py` 负责回答「**哪些文件**可以外发」（.deliveryignore + git 已跟踪对账 + 敏感词/secret 复核）；
...
用法（先解析真实 Python 3.10+）：
  <python> scripts/make_submission_package.py --out "C:/.../biodata-agent-提交版-2026-07-22" --manual "C:/.../说明书.pdf"
...
def build(out_root: Path, manual_pdf: Path | None) -> int:
    ...
    for src in files:
        # 仓库一级目录整理（2026-08-27）后，被迁移文件在提交包内仍按历史根目录布局落位
