...
两个层级（对齐设计文档 §5.1「provisioning success rate」——文件级 + 数据集级，别混成一个数）：
...
additive（2026-08-01）：integrity/load 两维改读台账真实档位（`"i"`/`"l"` 槽，由 provision / load_smoke
...
def build() -> int:
    if not _CURRENT.exists():
        print(f"[report] 活台账缺失：{_CURRENT}\n       先跑 py scripts/seed_inspection_ledger.py 播种。", file=sys.stderr)
        return 2
    ...
    report = {
        ...
        "file_level": {
            ...
            # 口径说明：problem 自 2026-08-01 起可能来自 integrity/load 两维（不只 reachable/size）。
