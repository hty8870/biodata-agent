...
def _health(base_url: str) -> str:
    ...
    # 10s 太短会把「忙」误判成「没起来」（ai-fallback 重拍真踩过）。
...
def _new_page(browser, base_url: str):
    ...
    #（重拍 results.png 真踩过：/api/interpret 发出后被弹窗拦住，/api/utterance
...
def _search(page, query: str, timeout_ms: int = 240_000) -> None:
    ...
    # ：服务端已配真实 AI key 时，零配置会话会自动切到真实 LLM——重排/润色走
...
def shot_results(page, path: Path) -> None:
    ...
    #（重拍真踩过：只等 resultsWrap 显形，把「- 18s」在途计时泡拍进了图。）
...
def shot_continue(page, path: Path) -> None:
    ...
    # dock1：范围弹层重排为「右坞双圆钮 + 单面板」。拍前点开弹层，
    ...
    #（：放切页签之前摘被重渲重新注入；还曾错摘 #cbProgPct——挂着的这颗是 #cbLivePct）。
...
def shot_act_receipt(page, path: Path) -> None:
    """一句话执行回执（p10：长在对话流里的执行总结）。
     ...
     取景前提更新：本仓库服务端现已配好真实 AI key（health.llm_server.key_detected），
    ...
    那是 10.5 边界 ④ 的文字内容，不再用图展示。
     ...
     dl-auto-1 取景前提更新：pack.download 档在模型首次调用即**自动开始下载**、
    ...
    """
    _open_settings(page)
    # 「AI 执行」现行开关（旧 #cfgAutoAct 已随 设置重构退役）。默认开，
...
def shot_ai_fallback(page, path: Path) -> None:
    ...
    page.wait_for_timeout(300)
    # 失败手法换代：旧手法 base_url 指死端口（127.0.0.1:9）——但那次调用会
    # 把服务端事件循环卡进同步网络栈的重试预算（实测 >8min，期间 health 都排队），
    # 拍图等不起、同实例其它图也被拖死。新手法：base_url 保持与服务端一致（# 服务端 key 按 _build_request_overrides 同约继续适用）、api key 留空、
    ...
    page.fill("#cfgApiKey", "")
    # （M1 环架构后取景改道）：必须关掉「AI 执行」。开着时查询先撞分流那次 LLM
