#!/usr/bin/env python3
"""外部模拟模型回路 harness（2026-08-14 跨模型实验批）——用**外部模型**（k3 子 agent、
Codex CLI 等无法走仓库 LLM client 的驱动方）驱动 `agent_exec.plan_with_agent` 的同一个
...
被 stub 的 LLM 调用点（循环后装饰，不影响 events/executions/plan_steps）：
  ...
  "deterministic"。capture 记录不含汇报文本，探针分析器也不消费它。
...
进程非零退出）。驱动方（k3 子 agent / Codex CLI / 人工程序）：
  ...
  2. 写 `W/decision_<i>.json`（建议先写临时文件再 rename，harness 对半截文件有
     ...
     - 工具调用应答：{"calls": [{"name": <工具表里的名字>, "args": {...}}, ...]}；
       ...
       events——这正是保真度探针要量的东西，请如实给出模型的全部输出）；
...
replay 模式的两个**已知不对称**（capture.jsonl 不记录这两类调用，只能重建）：
  - checklist 轮：探针 spy 只留 understand/decide 事件，清单应答原文未留档。replay
  ...
  - repair 轮：探针不留 repair 事件。replay 重放上一次 understand 的调用（等价
...
class _SimAnswer:
    ...
    `_message_text` 读 content，探针序列化器读 tool_calls/invalid_tool_calls，
...
def _build_answer(decision, valid_names: set) -> _SimAnswer:
    ...
    for c in calls:
        ...
        if not isinstance(name, str) or not name or name not in valid_names:
            ...
            # 第一个的语义不变），同时在 invalid 列留痕供探针事件对照。
...
class _ReplayDriver:
    """replay 模式驱动：从探针 DeepSeek 臂记录的 events 依次取应答（零外部模型）。
...
def _make_channel(driver, cur: dict):
    ...
    面——外部模型不产生服务端 400）；事件留痕与探针 spy 同构。"""
...
def _logging_run(cur: dict, verb: str, run):
    """与探针 `_logging_run` 逐行同构：executions 记录实际执行的调用（含失败步）。"""
...
def _load_replay_record(case_id: str, rd: int) -> dict:
    """探针主目录 capture.jsonl（DeepSeek 臂）里同 case+round 的**最后一条**记录。"""
