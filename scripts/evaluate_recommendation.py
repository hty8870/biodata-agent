...
# 冻结基线逐项 pin（AGENTS.md §7 正式基线：Top1 97.7% / Top5 97.7%）。
# 旧门只判 `Top5≥90` floor、从不判 Top1 —— 会让 97.6%→90.x% 的真回退静默全绿，
# 与"frozen"之名不符（评审确认的真缺陷）。改为 Top1 与 Top5 都 pin 到正式基线；
# 否定/模态等**受控重基线**若合法调整基线，在此同步更新常量（AGENTS.md §2/§7）。
# 2026-07-25 受控重基线（已获项目负责人授权）：「或」/hedge 由整句弃权改为照做，
...
# 2026-08-06：以下为 CLI 默认值的**模块级**常量；held-out 等非主集评测用 --expect-* 覆盖，
...
def evaluate(queries, records, settings, top_k: int = 5) -> dict:
    ...
    for q in queries:
        ...
        if expect_empty:
            ...
            # MRR（2026-08-09 调研-检索质量批 R1：Top1/Top5 97.7% 天花板下，名次移动
...
def main():
    ...
    # 正式指标不达标 → 非零退出，让 QUALITY_GATES 的 $LASTEXITCODE 门真正闭合（Codex 发现的存量门漏洞）。
    if not ok:
        raise SystemExit(1)
