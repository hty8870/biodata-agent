#!/usr/bin/env python3
"""多 tool_call **第 2..N 个调用合法率**微探针（2026-08-14 mcl1 批，只读实验）。

与 `probe_multicall_fidelity.py`（mcf1：测「预规划 vs 带真实状态重规划」的逐位保真度 →
整批乐观执行 NO-GO）互补：本探针回答一个更窄的问题——**只读且互相独立**的批量调用，
第 2..N 个调用的合法率上限有多高（schema 合法 + 语义合法），以及**提示词变体**
（明确邀请独立只读批量）能否稳定诱发多调用。判据：A 类 2..N 合法率 ≥85% 且变体稳定
诱发 → 支持给只读白名单开真并行；否则不施工。

实测事实背景（本探针再确认）：请求侧恒上 `parallel_tool_calls=False`，DeepSeek 不遵守；
...
产物：`research/reports/multicall-legality-probe/`（capture.jsonl /
...
# --------------------------------------------------------------------------- 提示词变体臂
# 三处「一次一个」锚的替换对（旧串必须在模块常量里逐字命中一次，否则探针直接报错——
# 提示词漂移宁可 fail-loud 也不静默测错对象）。
# 2026-08-15 dsv 批：变体文本已**采纳进生产**（agent_exec.py 三锚点替换，依据 dsv 臂
# 108 运行：多调用率 16%→50%、通道往返 -12%、完成率持平）。此后 _swap_once 幂等：
...
def _swap_once(text: str, old: str, new: str, where: str) -> str:
    ...
    if n_old == 0 and n_new >= 1:
        return text  # 2026-08-15 起变体已是生产基线：幂等放行
    if n_old != 1:
