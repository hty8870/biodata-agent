# -*- coding: utf-8 -*-
"""pack1（2026-08-16）打包回执 chip 精简 Playwright 实测。

验证点（用户需求原话三点）：
  p1) 自动打包（「下载top5」）回执：#taskPackPanel **保持关闭**；chip 行只剩
      「打开下载面板自己挑」（pack 系无撤回）；「按原话重新检索」「以后别自动执行」
      两颗 chip 在整条对话流里都不存在；
  p2) 点「打开下载面板自己挑」→ 面板打开、清单可勾选；
  p3) 显式预览（「我自己挑，先给我看看清单」）→ pack.preview 履约：面板**会**打开；
  p4) 回归：结果区顶部「下载这批数据」钮照常开面板；
  p5) 回归：设置里 cfgAgentExec（AI 执行开关）不受删 chip 影响（off chip 原是它
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        # ---- p1) 自动打包「下载top5」：面板保持关闭 + chip 行只剩「打开下载面板自己挑」 ----
        say("下载top5")
        ...
        check("p1：自动打包时下载面板保持关闭", p1["panelHidden"], str(p1))
        check("p1：chip 行只剩「打开下载面板自己挑」（无 research/off/undo）",
              p1["chips"] == ["panel"], str(p1["chips"]))
        check("p1：对话流无「按原话重新检索」「以后别自动执行」", p1["noResearch"] and p1["noOff"])
        page.screenshot(path=str(SHOTS / "p1-自动打包回执-面板保持关闭.png"))
        page.locator("#cbHistory .act-fix").last.screenshot(path=str(SHOTS / "p1b-chip行特写.png"))

        # ---- p2) 点 chip → 面板打开、清单可勾选 ----
        ...
        check("p2：点 chip 面板打开、清单行带勾选框", p2["visible"] and p2["nBoxes"] > 0, str(p2))
        if p2["nBoxes"] > 1:
            page.locator('#taskPackPanel input[type="checkbox"]').nth(1).click()   # 勾掉一行验证可改
            page.wait_for_timeout(250)
        page.screenshot(path=str(SHOTS / "p2-点chip面板打开可勾选.png"))
        page.click("#taskPackCloseBtn")   # 关面板，为 p3 的「会打开」造对照
        page.wait_for_timeout(300)
        check("p2：面板可关闭", page.evaluate("() => document.getElementById('taskPackPanel').hidden"))

        # ---- p3) 显式预览「我自己挑，先给我看看清单」→ 面板会打开（pack.preview 履约） ----
        say("我自己挑，先给我看看清单")
        ...
        check("p3：显式预览面板会打开（pack.preview 履约）", p3["visible"] and p3["rows"] > 0, str(p3))
        page.screenshot(path=str(SHOTS / "p3-显式预览面板打开.png"))
        ...
        page.wait_for_selector("#taskPackPanel:not([hidden])", timeout=15000)
        page.wait_for_timeout(400)
        check("p4：顶部「下载这批数据」钮照常开面板",
              page.evaluate("() => !document.getElementById('taskPackPanel').hidden"))
        page.screenshot(path=str(SHOTS / "p4-顶部按钮开面板.png"))
        page.click("#taskPackBtn")   # 再点收起（toggle）
        page.wait_for_timeout(300)
        check("p4：再点可收起", page.evaluate("() => document.getElementById('taskPackPanel').hidden"))

        # ---- p5) 回归：cfgAgentExec 开关在场可切（off chip 删了，开关本体不受影响） ----
        p5 = page.evaluate(
            """() => {
        ...
        check("p5：cfgAgentExec 开关在场、可切换、可切回", p5["ok"], str(p5))
