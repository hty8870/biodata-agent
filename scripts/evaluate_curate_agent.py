# -*- coding: utf-8 -*-
"""对话式数据库管护（curate）的 **execution-based** 冻结评测门（2026-08-09 五机制批）。

**借鉴 BIRD 的 EX 指标**（调研-对话式数据管理agent-2026-08-09 §机制3）：断言的不是 agent
...
LLM → raw 那一段由真机探针 `evaluate_agent_live.py` 覆盖，不在本门（本门要的是可复现）。
