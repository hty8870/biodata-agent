...
def main() -> int:
    ...
    required_home = [
        ...
        'id="cfgAgentExec"',    # AI 执行开关（维度 C，2026-08-03 agent2 合并旧「说了就直接做」+「Agent 规划执行」）
        ...
        "/api/curate/apply",        # 回传 confirm_token 才真执行（agent2：前端 runner 链式直推，问卷已退役）
        ...
        'id="onboardingProgress">1 / 14<',  # 14 步教程：第0步反馈承诺+高质量查询引导（tu1）+ 第2屏「一句话交代整件事」能力心智（ob3 前移）+ 真实 API 配置表单 + 排序说明（规则开箱即用/两种增强随时补）+ 润色/细化/详情页实拍/条件板 + 我的库介绍（任务C）+ 接进你自己的 AI 助手（fx2，任务C 后移精简）+ 使用反馈指路
        'id="nodeUsage"',                  # 设置里的使用反馈开关（默认态按版本分叉：强化版开/主线关，2026-08-13 ob1 起）
        'data-onboarding-visual="ranking"',
        'data-onboarding-visual="agent"',  # 教程「接进你自己的 AI 助手」视觉块（fx2；任务C 起后移至结尾、文案精简）
        "AI 润色只改说明，不改结果",
        "用细化筛选收窄，再查看数据集介绍",
        ...
        "/api/recommend",
        "放宽方式",             # 2026-08-01 ux6：coverage_caveats 展开开关文案——多档放宽策略选择（results.js）
        ...
        "/api/download/plan",       # dl2：真实数据下载分级（uids → 可下载清单，零副作用零网络）
        "/api/download/start",      # dl2：真实下载启动（预检 400/507/409 → 建目录 → 起线程）
        "/api/download/status",     # dl2：真实下载状态轮询（1s 进度）
        "/api/download/cancel",     # dl2：取消下载（保留 .part 可续传）
        "fetch(",
    ]
    for token in required_home:
        if token not in bundle:
            fail(f"frontend bundle missing token: {token}")
    # 版本分叉（2026-08-13 ob1）：反馈强化版带 benchmark 采集（导出反馈包），主线版没有。
