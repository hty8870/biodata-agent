...
#: MCP 真协议自检（批）：frozen MCP 入口 + 自检超时。构建/打包前必须跑
...
#: Setup。 交付审计 fix 4 增加 manifest 校验（存在性 + 关键字段 + 实物逐文件对账，
...
def verify_runtime_manifest(runtime_dir: Path) -> "tuple[bool, str]":
    """校验 `build_windows_runtime.py:91` 写出的 runtime-manifest.json（fail-closed）。
     ...
     交付审计 fix 4：此前 `--build-runtime` 只确认 onedir 目录/两个 exe 存在，不校验
...
def main() -> int:
    ...
    # ②a-fix4 runtime-manifest 校验（fail-closed； 交付审计 fix 4）——
    ...
    if args.skip_runtime_version_check:
        # 旁路要醒目警告（交付审计 fix 4）：只关 exe /api/health 版本比对，**不旁路**
