# -*- coding: utf-8 -*-
"""一键构建 BioData Agent 图形化安装器（安装器工程 W4，Inno Setup 6.7.3）。
...
流程（fail-closed）：
  ...
  2. 确认 PyInstaller onedir 产物存在（`<out>/dist/BioDataAgent/BioDataAgent.exe`）；
     `--build-runtime` 时先触发 `scripts/build_windows_runtime.py`（复用 W3 build-venv）。
  ...
  ov1-mcp1 批新增校验（均在 ISCC 前、fail-closed）：
...
用法：
  ...
  环境变量：BIODATA_BUILD_OUT / BIODATA_BUILD_VENV（与 W3 脚本同键）。
"""
...
# 产物命名模板（unsigned-dev 表明未签名开发构建；签名属后续批次）
...
#: MCP 真协议自检（ov1-mcp1 批）：frozen MCP 入口 + 自检超时。构建/打包前必须跑
...
#: Setup。2026-08-24 交付审计 fix 4 增加 manifest 校验（存在性 + 关键字段 + 实物逐文件对账，
...
def verify_mcp_selfcheck(runtime_dir: Path) -> "tuple[bool, str]":
    """frozen MCP 入口 --selfcheck 真协议探测（ov1-mcp1 批，fail-closed 语义）。
...
def verify_runtime_manifest(runtime_dir: Path) -> "tuple[bool, str]":
    ...
    2026-08-24 交付审计 fix 4：此前 `--build-runtime` 只确认 onedir 目录/两个 exe 存在，不校验
...
def main() -> int:
    ...
    # ②a-fix4 runtime-manifest 校验（fail-closed；2026-08-24 交付审计 fix 4）——
    ...
    # ②c MCP 真协议自检（ov1-mcp1 批，fail-closed）：跑 BioDataAgentMCP.exe --selfcheck
    ...
    if args.skip_runtime_version_check:
        # 旁路要醒目警告（2026-08-24 交付审计 fix 4）：只关 exe /api/health 版本比对，**不旁路**
