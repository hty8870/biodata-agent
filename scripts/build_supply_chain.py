# -*- coding: utf-8 -*-
"""BioData Agent 安装器供应链产物生成（安装器工程 W6，纯标准库、确定性）。

在隔离构建 venv（默认 `<仓库父目录>/build-venv`，也可用环境变量 BIODATA_BUILD_VENV
指定）上运行。`scripts/build_windows_runtime.py`（W3）**不改**——本脚本是它的供应链配套：
...
#: 诚实联网来源说明（§5/§6 供应链收口）：不得声称所有外部字节都被仓库 lock 直接覆盖。
...
#: Inno Setup 版本（W4 交付固定编译器版本；官方 jrsoftware GitHub release：
    ...
    *, tool_versions: dict[str, str], timestamp: str | None = None
    ...
    template: dict[str, Any] = {
        ...
        "verification_steps": [
            ...
            "W5 migration + end-to-end installer tests (test_migrate_from.py / test_installer_e2e.py)",
        ],
        "results": {
...
def installer_artifact_name(base: str, *, has_signing_credentials: bool) -> str:
    ...
    W4 的安装器构建脚本按此约定命名产物；本函数是命名逻辑的真源（契约测试钉死）。
    """
