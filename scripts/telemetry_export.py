#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""遥测落库数据 → 推荐评测 benchmark 原料导出管线（批，）。
...
KNOWN_BENCHFB_KEYS = {"id", "t", "kind", "q", "route", "search", "rating", "action", "env", "ms", "err",
                      ...
                      "contract_version", "prompt_version", "experiment_id", "experiment_arm", "propensity",
                      # （设计 §5.5）：任务卡/chip 生成文本提交轮次标记（bool）
                      "template_originated"}
...
def _export(engine, out_dir: Path, *, since: datetime | None, until: datetime | None,
    ...
    for p in packets:
        ...
        for rec in benchfb:
            ...
            if rid:
                # （设计 §5.5）：benchmark 候选**默认排除**「未经编辑直接发送」的模板轮次
    ...
    # 事件行； 起键含 recId，双源去重真正生效，同一轮次多条记录/多 rev 不再互相吞并）
