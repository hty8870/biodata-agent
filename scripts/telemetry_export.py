#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""遥测落库数据 → 推荐评测 benchmark 原料导出管线（ov1-bench1 批，PKG-E）。
...
产物（--out 目录，全部 UTF-8 JSONL / Markdown / HTML）：
  ...
  explicit_labels.jsonl       显式标注双源合并：usage kind:"label" 事件
                              ...
                              （label_source 区分来源；recId 为 ov1-fix2 新增，老数据缺失时空串兜底、
                              与 benchfb 侧不匹配属预期降级）
...
# eng-b3 追加 API Key 形态（sk-…/AKIA…/Bearer token/ghp_… → [API Key]，与接收端 app.py
...
KNOWN_USAGE_KEYS = {"event_id", "t", "k", "q", "n", "shown", "abstain", "unused", "cached", "ms",
                    ...
                    # ov1-tel1 schema v2 中央注入与 v3 自描述事件（imp/view/label/交互自带关联键）
...
KNOWN_BENCHFB_KEYS = {"id", "t", "kind", "q", "route", "search", "rating", "action", "env", "ms", "err",
                      ...
                      "contract_version", "prompt_version", "experiment_id", "experiment_arm", "propensity",
                      # eng-p6（2026-08-22，设计 §5.5）：任务卡/chip 生成文本提交轮次标记（bool）
                      "template_originated"}
...
def _candidate(rec: dict[str, Any], *, install_id: str, packet_id: str,
    ...
    ov1-fix1b 起 benchfb 不再制造 impression，也不再从记录 id 合成 iid——候选的 iid
    只能在全量曝光索引建完后按 (install_id,tid) 精确回填；
    ov1-fix2 起与 turns/labels 的关联键 tid = 记录内 tid 字段（轮次 id，与 usage/label 事件同语义），
    ...
    return {
        ...
        # ov1-bench1 新字段：与 turns 的关联键；iid 不再从 benchfb 合成（ov1-fix1b）。
        # ov1-fix2：tid 改取记录内轮次 id（与 turns/labels 的 tid 同语义），record_id 仍是记录 id。
        ...
        # eng-p6（2026-08-22，设计 §5.5）：模板轮次标记随候选携带（生成时默认已排除
...
def _quality_report(report: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        f"- 模板轮次候选（eng-p6，默认排除）：{e.get('candidates_excluded_template', 0)}",
        "",
        "## mcp 调用统计",
        "",
        f"- 调用总数 {e['mcp_total']}；按 install：{_fmt_counter(e['mcp_by_install'])}",
        "",
        "## 意见反馈（eng-b3：接收端已解密并遮蔽，这里只读遮蔽后形态）",
        "",
...
def _export(engine, out_dir: Path, *, since: datetime | None, until: datetime | None,
    ...
    ov1-fix1b 起为**两阶段全量扫描**：先把所有包的事件扫成曝光/标注索引（跨包 join——
    ...
    # (install_id,tid,recId) 合并（ov1-fix2：recId 区分同一轮次下的多条记录/多 rev，两侧键空间统一，
    ...
    feedback_rows: list[dict[str, Any]] = []   # eng-b3：意见反馈（payload 内为解密遮蔽后形态）
    ...
    report: dict[str, Any] = {
        ...
        "feedback_total": 0, "feedback_with_diag": 0,   # eng-b3
        "agent_trajectories_total": 0,
        "candidates_excluded_template": 0,   # eng-p6：默认排除的模板轮次候选计数
    }
    ...
    for p in packets:
        ...
        # --- benchfb 轮次：只作 turn / label / candidate 来源（不再制造 impression，ov1-fix1b）---
        for rec in benchfb:
            ...
            if isinstance(rating, dict):
                ...
                if rid and _rating_nonempty(rating):
                    ...
                    # 事件时被事件覆盖（ov1-fix2：键 = 记录内 tid + record id，与 label 事件键空间统一；
            ...
            if rid:
                # eng-p6（设计 §5.5）：benchmark 候选**默认排除**「未经编辑直接发送」的模板轮次
        ...
        for ev in usage:
            ...
            if kind == IMP_KIND or (kind == SEARCH_KIND and ev_iid):
                ...
                # ov1-fix2：键 = (install_id, tid, recId)——同一轮次下多条记录的评分互不吞并（1c），
        ...
        # --- eng-b3：意见反馈（接收端已解密并过 API Key/值级遮蔽；这里只读遮蔽后形态）---
    ...
    # 事件行；ov1-fix2 起键含 recId，双源去重真正生效，同一轮次多条记录/多 rev 不再互相吞并）
    ...
    products = [
        ...
        # eng-b3：意见反馈按 feedback_id 去重（同一 feedback_id 重传只留首次，与接收端幂等同键）
