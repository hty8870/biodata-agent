# -*- coding: utf-8 -*-
"""执行侧 agent 的**真机性能探针**（2026-08-08 v5：codex 静态评审处置批——假阳性通道清零；
v6：报告加「失败聚类」段 + 新维 number_grounded 汇报数字出处不变量）。

与单测的本质区别：单测用 fake model 钉行为（永远满分，无评估意义）；本探针用**真实 LLM**
...
已知未覆盖（codex 评审盲区清单，2026-08-08 登记，刻意只记录不实现）：
...
离线自验（零 API，2026-08-08 v5）：`--selftest`——启动自检过当前用例集 + 全维度
...
PAYLOADS: dict[str, dict] = {
    ...
    # 2026-08-15 ta1：search_online「全部撞重零写入」合法契约（corpus 层 filename=None +
    ...
    # 2026-08-22 ov1-wf1：rank 剧本负载（混合诉求用例）——六键过 RankResult 形状闸；
    # displayed=True 但 batch=None（剧本不装配 M3 批次原料，RankResult 允许 None）。
    "rank_ok": {
...
_TOOL_META = {
    ...
    "fair.check": {"label_zh": "检查 FAIR 就绪度", "card_kind": "fair_check",
                   "readonly": True},
    # 2026-08-22 ov1-wf1：rank 入剧本面（混合诉求用例的检索半）。同样刻意**不**带
...
_PAYLOAD_VERB_PREFIX = {
    ...
    "rank": "rank",  # 2026-08-22 ov1-wf1：rank 剧本负载随混合用例登记
}
...
def load_cases(path: Path) -> list[dict]:
    ...
    for ln_no, ln in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        # 2026-08-08 v5（致命-2）+ v5.2（高-2/中-7）：first 钉了 tools 提供的 loop 动词
...
#: expect_invalid/or_invalid 口径里的「护栏拒收」判定（2026-08-08 v5.2 中-8 收窄）：
...
def _script_expected_ok(tools: dict, verb: str, slots: dict) -> "bool | None":
    """剧本隐含期望（2026-08-08 v5，致命-1）：从用例 tools 剧本推导该步应有的 ok——
...
def _script_verdicts(tools: dict, steps: list[dict], matched: list[int],
    ...
    """matched 步的**剧本 ok 核对**（2026-08-08 v5.3 高-2）：每个命中步的 ok 必须与
...
# ---- report_covers 数字匹配加固（2026-08-08 v5，高-3）----
...
def _num_re(n: int) -> re.Pattern:
    ...
    0 额外排除 `-` `:` `/` 邻接——快照日期/时间（2026-08-01T00:00:00）里的「08」「00」
    不许被当成数字 0 命中。"""
    extra = ":/-" if n == 0 else ""
...
#: 零值同小句判定的隔断表（2026-08-08 v5.3 中-4）：`_CLAUSE_BREAKS_ZH` 去掉「，」「：」
...
def _report_covers_misses(steps: list[dict], report: str) -> list[str]:
    ...
    def zero_covered(label: str, all_labels: list[str]) -> bool:
        ...
        句形被误判缺——探针 v10 l08 假阴性现场）；命中才消费游标，未命中不消费。"""
...
# ---- number_grounded 汇报数字出处不变量（2026-08-08 v6）----
...
def score_case(case: dict, plan: dict, trace: list[dict], exc_kind: str = "",
    ...
    # ---- 异常路径（2026-08-08 v5 / v5.2 中-8 收窄 + 高-4 双结局）----
    if exc_kind:
    ...
    # on_script（2026-08-08 v5，致命-2）：plan.verb / steps 里出现 tools 外的 loop 动词
    ...
    if "must_steps_unordered" in exp:
        ...
        if not chain_done and len(wants) >= 2:
            ...
            chain_done = True
    if "must_when" in exp:
        # 路径条件断言（2026-08-08 v5.2 高-2 + v5.3 中-3）：按 plan.verb 选子句，命中
    ...
    if "check_sources" in exp:
        # 覆盖维（2026-08-08 v5.2 高-3）：所有 ok check_updates 步的 slots.source
    ...
    if "search_topics" in exp:
        # 覆盖维（2026-08-08 v5.2 高-3）：所有 ok search_online 步的 keywords 并集
        # （大小写不敏感拼接）必须逐串含各主题子串——空关键词/漏主题计败。
    ...
    # number_grounded（2026-08-08 v6，汇报数字出处不变量）：steps 非空且汇报非空时
...
def _cluster_failures(fails: list[dict]) -> list[tuple[tuple[str, str, str], list[str], str]]:
    """失败聚类（2026-08-08 v6）：把失败记录按 (首败维度, 失败发生的首个 agent 节点,
...
def main() -> int:
    ...
    def _execute_case(case: dict, round_no: int) -> dict:
        ...
        if not exc and plan.get("report_source") == "deterministic":
            # fallback 率信号（2026-08-08 v5，不计分）：确定性兜底汇报逐例留痕
...
def _run_selftest(cases_path: Path) -> int:
    ...
    check("report_covers 零值同小句否定（中-6a）：隔壁小句的「没有」不许借", lambda: (
        (lambda a, b, c: (a is False and b is True and c is True, f"{a} / {b} / {c}"))(
            # codex 例：「网络没有异常」的没有不许覆盖 ArrayExpress 的 new_count=0
    ...
    # 2026-08-08 v5.1 游标回归钉（k01/l08 假失败病灶）：hit_number 取正则与措辞候选的
