# -*- coding: utf-8 -*-
"""执行侧 agent 的**集成性能验证**（v5：验证 处置批——假阳性通道清零；
...
用例字段：
  ...
  expect（全软断言，逐项计分，不存在「一错全否」）：
    ...
    must_when: [{"if_first": 动词, "must": [条目...]}]——**路径条件断言**（v5.2，取代行动型用例的 allow_no_exec）：按 plan.verb 选子句，命中子句的
...
自动参与的维（无需在 expect 里点名）：
    chain_complete: must_steps（或 must_steps_unordered，或 must_when 命中子句）≥2 条时
                 ...
                 而「剧本该成却败」依旧记败。
    on_script: plan.verb / steps 里出现 **tools 外的 loop 动词** → False（v5：off-script 空执行此前是恒过通道——execute 对注册表外动词空过，
    ...
    "db_status_ok": {
        ...
        "sources": [{"source": "10x", "label": "10x Genomics", "local_count": 774,
                    ...
                    {"source": "arrayexpress", "label": "ArrayExpress", "local_count": 1784,
                     "snapshot_date": "2026-08-01"}],
        # v5（高-4）：total_records 与 corpus_status 定义一致 = 各源 local_count
...
_TOOL_META = {
    ...
    "curate.sync_updates": {"label_zh": "检查更新并同步入库", "card_kind": "sync_updates",
                            "readonly": False},
    # 四工具批：环内结果处理四工具入剧本面（缺省对象由剧本语境承载——
...
def _outcome_to_result(outcome: dict, slots: dict | None = None):
    ...
    # v5（高-4）+ v5.2（中-5 补全）：来源感知修正扩展到 search_online 负载——
...
# v5（中-6）：payload↔契约、动词合法性、冲突/类型/形状/重复键/未知字段、
...
#: 四工具批：compare/cite/compat/fair 四前缀随剧本面登记同步加入。
...
def load_cases(path: Path) -> list[dict]:
    ...
    for ln_no, ln in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        # v5（致命-2）+ v5.2（高-2/中-7）：first 钉了 tools 提供的 loop 动词
...
#: expect_invalid/or_invalid 口径里的「护栏拒收」判定（v5.2 中-8 收窄）：
...
def _script_expected_ok(tools: dict, verb: str, slots: dict) -> "bool | None":
    """剧本隐含期望（v5，致命-1）：从用例 tools 剧本推导该步应有的 ok——
...
def _script_verdicts(tools: dict, steps: list[dict], matched: list[int],
    ...
    """matched 步的**剧本 ok 核对**（v5.3 高-2）：每个命中步的 ok 必须与
...
class _CoverCursor:
    ...
    def hit(self, *candidates: str) -> bool:
        ...
        游标未命中时从 0 起**不消费地**全局再找一次（v5.2 中-6b：
    ...
    def hit_number(self, n: int, *, _bare_zero_ok: bool = True) -> bool:
        ...
        **取正则与措辞候选的最早命中消费**（v5 画廊实证：正则先行会跳到
...
#: 零值同小句判定的隔断表（v5.3 中-4）：`_CLAUSE_BREAKS_ZH` 去掉「，」「：」
...
def _report_covers_misses(steps: list[dict], report: str) -> list[str]:
    ...
    （v5.2 中-6a + v5.3 中-4）：0/零、固定否定短语、label 后备的命中
    ...
    def zero_covered(label: str, all_labels: list[str]) -> bool:
        ...
        句形被误判缺——验证 v10 l08 假阴性现场）；命中才消费游标，未命中不消费。"""
...
def _number_grounded_misses(steps: list[dict], report: str) -> list[str]:
    ...
    for m in _INT_RE.finditer(text):
        ...
        if int(tok) == 0 and zero_grounded:
            continue  # ③ 空容器/显式零值在场的「0」（v17aug 误伤主体，）
        if tok in seen:
            continue
        seen.add(tok)
...
def score_case(case: dict, plan: dict, trace: list[dict], exc_kind: str = "",
    ...
    # on_script（v5，致命-2）：plan.verb / steps 里出现 tools 外的 loop 动词
    ...
    if "must_steps_unordered" in exp:
        ...
        if not chain_done and len(wants) >= 2:
            ...
            chain_done = True
    if "must_when" in exp:
        # 路径条件断言（v5.2 高-2 + v5.3 中-3）：按 plan.verb 选子句，命中
    ...
    if "check_sources" in exp:
        # 覆盖维（v5.2 高-3）：所有 ok check_updates 步的 slots.source
    ...
    if "search_topics" in exp:
        # 覆盖维（v5.2 高-3）：所有 ok search_online 步的 keywords 并集
        # （大小写不敏感拼接）必须逐串含各主题子串——空关键词/漏主题计败。
    ...
    # number_grounded（v6，汇报数字出处不变量）：steps 非空且汇报非空时
...
def _cluster_failures(fails: list[dict]) -> list[tuple[tuple[str, str, str], list[str], str]]:
    """失败聚类（v6）：把失败记录按 (首败维度, 失败发生的首个 agent 节点,
...
def main() -> int:
    ...
    def _execute_case(case: dict, round_no: int) -> dict:
        ...
        if not exc and plan.get("report_source") == "deterministic":
            # fallback 率信号（v5，不计分）：确定性兜底汇报逐例留痕
...
def _run_selftest(cases_path: Path) -> int:
    ...
    # v5.1 游标回归钉（k01/l08 假失败问题）：hit_number 取正则与措辞候选的
