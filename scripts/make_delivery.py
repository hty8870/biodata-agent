# -*- coding: utf-8 -*-
"""交付打包 + 交付安全复核（把 DEVELOPMENT §11 的"提交包不应含内部材料"从规定变成机制）。

做两件事：
1. **剔除**：按仓库根 `.deliveryignore`（gitignore 风格）+ 硬排除（.git/.venv/__pycache__/秘密/`.userdata`）
   + **`.gitignore` 对账**收集对外交付文件集，把开发日志、协同台账、内部评审/规划报告、Agent 契约等
   内部材料挡在包外。
2. **复核**：对**留在包里**的文本文件做敏感词扫描（个人化敏感词 + 本机绝对路径）；命中即视为
   泄漏，拒绝打包并逐条报告（`file:line token=…`，不回显整行）。三门全绿拦不住"打包时把内部件塞进 ZIP"，
   这个脚本就是补上的那道机制。

**为什么要和 `.gitignore` 对账**（2026-07-17 全盘审计发现的根因）：`.deliveryignore` 曾是与 `.gitignore`
平行的第二份**手抄** denylist，两份从未对账。结果 `.gitignore` 里明明写着「`.userdata/` —— 本地账户数据
（用户名 + scrypt 密码哈希，仅本机、**绝不入库/交付**）」，交付集里却躺着 `.userdata/accounts.json`，
而 `--check` 照样打印「复核通过：0 内部专名 / 0 secret 值命中」——secret 扫描器抓不到纯 hex 的 salt/hash，
敏感词扫描器也抓不到（文件里没中文专名）。同批漏网的还有 `models/`(2.27 GB 权重)、`artifacts/`、`outputs/`，
共 28 个文件 / 2.32 GB。逐条往 `.deliveryignore` 补 glob 只能修掉这 4 个出口，第 5 个新目录仍会漏——
所以改成**结构性**绑定：**git 拒绝跟踪的文件，一律不进交付集**。干净 clone 里本来就没有它们，
交付包与干净 clone 对齐才是正确语义。

用法（先解析真实 Python 3.10+，见 docs/agent/WINDOWS_ENVIRONMENT.md）：
  <python> scripts/make_delivery.py --check            # 只复核：有泄漏/内部件命中→退出 1，干净→0（CI/交付前门）
...
本文件在 `.deliveryignore` 中被排除 → 不进交付包、也不被自身内容复核（它必须字面持有下列检测词）。
"""
...
# 交付包**绝不能出现**的敏感词（内部/个人/未发表）。命中即拒绝打包。
# 分两层，避免把任何个人身份信息留在源码里：
# - **通用层**（源码持有）：无歧义的本机个人绝对路径形态。源码里留下的是守卫机制要匹配的检测词
#   本身、不是泄漏；教程里的 C:\Users\<用户名> 模板不算泄漏、刻意不匹配。需要时再扩。
GENERIC_FORBIDDEN_TOKENS: tuple[str, ...] = (
    r"Users\ROG",      # 本机个人绝对路径（Windows 反斜杠）
    "Users/ROG",       # 本机个人绝对路径（正斜杠 / git-bash 形，大小写按实际盘符）
    "Users/rog",
)
...
# 仓库一级目录整理（2026-08-27）：启动脚本群迁入 launchers/、requirements 群迁入
...
# 无论 .deliveryignore 是否列出、也无论 git 是否可用，都硬排除的目录/文件名（秘密与缓存绝不外发）。
...
# 这些行是**故意**放置形似 OpenAI key 的字符串（sk-abcdefghijklmnopqrstuvwxyz0123）来验证值级遮蔽
# 数据在遮蔽后不出现在产物/响应里（tests/test_telemetry_export.py / test_telemetry_receiver.py 的
# 还原测试夹具）——并非真实凭据。只豁免「精确 (文件, 行)」，绝不整文件、整行段落或整目录豁免；
...
class GitUnavailable(RuntimeError):
    ...
    本脚本自身被 .deliveryignore 排除、只在仓库工作树里运行，因此 git 缺失属异常环境、应当报出来。
    """

# 只扫这些扩展名的文本文件（模型权重/图片/压缩包等二进制不扫、也很少进交付）。
_TEXT_SUFFIXES = {
    ".py", ".js", ".ts", ".html", ".htm", ".css", ".md", ".markdown", ".txt",
...
def tracked_paths(root: Path = REPO_ROOT) -> set[str]:
    ...
    **为什么交付集要与「已跟踪」取交集**（2026-07-21 补）：`gitignored_paths` 只挡「git 明确忽略」的文件，
    挡不住**未跟踪且未被忽略**的那一类——个人汇报、临时导出、别人放在工作树里的草稿。历史实测：仓库根
    躺着一份个人周报 PDF（未跟踪、文件名没被 `.gitignore` 的锚定模式覆盖），它进了交付集，而 `--check`
    照样打印「复核通过」——因为 `.pdf` 不在 `_TEXT_SUFFIXES` 里，两个扫描器都不看它。又一次 fail-open。
...
def collect_delivery_files(root: Path = REPO_ROOT) -> list[Path]:
    ...
    2. **git 对账**——只保留**被 git 跟踪**的文件（`tracked_paths`）。这一步同时覆盖了「被忽略」和
       「未跟踪但没被忽略」两类：干净 clone 里两者都不存在，客户也不该收到；
    3. `.deliveryignore`——已跟踪、但属内部材料（开发日志 / 协同台账 / Agent 契约）的那批。

    第 2 道是 2026-07-17 补的根因修复、2026-07-21 收紧：最初 1 和 3 是两份**手抄** denylist，
    `.gitignore` 写着「`.userdata/` 绝不入库/交付」而交付集里就躺着 `.userdata/accounts.json`；
    第一版修复改成「剔除 git 忽略件」，仍漏掉「未跟踪、也没被忽略」的个人文件（实测漏过一份个人周报
    PDF，且因 `.pdf` 不是扫描后缀而毫无告警）。语义上，交付包应当与**干净 clone** 一致，
    所以这里直接按「已跟踪」取交集。
    """
