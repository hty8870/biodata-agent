...
# W1：sys.path 必须锚定**真实源码位置**（本文件上两级 = 仓库根），与运行 cwd 无关；
...
# 公网护栏硬化（2026-08-26）：护栏模式（BIODATA_REQUIRE_ACCOUNT=1）的启动期 fail-closed 校验。
# 灰度曾允许「缺邀请码/缺 Host 白名单/配额用默认值」起手——缺任何一项，公网实例都会在
# 降级形态下裸奔（注册整体关闭只剩管理员干瞪眼、Host 白名单空 = 只剩 loopback、配额默认值
# 未必是部署者本意）。缺省关 → 本机形态零校验零行为变化。
...
def main() -> int:
    ...
    # （T1 灰度：容器内 0.0.0.0，宿主 Host 守卫白名单另见 webapp 的 BIODATA_TRUSTED_HOSTS）。
    host = os.getenv("BIODATA_WEB_HOST", "").strip() or "127.0.0.1"
    # ev1 批：PORT 是无前缀通用名（云平台/开发工具常设），ambient 残留非数字值曾让启动
    ...
    # 公网护栏 fail-closed（2026-08-26）：护栏模式缺必需配置直接 return 2，不起服务；
