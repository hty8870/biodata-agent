...
- **两段式抓取换取干净的 tissue/disease**：搜索接口只给自由文本 `content`；本脚本对每条命中再拉
  ...
  `Organism part`/`Tissue`/`Disease` 等结构化字段，取值质量明显优于纯文本正则。
...
# 口径（2026-07-19，xdc1 交接方向 1）：与 CELLxGENE / EBI SCEA 一致——「健康」是**已知事实**，
# 不是「没标注」。旧口径把健康态抹成空串，代价是诚实层的三个症状（实测见交接）：
...
# 与 CELLxGENE 1847 条 / SCEA 174 条同口径）。详见 `协同/交接/cross-source-disease-convention-xdc1.md`。
_HEALTHY = {"normal", "healthy", "control"}
# NA 占位过滤（`_NONVALUE` / `_norm_token` / `_is_informative`）已上移到 `ingest_cellxgene`，
# 与 `_clean_join` 同处共享助手模块 → 四个适配器共用单一真源（2026-07-16：SCEA 补字段时漏用它、写脏了快照）。
...
# 恰好一个家族出现才给标签，2+ 个出现（真并列 / 对照提及）一律留空（见 classify_platform 头，对抗评审
...
_PLATFORM_PATTERNS: "list[tuple[str, list[str]]]" = [
    ...
    # 仪器，只凭 "fluidigm" 断言具体 C1 化学是 overclaim；留家族名更诚实（对抗评审 nit）。
    ("Fluidigm", [r"fluidigm"]),
    # 单一模式同时覆盖 "STRT" 与 "STRT-seq"，避免两条正则对同一 token 双计（对抗评审 minor）。
...
# 字符类**不含 `;`**：否则 "10.x;10.y" 两个拼接 DOI 会被当成一个合并 token（对抗评审 nit）。
...
def classify_platform(*texts: str) -> str:
    ...
    **按家族是否出现（二值）判定，不按别名命中频次**（对抗评审 CONFIRMED major）：一个平台若用多个
...
def to_record(hit: dict, detail: dict | None) -> dict | None:
    ...
    # xdc1 方向 1：健康态写入 "normal"（源库显式声明健康）；多个健康标签经 _clean_join 去重成一个。
    ...
    # 平台家族（N14 修复）：把详情 protocol Description / Software / Study type 一并喂进关键词分类器
...
def enrich_existing_snapshot(limit: int = 0, sleep: float = 0.2) -> int:
    """**Surgical enrichment（N14）**：只补 `platform` + `collection_doi` 两字段进现有快照，
    其余字段（tissue/disease/species/description…）**一字不动**——避免无关漂移，也不碰 xdc1 的
    disease 口径决策。
    ...
    for i, rec in enumerate(todo, 1):
        ...
        # 瞬时抓取失败/正则漏配抹掉已填 DOI（对抗评审 minor：re-runnable enrich 的 DOI 也要有单调保护）。
...
def main() -> int:
    ...
    ap.add_argument("--enrich-existing", action="store_true",
                     help="surgical 模式：只补 platform + collection_doi 进现有快照，不重抓其余字段（N14）")
    args = ap.parse_args()
