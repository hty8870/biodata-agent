# -*- coding: utf-8 -*-
"""NLU 孪生差分门（2026-08-10 codex 架构评审裁决落地）。

`eval/curate_nlu/rule_parser.py` 是 NLU 选型实验的规则探针，其极性门刻意复刻生产
...
_MASK_BATTERY = [
    ...
    # 顺承「没」（2026-08-15 审计 C-4：掩掉——「没」修饰前一动词，不否定「就/才」后动作）
...
def test_polarity_outcomes_agree_on_typical_cases():
    """单锚点用例：生产 polarity_blocked 与探针 _all_blocked 同出同入。"""
    cases = [
        ("别打包了", "打包", True),
        ("不要下载那5条", "下载", True),
        ("能不能打包一下", "打包", False),
