...
function setup(scope, n) {
    ...
    // consent v2（ov1-fix1a）：值是同意时刻 ISO 串（"0"=拒绝；旧写法 "1" 仍兼容读，但测试钉新写法）
...
globalThis.fetch = (url, opts) => {
    const u = String(url);
    // MCP 中继（ov1-tel1）默认按「接收端未配套」立即 404——否则中继 GET 会进 pending 数组，
...
// 明文公网 HTTP 显式白名单（tc1 合并裁决）：未登记主机 fail-closed，登记后才放行。
...
// ov1-tel1：单条 benchfb 即触发上传（阈值 3→1）+ MCP 中继 attach / ack / offset 推进。
...
for (let i = 0; i < 100 && !relayCalls.some((c) => c.kind === "ingest"); i++) await new Promise((r) => setTimeout(r, 0));
{
    ...
    // ov1-fix1a：中继改相对同源——不带绝对 origin、不带 X-Ingest-Token，分页参数随请求带上
...
// ov1-tel1：超大 benchfb 记录先截 top-20（truncated:true）照常上传，不再整条 manual-only。
// ov1-fix1a：截断点之外的用户标注名次（useful_idx>20）浅拷贝加 _preserved_idx 追加尾部。
...
// ov1-fix1a：usage/benchfb 都空但有 MCP 中继记录时也发包（MCP-only），packet_id 材料含 call_id。
...
// ov1-fix1a：ack 只前进——本地 offset 已 50，对端页 next_offset 42 不得把本地回退。
...
// ov1-fix1a：keepalive 尽力档 30s 最小间隔——上一发（无论成败）后 30s 内不再发。
...
// ov1-fix1a：常规上传 3 分钟最小间隔——距上次成功 < 3min 即使有新 benchfb 记录也不再发
...
// ov1-fix2：since_ts 过滤墙——中继空页但 next_offset 已推进（本页行全被滤掉、offset 照常前进）
...
// ov1-adapt1：200 server_hint 采用 + 持久化（跨刷新生效——模块每次上传都重新读 meta）。
...
// ov1-adapt1：hint 钳制上界——batch_threshold 999→50、min_interval_ms 1000→15s。
...
// ov1-adapt1：hint 钳制下界——batch_threshold 0→2、min_interval_ms 10_000_000→10min。
...
// ov1-adapt1：429 → 临时高档（20 条 / 5 分钟）持久化；下一次 200 hint 覆盖。
...
// ov1-adapt1：默认阈值 2 + 老接收端（200 无 server_hint）不动动态值（fail-safe）。
...
// ov1-adapt1：MCP-only 包不受动态阈值拦（即使 meta 已是 429 高档 20 条）。
