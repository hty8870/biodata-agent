"use strict";
/* 行动流（act_run.js）真行为规格。钉两条 R2-2 对抗评审修复的不变量：
   P1-1 两通道一致：plan.trace 回放/流式上屏的每一步必须带 state+detail——
        失败节点（ok:false）渲染成 ✗ 且原因可进总结泡 details，与快照通道逐位同源；
   P1-2 僵尸流：arxFinish 后的 420ms 折叠窗口内 arxActive() 必须为假——新派发 arxBegin
...
], "P1-1 快照必须带 state+detail（失败节点 ✗ + 原因）");
const html = arxTraceHtml(snap);
ok(html.indexOf("✗") >= 0 && html.indexOf("第一次规划不合法") >= 0, "P1-1 总结 details 里失败节点亮 ✗ 且带原因");
ok(html.indexOf("arx-trace-failed") >= 0, "P1-1 失败行带 failed 样式类");
...
ok(arxActive(), "P1-2 新流越过旧折叠死线后必须还活着（旧计时器不得误杀）");
const snap3 = arxFinish();
eq(snap3.map((s) => s.text), ["步骤乙"], "P1-2 第二句的步骤不夹带第一句（不张冠李戴）");
