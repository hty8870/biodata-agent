# -*- coding: utf-8 -*-
"""F1 课题 UI（projects_core.js / projects.js，engagement）的结构契约门。

设计：engagement 落地包 §2/§3（F1 课题）/§3.3（上下文卡）/
§3.4（coachmark）。
...
3. **P4/P5 挂点**：课题详情检查条件区 / 导出记录区的 `` / `` 注释标记
   必须存在于 projects.js 与 index.html（后续批直接按标记挂载）。
4. **入口/骨架在场**：导航「档案」（#archiveNav， 起课题并入档案浮窗 projects tab）、
...
def test_p4_p5_mount_markers_present() -> None:
    """P4（更新检查）/P5（导出中心）按 / 注释标记挂载。
    P4 挂点只在 projects.js（详情检查条件区）；P5 挂点也只在 projects.js（导出记录区随详情动态渲染）。
     corpus-sync 批：index.html 的 P4 静态骨架（浮窗头/首页条）随
