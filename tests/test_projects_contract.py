# -*- coding: utf-8 -*-
"""课题 UI（projects_core.js / projects.js）的结构契约门。
...
1. **纯逻辑核心零 DOM / 零网络 / 零 localStorage**：课题内容是用户研究内容（隐私红线）；projects_core.js 出现任何出网/DOM/存储原语即红（与 artifacts 同红线）。
2. **上下文卡硬 cap 常量在场**：≤2000 Unicode 字符 / 目标 ≤300 / 候选 ≤20——
   超限「另有 N 项未注入」不静默截断。
3. **追踪/导出挂点**：课题详情检查条件区 / 导出记录区的 `` / `` 注释标记
   必须存在于 projects.js 与 index.html（后续批直接按标记挂载）。
4. **入口/骨架在场**：导航「档案」（#archiveNav，课题并入档案浮窗 projects tab）、
...
def test_context_card_caps_are_present() -> None:
    """硬 cap 常量：2000 Unicode / 目标 300 / 候选 20（纳入排除与数据层同源 ≤8）。"""
    text = CORE.read_text(encoding="utf-8")
...
def test_watch_export_mount_markers_present() -> None:
    """更新检查/导出中心按注释标记挂载。
    检查条件区挂点只在 projects.js；导出记录区挂点也只在 projects.js（导出记录区随详情动态渲染）。
    index.html 的静态骨架（浮窗头/首页条）随
    全体「检查 N 个追踪的更新」按钮一并退役（全体检查太耗费资源；
    批量刷新改由 corpus.gen 哨兵驱动的登录后自动编排承担），index.html 不应再有检查条件挂点。"""
    ui = UI.read_text(encoding="utf-8")
    index = INDEX.read_text(encoding="utf-8")
    assert "PROJECT_UPDATES_MOUNT" in ui, "projects.js 缺更新挂点标记"
    assert "PROJECT_UPDATES_MOUNT" not in index, "index.html 不应再有更新静态挂点"
    assert "PROJECT_EXPORTS_MOUNT" in ui, "projects.js 缺导出挂点标记"


def test_check_section_does_not_render_when_empty() -> None:
    """检查条件展示区「内容为空时不渲染」（不伪造条件）；导出记录区同。"""
    ui = UI.read_text(encoding="utf-8")
...
def test_privacy_copy_covers_both_cases() -> None:
    """上下文卡隐私口径：远端模型明示会发往配置的 AI 服务商；本地模型「不出本机」。"""
    ui = UI.read_text(encoding="utf-8")
