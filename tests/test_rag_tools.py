# -*- coding: utf-8 -*-
"""RAG 工具组（rank / rerank，2026-08-17 scoped 路由批 M2）的常驻钉。

2026-08-17 nl1 转正：rank / rerank 常驻动词表与 LOOP_TOOLS 注册表；原环境开关与
OFF 逐位一致负向钉随代码一并摘除（开关全名与快照归档于
...
def test_verb_specs_registered():
    ...
    # 尾部追加，旧动词一个不动；2026-08-18 四工具批追加在 route.request 之后。
    ...
    # rb1（2026-08-17）刻意更新：curate.rollback 同为环内专属（回滚目标依赖本轮 steps
    # 实录的快照锚，前端单步 runner 没有这个现场）——补录豁免清单。
    # 2026-08-18 四工具批刻意更新：compare.datasets / compat.find / fair.check 同为环内
...
def test_loop_tools_registered():
    ...
    # 2026-08-18 四工具批刻意更新：新四工具追加在 route.request 之后（顺序表尾部）。
...
def test_network_classification():
    ...
    rb1（2026-08-17）刻意更新：curate.rollback 是本地文件操作（不触网），同归纯本地。
    2026-08-18 四工具批刻意更新：compare/cite/compat/fair 全本地（结果处理不触网）。"""
...
def test_rewrite_query_records_usage():
    """2026-08-17 codex 评审 中4：rerank 的独立改写是真实 LLM 调用，给了 usage_sink
...
def _fake_meta(total=7, rows=None, filters=None):
    ...
    # 「dict() 强转列表必炸」的真 bug（2026-08-17 run2 复盘）。
    return SimpleNamespace(
        result_total=total,
...
def test_loop_rank_display_produces_batch(fake_pipeline):
    ...
    # query_raw = 本轮用户原话（M3 契约；2026-08-17 codex 评审 中6）——ctx 带 utterance
...
def test_loop_rerank_rewritten(fake_pipeline, monkeypatch):
    ...
    # query_raw = 本轮用户原话（中6）；原始坏 query 在结果顶层 original_query 键里。
...
def test_loop_rank_carries_structured_conditions(fake_pipeline):
    """ku1（设计 §10.1）：rank 把 facet/suppressed/lenient/date 原样带进管线——
    缺任何一项都是弱批顶掉好结果的回归。"""
    ax, calls = fake_pipeline
    ctx = {
        "search_sources": ["GEO"],
...
def test_loop_rank_scope_kept_backfills_applied(fake_pipeline, monkeypatch):
    """ku1（设计 §10.1）：scope 完整保留时，payload 回填 applied_*（与 search.rerun 同义务）
...
def test_loop_rank_fail_closed_when_date_lost(fake_pipeline):
    """ku1（设计 §10.1）：显式日期在 interpretation 投影里带不出 → fail-closed——不回填 batch、
...
def test_loop_rerank_carries_structured_conditions(fake_pipeline, monkeypatch):
    """ku1（设计 §10.1）：rerank 的改写只换检索句，结构化条件同样原样带进管线。"""
    ax, calls = fake_pipeline
...
def test_loop_rerank_fail_closed_when_date_lost(fake_pipeline, monkeypatch):
    """ku1（设计 §10.1）：rerank 同样 fail-closed——显式日期带不出则不出批。"""
    ax, calls = fake_pipeline
...
def test_batch_extras_count_against_rag_budget():
    """2026-08-17 codex 对抗评审 高1（decide 侧）：同批只读消费的第 2..N 个调用必须
...
def test_execute_batch_fuse_rechecks_rag_budget(monkeypatch):
    """2026-08-17 codex 对抗评审 高1（execute 侧，gr2 批内熔连同哲学）：主步/前序
