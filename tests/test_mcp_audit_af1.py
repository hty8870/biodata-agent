# -*- coding: utf-8 -*-
"""审计修复批 af1（CLM-20260821-0530-zcode-audit-af1）的 MCP 层回归门。

钉住的病形（详见 repo 外《BioData-Agent-代码审计报告-2026-08-21》）：
- S-1  upload_dataset 的 path 读取面无限制：`.userdata/sessions.json`（明文会话 token）这类
        合法 JSON 可被摄入外部库再经 browse_datasets 读回（跨信任边界外泄链）→ 敏感位置
        fail-closed（.env* / 仓库内 .userdata/.git/.venv/database / 任意同名敏感目录）；
- C-2  build_task_pack 的 facet/suppressed/lenient 原样透传（recommend 先过共用 sanitize）：
        sanitize casefold 而 retriever 不归一 → 同值在 recommend 生效、在 task-pack 静默
        0 命中；12 项上限也未应用 → 收敛为同一套 sanitize；
- S-2  verify_local_assets 的 max_files 仅挡 <=0：传 10**9 可对任意目录全盘 md5（I/O DoS）
        → 硬上限 = lab_ledger.MAX_SCAN_FILES；
- S-15 错误消息原样回显超长入参（响应放大器）→ _clip 统一截断 120 字符。
...
# ---------------------------------------------------------------- C-2：build_task_pack 过 sanitize
@pytest.fixture
...
def test_build_task_pack_facets_casefolded_like_recommend(captured_workflow):
    """C-2 核心病形：{"value":"Homo sapiens"} 原样透传时 retriever 按 casefold 键匹配必 0 命中；
    修复后与 recommend 同过 sanitize → 值被 casefold，params 回显与实际生效一致。"""
    r = M.build_task_pack(
