# -*- coding: utf-8 -*-
"""agent_schemas（schema 加固：pydantic 契约层）的确定性门。**全离线**：
...
def _legacy_tool_parameters(spec):
    ...
    slot_descriptions = {
        # 性能批：source 槽补「多点名来源只填第一个」——集成验证 b05/d08 坐实
        # 模型把「10x Genomics, ArrayExpress」挤进一个槽、被点名源护栏拦到 AgentPlanInvalid。
        # B1a：keywords 补出处锚（j03/l07）、target 补文件名实例。
        # （source 的多来源具体例在集成验证里把 k08 首步动词引偏，撤——记录在案。）
        # 验证裁决 N1：source 描述按 verb 分派（三个 curate 动词各有口径），
        ...
        # 四工具批：compare 的 a/b 与 compat/fair 的 uid（与生产侧
        # `_SLOT_DESCRIPTIONS_ZH` 逐字一致——diff 基准随新槽位登记同步）。
        "a": (
    ...
    # 验证决定 N1：三个 curate 动词的 source 描述分派（与生产侧 `_SOURCE_SLOT_DESCRIPTIONS_ZH`
...
def test_generated_schema_constraint_values():
    """约束值抽查：confidence 枚举逐位；source 枚举**按 verb 分派**（验证决定 N1：
...
def test_decide_rules_are_byte_identical_to_legacy_handwritten():
    ...
    钉字刻意更新记录： 验证 v3 问题 b12/g04（马拉松指令做两件就 finish）——
    INTRO 补「判断前先逐件核销」流程句、rule 10 补马拉松实例（「检查A…再检查B…完了告诉我C」
    是三件事），原文钉字随本次问题修复同步更新。
     验证 v4：rule 6 补「检查出疑似新增不等于已经入库」（问题 l07——检出疑似新增
    被当成已入库而跳过搜索入库步）；rule 7 补反向条件句（问题 k06——「没有 A 就做 B」里
    ...
    通道往返 -12%、完成率持平），见 multicall-fidelity-probe/dsv/arm-summary.md。
     检索工具化 Phase 1：注册表新增 search.rerun（换词重检工具化，decide 清单
    ...
    候选排除 rollback 自身、快照不可读 fail-closed，原文随真实边界同步。
     四工具批：注册表新增 compare.datasets / cite.export / compat.find /
...
def test_decide_tools_rules_are_byte_identical_to_legacy_handwritten():
    ...
    钉字刻意更新记录： 验证 b12/g04 提前收工问题第二刀——finish 说明句从
    「→ 调用 finish」改为「必须附 completion_report + 有一件没交代系统会拒收收尾并重问一次」
    （核销从可选 checklist 的纯外化 articulation 升级为机械闸输入）。
     验证 v4：rule 6/7 随 JSON 壳同批刻意更新（问题 l07 疑似新增≠已入库、
    问题 k06 反向条件——两壳共享同一份铁律真源 `_DECIDE_RULES_TAIL_ZH`）。
     策略 B 方案：finish 说明句补 reasked_write 对账义务（重问写动词
    从只读闸改放行 + 强制核销——tests/test_agent_decide_channel.py 同批专项测试）。
     B1a 调研六候选批：四个 LOOP_TOOLS 的 decide_zh 工具描述手术
    ...
    check_updates 表格行同步（依据同上，见 dsv/arm-summary.md）。
     检索工具化 Phase 1：注册表新增 search.rerun（decide 清单末尾追加一行，
    JSON 壳同批同步）。
     新逻辑转正：rank/rerank/route.request 常驻 decide 面，清单末尾追加三行
...
def test_decide_table_is_generated_from_the_registry():
    ...
    带 source 槽的行尾拼候选清单（验证决定 N1，与 schema 枚举同一真源）。"""
    parts = []
...
def test_decide_rules_carry_reverse_condition_and_suspected_new_sentences():
    ...
    - 问题 k06（反向条件丢失）：rule 7 必须带「没有 A 就做 B」是反向条件——A 没有时 B 必须做；
    - 问题 l07（检出疑似新增被当成已入库）：rule 6 必须带「检查出疑似新增不等于已经入库」。"""
...
def test_sync_updates_when_zh_carries_the_topic_word_litmus():
    """问题 j03（劣质指令 sync 磁吸回潮）：sync_updates 的 when_zh 必须带机械判定口径——
    原话出现任何主题词（疾病/物种/组织/技术）就是限定主题，一律 check + search 两步。
     w2a3 复跑后刻意更新：口径**前置进首句**（缀句尾时 JSON 兜底档的模型只读
    首句样例就磁吸，验证 j03 三轮两轮仍选 sync）。"""
...
def test_check_updates_when_zh_distinguishes_topic_question_from_source_question():
    """问题 j05（主题问句误选 check）：check_updates 的 when_zh 必须带对照句——
...
def test_network_loop_tools_are_the_registry_minus_db_status():
    ...
    同属「纯本地」排除项。
     四工具批刻意更新：compare.datasets / cite.export / compat.find /
...
def test_error_code_literals_match_actual_raise_points():
    ...
    assert set(get_args(corpus_curation.CurateCode)) == {
        ...
        # engagement B2：sync 整任务锁冲突（真实 raise 点在 sync_updates_critical_section）
        "sync_busy",
        # engagement B2：按 operation_id 撤回时查无此操作（真实 raise 点在 recall_sync_operation）
        "unknown_operation",
    }
