# -*- coding: utf-8 -*-
"""agent_schemas（2026-08-06 schema 加固：pydantic 契约层）的确定性门。**全离线**：
...
def test_search_online_result_zero_write_filename_none():
    """2026-08-15 fix-exec H1：候选全部已在库中的**零写入**是合法诚实回报
...
def _stub_checklist_call(monkeypatch):
    """任务4：清单轻量调用统一 stub（本文件钉钉图内行为/schema 契约，不含清单语义；
...
def _legacy_tool_parameters(spec):
    ...
    slot_descriptions = {
        ...
        # 2026-08-08 codex 裁决 N1：source 描述按 verb 分派（三个 curate 动词各有口径），
        ...
        # 2026-08-17 nl1 转正缝合：display（M2 既有）/ target_route 与 reason 通用兜底
        ...
        # 2026-08-20 pbt1 批：cite.export 的 uids 数组槽（与生产侧 `_SLOT_DESCRIPTIONS_ZH`
    ...
    # 2026-08-17 nl1：query/reason 的按动词专职描述（与生产侧 `_QUERY_SLOT_DESCRIPTIONS_ZH` /
    ...
    # codex 裁决 N1：三个 curate 动词的 source 描述分派（与生产侧 `_SOURCE_SLOT_DESCRIPTIONS_ZH`
...
#: uids→items（数组元素类型，2026-08-20 pbt1 批）。
...
def test_generated_schema_constraint_values():
    """约束值抽查：confidence 枚举逐位；source 枚举**按 verb 分派**（codex 裁决 N1：
...
def test_decide_rules_are_byte_identical_to_legacy_handwritten():
    ...
    钉字刻意更新记录：2026-08-08 探针 v3 病灶 b12/g04（马拉松指令做两件就 finish）——
    INTRO 补「判断前先逐件核销」流程句、rule 10 补马拉松实例（「检查A…再检查B…完了告诉我C」
    是三件事），原文钉字随本次病灶修复同步更新。
    2026-08-08 探针 v4：rule 6 补「检查出疑似新增不等于已经入库」（病灶 l07——检出疑似新增
    被当成已入库而跳过搜索入库步）；rule 7 补反向条件句（病灶 k06——「没有 A 就做 B」里
    A 没有时 B 反而必须做），原文钉字随本次病灶修复同步更新。
    2026-08-14 refine.bio 第 11 源接入：source 候选清单程序取自注册表，三行候选句尾
    追加「/ refine.bio」（check/sync 取 CHECK_UPDATE_SOURCES labels、search_online 取
    SOURCE_ADAPTERS labels），原文钉字随新源注册同步更新。
    2026-08-15 dsv 变体提示词臂实测采纳：check_updates 描述从「一次只查一个来源——本步查
    ...
    2026-08-16 检索工具化 Phase 1：注册表新增 search.rerun（换词重检工具化，decide 清单
    末尾追加一行，无 source 槽不拼候选），原文钉字随新工具登记同步更新。
    2026-08-17 nl1 新逻辑转正：rank/rerank/route.request 常驻 decide 面，清单末尾追加三行，
    原文钉字随转正同步更新。
    2026-08-17 rb1 回滚动词化：注册表新增 curate.rollback（动作套件写工具，清单在
    db_status 行后追加一行，无槽位不拼候选）。2026-08-18 钉字：回滚改独立预算、
    候选排除 rollback 自身、快照不可读 fail-closed，原文随真实边界同步。
    2026-08-18 四工具批：注册表新增 compare.datasets / cite.export / compat.find /
    fair.check（环内结果处理四工具，清单在 route.request 行后追加四行——compare/
    compat/fair 无 source 槽不拼候选，cite.export 同），原文钉字随新工具登记同步更新。
    2026-08-20 pbt1 批（依赖占位批量计划 v2）：JSON 兜底壳补「一次只能一个调用、
    顺序诉求分步发」提示句（本通道不支持同批依赖占位）；cite.export/rank/rerank 三行
    工具描述随 uids 槽与 top digest 扩字段同步，原文钉字随本次刻意更新同步。
    2026-08-23 改空也采纳（「换成猪的」投诉）：search.rerun 行工具描述从「改空了或
...
def test_decide_tools_rules_are_byte_identical_to_legacy_handwritten():
    """tools 主通道壳的**字节钉**（2026-08-08 补上，与 JSON 壳合成真正的双壳钉）。
    钉字刻意更新记录：2026-08-08 探针 b12/g04 提前收工病灶第二刀——finish 说明句从
    「→ 调用 finish」改为「必须附 completion_report + 有一件没交代系统会拒收收尾并重问一次」
    （核销从可选 checklist 的纯外化 articulation 升级为机械闸输入）。
    2026-08-08 探针 v4：rule 6/7 随 JSON 壳同批刻意更新（病灶 l07 疑似新增≠已入库、
    病灶 k06 反向条件——两壳共享同一份铁律真源 `_DECIDE_RULES_TAIL_ZH`）。
    2026-08-07 产品拍板 B 方案：finish 说明句补 reasked_write 对账义务（重问写动词
    从只读闸改放行 + 强制核销——tests/test_agent_decide_channel.py 同批专项测试）。
    2026-08-08 B1a 调研六候选批：四个 LOOP_TOOLS 的 decide_zh 工具描述手术
    （k08 sync 磁吸补具体反例 / k09 多来源只检一个 / j03·l07 条件成立后搜索步被放弃 /
    k03 库容是独立事项）——两壳表格行随真源同步（JSON 壳同批）。
    2026-08-14 refine.bio 第 11 源接入：source 候选清单程序取自注册表，三行候选句尾
    追加「/ refine.bio」（JSON 壳同批同步）。
    2026-08-15 dsv 变体提示词臂实测采纳：bullet 从「调用**恰好一个**对应的工具」改为
    「彼此独立且都是只读的事一次各发一个调用；有先后依赖或写库动作仍一次一个」，
    check_updates 表格行同步（依据同上，见 dsv/arm-summary.md）。
    2026-08-16 检索工具化 Phase 1：注册表新增 search.rerun（decide 清单末尾追加一行，
    JSON 壳同批同步）。
    2026-08-17 nl1 新逻辑转正：rank/rerank/route.request 常驻 decide 面，清单末尾追加三行
    （JSON 壳同批同步），原文钉字随转正同步更新。
    2026-08-17 rb1 回滚动词化：注册表新增 curate.rollback（db_status 行后追加一行，
    JSON 壳同批同步）。2026-08-18 钉字：同步独立预算/不回滚回滚/fail-closed 边界。
    2026-08-18 四工具批：注册表新增 compare.datasets / cite.export / compat.find /
    fair.check（route.request 行后追加四行，JSON 壳同批同步），原文钉字随新工具
    登记同步更新。
    2026-08-20 pbt1 批（依赖占位批量计划 v2）：tools 壳新增「同批依赖前一步检索结果」
...
def test_decide_table_is_generated_from_the_registry():
    ...
    带 source 槽的行尾拼候选清单（codex 裁决 N1，与 schema 枚举同一真源）。"""
    parts = []
...
# ------------------------------------------------- 2026-08-08 探针 v4 病灶钉（k06/l07/j03/j05/k08）
...
def test_decide_rules_carry_reverse_condition_and_suspected_new_sentences():
    ...
    - 病灶 k06（反向条件丢失）：rule 7 必须带「没有 A 就做 B」是反向条件——A 没有时 B 必须做；
    - 病灶 l07（检出疑似新增被当成已入库）：rule 6 必须带「检查出疑似新增不等于已经入库」。"""
...
def test_sync_updates_when_zh_carries_the_topic_word_litmus():
    """病灶 j03（劣质指令 sync 磁吸回潮）：sync_updates 的 when_zh 必须带机械判定口径——
    原话出现任何主题词（疾病/物种/组织/技术）就是限定主题，一律 check + search 两步。
    2026-08-08 w2a3 复跑后刻意更新：口径**前置进首句**（缀句尾时 JSON 兜底档的模型只读
...
def test_check_updates_when_zh_distinguishes_topic_question_from_source_question():
    """病灶 j05（主题问句误选 check）：check_updates 的 when_zh 必须带对照句——
...
def test_network_loop_tools_are_the_registry_minus_db_status():
    ...
    2026-08-17 nl1 转正刻意更新：rank/rerank 跑本地检索管线（不触网）、route.request 是
    ...
    # P1-5（2026-08-09 codex）：注入段如实写明离线 db_status 与离线快照源检查都不连坐
...
def test_error_code_literals_match_actual_raise_points():
    ...
    assert set(get_args(corpus_curation.CurateCode)) == {
        ...
        # 2026-08-22 engagement B2：sync 整任务锁冲突（真实 raise 点在 sync_updates_critical_section）
        "sync_busy",
        # 2026-08-22 engagement B2：按 operation_id 撤回时查无此操作（真实 raise 点在 recall_sync_operation）
        "unknown_operation",
    }
