# -*- coding: utf-8 -*-
"""cr1「并发分流与确定性 RAG 策略」· RAG flight 池级门（设计 §4.1/§4.6/§6）。**全离线**：

- flight 池/deferred 准入：准入信号量满 → 第四请求标 deferred **不起线程**（禁止内联，
  r3 P1-3）；join 点由调用线程同步补跑（=今天串行时序，正确性保底）。
...
- 预热闭合（r3 P1-5）：主线程先 ensure vector/env 初始化，flight 线程全程零
...
def _fresh_pool(monkeypatch):
    """每测试一个干净的准入信号量（不串槽）；rule_match_summary 默认替换为确定性探针
...
def test_flight_deferred_when_pool_full():
    ...
    assert flight._future is None, "池满时不起线程（禁止内联跑，r3 P1-3）"
    assert not flight.done()
...
def test_flight_bounded_pool_barrier_fourth_deferred(monkeypatch):
    """有界性（barrier 驱动，设计 §6）：三槽全被阻塞探针占满后，第四请求 deferred——
...
def test_warmup_then_flight_never_touches_env(monkeypatch):
    """预热闭合（r3 P1-5）：主线程先 ensure vector/env 初始化；flight 线程全程
...
def test_flight_has_hits_matches_gate_semantics():
    """has_hits = status==results ∧ total>0（与既有机械闸同口径，design §4.2）。"""
    flight = _flight()
...
def test_flight_ensure_payload_built_once(monkeypatch):
    """M3 原则：recommend_payload 建一次存 flight（发射与批次组卷不重跑）。"""
    calls: list = []
