...
设计经 3 视角对抗评审定稿；base 单细胞⟺chromium → 冻结门逐位不变，多源修召回（已授权 2026-07-12）。"""
import sys
...
def test_scrna_seq_recognized_not_abstained():
    """codex 反馈：scRNA-seq 此前 unresolved_term 直接弃权 → 现识别为 modality:single-cell。"""
...
def test_base_single_cell_equals_chromium_count():
    ...
    # 2026-08-17 promo1 受控重基线 774→784：晋升 10 条里 1 条 flexv2（chromium→single-cell），582→583
