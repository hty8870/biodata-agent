"""B3 解析失败的错误回灌（2026-08-08 调研六候选批）专项钉。
...
def test_rerank_glued_number_treated_as_invalid_not_candidate():
    """codex #5 点名的坑：「我选3x」不许被当成候选 3——判 invalid 触发回灌而不是静默错排。"""
...
# ---------- audit_query_only ----------
# 2026-08-16 检索工具化 Phase 1：空池独立审核档（audit_query_only / _grade_query_audit_text）
...
# ---------- D-01（2026-08-15 触发点审计）：_LAST_PARSE_RETRY 并发隔离 + 无跨请求残留 ----------
...
# ---------- D-02（2026-08-15 触发点审计）：宽 except 回退必须留下异常本体 ----------
