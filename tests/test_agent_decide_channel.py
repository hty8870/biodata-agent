# -*- coding: utf-8 -*-
"""decide 的 tool-calling 主通道 + 图单例并发（2026-08-07 langgraph 换装）的确定性门。**全离线**：

- decide 通道矩阵：finish / 空回答 / 散文拒答 / content JSON 双通道 / 幻觉工具名 /
  多 tool_call 取第一个（2026-08-09 病灶猎杀批改策，DeepSeek 不遵守 parallel_tool_calls=False）/
  unsupported_next_step 婉拒 / none 干净 done / tools 通道异常跌
  JSON 兜底（审计 node="decide"）/ parallel_tool_calls=False 契约；
- **非法应答重问一次**（2026-08-08 探针 v4/w2a3 病灶 b08/l07/k10）：主通道拿到非法应答
  重问一次；重问后的写动词 **放行 + 强制核销复核**（2026-08-07 产品拍板 B 方案，取代
  2026-08-08 只读闸）——放行的写步落 `reask_writes` 台账，finish 报告必须引用其步骤号
  单独交代结果，否则核销硬闸拒收；重问仍非法 / JSON 兜底档非法 → 照旧停环不多问；
- **violation 重问对称化**（2026-08-08 借鉴批，l07 r3 实锤）：续步提议没过 `_validate_raw`
...
- 图单例：编译计数器跨调用不增长；ThreadPoolExecutor 多请求交错跑同一 compiled graph
  不串味（codex P1-6 的验收钉）；
- **未决事项机械提示段**（2026-08-08 探针 v6：finish 核销被「带步骤引用的假豁免/假已做」
...
def test_finish_tool_call_is_a_clean_done():
    ...
    # 2026-08-08 任务4：换纯净 utterance（无入库诉求——pending 硬闸不拦 finish），
...
def test_empty_answer_is_reasked_once_then_done():
    """空回答 = 非法 → **重问一次**（2026-08-08 探针 v4/w2a3 病灶 b08/l07/k10 起刻意更新：
    旧钉「拿到但非法绝不多问第二次」（codex P0-2）细化为「重问一次」；重问后的写动词
    2026-08-07 产品拍板 B 方案——放行 + 强制核销，见下方专项测试）；
    重问仍非法 → 照旧停环，不再问第二次。"""
    model = _FakeModel(
        _understand_check(),
...
def test_multiple_tool_calls_accept_first_in_order():
    """decide 一次回两个 tool_call → **取第一个按序执行**（2026-08-09 病灶猎杀批，取代
    「多调用机械拒绝」：DeepSeek 实测不遵守 parallel_tool_calls=False，微探针实证 decide
...
# ---------------------------------------------------------------- 非法应答重问一次（2026-08-08 探针 v4/w2a3 病灶 b08/l07/k10）

def test_invalid_answer_reask_allows_readonly_continuation():
    """病灶主治①：decide 散文拒答 → 重问一次 → 重问后提议只读 db_status → 照常放行执行，
...
def test_invalid_answer_reask_write_verb_allowed_with_forced_accounting():
    """2026-08-07 产品拍板 **B 方案**（取代 2026-08-08 只读闸；旧钉「重问后的写动作绝不
...
# ---------------------------------------------------------------- violation 重问对称化（2026-08-08 借鉴批，l07 r3 实锤）
...
# ---------------------------------------------------------------- 未决事项机械提示段（2026-08-08 探针 v6：核销被「合法措辞」绕过族）
...
def test_pending_hints_rule_matrix():
    ...
    ①点名源未触碰（逐字规范名 ENCODE 也算点名——与点名源闸的 w2a2 豁免同口径）；
...
def test_pending_hints_import_denial_polarity():
    """2026-08-15 ta2 exec2（与 fix-exec H1 同族，提示层收尾）：规则 3 的入库诉求词表
...
def test_pending_hints_block_reaches_both_decide_channels():
    ...
    for prompt in (tools_prompt, json_prompt):
        ...
        # 段末重锚输出契约（w2a6 真机坐实：无此句时模型被「逐项核对」带进散文）
...
def test_parallel_tool_calls_disabled_and_tier_choices():
    ...
    # 2026-08-17 nl1 转正：general 套件 decide 面 = 8 loop（rank/rerank/route_request 常驻
    ...
    }, "decide 工具面 = 13 loop + 2 控制（不是全动词表，codex P1-5；2026-08-18 四工具批新增 compare/cite/compat/fair）"


def test_understand_accepts_first_of_multiple_tool_calls(_tmp_project_root):
    """understand 侧同策（2026-08-09 病灶猎杀批，取代「多调用判空跌兜底」）：微探针实锤
    ...
    assert "工具调用模式" in trace[1]["detail"], "多调用取第一个后不得再跌兜底通道"  # [0] 是常驻环首（nl1）
    log_path = _tmp_project_root / ".userdata" / "agent_fallbacks.jsonl"
