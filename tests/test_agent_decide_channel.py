# -*- coding: utf-8 -*-
"""decide 的 tool-calling 主通道 + 图单例并发（langgraph 换装）的确定性门。**全离线**：

- decide 通道矩阵：finish / 空回答 / 散文拒答 / content JSON 双通道 / 幻觉工具名 /
  多 tool_call 取第一个（问题猎杀批改策，DeepSeek 不遵守 parallel_tool_calls=False）/
  unsupported_next_step 婉拒 / none 干净 done / tools 通道异常跌
  JSON 兜底（审计 node="decide"）/ parallel_tool_calls=False 契约；
- **非法应答重问一次**（验证 v4/w2a3 问题 b08/l07/k10）：主通道拿到非法应答
  重问一次；重问后的写动词 **放行 + 强制核销复核**（策略 B 方案，取代
   只读闸）——放行的写步落 `reask_writes` 台账，finish 报告必须引用其步骤号
  单独交代结果，否则核销硬闸拒收；重问仍非法 / JSON 兜底档非法 → 照旧停环不多问；
- **violation 重问对称化**（借鉴批，l07 r3 实锤）：续步提议没过 `_validate_raw`
...
- 图单例：编译计数器跨调用不增长；ThreadPoolExecutor 多请求交错跑同一 compiled graph
  不串味（验证 的验收钉）；
- **未决事项机械提示段**（验证 v6：finish 核销被「带步骤引用的假豁免/假已做」
...
def test_finish_tool_call_is_a_clean_done():
    ...
    # 任务4：换纯净 utterance（无入库诉求——pending 硬闸不拦 finish），
...
def test_empty_answer_is_reasked_once_then_done():
    """空回答 = 非法 → **重问一次**（验证 v4/w2a3 问题 b08/l07/k10 起刻意更新：
    旧钉「拿到但非法绝不多问第二次」（验证）细化为「重问一次」；重问后的写动词
     策略 B 方案——放行 + 强制核销，见下方专项测试）；
    重问仍非法 → 照旧停环，不再问第二次。"""
    model = _FakeModel(
        _understand_check(),
...
def test_multiple_tool_calls_accept_first_in_order():
    """decide 一次回两个 tool_call → **取第一个按序执行**（问题猎杀批，取代
    「多调用机械拒绝」：DeepSeek 验证不遵守 parallel_tool_calls=False，微验证实证 decide
...
def test_none_means_clean_done_now():
    """content JSON 里 {"verb":"none"}： 起 = 干净的 done（旧版误入婉拒路径，
...
# ---------------------------------------------------------------- 非法应答重问一次（验证 v4/w2a3 问题 b08/l07/k10）
...
def test_reask_write_unaccounted_third_finish_is_failsafe_accepted():
    ...
    回灌 2 次，防原地空转；标注进 trace，人读可复盘）。
     调研-长程agent批 候选2：旧「第二击即放行」过软，升级为三击放行。"""
    model = _FakeModel(
        _understand_check(),
...
def test_pending_hints_import_denial_polarity():
    """ exec2（与 H1 同族，提示层收尾）：规则 3 的入库诉求词表
...
def test_pending_hints_block_reaches_both_decide_channels():
    ...
    for prompt in (tools_prompt, json_prompt):
        ...
        # 段末重锚输出契约（w2a6 集成坐实：无此句时模型被「逐项核对」带进散文）
...
def test_understand_accepts_first_of_multiple_tool_calls(_tmp_project_root):
    """understand 侧同策（问题猎杀批，取代「多调用判空跌兜底」）：微验证实锤
