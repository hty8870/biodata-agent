# -*- coding: utf-8 -*-
"""search 档真机探针 harness（scripts/evaluate_search_live.py）的离线单测 + 门控真机入口。
...
真机部分（默认 skip，`BIODATA_SEARCH_LIVE=1` 才跑——仓库没有真 LLM pytest 先例，
真机评测的既有约定是 scripts/ 下手跑；本用例只是给「想留在 pytest 里复跑」开的
显式门控入口）：单跑 s01 一例，断言共识落档 search、preliminary 已发、屏上有结果。
conftest 的全局 stub（route_consensus→general、清单空调用）是给 FakeModel 图内测试
的防错位垫片；真机入口用 import 期存的真引用恢复（与 tests/test_scoped_routing.py
...
# import 期存真引用（conftest autouse stub 先于测试体执行，真机用例在测试体内恢复）
...
def test_score_top1_and_totals():
    """2026-08-18 钉字：结构化字段决定正确性，标题子串只作辅助证据。"""
...
def test_score_anyof_group():
    """2026-08-18 钉字：任一命中组（受控同义类）——carcinoma 满足癌症类期望、healthy 不满足；
