# -*- coding: utf-8 -*-
"""前端加载期跨模块引用门——C4 起判据换代：boot.js 已是 ES Module（全应用唯一入口）。

旧判据（经典脚本时代，C1-C3 沿用）：收集所有模块顶层定义名当作共享命名空间，断言 boot.js
...
def test_boot_import_specifiers_resolve_via_package_imports() -> None:
    ...
    assert pairs, "boot.js 没有解析出任何 import（C4 起它必须是纯 ESM 入口——解析失真？）"
...
def test_init_calls_are_all_imported() -> None:
    ...
    assert not unresolved, (
        ...
        "C4 起 boot 是纯 ESM：忘了 import 的名字在浏览器里是 ReferenceError（ESM 不读 window 桥）。"
    )
