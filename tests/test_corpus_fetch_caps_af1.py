# -*- coding: utf-8 -*-
"""审计 S-6（2026-08-21，CLM-20260821-0530-zcode-audit-af1）：联网取数响应体大小上限的
...
（确定性失败），与 G-10 的「JSON 解析失败不重试」同哲学——退避重试只会再白读 64MB+。
...
钉四条：
  ...
  3. corpus_curation._fetch 超限 → CurateError(network_error) 且只调一次 urlopen
     （超限 raise 位于 except 链之外，不会被 G-10 的 except ValueError 误捕改写）；
...
def test_fetch_oversize_raises_curate_error_without_retry(monkeypatch):
    ...
    assert "不重试" in exc_info.value.hint and "不是合法 JSON" not in exc_info.value.hint, (
        "超限 CurateError 必须从 except 链之外抛出——在 try 内 raise 会被 G-10 的 "
