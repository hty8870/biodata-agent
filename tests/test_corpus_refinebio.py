# -*- coding: utf-8 -*-
"""refine.bio 第 11 源（2026-08-14 接入）的确定性门。**全程禁网**：corpus_net 侧在 fetch_json
...
#: /v1/search/ 列表项形状（2026-08-14 实测字段）：扁平、technology 单数、platform_names。
...
# ---------------------------------------------------------------- G-04（2026-08-15）：词表外物种两入口同口径、均如实提示

def test_net_outofvocab_species_is_noted(tmp_path, monkeypatch):
    """G-04：net 入口词表外物种不打 organism 过滤，但 note 必须如实写明（此前静默透过滤）。"""
    seen: list[str] = []
    monkeypatch.setattr(cn, "fetch_json",
                        lambda url, **kw: (seen.append(url) or (REFINEBIO_SEARCH_PAYLOAD, 200)))
...
def test_net_outofvocab_species_no_results_also_noted(tmp_path, monkeypatch):
    """G-04：零结果时用户更要能区分「没有这个物种的数据」与「词没被认出来」。"""
...
def test_curation_outofvocab_species_not_fabricated(tmp_path, monkeypatch):
    """G-04：curation 入口不再把带空格的词表外词当二名法学名透传
