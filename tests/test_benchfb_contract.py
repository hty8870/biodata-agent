...
1. **零出网原语**：出网只经 `usage_upload.js`（2026-08-20 tl1 起脱敏自动上传）；
...
3. **脱敏是结构性的**：api_key 落盘前必经 benchfbStripRequest 整键删除——静态钉住
   调用链，行为由 node 规格逐条断言。
4. **与使用反馈同一开关**：采集闸 = usageEnabled（单版本化，2026-08-20 tl1：恒默认开），
   不开独立后门。
...
def test_none_and_error_turns_do_not_render_rating_cards() -> None:
    """设计 §4（2026-08-20 tl1 S5）+ T1 任务7：none/error 轮**不再主动渲染完整评分卡**（记录照攒，
    纯埋点）；2026-08-22 ux1 起该判定连同「会话降频闸」收口进 `_collapsedRate`——none/error 轮、
...
def test_none_error_turns_expose_a_rate_toggle_button() -> None:
    ...
    小字「评价」按钮，点击原位展开完整评分卡（ux1 起为完成度三选+原因 chips+评语；「标出有用」
    仅检索轮，复用 _fillCard canMark 判据），再点「收起」回到按钮。2026-08-22 ux1 起被会话
...
def test_turn_records_carry_duration_ms() -> None:
    """设计 §5（2026-08-20 tl1 S5）：benchfb 轮次记录补 `ms`（turn begin→close 的时长）。"""
    code = _strip_js_comments(LOG.read_text(encoding="utf-8"))
    close = re.search(r"function _closeTurn\(kind\)\s*\{(?P<body>.*?)\n\}", code, flags=re.S)
    assert close, "_closeTurn 不存在"
...
def test_rating_card_is_tristate_not_stars_and_records_carry_tid() -> None:
    """2026-08-22 ux1 评分结构改版 + 轮次 id 接线：
