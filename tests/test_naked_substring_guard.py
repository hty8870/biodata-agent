# -*- coding: utf-8 -*-
"""裸子串匹配这一族缺陷的统一回归网（2026-07-22 夜，批量测试 + 对抗评审 + 场景覆盖）。

这个项目栽在**同一个病根**上已经四次了，每次只修了其中一半：

    2026-07-22 白天  alias 的 ASCII 半边   `integrated` 里的 `rat` → 悄悄多一个物种约束     已修（词边界）
    2026-07-22 夜①   alias 的中文半边       「高血压」含「血」→ 组织=Blood                  已修（登记本体）
    2026-07-22 夜②   保护表只收了一个成员   「肾上腺素」→ 组织 Adrenal Gland                本文件
    2026-07-22 夜③   free_text 打分         查「AD」→ 命中标题里的 ad**ult**                 本文件
...
def test_long_free_text_token_still_matches_as_substring():
    ...
    # （2026-08-18 起判据集中在 `_term_hits_text`，`_rank_score` 经 `_any_synonym_hits`
