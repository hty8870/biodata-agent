# -*- coding: utf-8 -*-
"""课题更新检查（project_updates_core.js / project_updates.js）结构契约门。

与 sync_button/projects/artifacts 契约同一套思路：
...
6. **挂点接线**：projects.js 在详情检查条件区调 updateDetailMount、行检查按钮走 runProjectCheck
...
def test_core_layer_has_no_dom_or_localstorage() -> None:
    """零 DOM、零 localStorage——纯逻辑核只做结构推导（设计约定隐私红线）。"""
    code = _strip_js_comments(CORE.read_text(encoding="utf-8"))
...
def test_shell_never_stores_sync_status_in_localstorage() -> None:
    """「本地目录同步于」是实例级事实（设计约定，验证#6）——壳层零 localStorage。"""
...
def test_design_copy_present_verbatim() -> None:
    """设计约定关键文案逐字在场（上屏语是行为承诺，漂一个字的成本是用户误解）。"""
    text = CORE.read_text(encoding="utf-8")
...
def test_no_auto_verify_path_in_shell() -> None:
    ...
    （设计约定硬性：任何自动流程不得直接改纳入表；终态由用户在候选区核验/排除）。"""
    code = _strip_js_comments(UI.read_text(encoding="utf-8"))
...
def test_batch_button_retired() -> None:
    """全体「检查 N 个追踪的更新」按钮已撤（用户：点一下太耗费资源）：壳层零批量面板/批量钩子
...
def test_corpus_sync_orchestration_present() -> None:
    """上游同步编排结构在场：纯核六个新纯函数；壳层经 API 真源轮询 job + 哨兵读写走
...
def test_delta_entry_links_to_dataset_page() -> None:
    """设计约定「待查看更新逐条可点开看数据集」：每条 uid 提供 /dataset?uid=… 链接
...
def test_projects_mount_wiring_present() -> None:
    """projects.js 的 区域接线：详情检查条件区调 updateDetailMount、行检查按钮走 runProjectCheck
    （探测式降级——typeof 守卫，追踪壳未加载不报错）、注册自动刷新完成钩子重渲列表徽标。"""
    projects = PROJECTS.read_text(encoding="utf-8")
    assert 'from "#project_updates"' in projects, "projects.js 未 import #project_updates"
    assert "typeof updateDetailMount === \"function\"" in projects, "详情挂载缺探测式降级守卫"
...
def test_both_pages_load_shell_scripts() -> None:
    """index.html 显式加载壳 + 纯核（防 import 边误删的静默失能，双保险）；
    dataset.html 不再加载（联动钩子随全体批量按钮退役）——
    ...
    assert 'src="/static/js/core/project_updates.js?v=' not in dataset, \
        "dataset.html 不应再加载 project_updates.js（联动钩子已退役；importmap 键保留）"
