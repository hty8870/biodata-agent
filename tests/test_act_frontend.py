...
def _card_in_node(src: str, name: str, expr: str, payload: object, deps: "list[str] | None" = None) -> object:
    """cd1（2026-08-19）：在 node 里跑 act.js 的四工具卡片构造函数（真行为）。
    ...
    cd2 曾因手桩掩盖 act.js 漏 import tpBytes 的 ReferenceError，import 缺失由结构门
...
def test_every_exec_verb_has_a_runner_and_no_runner_is_invented():
    ...
    「有独立执行入口、前端暂未接线」的动词。curate.* 六动词已全部毕业（前四个 2026-08-01 cur1、
    restore 同日随统一对话窗口 uni1 接线、check_updates 2026-08-03 随 agent1 批接线）。
...
def test_landing_goes_through_one_shared_entry():
    """设计 §4：runRecommend 两个落地点与救回换屏**同走 landRecommendResult**——
    ...
    # ku3-w5：救回换屏路径已退役（救回不再发 /api/agent/search-rescue），共享入口只剩缓存 + 真请求
...
def test_auto_rescue_is_retired_from_search():
    """ku3-w5：自动救回链退役——不再有 maybeSearchRescue / handleSearchRescue、不再自动调
...
def test_rescue_option_strip_is_wired_in_board():
    """ku3-w5：救回选择条接线钉——零命中 pill 渲染「点击处理」+ 视觉区分类（ft-pill--zero）+
...
def test_rescue_option_derivation_lives_in_batch_select():
    """ku3-w5：零命中救回纯逻辑集中在 batch_select.js——isZeroHitBatch（payload.results 空数组）、
...
def test_plansteps_wiring_is_kept_for_trace_summary():
    """ku3-w5：planSteps 透传不是救回专用的——它是信息流（ku3）轨迹摘要「执行了 N 次检索」
...
def test_adopt_swap_sys_line_prefers_batch_disclosure():
    ...
    回退既有通用句——回退句不许删（无披露句的采纳批仍要有留痕）。ku1-覆盖策略修复后，披露读取
    与「已更新」词句一并收进共享应用函数 _applyBatchDecision（search a 档与 route=tool 档共用）。"""
    apply_fn = _strip_comments(_fn_body(BOARD, "function _applyBatchDecision(text, reply, decision, opts)"))
    assert "_ba.disclosure_zh" in apply_fn, "换屏留痕没读活跃批随行的 disclosure_zh"
    assert "深入思考后找到了更匹配的结果，已更新。" in apply_fn, "无披露句时的回退通用句被删了"
    # ku1（设计 §10.3）：两档共用 selectDisplayBatch——「严格更高级才自动换屏/去重/备选」统一真源。
    sub = _strip_comments(_fn_body(BOARD, "function ubDispatch(text, reply, fromChat, wasChat)"))
...
def test_search_rerun_step_card_is_summary_only_and_never_a_runner():
    """search_rerun 步骤卡（设计 §4）：只出摘要三要素（改写词 + 采纳/拒绝 + n_before→n_after），
...
def test_four_tool_step_cards_are_wired_and_read_their_result_fields():
    """cd1：actLoopStepCardHtml 必须有四种 card_kind 分发；四个构造函数存在且读各自 result 的
...
def test_act_finish_passes_four_tool_cards_through_html_channel():
    """cd1：actDispatchPlan 图内通道收集四工具卡（按执行顺序拼接）交给 actFinish 的 html 通道
    （.cbh-sys-extra，entry.html 是重画真源 → 历史重画随 html 一起恢复）；非四工具步不上卡
    （ux1 精简不回流）；actSummaryHtml 的 ux1 精简契约保持不动。"""
    ...
    assert "execSummary" not in finish, "ku3-w3：execSummary 通道已退役（摘要归信息流压缩行）"
    assert "cbExecReceiptCovered" in finish and "_actRetrievalOnly" in finish, (
        "ku3-w3 唯一气泡规则：纯检索计划且批次回执已认领时 actFinish 必须抑制第二颗气泡"
    )
    assert "detailLines:" not in finish, "ux1 明细折叠区不得回流"
    summ = _strip_comments(_fn_body(ACT, "function actSummaryHtml(opts)"))
    assert '<details class="arx-trace">' not in summ, "ux1 执行过程折叠区不得回流"


def test_act_js_imports_tp_bytes_from_act_core():
    """cd2 根因回归门：actCiteExportCardHtml 在 files 非空时调用 tpBytes（861 行），但 act.js
...
def test_combo_turn_step_sequence_dispatches_cite_card():
    """cd2 真行为：组合轮 plan.steps=[rank, cite_export] 经 actDispatchPlan 图内通道的卡片
    收集逻辑（actLoopStepCardHtml 按执行顺序分发）产出 cite 卡 + 每个文件一个下载链；
    rank 步非四工具不上专项卡（ux1 精简兜底句）。这是「检索 + 导出 BibTeX」组合轮真正派发
    ...
    assert "这一步已经跑完，没有需要展示的内容" in out, "rank 步走 ux1 精简兜底（不上专项卡）"
...
def test_auto_execution_only_fires_on_a_hand_typed_submit():
    """自动执行只能由「用户亲手提交」触发。统一对话窗口（uni1）后，两条亲手提交路径
...
def test_the_route_llm_follows_api_capability_not_a_master_switch():
    """2026-08-03 agent2 设置三维度化：大模型总开关退役，`getConfig().use_llm` 由
...
def test_curate_endpoints_live_in_the_core_api_dict():
    ...
    # check_updates（2026-08-03 agent1，只读无 token）同纪律：地址只在 core.js，act.js 只认常量。
...
def test_curate_apply_sends_back_the_plan_confirm_token():
    """两步端点的机械核心（agent2 全自动化后由 runner 链式直推，契约不变）：apply 必须
...
def test_curate_search_online_marks_the_network_fetch_before_it_happens():
    ...
    不能等查完/入完库才说（agent2 全自动化：播报即行动流条目，无问卷）。"""
...
def test_the_decision_survey_is_retired_by_full_automation():
    """2026-08-03 agent2 执行侧全自动化：问卷弹窗（survey.js）整建制拆除——
...
def test_the_dead_chat_note_and_prefill_strip_patch_are_gone():
    """agent1 清理清单（设计 §1.5）：actChatNote 已无生产调用方（p10 起执行全程由行动流 +
...
# ---------------------------------------------------------------- agent1（2026-08-03）：check_updates / cfgAgent / plan.trace

def test_curate_check_updates_is_a_readonly_surveyless_runner():
    """curate.check_updates（agent1 新动词，「检查来源更新」从 search_online 拆出）：
...
def test_curate_db_status_runner_uses_observation_then_endpoint_fallback():
    """curate.db_status（agent2 P1-B6，只读状态汇报）：runner 双通道同一真源——
...
def test_the_route_body_carries_the_agent_exec_switch():
    """AI 执行开关（cfgAgentExec，默认开；2026-08-03 agent2 合并旧 autoAct+agent）随
    ...
    # 门控语义（P0-3 开关语义统一）：未配 key 禁点并指路；已配必可开关——不再有第二道闸锁开关
...
# ---------------------------------------------------------------- agent2（2026-08-03）：流式规划（设计 §5.2）+ 一句话收尾（§5.3）
...
def test_streaming_route_only_fires_when_agent_on_and_extension_available():
    """流式规划（claudecode 式，设计 §5.2）：`stream:true` 只在 AI 执行开且扩展可用时出现。
    R2-9 P1-1 × R2-10 P1-1（2026-08-04 产品所有者钦点改行为）：流式档**也起跑 startProgress**
    ——只挂 loading 静态视觉时没有在途旗标，进度泡不显示；ux2 起数字里程表退役，loading 与
    ...
    # ux2（2026-08-18）：数字百分比撤下——进度泡＝三点动画；流式档带不确定态文案，非流式档只有三点。
    assert 'cbProgressBegin(streamAgent ? "规划中…" : "")' in body
    assert "Math.floor(progressPct())" not in BOARD, "ux2：进度泡不许再回退到 progressPct() 百分比"
...
def test_streaming_sse_parser_frames_across_chunks_and_fails_loud():
    ...
    让调用方走回退——不许拿半截响应当答案。"""
    # 2026-08-08 幂等批：签名加 reqId（断流重发同号，服务端幂等认领）——钉字刻意更新。
    # 2026-08-22 eng-p6：签名尾部再加 opts（suggestedRecipe 透传，流式/非流式同口径）——钉字刻意更新。
...
def test_streamed_steps_are_not_rendered_twice():
    """ku3-w3（用户重申三段结构）：流式期间过程展示**全部**归信息流工具行（flowPushEvent，
...
def test_brief_summary_replaces_body_and_exec_disclosure_is_lean():
    """一句话收尾（设计 §5.3）+ 执行披露精简：/api/act/summary 带 brief:true；
    拿到 summary_zh 后原位替换总结泡**正文**（「AI 总结」标保留）。
    ux1 起工具结果卡 / 「明细」/「执行过程」折叠条撤下；ku3-w3 起 ux1 的 execSummary
    ...
    assert "execSummary" not in finish, "ku3-w3：execSummary 通道已退役（摘要归信息流压缩行）"
    assert "detailLines:" not in finish, "明细折叠区已撤（ux1）"
    summ = _strip_comments(_fn_body(ACT, "function actSummaryHtml(opts)"))
    assert '<details class="arx-trace">' not in summ, "执行过程折叠区已撤（ux1）"
    ...
    assert "compressFlow" in flow and "执行了 " in flow, (
        "ku3-w3：工具调用计数压缩句必须在纯核 flow_trace（可单测）"
    )


def test_loading_never_double_fires_with_streaming():
    """ux2（2026-08-18）：loading 全程只挂一次——startProgress 的幂等守卫在（runRecommend 接手不重启）；
    流式档与非流式档同走这一个起跑点（R2-9/R2-10 P1-1 后不再手工占 loading），
...
def test_loading_is_indeterminate_and_cache_hit_finishes():
    """ux2（2026-08-18）：数字里程表退役——startProgress 只挂 loading 态与在途旗标（不再起 rAF 翻数），
    ...
    assert "requestAnimationFrame" not in start, "ux2：数字里程表退役，startProgress 不许再起 rAF 翻数"
    ...
    assert "requestAnimationFrame" not in finish, "ux2：finishProgress 同步收尾，不许再补满翻数"
...
# ---------------------------------------------------------------- dl2（2026-08-19）：真实下载 UX 契约

def test_download_endpoints_declared_in_core_api():
    """dl2：4 个 /api/download/* 端点必须在 core.js API 集中声明（照 citationsDownload 模式）——
...
def test_task_pack_download_calls_use_api_declarations():
    """dl2：task_pack.js 的下载区只经 API.downloadXxx 调端点——字面路径一出现就与 core.js 声明漂移。"""
...
def test_task_pack_has_primary_real_download_and_fallback_buttons():
    """dl2：面板底部主/次双按钮——主按钮「直接下载真实数据」（/api/download/plan 有 supported 才显示），
...
def test_pack_bytes_label_says_real_data_size_not_zip_size():
    """dl2（B 文案）：按钮 label 的体积数字必须是「真实数据共约 X」——
...
def test_cache_hit_branch_wraps_finally_around_the_reset():
    """dl2（D2 小修）：缓存命中分支用 try/finally 包住「渲染 + 收尾」——
...
def test_dl_final_stats_are_honest():
    """dl2 真行为门：终态文件统计（dlFileStats 真函数）——成功字节只算 ok/size_ok；
...
def test_pack_download_verb_starts_real_download_automatically():
    """dl-auto-1：pack.download 动词入口（actRunPackDownload）分级 OK 后**直接开始真实下载**，
    不再停确认闸——调可复用的启动内核 tpDownloadStart；失败如实降级任务包兜底。旧「请在面板点
    开始下载确认」的确认闸措辞必须消失（这是本批的目的性变更，不是改测试凑绿）。"""
...
def test_reset_task_pack_guards_an_in_flight_download():
    """dl2 真机抓到（回归钉）：下载进行中（_dl.stage==='running'）resetTaskPack 必须跳过整个重置——
...
# ---------------------------------------------------------------- ku1-覆盖策略修复：批次覆盖语义（设计 §10.3）

def test_batch_rank_suffix_labels_the_sorting_layer() -> None:
    """ku1（设计 §10.3）：非活动备选批的排序层标注——规则=1 / 规则+本地精准重排=2 /
...
def test_batch_select_spec_passes_in_node() -> None:
    """ku1（设计 §10.3）真行为：node 直跑 batch_select_spec.mjs（断言失败 → 非零退出）。"""
    node = _resolve_node()
...
def test_relax_phrase_templates_consistent_results_and_batch_select() -> None:
    """ku3-w6 任务3c 契约钉：results.js 空态卡放宽 chips（RELAX_GROUPS.verb）与 batch_select.js
