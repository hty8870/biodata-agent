# -*- coding: utf-8 -*-
"""安装器工程 W5：`--migrate-from` 迁移器契约测试（desktop_launcher.py 迁移段）。
...
# ===========================================================================
# 11. A1-M2：symlink/junction 链接穿透拒绝（resolve 后校验仍在源根内）
# ===========================================================================
...
def test_plan_rejects_junction_escaping_source(tmp_path):
    """A1-M2 负向：.userdata/recycle 是指向源根外的 junction → 进 rejected，不迁移。"""
...
def test_execute_rejects_item_outside_source(tmp_path):
    """A1-M2：run_migration staging 前对每个源条目重新校验——位于源根外的条目拒绝
