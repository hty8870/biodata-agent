# -*- coding: utf-8 -*-
"""档案浮窗（2026-08-22 fx2：历史记录 + 我的课题合并为单窗双 tab）的前端静态门。
...
def test_window_is_draggable_and_resizable():
    """标题栏拖动 + 右下角缩放：指针事件接线都在（ku1 起骨架属主在 shell.js initFloatingWin 工厂），
    且都有视口约束（不许拖丢/拉爆）。"""
...
def test_each_conversation_row_click_and_actions():
    """每条对话（2026-08-03 用户点4）：**点行本体**＝在本标签页找回（hist-main → viewHistorySnapshot，
...
def test_chat_only_rows_render_and_restore_honestly():
    """A2（2026-08-04）仅对话历史行：展示照实报「仅对话 · N 条消息」、不给「重新检索」按钮
...
def test_resize_uses_real_drag_geometry():
    """D1（2026-08-04）：右下角缩放按真实拖拽几何——setPointerCapture（滑出把手不断拖）
    + 按下时记偏移（第一步不再跳缩 11px）+ 双向正确 + 最小/最大钳制保留。
    fx2：骨架属主迁到 shell.js（initArchiveWin），几何参数不变。"""
...
def test_first_placement_stays_inside_the_viewport_on_mobile():
    """D2（2026-08-04）：落位的 +110 桌面右偏在 ≤780px 视口豁免（实测移动宽度右缘溢出 98px、
    按钮不可达）；任何宽度下右缘钳进视口。fx2：落位逻辑在 shell.js openArchiveWin。"""
...
def test_open_window_is_reclamped_on_resize_and_close_is_animated():
    """R2-2 P2-1 + R2-10 P2-3（2026-08-04）：① 浮窗开着时缩窗要把它重钳回视口
    （钳制此前只在首开算一次，缩窗后关闭按钮出屏不可达）；② 关闭播退出动画（与入场
    histWinIn 对称），动画播完才 hidden；③ Esc 逐层链接管我的库/历史浮窗。
    ku1：骨架属主迁到 shell.js initFloatingWin 工厂（reclamp / close，单一关窗计时器每窗一份）。"""
