# -*- coding: utf-8 -*-
"""安装器工程 W5：E2E harness（scripts/installer_e2e.py）契约测试。

- **报告结构**：frozen 段 / installer_matrix 段形状与矩阵 ID（m01-m13）稳定。
- **frozen 段**：build-out 存在 + 本会话无运行实例 + 7860-7869 空闲 → 真跑真实
  `BioDataAgent.exe` 四用例（f01 health / f02 固定端口跨重启 / f03 二次启动 attach /
  f04 退出端口释放）全 PASS；条件不满足 → `pytest.skip` 并给明确原因（**环境缺失
  不写成通过**）。
- **安装器段**：真实安装器 exe 未构建（W4 并行施工中）→ 整矩阵 13 项 SKIP 且标注
  「待 W4 联调」（m13=桌面壳未静默缺失，前置 WebView2 缺失时也 SKIP）；W4 合并后
  exe 就位时自动真跑。
...
def test_find_installer_returns_none_or_matching_name():
    ...
    if installer is not None:
        ...
        # 当前 W4 未构建 → 整矩阵必须 SKIP（由报告测试兜底），此处仅为语义说明
        pass
...
def test_find_installer_prefers_current_version_over_walk_order(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(e2e, "DEFAULT_W4_WORKTREE", tmp_path / "missing")
...
# ===========================================================================
# 报告结构 + 安装器矩阵（当前必然 SKIP 且标注待 W4 联调）
# ===========================================================================
def test_report_shape_and_matrix_ids(tmp_path):
    ...
    assert [m["id"] for m in matrix] == [f"m{i:02d}" for i in range(1, 14)]
    for item in matrix:
        assert item["status"] == "skip", f"{item['id']} 当前必须 SKIP：{item}"
        assert "待 W4 联调" in item["detail"], f"{item['id']} 必须标注待 W4 联调"
...
def test_installer_matrix_skips_when_exe_absent(monkeypatch, tmp_path):
    ...
    assert all("待 W4 联调" in m["detail"] for m in matrix)
...
def test_frozen_e2e_real_exe(tmp_path):
    ...
    if frozen_root is None:
        pytest.skip("frozen runtime 未找到（build-out/dist/BioDataAgent 缺失），待 W3 构建产物")
    ...
    assert case_ids == ["f01", "f04", "f02", "f03"], case_ids
    by_id = {c["id"]: c for c in frozen["cases"]}
    # f01/f04 是真跑必过项（health + 退出端口释放）
    assert by_id["f01"]["status"] == "pass", by_id["f01"]
    assert by_id["f04"]["status"] == "pass", by_id["f04"]
    # f02/f03 为启动器专用：能力就绪（W7 已切入口）则 pass；产物若缺启动器能力
...
# ===========================================================================
# A2-H1：_wait_for 防空转（进程死了立即失败，不空转到超时）
# ===========================================================================
class _DeadProc:
