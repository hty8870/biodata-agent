...
# 包身份单一真源（2026-08-10 codex 架构评审裁决落地）：历史上一半测试经
...
# src 给包；2026-08-27 一级目录整理后 mcp_server 也入包为
...
def _stub_checklist_call_globally(monkeypatch):
    """任务4（2026-08-08 清单核销）：understand 的清单轻量调用全局默认 stub。
    ...
    2026-08-10 起包身份统一为 `dataset_recommender.*` 单名（历史双名同 stub 补丁退役，
...
def _stub_route_consensus_globally(monkeypatch):
    """nl1 转正（2026-08-17 新逻辑摘除批）：route_consensus 恒为环首，全局默认 stub。

    agent 图内测试的 FakeModel 应答序列普遍不预置分流投票应答——共识投票会吃掉下一个
    预置应答造成全线错位（与上方清单 stub 同一病灶）。统一默认 stub 成 general（全集
    ...
    掩盖风险（2026-08-17 nl2 codex 评审 中3，如实留痕）：本 stub 把所有图内测试钉在
    **general 面**——scoped 套件面（search/action）下的 understand/repair 提示词装配、
    JSON 兜底壳的按面铁律（action_plan._constraints_zh）在图内全无覆盖，nl2 高2 的
