...
本护栏跑**全部来源**（~5667），确保第二批词表扩充让外部平台库真正可检出，且跨源检索仍：
  1) 硬违规 = 0%（返回项必满足期望约束——0% 违规保证在多源下也成立）；
  2) 该弃权的全弃权、该空集的全空集（fail-closed 语义跨源不破）；
  3) Hit@5 不塌、五个来源都能被检出（第二批词表扩充的召回成果不回退）。
...
# 基线来源：**按名字钉**，不按个数钉。
#
# 原写法是 `n_sources == 5` / `len(sources_seen) == 5`。它想保护的是「第二批词表扩充的召回成果
# 不回退」——即这五个来源**每一个都仍然检得出来**；而它实际写成的是「来源总数恰好是 5」。
# 两者在只有五个来源时等价，一旦要接入第六个来源就分道扬镳：数据侧做完了合规的候选，
# 却被一条与数据质量无关的计数断言挡住。验证 的数据库扩展工作流因此停在这里（它自己
