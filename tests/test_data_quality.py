# -*- coding: utf-8 -*-
"""数据内容一致性检查（codex 试用反馈 #5）：字段 vs 描述文本不一致检测。

关键：字段常多值（species='Mouse, Human'、tissue='Brain, Lung'）——描述提到字段里**已有**的另一值
不得误报冲突（否则混样/多组织数据集会刷屏假阳）。内联 caveat 只查物种（高精度，正是 codex 报告场景），
...
def test_species_field_description_conflict_detected():
    """标注 Mouse、描述文本出现 Human → 物种冲突（codex 报告的 Mouse/Human-Kidney 场景）。"""
