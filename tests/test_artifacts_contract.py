# -*- coding: utf-8 -*-
"""课题数据层（artifacts.js，engagement 落地包 B1）的结构契约门。

设计：`docs/工作记录/设计_engagement落地包_2026-08-22.md` §2/§3.1/§3.2（F1 课题）。
...
def test_artifacts_layer_has_no_dom_or_localstorage() -> None:
    ...
    assert not hits, f"artifacts.js 出现 DOM/localStorage 访问：{hits}（设计 §3.2：课题数据只进 IndexedDB）"
...
def test_schema_and_status_constants_are_present() -> None:
    ...
    assert 'export const ARTIFACTS_DB_NAME = "biodata-artifacts";' in text, "库名常量缺失（设计 §3.2 的 biodata-artifacts）"
    assert 'export const DEFAULT_CANDIDATE_STATUS = PROJECT_STATUS.PENDING;' in text, "新候选默认待核验常量缺失（设计 §3.1 硬性）"
    # schema_version 字段必备：规整函数必须在每条记录上落当前版本
    assert "schema_version: ARTIFACTS_SCHEMA" in text, "记录 schema_version 字段缺失（P4/P5 建库前提）"


def test_quota_error_is_captured_honestly() -> None:
    """写路径必须捕获 QuotaExceededError 并如实上报（设计 §3.2：不静默吞掉「写不进去」）。"""
    text = CORE.read_text(encoding="utf-8")
...
def test_composite_key_carries_profile_scope() -> None:
    """复合主键带 profile scope（设计 §3.2）：scope + project_id 确定性编码 + by_scope 索引。"""
    text = CORE.read_text(encoding="utf-8")
...
def test_importmap_registration_done_by_wave1b() -> None:
    """#artifacts 的 importmap/package.json 登记在 Wave 1B（F1 课题 UI，2026-08-22）完成——
    B1 数据层的「不登记」守卫已退役（那段守卫防的是 B1 越界；Wave 1B 收口登记是设计 §11 的
    既定顺序）。登记必须三处齐全且两页映射一致，否则 test_frontend_import_graph 会红。"""
    pkg = (ROOT / "package.json").read_text(encoding="utf-8")
    assert '"#artifacts"' in pkg, "package.json 缺 #artifacts——F1 已登记，缺失即漂移"
    maps = {}
    for page in ("index.html", "dataset.html"):
        html = (ROOT / "web" / "static" / page).read_text(encoding="utf-8")
        assert '"#artifacts"' in html, f"{page} importmap 缺 #artifacts——F1 已登记，缺失即漂移"
