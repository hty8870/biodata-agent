# -*- coding: utf-8 -*-
"""统一对话窗口（uni1 起； cur3 微信式输入行）的前端接线门 + **真行为**门（后者在 node 里跑真函数）。
...
def test_ubsubmit_has_its_own_inflight_gate_and_seq():
    ...
    # p10：_curateSeq 退役； 起管护并发由「_actBusy 串行闸 + runner 链式直推」单闸接管
...
def test_route_kind_none_echoes_back_never_silent_full_search():
    ...
    （uxfix1：零检索信号的歧义句被静默搜全库、连一句回音都没有，正是这次修掉的缺陷）。
...
def test_sidebar_stays_put_during_a_search_and_animates_in_afterwards():
    """图4：检索**进行中**侧栏工作卡不许在落地页提前弹出——只有 say 不够格，
...
def test_research_fix_chip_retired_but_keep_conv_survives():
    """图5的 keepConv 机制仍在（runRecommend 的 keepConv 档：分面/抑制照清，
    cbLogClear 不调、对话记录保留）；但回执 chip「按原话重新检索」已于 pack1
...
def test_curate_restore_has_a_named_runner_and_direct_flow():
    """restore 在派发表里指向一个**真实存在**的 runner； 起全自动化直推
...
def test_act_match_file_zero_hits_falls_through_to_the_listing_branch():
    """H-01（触发点审计 frontend.md）：target 非空但零命中时 actMatchFile 曾返回
...
def test_failopen_fallback_alerts_user_before_literal_search():
    """H-02（触发点审计 frontend.md）：/api/utterance 非 503/409 失败（含后端 500、
...
# ---------------------------------------------------------------- owner 疑点 A/B：动作句的 query 真源与回注目标
