...
def test_exec_downgraded_to_none_marks_downgraded_from():
    """2026-08-15 审计 C-7：EXEC 缺 quoted 被机械降成的 none 是**降级 none**，不是真 none——
...
def test_action_verb_itself_is_not_a_negation_morpheme_cn():
    ...
    2026-08-03 agent1 模拟剧本抓到的真 bug：极性门直接消费检索弃权守卫表
...
# ---------------------------------------------------------------- 疑问「没」掩码（2026-08-09 病灶猎杀批）

@pytest.mark.parametrize("utterance,quoted", [
    # cr4final 实锤四族——疑问「没」落在续步 quoted 紧邻左窗，曾被误判 cancelled 掐死多步链：
...
def test_year_identifier_or_date_digits_do_not_count_as_said(utterance, limit):
    """2026-08-15 审计 C-5：裸子串会把年份/编号/日期里的数字当成「用户说过的条数」，
...
def test_scope_all_is_refused_with_an_explicit_delta():
    ...
    scope=all 是公开能力；2026-08-09 codex P0-6 把旧「产品不支持」假话文案改成如实口径）。"""
...
# ---------------------------------------------------- rule_fallback 与 C-1 共用名词用法反向闸（审计 ta2）
...
# ---------------------------------------------------- rule_operation_marker 的名词用法反向闸（审计 C-1）
...
def test_raw_shape_violations_is_agent_mirror_prefix():
    """2026-08-10 codex 架构评审裁决落地：agent_exec._validate_raw 的公共形状校验
...
def test_plan_action_verbs_are_the_table_minus_loop_only_verbs():
    """2026-08-17 nl2 codex 对抗评审 高1：plan_action 保底通道的允许表 = 闭环动词表
    减去环内专属动词（search.rerun/rank/rerank 环内检索工具 + route.request 换线
    元动词 + curate.rollback 回滚——rb1（2026-08-17）补录：回滚目标依赖本轮 steps
...
def test_mcp_doc_verb_list_covers_plan_action_verbs():
    """2026-08-17 codex nl2 复核（低）：MCP 模型可见文档的动词枚举曾两度漂移（这次
...
def test_plan_action_never_returns_loop_only_verb():
    """高1 行为钉：模型在保底通道答了环内动词 rank → 机械拒（进 rejected、降 none），
...
def test_build_plan_from_raw_allowed_verbs_gate():
    """高1 闸层钉：`allowed_verbs` 缺省 None = 全表放行（agent 环内路径不变——rank 在
...
def test_scoped_face_prompt_has_no_retired_route_verbs():
    """2026-08-17 nl2 codex 对抗评审 高2：scoped 收窄面的 JSON 壳提示词里，面内不存在
