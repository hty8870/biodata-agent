...
- 空池/弃权档：空池独立审核（mode="empty"）已删（2026-08-16 检索工具化 Phase 1）——
...
def test_parse_audit_infinity_order_no_crash(blob):
    # 回归（对抗评审 #1）：order 含 inf（int(inf) 抛 OverflowError）绝不能穿透 fail-open 链。
...
# ----------------------------- 空池独立审核档：已删（2026-08-16 检索工具化 Phase 1）-----------------------------

def test_empty_pool_independent_audit_branch_removed():
    """空池独立审核档（rerank.audit_query_only 族四件）2026-08-16 随「检索工具化」删除——
...
def test_same_hard_filter_order_insensitive():
    # 对抗评审 #1 回归：约束是 OR 匹配、顺序无关；同一实体集换 alias 会翻转多值列表顺序 →
...
def test_workflow_audit_verdict_true_with_rewrite_ignored(wf, good_pair, monkeypatch):
    # 回归（对抗评审 #3）：LLM 自相矛盾地给 keywords_ok=true + 非空改写 → 信 verdict、不改写，
...
# ----------------------------- workflow：空池 / 弃权档 -----------------------------
# 2026-08-16 检索工具化 Phase 1：空池独立审核（发现B 扩展）已删——空池救回改由
...
def abstain_query(wf):
    ...
    2026-07-22 夜换载荷：原载荷是「小鼠大脑发育的scRNA-seq」，它弃权的唯一原因是「发育」
...
def test_workflow_audit_empty_pool_no_longer_rescued(wf, empty_query, abstain_query, monkeypatch):
    """2026-08-16 检索工具化 Phase 1：空池/弃权档的独立审核删除——空池救回改由
