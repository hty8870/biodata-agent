# -*- coding: utf-8 -*-
"""数据库状态汇报（2026-08-03 agent2 P1-B6 `curate.db_status`）的确定性门：

- `corpus_status.db_status`：只读离线、不抛（空项目根也如实降级）、形状稳定；
- `/api/curate/status`：same-origin 闸 + 同一份事实真源；
- agent execute 节点（2026-08-04 起取代 observe 只读节点；LOOP_TOOLS 注册表）：命中 →
...
def test_curations_failure_carries_error_field(tmp_path: Path, monkeypatch):
    """G-11：外部库/回收站清单失败不得呈现成真空清单——顶层带 curations_error。"""
    def boom(**kw):
