# -*- coding: utf-8 -*-
"""中文别名子串劫持的回归网（2026-07-22 夜）。

**病根**：受控词表的别名匹配是裸子串。ASCII 那一半在 2026-07-22 白天已按词边界收紧
...
# 每条都是实测抓到过的真实错筛（详见开发日志本轮条目），不是假想。
HIJACK_CASES = [
...
def test_protected_compounds_do_not_leak_a_constraint(compound, leaked_dim):
    ...
    2026-07-22 夜修正：此前这条还顺带断言 `parse_status == "executable"`，
...
def test_unknown_protected_compound_still_abstains(compound):
    ...
    这是 2026-07-22 夜对抗评审抓到的 major：屏蔽发生在 alias 消费**之前**，于是残差弃权门
