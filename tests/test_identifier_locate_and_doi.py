# -*- coding: utf-8 -*-
"""2026-08-04 普查 P0-1 / P1-2 修复的回归门。

P0-1 「查看介绍」/ FAIR 自检张冠李戴：旧实现线性扫描语料、uid/url/name 任一命中即早退——
...
P1-2 裸贴 DOI 未命中 → 全库 5712 条冒充结果：DOI 被词面解析拆成数字残片静默丢弃 →
...
#: 语料里的同名两条（普查 P0-1 的天然测试夹具）：同名、同来源、不同 uid/url/发表时间/样本量。
UID_A = "visium-hd-cytassist-gene-expression-libraries-human-breast-cancer-ff-ultima"   # 2025-02-20 · 583104
UID_B = UID_A + "-4"                                                                     # 2025-07-24 · 38993
...
# ---------- P0：locate_record 优先级与消歧（纯单元，不依赖真实语料） ----------

def test_locate_uid_pass_beats_earlier_name_match():
    """uid 精确键必须全扫一遍：靠前的 name 命中不得截胡靠后的 uid 命中（= P0-1 病形）。"""
...
def _same_name_pair():
    ...
    if UID_A not in by_uid or UID_B not in by_uid:
        # 夹具缺席必须**炸出来**而不是静默 skip——否则 P0-1 的 API/MCP 级回归门蒸发且无人察觉
        #（2026-08-04 对抗评审 tests P2-3）。语料更新掉了这对同名记录时，请换一对同名夹具并更新断言。
        missing = [u for u in (UID_A, UID_B) if u not in by_uid]
        pytest.fail(f"P0-1 同名夹具已不在语料里（缺 {missing}），本文件 6 条 API/MCP 级断言失去载体——"
...
def test_api_introduction_full_params_belong_to_uid_record():
    ...
    assert "583104" in blob          # 本卡（2025-02-20 · 583104 Spots）
    assert "38993" not in blob       # 同名另一条（2025-07-24 · 38993 Spots）不得混入
...
def test_recommend_bare_doi_miss_is_honest_fail_closed_not_full_corpus():
    """P1-2 病形：裸贴库里没有的 DOI 曾返回全库 5712 条「满足基本检索条件」。"""
...
def test_recommend_bare_gse_same_honest_channel():
    """对照组：GSE 编号原有弃权通道不被改动破坏（2026-08-06 起 GSE 已入库 indexed=True，
