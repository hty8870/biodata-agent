# -*- coding: utf-8 -*-
"""多批检索结果（2026-08-17 过夜批 M3，设计 §M3）后端专项门。**全离线**：

2026-08-17 nl1 转正：批次机制常驻——轮内累积批次（preliminary 在前、环内上屏批
...
2026-08-17 codex 评审 中5）；preliminary_final 的 b 档判定用独立哨兵 loop_payload，
不再拿 result_payload is None 充当。原环境开关与 OFF 逐位一致负向钉
随代码一并摘除（开关全名与快照归档于 `docs/归档/旧逻辑_scoped路由替代_2026-08-17/`）。

- 溯源（2026-08-17 codex 评审 中6）：各批 query_raw 恒 = 本轮用户原话；
...
def _stub_agent(monkeypatch, plan, *, verdict: str = "search"):
    """agent 图替身（与 test_prelim1_backend 同 seam）：cr1 后 preliminary 发射点在
...
def test_search_rerun_batch_carries_disclosure_zh(monkeypatch):
    """r2p（2026-08-18）：采纳档批 additive 带 disclosure_zh（供 final a 档换屏的
