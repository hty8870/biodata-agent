# -*- coding: utf-8 -*-
"""成功经验 few-shot 库（2026-08-09 五机制批；Vanna auto_train 式）的确定性门。

2026-08-13 起两级结构：机械收录只进**候选池**（`curate_example_candidates.jsonl`，不注入）；
...
# ---------------------------------------------------------------- 2026-08-15 ta2 exec2：观测留痕（cross-trace D3/D4）

def test_corrupted_candidates_pool_warns_and_returns_empty(tmp_path, capsys):
    """cross-trace D4：候选池文件损坏 → 仍按空表返回（口径不变，不掀翻主流程），
...
def test_corrupted_ledger_warns_and_injects_nothing(tmp_path, capsys):
    """cross-trace D4：正式库损坏 → 注入侧静默降级回纯静态 few-shot 的口径不变，
...
def test_candidate_write_failure_warns(tmp_path, capsys, monkeypatch):
    """cross-trace D3：候选池写盘失败不再零痕迹——收录语义不变（不掀翻主流程），
    stderr 留一行可定位的痕迹。"""
...
def test_audit_write_failures_warn(tmp_path, capsys, monkeypatch):
    """cross-trace D1/D2：联网账本与降级审计账本的写盘失败统一 `_warn_once` 纪律——
