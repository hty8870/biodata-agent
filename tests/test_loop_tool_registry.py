# -*- coding: utf-8 -*-
"""LOOP_TOOLS 注册表一致性闸（2026-08-06「工作流即工具」批；蓝本 pydantic-ai 的
...
钉四条：
  ...
  4. 每个动词声明的槽位都有**专职**描述（`_SLOT_DESCRIPTIONS_ZH`；limit 为内置槽豁免）——
     回退泛模板是「病灶温床」时期的兜底（2026-08-03 agent1b 病灶：泛泛模板让 LLM 随手填
...
def test_every_loop_tool_entry_is_complete_and_exec():
    for verb, entry in agent_exec.LOOP_TOOLS.items():
        ...
        # 2026-08-17 nl1 转正：route.request 是注册表里**唯一**的 ROUTE 类成员（环内元动词——
