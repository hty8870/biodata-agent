# -*- coding: utf-8 -*-
"""使用反馈（L2 埋点）的隐私与诚实性契约门 —— 2026-08-20 tl1 反转版。
...
def test_the_onboarding_points_at_the_management_row() -> None:
    """2026-08-13 ob1：教程最后一步不再当场问开不开，改为打开设置抽屉、高亮管理入口。
...
def test_consent_is_per_account_profile() -> None:
    ...
    # v2（2026-08-22 ov1-fix1a）：值为同意时刻 ISO 串（MCP 中继 since_ts 下界要用）；"0"=拒绝。
...
def test_training_consent_is_separate_opt_in_and_contract_is_versioned() -> None:
    """产品改进授权不得偷换成训练授权；训练默认开启（2026-08-25 产品所有者决策，
...
def test_upload_thresholds_stay_at_or_below_design() -> None:
    """触发阈值常量存在且 **≤ 设计值**。2026-08-22 ov1-tel1 激进上传改版：

    benchfb ≥1 轮 / usage ≥10 条（原 3 轮/50 条——benchfb 单条即一轮完整检索现场）；
    ov1-adapt1 起 usage 默认降到 **2 条**（无 hint 时几乎实时），阈值/间隔随服务器
    ...
    assert m and int(m.group(1)) <= 1, f"benchfb 阈值必须 ≤ 1（ov1-tel1 激进上传），实际 {m and m.group(1)}"
    m = re.search(r"const UPLOAD_EVENTS_MIN\s*=\s*(\d+);", code)
    assert m and int(m.group(1)) <= 2, f"usage 默认阈值必须 ≤ 2（ov1-adapt1），实际 {m and m.group(1)}"
    ...
    # tc1 合并裁决：生产明文 HTTP 端点以显式配置启用（风险已知、用户知情授权）。
...
def test_adaptive_upload_threshold_hint_pins() -> None:
    """ov1-adapt1：上传阈值/节奏随服务器 server_hint 自适应（结构性钉子，断真代码）：
...
def test_payload_goes_through_the_sanitizer() -> None:
    ...
    # 2026-08-22 ov1-tel1：值级遮蔽（键级剔除挡不住「值里夹带」的自由文本 PII）
...
# ------------------------------------------------- schema v3（2026-08-22 ov1-fix1a）

def test_schema_v3_impression_label_and_seen_wiring() -> None:
    """ov1-fix1a 遥测缺陷修复批的结构性钉子（断真代码，全走注释剥离后的源码）：
...
def test_feedback_core_spec_passes_under_node() -> None:
    """eng-b3：意见反馈核心真行为门——队列（幂等/遮蔽/上限/状态流转/隔离）、
...
def test_feedback_channel_is_isolated_and_independent() -> None:
    """eng-b3 F5 结构性钉子（断真代码，防回潮）：
    - `feedback_core.js` 是纯逻辑核心：公钥配置点（ENG-B3-PUBKEY 醒目注释）、
    ...
    assert "ENG-B3-PUBKEY" in raw_core, "必须有公钥配置点醒目注释"
    assert "收口已填生产公钥" in raw_core, "ENG-B3-PUBKEY 注释必须如实说明生产公钥已填（2026-08-22 收口）"
