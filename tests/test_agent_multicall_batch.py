# -*- coding: utf-8 -*-
"""多 tool_call **同批只读消费**（2026-08-14 mcl1 批）的确定性门。**全离线**（FakeModel + 假注册表）。
...
- 2026-08-20 pbt1 批（依赖占位批量计划 v2，设计 §1）：占位接地续步（compare/compat/
...
def test_write_cite_export_without_placeholder_truncates():
    """pbt1（2026-08-20）写动词截断系列补位：cite.export 是写动词（引文落盘）——**无占位**
    在批中照旧截断整尾（其后 db_status 也可能写给后状态，回炉重判），与 mcl1 的 search_online
...
def test_placeholder_grounded_cite_export_enters_batch(monkeypatch):
    """pbt1（2026-08-20）矩阵内占位放行：cite.export(uids=[\"$1.top[0].dataset_uid\"]) 随
    rank 主步进批（写动词**带占位接地**即可进批——v2 与 mcl1 只读白名单的关键差别，设计 §1）。"""
...
def test_batch_breaker_recuts_between_extras(monkeypatch):
    """2026-08-15 gr2 **批内熔断**（codex 对抗评审 Critical-2）：批 = [check(ENCODE),
    ...
    # 2026-08-15 fix-exec M4：熔断剔步「留痕不留步」——步骤照旧不产生（上行钉不变），
...
def test_batch_breaker_ban_branch_leaves_trace(monkeypatch):
    """2026-08-15 fix-exec M4（禁提分支）：批 = [check(AE), check(ENCODE), check(10x),
...
# ---------------------------------------------------------------- understand 首步同批（2026-08-22 ov1-wf1）
