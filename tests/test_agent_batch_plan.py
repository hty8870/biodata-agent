# -*- coding: utf-8 -*-
"""依赖占位批量计划 v2（2026-08-20 pbt1 批）的确定性门。**全离线**（FakeModel + 假注册表
+ 合成记录）。设计：`docs/工作记录/设计_依赖占位批量计划_2026-08-20.md`（v2 权威，§1-§3、§7）。
...
def test_single_call_placeholder_resolves_on_main_step_end_to_end(monkeypatch):
    """真模型自然形态（pbt1 探针实测）：understand 先 rank（第 1 步），decide **单发**
...
def test_dependency_unavailable_cascades_skip_with_trace(monkeypatch):
    """级联跳过（设计 §3 dependency_unavailable）：主步 rank 真失败 → compare（$1）引用
...
def test_resolver_error_index_out_of_range_not_banned(monkeypatch):
    """resolver_error 不进熔断（设计 §3）：top 只有 1 条、引用 $1.top[1] → 解析失败跳过；
...
def test_cancelled_placeholder_extra_rejected_at_decide(monkeypatch):
    ...
    # 零采纳 → 留痕措辞是 mcl1 的「按顺序先执行第一个」（既有语义）。
...
# ---------------------------------------------------------------- top digest / cite.export（设计 §7-5/7-6）
...
def test_cite_export_consumes_explicit_uids(monkeypatch, tmp_path):
    """cite.export uids 真实消费（设计 §4）：提供 uids 清单 → 按清单导出（保序、不叠加
...
def test_batch_results_do_not_carry_over_across_batches():
    """batch_results 局部化（设计 §3）：解析源是 execute 局部 dict——同一函数两次调用
