...
4. **挂点契约**：projects.js 的 区域内必须有 `data-p5-mount-export`（带课题 id）
...
def test_projects_js_mount_region_contract() -> None:
    """projects.js 的 区域内：挂点带课题 id、区域默认整段隐藏（无 P5 不渲染）。"""
    ui = PROJECTS.read_text(encoding="utf-8")
    assert "PROJECT_EXPORTS_MOUNT" in ui
    assert "data-p5-mount-export" in ui, "导出挂点区域缺 data-p5-mount-export 挂点"
