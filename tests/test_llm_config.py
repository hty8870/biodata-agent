...
def test_env_and_env_zhipu_are_both_loaded(tmp_path: Path, monkeypatch):
    """E-01（2026-08-15 触发点审计）：.env 与 .env.zhipu 并存时两份**都加载**（此前第一个
...
# ------------------------------------------------------------- P1：ENABLE_LLM 未设置时，默认 = 当前 provider 有真实 key
# 产品所有者 2026-07-19 P1 决策：「如果填了apikey就默认开启，否则默认关闭」。判定单一真源 =
...
def test_settings_and_llm_client_share_one_default(monkeypatch, tmp_path, _fresh_settings_cache):
    ...
    （如开发机），delenv 清的变量会被 .env setdefault 复活，用例不再密封。E-01（2026-08-15
    触发点审计）起 load_env_candidates 加载**所有**存在的候选（.env 与 .env.zhipu 不再互斥），
...
def test_default_max_tokens_is_8000_in_both_places(monkeypatch, tmp_path):
    """C2（2026-08-04）：3000 对 deepseek 预设偏小——实测 6 次检索 2 次 finish_reason:length
