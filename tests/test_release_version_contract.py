"""版本 / 工具数的**跨文件单一真源**门。
 ...
 全盘审计的一批文档漂移，根因都在这里：这个门原本只钉 `webapp.py` ↔ `launch_web.ps1`
...
def test_launcher_expected_version_matches_web_health_version() -> None:
    ...
    # 2.2.0：新增 `/api/curate/sync-updates`（「工作流即工具」批 `curate.sync_updates`：
    ...
    # 2.5.0：engagement 落地包新增 4 端点（各端点先落地、随后统一
    ...
    # 2.6.0：新增 `/api/download/update`（dl-auto-1 在途增删：对运行中的下载队列做
    ...
    # （语料定期更新批：进程内单飞语料同步 job，admin 双闸端点 + 用户侧异步
...
def test_developer_doc_route_table_counts_every_decorator() -> None:
    """DEVELOPMENT.md §6 自称「与 webapp.py 的路由装饰器一一对应，共 N 个」。
     ...
     抓到：那句话说 24，实际 25 —— 少的是 `/dataset`（详情页独立路由， 那轮加的），
...
def test_acceptance_note_states_the_real_web_api_version() -> None:
    ...
    合理反应是「这个包是不是发错了」。 抓到它停在 1.5.0 而代码已是 1.6.0。
    """
    real = _web_api_version()
...
def test_customer_manual_states_the_real_versions_and_tool_count() -> None:
    """《使用说明书》是交付给客户的主文档，封面与末尾都自报版本与工具数。
     ...
     抓到：正文已经写进了 1.6.0 才有的「说了就直接做」一节，封面却仍是
...
def test_customer_manual_figures_are_numbered_in_order_and_all_files_exist() -> None:
    """说明书的图是交付物，掉一张图 / 序号乱一位，客户拿到的就是残缺的 PDF。
     ...
     这一轮往正文中间插图（§5.1 的「AI 没能完成」那张），后面四张的序号全要顺移——
...
#
# 审查抓到：客户安装教程正文写「九个工具」+ 9 行表，实际 14 个——**穿过 v1.13→v1.22 五个
...
def test_all_first_party_static_assets_share_one_cache_generation() -> None:
    ...
    assert len(versions) == 88   # 验证 88（新增 #flow_trace importmap 条目；构成 = 1 css + script 标签 + importmap 条目，随模块增删以验证为准。历史： 87 / eng1 86 / b1 60）
...
def test_dataset_page_loads_the_usage_layer_it_actually_calls() -> None:
    """`dataset.html` 必须加载 usage_core.js + usage_log.js。
     ...
     集成抓到：详情页只加载 core.js / cards.js / fav_folders.js / dataset_page.js，
...
def test_dataset_page_shares_the_same_cache_generation() -> None:
    """独立详情页 `dataset.html` 必须与首页**同一代**令牌。
     ...
     验证漂移：首页已 bump 到 `20260720-`，`dataset.html` 还停在 `20260720-`——
...
#
# 为什么需要一道「内容指纹」门（验证抓到的真事故）：本项目原有两道令牌守卫 ——
...
# 同类事故记过两次（一次 app.css、一次 tabindex，均为「真交付 bug」）。
#
...
CACHE_GENERATION = "20260827-sec4"  # First-party static assets share this release generation.
STATIC_ASSETS_SHA256 = "433fc2d4b03e6337ae31135838cd5e6475868b5a265b84c7c029c77b7ce7b54d"
