...
这个门原本只钉 `webapp.py` ↔ `launch_web.ps1` 两处，**没钉面向客户的文档**——
代码版本前进时，README / DEVELOPMENT 的版本声明与工具表可能原地不动，
客户文档与真实能力脱节。
...
def test_launcher_expected_version_matches_web_health_version() -> None:
    ...
    # 2.2.0：新增 `/api/curate/sync-updates`（`curate.sync_updates`：
    #        检查更新→有新增则自动入库的复合流；无 token——原子调用无信任边界，回收站可撤）。
    # 2.3.0：`/api/utterance` 入参 additive 新增 `req_id`（断流重发幂等——
    #        服务端按 req_id 占用，同号在途等 owner 收尾回缓存体，不再二次执行写工具；
    #        无 req_id 行为逐位不变；响应体形状零变化）。
    # 2.4.0：新增 `/api/curate-examples/pending` + `/approve` + `/dismiss`（操作样例库
    #        改用户挑选入库：机械收录只进候选池，勾选迁入正式库，注入侧只读正式库）。
    # 2.5.0：新增 4 端点：`/api/curate/sync-status`（实例级同步状态）+ `/api/curate/recall`
    ...
    # 2.6.0：新增 `/api/download/update`（对运行中的下载队列做
    ...
    # （进程内单飞语料同步 job，admin 双闸端点 + 用户侧异步
...
def test_developer_doc_route_table_counts_every_decorator() -> None:
    ...
    数字从代码里数出来——新增路由而文档漏改时，这里立刻红：
    「一一对应」这种自我声明没有门就是一句空话。
    """
...
def test_customer_manual_states_the_real_versions_and_tool_count() -> None:
    ...
    正文能力描述与封面版本号、工具表必须同步前进，缺一处客户就会看到自相矛盾的交付物。
    """
...
def test_selfcheck_tool_count_in_tutorial_matches_code() -> None:
    ...
    教程正文与各平台附录都可能写这个数，漏改任何一处都会误导对应平台的用户。
    这条把「每一处」都纳进来。
    """
...
#
# 教程正文与工具表不能只靠 `SELFCHECK_OK tools=N` 那一行守卫——那一行对不上时才红，
# 正文和工具表的漂移它看不见。这里把「每个工具名都必须在文档里出现」纳入门：新增工具却漏改
...
def test_all_first_party_static_assets_share_one_cache_generation() -> None:
    ...
    assert len(versions) == 88   # 构成 = 1 css + script 标签 + importmap 条目，随模块增删以实测为准
...
def test_dataset_page_loads_the_usage_layer_it_actually_calls() -> None:
    ...
    详情页若漏加载 usage 层，`core.js` 的 `toggleFav` 与 `cards.js` 的 `buildCard`
    会因**直接**调 `usageLog` / `USAGE_KINDS`（本仓库刻意不给跨模块调用加 `typeof` 守卫）
...
def test_dataset_page_shares_the_same_cache_generation() -> None:
    ...
    上面那条只读 index.html，从不看详情页。两页**共用** core.js / cards.js /
    fav_folders.js：令牌不同代 = 详情页用户继续吃旧缓存，且首页新 JS 与详情页旧 JS 混用，
    会抛「混合缓存 ReferenceError」（见下方内容指纹门注释）。
    """
    html = (ROOT / "web" / "static" / "dataset.html").read_text(encoding="utf-8")
    versions = re.findall(r'/static/(?:css|js)/[^"?]+\?v=([a-zA-Z0-9-]+)', html)
...
#
# 为什么需要一道「内容指纹」门：本项目原有两道令牌守卫 ——
# `test_onboarding_contract.py` 查「每个引用都带令牌」、上面那条查「所有令牌同一代」——
# 但它们都是**一致性**检查，**不是变更检测**：整轮不 bump，两道全绿。后果是：
#   1. `webapp.py` 用裸 `StaticFiles` mount、不发 Cache-Control（只有 etag/last-modified）
#      → 浏览器走**启发式**新鲜度 → 回访用户 URL 逐字节相同 → 直接用旧 JS，**修复等于没发布**；
#   2. 更糟：浏览器按**文件**独立淘汰缓存。新 JS + 旧 JS 的混合缓存下，
#      新模块会调到旧文件里尚不存在的导出 → `ReferenceError`，且该调用可能在 try 之外
#      → unhandled rejection → 功能照样坏。**一个修 bug 的变更反而制造另一个 bug。**
#
...
CACHE_GENERATION = "20260829-sec10"  # First-party static assets share this release generation.
STATIC_ASSETS_SHA256 = "0ba302a854b4ab43e9bedf1ec34f377119c5f441753c43f07d55e0647e79e857"
