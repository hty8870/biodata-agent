...
class TestUninstallBoundary:
    ...
    def test_delete_boundary_code_present(self, iss_text):
        ...
        assert "W4DeleteRejected" in code_section or "W4DeleteRejected" in iss_text
    ...
    def test_data_root_redirect_hook(self, iss_text):
        """W7：数据根隔离钩子（E2E 联调前置）——安装期读 BIODATA_DATA_ROOT env 持久化
        ...
        assert "W4_DATA_ROOT_REG_VALUE" in code_section
        assert "BIODATA_DATA_ROOT" in code_section
        assert "GetEnv('BIODATA_DATA_ROOT')" in code_section
        assert "RegWriteStringValue(HKEY_CURRENT_USER, W4_UNINSTALL_KEY, W4_DATA_ROOT_REG_VALUE" in code_section
        assert "RegQueryStringValue(HKEY_CURRENT_USER, W4_UNINSTALL_KEY, W4_DATA_ROOT_REG_VALUE" in code_section
    ...
    def test_uninstall_delete_data_switch(self, iss_text):
        """W7：卸载删数据开关（非交互验证）——卸载器 /DELETEDATA 命令行参数直接触发
    ...
    def test_deletedata_parsed_by_token_not_substring(self, iss_text):
        """A1-L1：/DELETEDATA 必须按 token 边界解析（不再子串匹配）——
    ...
    def test_install_delete_upgrade_cleanup_present(self, iss_text):
        """A2-M3：升级前清理 install_root 的 onedir 旧内容——[InstallDelete] 段被
    ...
    def test_normalize_path_canonicalizes_and_rejects(self, iss_text):
        """A1-M1：删除边界规范化——GetFullPathNameW 真实规范化；拒绝含 .. 段、
...
# ── 安装器边缘修复（波次B 边缘项，install-edge）静态契约 ─────────────
class TestEdgeFixesStatic:
    ...
    def test_initialize_setup_dir_mismatch_guard(self, iss_text):
        ...
        2026-08-21 修复：原实现在此处用 `ExpandConstant('{app}')` 取目标目录，但 {app}
        在 InitializeSetup 返回后才初始化，会抛 "app constant before initialized" 致命错误
        并在无 /SUPPRESSMSGBOXES 时弹模态框挂死（m03/m04 300s 超时根因）。改为
        ...
        assert "W4DirMismatchInstalled" in code
        assert "english.W4DirMismatchInstalled=" in iss_text
        assert "chinesesimplified.W4DirMismatchInstalled=" in iss_text
        ...
        # 版本守卫（W4NewerInstalled）同样走 ReportSetupBlocked 的静默防护
        assert "W4NewerInstalled" in code


# ── runtime-manifest 校验（2026-08-24 交付审计 fix 4，fail-closed）─────────────
