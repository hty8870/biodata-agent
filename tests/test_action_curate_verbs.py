...
2026-08-01 用户授权把删除类以**回收站式可逆删除**为前提纳入封闭动词表
...
def test_curate_verbs_have_graduated_from_the_frontend_exemption_list():
    """curate.* 已全部从前端豁免清单**毕业**：前四动词 2026-08-01 cur1 接线、restore 同日
    随统一对话窗口（uni1）接线、check_updates（2026-08-03 agent1）随同批前端改动接线
...
def test_negated_curate_action_is_never_treated_as_authorization(utterance, quoted, verb):
    ...
    2026-08-01 口径升级（`eval/curate_nlu/FINDINGS.md` §5③）：动词照判 + `cancelled=True`，
...
# ---------------------------------------------------------------- curate.restore（2026-08-01 补表：实验发现的真实缺口）
...
def test_curate_restore_has_graduated_from_the_frontend_exemption_list():
    ...
    豁免机制本身保留：常量仍在、必须是 EXEC 子集——将来新动词可凭独立执行入口加回。
    `curate.check_updates`（2026-08-03 agent1）亦已随同批前端改动接线毕业
    （act.js `actRunCurateCheckUpdates`，只读无问卷）。
    2026-08-16 刻意更新：search.rerun（检索工具化 Phase 1）凭**独立执行入口**登记进
    豁免清单——独立入口 = langgraph 图内 LOOP_TOOLS 工具 + `/api/agent/search-rescue`
    端点。2026-08-16 Phase 2 再定：**永久豁免、不毕业**——前端只接步骤卡渲染
    （act.js actSearchRerunCardHtml），刻意不写 runner：本工具的采纳由后端机械择优闸
    裁定（改空/同集如实拒绝），前端 runner 直打 /api/recommend 会绕过择优闸换屏。
    2026-08-17 rb1 钉字更新：curate.rollback（rollback 动词化批）凭同哲学入清单——
    ...
    assert AP.FRONTEND_UNWIRED_EXEC_VERBS == ("search.rerun", "rank", "rerank",
        ...
        "rank/rerank（2026-08-17 M2 登记、nl1 转正常驻；均无前端 runner）以及 "
        "curate.rollback（2026-08-17 rb1：目标靠本轮 steps 快照锚，前端无现场），"
        "以及 compare.datasets / compat.find / fair.check（2026-08-18 四工具批："
...
# ---------------------------------------------------------------- curate.check_updates（2026-08-03 agent1）

def test_check_updates_is_in_the_closed_table_as_exec():
    """「检查更新」从 search_online 剥出、各自专职（设计 §1.3，病灶见 §0.1）。"""
...
def test_search_online_no_longer_claims_the_check_updates_semantics():
    """病灶修复的钉死：search_online 的 when_zh 不再把「检查某个库有没有更新」并进自己的语义
