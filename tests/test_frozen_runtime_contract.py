...
REQUIRED_DATAS_TARGETS = {
    ...
    "assets",   # 桌面壳窗口图标（2026-08-21 壳批）：webview_shell 在 frozen 下从 _MEIPASS/assets/ 取
    # ov1-mcp1 批：安装版用户 MCP 接入教程（/api/guide/agent-prompt 读取侧）与随包 skill
...
# （"research" 覆盖迁移前的 "workstream"：2026-08-27 起 research 是流水线归档的新顶层。）
...
def test_spec_database_external_is_whitelist_not_recursive():
    """2026-08-24 交付审计 fix 1（Critical）：database/external 必须**白名单**（只打包官方快照），
...
def test_runtime_manifest_shape_when_present():
    ...
    # ov1-mcp1 批：新随包三件（MCP 教程目录 / Skill 教程目录 / skill 副本）只在**新构建**的
