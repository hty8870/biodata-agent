...
负缓存纪律 D-06 不动（只清正缓存与确定性标志，测试后原样恢复）。
"""
import sys
import threading
import types
...
def test_cold_start_load_cross_encoder_no_deadlock(_cold_start, _fake_st, tmp_path):
    """P0 回归：冷启动（清缓存+确定性标志）经 load_cross_encoder 完整走一遍——修复前
...
def test_cold_start_load_embedder_no_deadlock(_cold_start, _fake_st, tmp_path):
    """P0 回归（embedder 侧）：冷启动经 load_embedder 完整走一遍不死锁（同款修复：
