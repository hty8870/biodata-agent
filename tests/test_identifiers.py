...
def test_geo_and_indexed_flags():
    # 2026-08-06 三源接入：GEO Series（GSE）随 geo.json 入库转为 indexed；GSM（Sample 级）仍结构性不索引。
...
def test_scp_accession_boundary_not_prefix_swallowed():
    """「SCP1」必须精确命中 1 条，不被 scp10/scp101/… 前缀吞并（2026-08-06 真机抓获：曾返 241 条候选）。"""
    corpus = _corpus()
...
def test_lookup_refinebio_alternate_gse_hits_mirror_record():
    """refine.bio 镜像条目（2026-08-14 第 11 源）的 GSE 副号直达：
...
def test_recommend_carries_identifier_lookup():
    ...
    # GEO 号 → 非 null；2026-08-06 起 GSE 已入库（indexed=True），未命中时如实说「未匹配」
