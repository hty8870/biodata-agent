# -*- coding: utf-8 -*-
"""B2 decide 状态栏（调研六候选批）专项钉。

字段纪律（验证）：只放真实机械状态——步数/写步/失败/联网暂停/已搜主题/finish_vetoes/
reask_writes/清单未决；虚构的「统一重问预算」与不可达的「8/8 到顶」提示不进。
操作策略行只在「剩余=1」这个**可达**状态出现。
（约束放松批：MAX_STEPS 3→6、新增写步数字段，断言同步。）
（批：MAX_STEPS 6→8 + 到顶结算闸 + 批内熔断 + sync 点名半闸 +
...
def test_write_budget_exempts_search_network_error():
    """ （验证-5）：search_online 的 network_error 失败可证零副作用
