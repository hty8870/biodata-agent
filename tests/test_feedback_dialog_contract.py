# -*- coding: utf-8 -*-
"""意见反馈对话框（feedback_dialog_core.js / feedback.js，engagement Wave 1B F3）的结构契约门。

设计：`docs/工作记录/设计_engagement落地包_2026-08-22.md` §8（F5 向开发者发送意见）。
...
4. **授权与诚实语义**：发送路径必须「先入队后 sendFeedback」（feedbackEnqueue + sendFeedback
   ...
   （feedback_sent{with_diag} 计数在接收端入库时完成，B3 已实现）。
5. **禁碰 dataset.html**：本包与 F2 并行（F2 拥有 dataset.html），importmap 只在本页登记；
...
def test_boot_does_not_import_the_dialog_modules() -> None:
    """两模块不进 boot 的静态 import 表（F2 并行包拥有 dataset.html，新增 # 键会红 parity 门）。"""
...
def test_no_channel_copy_fallback_copy_is_present() -> None:
    """公钥未配置/WebCrypto 不可用 → 复制兜底文案与按钮语义必须在场（设计 §8）。
...
def test_no_client_side_telemetry_for_feedback_text() -> None:
    """客户端不另埋文本类事件：feedback_sent{with_diag} 计数在接收端入库时完成（设计 §8/§10）。"""
    for path in (UI, CORE):
...
def test_importmap_registration_stays_on_index_page_only() -> None:
    """本包 importmap 登记只在本页 + package.json（dataset.html 归 F2 并行包，不碰）。"""
