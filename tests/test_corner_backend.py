# -*- coding: utf-8 -*-
"""后端离线 corner-case 探针（QA A 路，2026-08-04 夜班）。

**只发现与复现，不改 src/**。每一类钉「不 500 + 诚实口径」的底线；断言写的是
*应然*（诚实方向），跑出红的就是真缺陷，逐条进 `.fix-shots/reports/A-backend.md`。
...
def test_recommend_top_k_out_of_range_rejected(bad_top_k):
    """top_k 越界（Field ge=1 le=50，2026-08-08 由 le=20 放宽）：必须 4xx 拒掉，不许 500 也不许静默当默认。"""
...
def test_identifier_lowercase_gsm_fail_closed_before_load():
    ...
    （2026-08-06 起 GSE 已入库走反查；Sample 级 GSM 仍结构性不索引。）"""
...
# ---------------------------------------------------------------- D 路 MCP 修复回归门（2026-08-05 夜班）

def test_mcp_curate_payload_json_is_plain_str_annotation():
    """D 路 P1：FastMCP `pre_parse_json` 会对**非纯 str 注解**的字符串入参一律 json.loads——
...
def test_mcp_plan_action_uses_strict_scalar_types():
    """D 路 P2：plan_action 的 has_results/result_total 此前是普通 bool/int——"5"→5、
...
# ---------------------------------------------------------------- E 路 HTTP 修复回归门（2026-08-05 夜班）

def test_recommend_query_length_cap():
    """E 路 P2：recommend query 无上限（200KB 照跑）→ 与 MCP bad_query 同口径 2000 字闸。"""
...
def test_import_record_count_cap(tmp_path):
    """E 路 P1：150 万条合法 JSON 曾能入库、随后 /api/datasets 37.8s 级联拖死全站——
...
def test_import_external_total_budget_cap(tmp_path, monkeypatch):
    """K5/N8（2026-08-08 codex 裁决）：单文件 20 万条上限挡不住「连续导入多个 20 万」——
...
def test_static_cache_control_headers():
    """E 路 P2：/static 带 ?v= 指纹 → immutable 长缓存（内容变则令牌必变，契约门保证）；
    无指纹 → no-cache 回源再验证。"""
    from fastapi.testclient import TestClient
    from dataset_recommender.app.webapp import app
    client = TestClient(app, base_url="http://127.0.0.1")
...
# ---------------------------------------------------------------- B 路前端功能修复回归门（2026-08-05 夜班）

def test_unified_box_user_searches_carry_usersubmit_flag():
    """B 路 P3-1 的根：统一框路由来的检索没带「亲手提交」标记 → usageLogSearch 闸全拒，
    ...
    # prelim1（2026-08-16）：final a 档（环内采纳 result_payload 换屏）也是用户亲手提交的落地——
    # 它的 prefetched runRecommend 共用一份 `_a`（handSubmit:true），故 3 → 4；漏带会让 a 档
    # 落地被 usageLogSearch 闸拒，「检索 N 次」少记一类用户检索。
    # mb2（2026-08-17 M3 波2）：tool 档消费补链——三 flag ON 时检索由环内 rank/rerank 工具完成、
    # 整轮 route=tool，tool 分支把环内上屏批 prefetched 落地；这同样是用户亲手提交那句引发的
    # 检索落地。仅 preliminary 批镜像只摘徽标不落地，不新增计数。
    # ku1（2026-08-23 覆盖策略修复）：search a 档与 route=tool 档的落地统一收进 `_applyBatchDecision`，
