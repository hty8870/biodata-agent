# -*- coding: utf-8 -*-
"""prelim1「初步结果先行 + 信息流升级」（2026-08-16，设计 §2/§7）后端专项门。**全离线**：

- 机械闸（设计 §2.2，全与）逐项击破：正例（"human blood"，261 条真实命中）只发一次
...
- final additive 键三档：result_payload None（无批）/ 非空（环内 search.rerun 采纳，
  从 plan.steps 实录扫出；**或镜像 active 批**——2026-08-17 nl1 转正后 M3 常驻，
  仅 preliminary 批时 legacy result_payload 镜像该批，b 档判定改用独立哨兵
  loop_payload）；preliminary_final 显式判定「润色不会跑 = ¬(LLM 武装 ∧
  polish 子开关)」（2026-08-16 收尾：ubRouteBody 第 10 参 polish 落地，b 档解锁——
...
CFG = LLMConfig(enable_llm=True, api_key="sk-prelim1-test")
...
def _stub_agent(monkeypatch, plan, *, verdict: str = "search"):
    ...
    cr1（并发分流 v3.1）重钉：preliminary 发射点从图前移进 understand 节点入口——
...
def test_gate_positive_emits_preliminary_exactly_once(monkeypatch):
    ...
    # 2026-08-17 nl1 转正（M3 常驻）：仅 preliminary 批时 legacy result_payload **镜像
    # active 批**（过渡期回退兼容，设计 §M3）——不再是 None；批次两键随批出现。
...
def test_preliminary_final_true_when_polish_off(monkeypatch):
    """b 档解锁（2026-08-16 收尾，ubRouteBody 第 10 参 polish）：polish=false 显式关闭
    ...
    assert out["route"] == "search" and out["via"] == "agent"
    # nl1（M3 常驻）：result_payload 镜像 preliminary 批（非 None）——b 档判定看的是
...
def test_gate_breaks_on_action_markers(monkeypatch):
    ...
    cr1：marker 命中 → 不起 flight 直接进图（纯执行不打印）；action 路线 never 发射。"""
    ...
    # cr1 breaking：tool 路线 retrieval 由摘要 dict 变 None + additive retrieval_note。
...
def test_graph_tool_start_precedes_matching_step(monkeypatch):
    ...
    assert kinds[0] == "tool_start", "分流共识的 node_start 是全链第一帧（nl1 常驻环首）"
...
def test_endpoint_non_stream_body_additive_only(monkeypatch):
    ...
    ov1-fix1b 再加 `policy_id`（route=="search" 且组装成功时非 None；本用例检索句走
...
def test_endpoint_stream_kind_passthrough(monkeypatch):
    """流式 kind 透传 + cr1 确定性帧序（v3.1，r3 P1-4 裁定）：tool_start(共识) →
...
def test_endpoint_idem_fingerprint_covers_search_params(monkeypatch):
    ...
    _capture_route_turn(monkeypatch, sink)
    client = TestClient(app, base_url="http://127.0.0.1")
    req = {"utterance": "human blood", "req_id": "prelim1-fp-001", "recall": "off"}
