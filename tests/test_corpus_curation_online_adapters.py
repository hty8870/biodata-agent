...
- 任务 A 端点核实回归钉（curl 验证）：HuBMAP 必须走 /v3/、SCP 列表必须走 site/studies、
...
def test_hubmap_species_filter_after_backfill(tmp_path, monkeypatch):
    ...
    plan = cc.plan_search_online("评审", "hubmap", species="Mouse", project_root=tmp_path)
    assert plan["record_count"] == 1
...
def test_hubmap_species_filter_paginates_to_fill_limit(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cc, "_fetch", _fake)
    plan = cc.plan_search_online("评审", "hubmap", species="Human", limit=2, project_root=tmp_path)
    assert plan["record_count"] == 2
...
# ----------------------------------------------------------------------------------------------
# 任务 A（curl 验证）端点回归钉：验证确认过的现行口径不许回退
# ----------------------------------------------------------------------------------------------
...
# 配方见数据源 API 调研记录§4："GSE"[Entry Type] 枚举 + [Organism] 物种过滤 +
