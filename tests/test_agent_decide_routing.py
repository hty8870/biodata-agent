...
def _tmp_project_root(monkeypatch, tmp_path):
    ...
    # V4 旋钮 env 默认清零（任务1）：各测试要配自己 setenv，
    # 外部环境里的真实配置不许串进离线钉。
...
@pytest.mark.parametrize("utterance,expected", [
    # 验证 克制守卫：词面 4 分（+如果+再搜+最后）但全句是叫停语义 → 一票留 simple。
    ...
    # 验证 词表增补（另外/以及/并且）能被计分；l04 型两分句 score 1，但叠库容加档 → complex
...
def test_sync_empty_source_named_scope_helper():
    """ exec2（exec-gates M5）：点名单源 + sync 空槽 = 按全部在线源同步——
...
def test_pending_hard_gate_import_denial_polarity():
    """ H1：pending 硬闸规则 3 补否定极性——「有新增也不要入库」是
