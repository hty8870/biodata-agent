# -*- coding: utf-8 -*-
"""复杂度路由（2026-08-07 第三批任务1，设计/定标见 docs/工作记录/任务记录_2026-08-07_第三批.md §1）的确定性门。
**全离线**：
...
# 任务4 清单函数的真引用（import 期存根——autouse fixture 会 stub 掉模块属性，
...
@pytest.mark.parametrize("utterance,expected", [
    # 库容加档（b08 实锤：chat 3/3 断链、reasoner 3/3 治愈）：库容问句 + 另一事项信号 → complex。
...
@pytest.mark.parametrize("utterance,expected", [
    # codex P1-3 克制守卫：词面 4 分（；+如果+再搜+最后）但全句是叫停语义 → 一票留 simple。
    ...
    # codex P1-2 词表增补（另外/以及/并且）能被计分；l04 型两分句 score 1，但叠库容加档 → complex
...
def test_explicit_decide_model_seam_is_used(monkeypatch):
    """显式 decide_model 注入缝（codex P2-5）：给了就用它，即使 chat_model 也是注入的。"""
...
def test_pending_hard_gate_search_word_narrowed():
    """pending 升硬闸的入库诉求词表剔「搜」（任务4 v2.1 裁决：「搜搜有没有」是检索语境，
...
# -------------------------------------------- 任务4 codex 二审修复钉（2026-08-08）

def test_width_fold_punctuation_pairs():
    """全半角映射逐对钉（codex 二审 P0-2 实锤初版 。→; 错配）：。→. ；→;。"""
...
def test_task_settled_multi_source_coverage():
    ...
    # 全来源步（空 source）按 result.sources 还原覆盖（P1-4）
...
def test_unsupported_items_exempt_one_per_declined():
    """P1-3 修订：一次婉拒只豁免一条 unsupported；第二条仍未决。"""
...
def test_sync_named_source_half_gate():
    """2026-08-15 gr2（codex 对抗评审 Critical-3）：sync_updates 纳入点名源一致性**半闸**——
...
def test_sync_empty_source_named_scope_helper():
    """2026-08-15 ta2 exec2（exec-gates M5）：点名单源 + sync 空槽 = 按全部在线源同步——
...
def test_curate_extra_named_sources():
    """P1-5：curate 侧补充点名表（Zenodo 等）；GEO 裸词命中是检索词表既有行为（第一趟），
...
def test_pending_import_per_source_diff():
    """P0-1 附带：检出未入库按来源差集——AE 检出、只搜了 10x → AE 仍未入库。"""
...
def test_pending_hard_gate_import_denial_polarity():
    """2026-08-15 fix-exec H1：pending 硬闸规则 3 补否定极性——「有新增也不要入库」是
...
def test_pending_count_query_jitiao_boundary():
    """2026-08-15 fix-exec M3：「十几条/这几条/好几条」不是库容问句——裸子串「几条」
