...
钉的两条不变量（R2-2 对抗评审修复，2026-08-04）：
- P1-1 两通道一致：plan.trace 的 state/detail 必须随 arxStep 进行动流、随 arxFinish 快照进
  总结泡 details——失败节点亮 ✗ 且带原因，空闲通道与 busy 快照通道同一事实同一渲染；
- P1-2 僵尸行动流：arxFinish 后的 420ms 折叠窗口内 arxActive() 为假，新派发 arxBegin 另开
