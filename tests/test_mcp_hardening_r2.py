# -*- coding: utf-8 -*-
"""第二轮对抗评审（R2-5 MCP 路）修复的回归门。

钉死的病形（详见 .census/adversarial2/mcp.md）：
- P1-1  pack_failed/plan_failed ×3 把 {exc}（可含绝对路径）直送客户端且 stderr 无堆栈
        → 统一走 _internal_error：客户端只见类型名，堆栈留 stderr；
- P1-2  build_task_pack 日期零校验 + 空分支零回显 → 与 recommend_datasets 同一把
        「年份打头」轻校验尺，空分支回显实际生效的 retrieval_params；
        【2026-08-08 产品所有者拍板拉齐为严格档（登记册 D12 清零）】轻校验尺已退役：
        两侧统一为 Web 严格档——非法日期/倒挂窗口 → bad_param 点名，空分支回显保留；
- P2-3  build_reuse_pack 单条 uid 无长度上限 → 200 字符上限，超限 bad_param；
- 同族顺手收口：build 模式空指纹「if 给了才比」短路 → 缺一即 bad_param（Web 侧 R2-3
        P2-2 T2 的同型病，文档口径本就是 build 必须带全）。
...
def _assert_sanitized(func, capsys, *args, **kwargs):
    ...
    assert "r2_secret" not in msg          # 异常正文（绝对路径）不进客户端
...
def test_build_task_pack_internal_error_is_sanitized(monkeypatch, capsys):
    def boom():
        raise PermissionError(13, "Permission denied", "Z:\\r2_secret\\corpus")
...
def test_plan_query_edit_internal_error_is_sanitized(monkeypatch, capsys):
    ...
    def boom(*args, **kwargs):
        raise PermissionError(13, "Permission denied", "Z:\\r2_secret\\board")
...
def test_plan_action_internal_error_is_sanitized(monkeypatch, capsys):
    ...
    def boom(*args, **kwargs):
        raise PermissionError(13, "Permission denied", "Z:\\r2_secret\\action")
...
# ---------------------------------------------------------------- P1-2：日期校验 + 空分支回显
# 【2026-08-08 产品所有者拍板拉齐为严格档（登记册 D12 清零）】原「非年份打头 → 忽略=不限」
# 宽松档退役，与 Web `_require_iso_date` 同口径：非法 → bad_param，倒挂 → bad_param。
...
def test_build_task_pack_garbage_date_is_rejected_not_literal_compared(fake_empty_retrieval):
    """P1-2 病形：date_from='zzz' 曾直传检索层做字面比较（'zzz' 字典序大于一切 ISO 日期），
    把全库静默归零再谎称「没有命中」。2026-08-08 产品所有者拍板拉齐为严格档（登记册 D12 清零）：
...
def test_build_task_pack_nonexistent_date_and_inverted_window_rejected(fake_empty_retrieval):
    """日历不存在的日期（2026-02-30）与 from>to 倒挂窗口 → bad_param（D12 严格档）。"""
...
def test_mcp_copy_uses_unified_terms():
    ...
    【2026-08-08 产品所有者拍板拉齐为严格档（登记册 D12 清零）】：非法日期两侧统一显式报错，
