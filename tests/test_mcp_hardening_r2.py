...
-   build_task_pack 日期零校验 + 空分支零回显 → 与 recommend_datasets 同一把
        「年份打头」轻校验尺，空分支回显实际生效的 retrieval_params；
        【 产品侧拍板拉齐为严格档（登记册 D12 清零）】轻校验尺已退役：
...
# ---------------------------------------------------------------- ：日期校验 + 空分支回显
# 【 产品侧拍板拉齐为严格档（登记册 D12 清零）】原「非年份打头 → 忽略=不限」
...
def test_build_task_pack_garbage_date_is_rejected_not_literal_compared(fake_empty_retrieval):
    ...
    把全库静默归零再谎称「没有命中」。 产品侧拍板拉齐为严格档（登记册 D12 清零）：
...
def test_build_task_pack_nonexistent_date_and_inverted_window_rejected(fake_empty_retrieval):
    """日历不存在的日期与 from>to 倒挂窗口 → bad_param（D12 严格档）。"""
...
def test_mcp_copy_uses_unified_terms():
    ...
    【 产品侧拍板拉齐为严格档（登记册 D12 清零）】：非法日期两侧统一显式报错，
