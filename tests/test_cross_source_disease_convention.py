# -*- coding: utf-8 -*-
"""跨源 disease 口径不变量（xdc1 交接，方向 1）：四库统一「健康态写入 disease="normal"」。

背景（交接 `协同/交接/cross-source-disease-convention-xdc1.md`）：四库曾有两套相反口径——
...
方向 1 修复后（2026-07-19）：AE/HCA 适配器把健康态标签（normal/healthy/control）统一写成
...
def test_healthy_state_only_appears_as_canonical_normal(name):
    ...
    （否则分面碎裂、跨源口径再次分裂）。这是交接点名的「现在没有任何测试覆盖跨源口径」的不变量。"""
    bad = []
    for r in _load(name):
