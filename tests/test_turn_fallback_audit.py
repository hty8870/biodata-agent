"""2026-08-10 codex 架构评审裁决落地：`turn.route_turn` 的 agent 路径异常分诊钉。
...
def test_agent_error_fallback_is_flagged_on_the_plan(monkeypatch):
    """2026-08-15 审计 C-6：AgentError 安静降级契约不变（不落审计行），但保底 plan 上
...
def test_audit_fallback_write_failure_is_warned_not_silent(monkeypatch):
    """cross-trace D2 移交项（ta2-exec2 → dispatch2）：turn 调 `_audit_fallback` 的那层
