# -*- coding: utf-8 -*-
"""F2 数据集页一键同步（sync_button_core.js / sync_button.js，engagement）结构契约门。

设计：engagement 落地包 §7（F4 一键同步）/§1.1（如实第一）/
§10（埋点清单）/§4.4（P4 联动）。
...
5. **三端点走 API 真源**：sync-updates / sync-status / recall 必须经 core.js API 常量
   （端点真源单一化，不散裸串）。
6. ** 已退役（corpus-sync 批）**：「并检查 N 个追踪的更新」全体批量
...
def test_shell_never_stores_sync_status_in_localstorage() -> None:
    """设计 §7 硬约束：实例级事实不得存 per-profile localStorage（验证#6 裁决）。"""
