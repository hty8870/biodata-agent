# -*- coding: utf-8 -*-
"""F5 组（后端安全 / 联网 / 文案）对抗评审修复的回归门。

每条用例对应 2026-08-04 十路对抗评审的一条发现，病形与证据见 `.census/adversarial/`：
- xss-sec P1-1      联网限速并发绕过（`_polite_wait` 裸 dict 无锁）；
- copy P1-1         write_boundary 成功回执机制词三连上屏；
- docs-arch P2-4    10x「部分覆盖」诚实声明逐字手抄两份（2026-08-08 起随全量 API 通道退役，改钉不许复活）；
- copy P2-3         duplicate_content hint 残留「（force=True）」；
- docs-arch P2-9    `_NONVALUE` 缺失哨兵第二定义（与 normalizer 真源漂移隐患）；
- xss-sec P2-1      全站零安全响应头（localhost 点击劫持面）；
- backend-probe P2-1 Host 守卫对畸形 Host fail-open（starlette 洗白成绑定地址）；
- xss-sec P2-2      同源闸在 pydantic 校验之后（跨源非法 body 吃 422 而非 403）；
- backend-probe P2-2 `/api/recommend` 非法日期静默吞 / 冒充生效条件上屏；
- p0-record P2-c    MCP `internal_error` 外抛异常正文（可含本机绝对路径）；
- docs-arch P2-7    rerank `_LAST_LLM_ERROR` 模块级槽并发互踩误标。
...
# ======================================================================================
# xss-sec P1-1：限速锁——8 线程并发下相邻放行间隔仍 ≥ min_interval
# ======================================================================================

def test_polite_wait_enforces_interval_under_8_thread_concurrency():
    """评审实测病形：无锁时 8 线程 1.00s 内全部放行、相邻间隔违规 6 次。
...
# ======================================================================================
# copy P1-1：write_boundary 成功回执——机制词退结构化字段，诚实语义一字不丢
# ======================================================================================
...
# ======================================================================================
# docs-arch P2-4：10x 诚实声明单一真源 → 2026-08-08 起整段退役
# ======================================================================================
# 原病灶：「部分覆盖」声明逐字手抄两份。2026-08-08 10x 接入官网私有搜索 API（全量 786 条，
# 见《数据源API调研-2026-08-08.md》§2）后，「页面只列精选条目」的事实本身不再成立——
...
# ======================================================================================
# copy P2-3：duplicate_content——人话 hint 不带 API 参数名，参数名走结构化字段
# ======================================================================================
...
# ======================================================================================
# docs-arch P2-9：_NONVALUE 与 normalizer 单一真源对齐（import 派生，防漂移）
# ======================================================================================
...
# ======================================================================================
# xss-sec P2-1：安全响应头（含 Host 守卫直返的 403 也要带头）
# ======================================================================================
...
# ======================================================================================
# backend-probe P2-1：畸形 Host fail-closed（读原始头自解析，不信 starlette 洗白）
# ======================================================================================

@pytest.mark.parametrize("host", [
    "127.0.0.1:abc",            # 畸形端口（评审实测旧代码 200）
    "127.0.0.1:7981:80",        # 双端口（评审实测旧代码 200）
    "127.0.0.1:abc@evil.com",   # userinfo 混入（评审实测旧代码 200）
...
# ======================================================================================
# xss-sec P2-2：同源闸提到 pydantic 校验之前——跨源非法 body 吃 403 而非 422
# ======================================================================================
...
# ======================================================================================
# backend-probe P2-2：/api/recommend 非法日期 → 400（不静默吞、不冒充生效）
# ======================================================================================
...
# ======================================================================================
# p0-record P2-c：MCP internal_error 脱敏——客户端只见类型名，细节留 stderr
# ======================================================================================
...
# ======================================================================================
# docs-arch P2-7：rerank 错误归因 per-call 隔离——并发下 401 不被误标成临时故障
# ======================================================================================
