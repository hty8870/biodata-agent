# -*- coding: utf-8 -*-
"""第二轮对抗评审（R2-3 backend 路）Web 侧修复的回归门。

钉死的病形（详见 .census/adversarial2/backend.md）：
- P1-2  /api/upload 无体积上限 → 64MB 闸，超限 413 人话；
- P1-3  /api/task-pack/preview 日期未铺 _require_iso_date → 非法即 400；
- P2-1  Host 守卫放行 userinfo → 403（尾随点放行是有意行为，一并钉住）；
- P2-2  build 空指纹跳校验 → 缺一即 400；retrieval_date 未校验（引号注入响应头 /
        中文未捕获 500）→ 非法即 400；500 档缺安全头 → middleware 兜底；
- P2-4  _validate_pack_sources 对 str 逐字符枚举 → 先判型，给「sources 需要数组」人话。
...
def test_upload_over_64mb_rejected_413(upload_tmp_root):
    """P1-2 病形：68.7MB 曾 200 落盘。现在超限 413 + 人话，且一个字节不落盘。"""
...
def test_preview_rejects_garbage_dates_before_retrieval(monkeypatch):
    """P1-3 病形：date_from=not-a-date 曾 200 谎称「没有命中」；2020-13-45 曾冒充生效条件。"""
...
def test_build_rejects_empty_fingerprints(monkeypatch):
    """P2-2 T2 病形：空指纹三件套 + 篡改 limit 曾 200 出包。现在缺一即 400，检索都不跑。"""
...
def test_build_rejects_bad_retrieval_date(monkeypatch):
    """P2-2 T3/T4 病形：引号曾注入 Content-Disposition 第二参数；中文曾炸成未捕获 500。"""
    monkeypatch.setattr(webapp, "_task_pack_retrieval", _ShouldNotRun)
...
def test_500_responses_carry_security_headers(monkeypatch, capsys):
    """P2-2 500 档病形：未捕获异常曾是唯一缺 XFO/nosniff/Referrer-Policy 的档位。
    现在 middleware 兜底：三头带齐、客户端只见通用文案（异常正文不上屏）、堆栈留 stderr。"""
    def boom(_params):
        raise RuntimeError("boom at Z:\\r2_secret\\corpus")
    ...
    assert "r2_secret" not in res.text
    assert "RuntimeError" in capsys.readouterr().err


# ---------------------------------------------------------------- P2-1：Host 守卫 userinfo
...
def test_build_sources_string_gets_shape_error_not_char_enum(monkeypatch):
    """P2-4 病形：sources='10x Genomics' 曾报「不认识这些数据来源：1、0、x、 …」。"""
    monkeypatch.setattr(webapp, "_task_pack_retrieval", _ShouldNotRun)
