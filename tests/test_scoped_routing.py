# -*- coding: utf-8 -*-
"""scoped 路由（2026-08-17 过夜批 M1）的常驻钉。

2026-08-17 nl1 转正：scoped 路由成为**唯一路径**——route.request 常驻动词表、
...
def test_suites_and_faces():
    """三套件常驻装配（rank/rerank 同在注册表——M2 已同步转正）。
    ...
    assert AX._SUITE_LOOP_VERBS["action"] == (
        ...
        "curate.db_status", "curate.rollback",  # rb1（2026-08-17）：回滚动词入动作套件
        "compare.datasets", "cite.export", "compat.find", "fair.check",
    )
...
def test_consensus_single_valid_vote_not_consensus():
    """2026-08-17 codex 对抗评审 高2：只有 1 张有效票的「唯一最高」不是共识——
...
def test_route_consensus_never_sees_result_titles():
    """2026-08-17 codex 对抗评审 高3（诚实不变量哨兵钉）：分流模型的输入只允许
...
def test_route_consensus_records_llm_usage():
    """2026-08-17 codex 评审 中4：共识投票是真实 LLM 调用，必须过 `_usage_record`
...
# ---------------------------------------------------------------- nl2 高2/中3：JSON-only 模型 search 面全图

class _JsonOnlySpyModel:
    """JSON-only 替身（nl2 高2/中3）：bind_tools 抛错（模拟 provider 不支持
...
def test_json_only_model_walks_search_face_full_graph(monkeypatch):
    """2026-08-17 nl2 高2 + 中3：真共识（本文件 autouse fixture 恢复真引用，不受
    ...
    此前在图内零覆盖，本条是对冲钉）。"""
    monkeypatch.setattr(AX, "_audit_loop_tool", lambda *a, **k: None)
    # 审计台账桩（codex nl2 复核中指）：JSON 兜底成功会经 _audit_fallback 写真实
