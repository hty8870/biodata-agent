# -*- coding: utf-8 -*-
"""scoped 路由（过夜批 M1）的常驻钉。

 转正：scoped 路由成为**唯一路径**——route.request 常驻动词表、
...
def test_route_request_registered():
    ...
    # 四工具批刻意更新：route.request 不再是顺序表末位（compare/cite/compat/
...
def test_suites_and_faces():
    """三套件常驻装配（rank/rerank 同在注册表——M2 已同步转正）。
     四工具批：结果处理四工具（compare/cite/compat/fair）同时入 search 与
    ...
    assert set(AX._SUITE_UNDERSTAND_VERBS["search"]) == {
        ...
        # 四工具批：结果处理四工具入检索首步面（检索后追问「对比/兼容/FAIR」）。
        "compare.datasets", "cite.export", "compat.find", "fair.check"}
    ...
    # curate.rollback（：回滚是本地文件操作，不触网）； 四工具批
...
def test_route_consensus_records_llm_usage():
    """ 验证 中4：共识投票是真实 LLM 调用，必须过 `_usage_record`
...
def test_json_only_model_walks_search_face_full_graph(monkeypatch):
    ...
    此前在图内零覆盖，本条是对冲钉）。"""
    monkeypatch.setattr(AX, "_audit_loop_tool", lambda *a, **k: None)
    # 审计台账桩（验证 复核中指）：JSON 兜底成功会经 _audit_fallback 写真实
