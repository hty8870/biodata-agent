"""F1 追踪 UI 真机端到端冒烟（默认本地测试服 7988，可由 BIODATA_E2E_BASE 覆盖；系统 Edge）。
...
with sync_playwright() as pw:
    ...
    # 再进详情 → 在对话中使用 → 上下文 chip（ku2-w2：侧栏窄框态 = 小圆徽章 + hover/点击 popover）
    ...
    # ku2-w2 任务 1a：hero 态（无结果）主输入框也要立即出现完整 chip
    page.goto(BASE, wait_until="load")
    page.wait_for_timeout(1500)
    try:
        if page.locator("#onboarding").is_visible():
