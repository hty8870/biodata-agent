...
# 2026-08-03 受控重基线（用户授权 7 个 Visium HD CytAssist 新数据集入库）：767→774，指纹同步重算。
# 2026-08-17 受控重基线（promo1 任务授权：curate sync 运行时 upload 晋升 tracked 快照
...
def test_base_corpus_is_frozen_784() -> None:
    ...
    不必等官方评测才发现（补 codex 评审指出的『pytest 只查 len>0、未锁 767』覆盖缺口）。
    2026-08-03 受控重基线（用户授权 7 个 Visium HD CytAssist 新数据集入库）：767→774。
    2026-08-17 受控重基线（promo1 任务授权：sync 运行时 upload 晋升 10x-synced.json）：774→784。"""
    settings = get_settings()
    records = load_raw_records(settings.data_dir)
