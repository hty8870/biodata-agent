...
契约真源：docs/工作记录/设计_遥测上传与单版本化_2026-08-20.md §2 / §6。
前端脱敏后经 POST /v1/ingest 上传 usage / benchfb 数据，本服务校验、限流
并原样落库（jsonb 原样存，查询/物化留给后续分析）。

过夜改造批 ov1-bench1（2026-08-21，PKG-E，additive）：
...
安全修复批 S2（2026-08-21，SEC-C02 修 4/5 + PERF-M02 + SUP-M01）：
...
ov1-fix1b（2026-08-22，FIX-B，additive）：
...
ov1-adapt1（2026-08-22，自适应上传阈值，additive）：
...
eng-b3（2026-08-22，意见反馈通道 F5 后端，additive）：
...
# eng-b3：意见反馈解密（ECDH+HKDF+AES-256-GCM；顶层 requirements.txt 已补 cryptography）。
...
MAX_MCP_RECORDS = 200              # mcp_records 每包条数上限（ov1-bench1 批；与 usage/benchfb 同风格的条数上限）
MAX_FEEDBACK_RECORDS = 20          # feedback_records 每包条数上限（eng-b3；与客户端 per-profile 队列上限同源）
...
DEFAULT_EXPORT_DIR = "/data/biodata-telemetry/export"
...
# --- eng-b3：意见反馈加密协议参数（客户端 feedback_core.js 同源，改动必须两端同步）---
...
class Settings:
    ...
    def __init__(
        ...
        # eng-b3：意见反馈解密 P-256 私钥（PEM 或 base64 DER）。不配置时 feedback_records
...
# --- eng-b3：意见反馈记录模型（严格 pydantic，顶层 extra="forbid"）---
...
class IpRateLimiter:
    ...
    def __init__(
        ...
        # ov1-adapt1：窗口内请求尝试计数（含被拒），供 server_hint 压力计算；窗口滑动即归零。
    ...
    def allow(self, key: str) -> bool:
        now = time.monotonic()
        # 窗口内尝试计数（ov1-adapt1）：包含被拒的请求——被拒同样是服务器压力；
        # 窗口滑动即归零，避免计数无限增长。
...
class InFlightGauge:
    """在途请求数轻量计数器（ov1-adapt1，server_hint 的分量之一）。
...
# --- ov1-fix1b：落库前净化（与客户端 usage_core.js telemetryStrip 同规则，逐条对齐）---
...
# eng-b3：API Key 形态遮蔽**追加在既有值级遮蔽之后**（usage 既有遮蔽语义不动）。
...
# --- eng-b3：意见反馈解密（ECDH(P-256) + HKDF-SHA256 + AES-256-GCM）---
...
def _sanitize_feedback_plain(plain: Any, counter: list[int]) -> dict[str, Any]:
    ...
    含 eng-b3 追加的 API Key 形态——第二层防御，客户端加密前已过同一套）。
...
def _store_ingest_packet(engine: Engine, payload: IngestPayload, settings: Settings,
    ...
    ov1-fix1b：三类明细**先落库前净化再 claim/存包**（normalized/packet_id 摘要均为净化后
    形态，重试同包净化结果确定，幂等不受影响）；mcp 事件幂等键用 install_id（整机口径）。
    eng-b3：feedback 明细**先解密再净化**（解密失败 → 422，包整体不落库、明文不回显）；
    ...
    # eng-b3：解密发生在净化计数阶段（事务外、不占连接）；无私钥/解密失败在此抛 422。
    ...
    with engine.begin() as conn:
        ...
        if packet.duplicate:
            return {
                ...
                "accepted_feedback": 0,  # eng-b3：重复包本次零写入（与其它三类同口径）
        ...
        mcp_records = claim_new_events(
            # ov1-fix1b：mcp 幂等键用 install_id 而非 identity——MCP 调用属整机安装，
        ...
        if feedback_decrypted:
            # eng-b3：feedback 幂等键 = 记录内 identity（客户端按 profile/install 语义填，
...
def create_app(settings: Settings) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        # 启动即跑一次，确保部署/重启后无需等待 24h；随后每天执行。宿主机 cron 仅作双保险。
        await _retention_safely()
    ...
    # 低-3（审核）：关闭交互式文档与 OpenAPI 暴露——本服务只有两个内部端点，无需可发现性
    app = FastAPI(
    ...
    async def ingest(request: Request) -> dict[str, Any]:
        # 在途计数（ov1-adapt1）：进入处理器即计入，finally 归还；错误路径同样递减。
        gauge.incr()
        try:
    ...
    async def _ingest_impl(request: Request, settings: Settings, engine: Engine,
        ...
        # ov1-adapt1：200 响应带 server_hint（additive，老客户端不读该字段无影响）——
