...
2) claim identity+kind+event_id（重叠 batch 只保留首次事件）。identity 由调用方定：
   ...
   install_id**（ov1-fix1b——MCP 调用属整机安装，同一安装切换匿名账户重传同一
   call_id 不再重复入库）；**feedback 用记录内 identity 字段**（eng-b3——客户端按
...
def _event_id(event: dict[str, Any], *, kind: str, index: int) -> str:
    ...
    feedback→feedback_id（eng-b3）；缺省时退化为规范 JSON 摘要
...
def claim_new_events(
    ...
    *,
    ...
    （identity_of），mcp 由调用方传 install_id（ov1-fix1b，防账户切换重复入库），
    feedback 由调用方传记录内 identity（eng-b3，客户端按 profile/install 标识语义填）。"""
