#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""遥测数据保留期清理（安全修复批 S2，SEC-C02 修 5）。
...
- 手动执行（receiver 容器内，env 齐备）：
    sudo docker compose -f /opt/biodata-telemetry/docker-compose.yml exec -T receiver \
        python scripts/telemetry_retention.py --days 90
- cron（服务器每日 03:30，避开 03:00 数据备份窗口）：
    sudo crontab -e
    30 3 * * * cd /opt/biodata-telemetry && sudo docker compose exec -T receiver \
        python scripts/telemetry_retention.py --days 90 --quiet \
        >> /var/log/biodata-telemetry-retention.log 2>&1
