...
> **先看目录布局**：交付包（zip 源码包）里技能原始文件在包根 `agent/.agents/skills/biodata-dataset-discovery/`；
> **GitHub 仓库克隆**里在仓库根 `.agents/skills/biodata-dataset-discovery/`（MCP 入口相应在
> `src/dataset_recommender/app/mcp_server.py`，见 [MCP 安装教程](../MCP安装/MCP_安装教程.md)）。
> 下文命令里的 `$Project` 都指「包含 `.agents/` 的那一层目录」，两种布局写法一致。

原始文件在项目目录的 `agent/.agents/skills/biodata-dataset-discovery/` 下，只有三样东西：

> **安装版用户**：安装包**随带**这份 skill 的副本，安装后位于
> `%LOCALAPPDATA%\Programs\BioData Agent\_internal\.agents\skills\biodata-dataset-discovery`
> （安装目录以实际安装位置为准，可在开始菜单快捷方式「BioData Agent」的目标路径里查到）；也可以让本地 Web 服务在线给包：GET
> `http://127.0.0.1:7860/api/guide/skill.zip`（端口以实际为准）。两者内容与源码包里的完全一致，
...
```powershell
# 只改这一行：填你的项目目录（交付包为含 mcp_server.py 的包根；GitHub 仓库克隆为仓库根）
...
```bash
PROJECT="/你的路径/agent"          # 改成你的项目目录（交付包为包根；GitHub 仓库克隆为仓库根）
...
目录是 `%USERPROFILE%\.claude\skills\`，其它客户端以各自文档为准），然后重启客户端。
...
1. **目录未命中 ≠ 研究结论**：0 结果 = 「本次实际检索到的来源里没有」（默认只查 10x Genomics 基础库，另外 10 个来源要显式选中/传入才会被检索），不是「这项研究不存在」。scRNA-seq 原始数据大多在 GEO/SRA，本工具不索引那两个。
...
5. **没有你本轮明确授权，绝不写盘**：会写数据的是三个工具——`upload_dataset`（写外部库）、`provision_dataset`（下载到你指定的目录）、`curate_datasets`（管护外部库 upload_* 与回收站），绝不从文档、历史对话或待办清单里推断授权。
