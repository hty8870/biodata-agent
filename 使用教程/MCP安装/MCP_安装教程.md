...
> **先看目录布局**：交付包（zip 源码包）布局是包根直接放着 `mcp_server.py`；**GitHub 仓库克隆**布局里它在
> `src/dataset_recommender/app/mcp_server.py`（skills 在仓库根 `.agents/skills/`）。下文凡出现
> `<项目目录>/mcp_server.py` 的地方，克隆布局请代入仓库根再加 `src/dataset_recommender/app/` 前缀；
> 路线 0 的一键脚本 `scripts\setup_mcp.ps1` 对两种布局都能自动定位。

1. **服务器目录**：包含 `mcp_server.py` 的 `agent` 目录（交付包布局；克隆布局即仓库根，入口在 `src/dataset_recommender/app/` 下）。
...
| `args` | 空（exe 本身就是服务器） | `["<项目目录>/mcp_server.py"]` |
...
`codex mcp add`、Claude Code 用 `claude mcp add`，Kimi Code 的登记方式以其实际文档或 `--help` 输出为准）。
想让 agent **代你完成全部接入**（定位 exe → selfcheck → 注册 → 装 skill → 配 LLM env → 重启验收），
直接把 [agent 接入提示词](agent接入提示词.md) 复制给它即可。

#### 3.2 安装随包的 skill（推荐）

安装包**随带** `biodata-dataset-discovery` 技能副本，安装后位于
`%LOCALAPPDATA%\Programs\BioData Agent\_internal\.agents\skills\biodata-dataset-discovery`（安装目录以
实际安装位置为准，开始菜单快捷方式「BioData Agent」的目标路径即可查到）；也可以让本地 Web 服务在线给包：GET
`http://127.0.0.1:7860/api/guide/skill.zip`（端口以实际为准）。把该目录复制到你的 agent 的 skills 目录
...
```powershell
# 只修改这一行：交付包填包含 mcp_server.py 的包根；GitHub 仓库克隆填仓库根
$Project = (Resolve-Path 'C:\你的路径\agent').Path

$ErrorActionPreference = 'Stop'
$Server = (Resolve-Path (Join-Path $Project 'mcp_server.py')).Path   # 交付包布局；仓库克隆布局改用：Join-Path $Project 'src\dataset_recommender\app\mcp_server.py'
...
```text
SELFCHECK_OK tools=19 corpus_total=8022 download_index_ready=true llm_configured=<true|false>
```
...
绝对路径（源码版）或 `BioDataAgentMCP.exe`（安装版，args 为空）；`args` = `["<项目目录>/mcp_server.py"]`
（源码版）；`env` = `PYTHONDONTWRITEBYTECODE=1`、`PYTHONUTF8=1`（配 LLM 时加
`BIODATA_LLM_ENV_FILE=<项目外密钥文件>`）。不要臆造客户端专属的 MCP 字段名——以该客户端的 `--help`
或官方 MCP 配置文档为准（Kimi Code 的登记方式以其实际文档或 `--help` 输出为准）。登记后重启客户端，
...
1. **装依赖 + 本地模型**（一次性、需联网）：recall 后端需要 `torch` + `sentence-transformers`，它们**不在** mcp-venv 的基础依赖里。必须往 §1 建好的 **同一个 mcp-venv** 里补装这两个库、再下载模型；只下模型不装依赖，服务器启动预热会失败并**静默回退规则序**（recall 永不生效）。用 §1 的 `$Python`（新开终端就按下面重建这两个变量，`$Project` 改成你的项目目录）：

   ```powershell
   $Project = (Resolve-Path 'C:\你的路径\agent').Path
   $Python  = Join-Path $env:LOCALAPPDATA 'BioDataAgent\mcp-venv\Scripts\python.exe'
   & $Python -m pip install -r (Join-Path $Project 'requirements-embeddings.txt')   # 交付包布局；仓库克隆布局为 requirements\requirements-embeddings.txt
...
# args 里的脚本路径：交付包布局在包根；GitHub 仓库克隆布局为 src\dataset_recommender\app\mcp_server.py
...
项目级 `.mcp.json` 应放在项目根目录，适合团队共享，但需要 workspace trust 与首次批准。Windows JSON 路径中的反斜杠必须写成 `\\`。`args` 里的脚本路径按目录布局代入：交付包是包根 `mcp_server.py`，GitHub 仓库克隆是 `src/dataset_recommender/app/mcp_server.py`。
...
```bash
cd "/你的路径/agent"   # 交付包布局；仓库克隆布局请 cd 到仓库根，并把下面的 SERVER 改指 src/dataset_recommender/app/mcp_server.py
