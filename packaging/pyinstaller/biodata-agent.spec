...
"""PyInstaller onedir spec —— BioData Agent 冻结运行时（安装器工程 W3；W7 收口入口与图标）。
...
- 入口（W7）：Web exe 改走 `desktop_launcher.main`（W2 契约 1 落地）——entry_web.py
  是薄转发（import + raise SystemExit(main())），托盘/单实例/固定端口/attach 全部由
  启动器接管；frozen 冒烟 `--tray-selfcheck` 输出 TRAY_SELFCHECK OK（见交接 W3 验收）。
- 图标（W7）：两个 EXE 都内嵌 packaging/assets/BioDataAgent.ico（W4 品牌资产，
  安装器/快捷方式侧已用同一 ico 兜底；exe 资源图标为 W4 交接验收项 1）。
"""
import sys
...
#    dataset_recommender/data/...。**不打** data/inspection/snapshots/（4.2M，运行时无读取点）。
# ③ 明确不打：research（219M）、models/、.env、tests/eval/services/docs/协同/开发日志。
# ── database/external 官方快照白名单 ────────────────────────────────────────────────
# 2026-08-24 交付审计 fix 1（Critical）：此前 datas 把 external 目录**整体递归**打进——用户上传的
...
# A3：filter_submodules 只收窄模块（pure）——mcp 包的 data 文件（mcp/cli/**，约 25K）
...
# 2026-08-24 wvfix：modulegraph 只收代码不收 package data/binary——Windows 壳依赖
...
# ── 排除清单（P0 ①）：重依赖不随包，运行时函数级 import + try/except 降级 ─────────
_EXCLUDES = [
