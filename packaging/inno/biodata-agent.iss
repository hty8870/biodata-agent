...
;   · AppUserModelID=BioDataAgent.Desktop（Inno 无内置指令，见 [Code] W4_APP_USER_MODEL_ID
;     常量：安装后经 PropertyStore 写入开始菜单/桌面快捷方式）
;   · AppMutex=Local\BioDataAgent.Desktop.E249D2BA84574B8AB2C9B7CFA234BA79
;     （与 W2 desktop_launcher.MUTEX_NAME 逐字一致；运行中二次安装/升级被 mutex 拦）
...
;     （A1-M1：GetFullPathName 真实规范化 + 拒绝含 .. 段/非绝对路径/\\?\ 前缀，
;     再与解析出的预期 data root 精确比较；junction/reparse point 拒绝跟随）
;   · 同 AppId 原位升级；旧版安装器遇到更高已装版本 → 终止并说明
;   · 升级前清理 {app} 旧 onedir 内容（A2-M3；[Code] CleanupStaleInstallRoot 在
...
; AppUserModelID=BioDataAgent.Desktop —— Inno 无内置指令，由 [Code] 常量 W4_APP_USER_MODEL_ID
; 逐字钉住；快捷方式属性写入属后续批次（见 协同/交接）。
AppMutex=Local\BioDataAgent.Desktop.E249D2BA84574B8AB2C9B7CFA234BA79
...
; 品牌图标（快捷方式/卸载显示用，exe 内嵌图标属后续批次 spec 交接）
Source: "..\assets\BioDataAgent.ico"; DestDir: "{app}\assets"; Flags: ignoreversion

[Icons]
; 桌面壳批 ds1（2026-08-21）：快捷方式显式传 --window（安装版默认原生窗口）；
...
english.W4DataAskDelete=Do you also want to delete BioData Agent's local data?%n%nPath: %1%n%nDeleting is IRREVERSIBLE. Click "Yes" to delete it (a normal uninstall keeps it), or "No" to keep it.
english.W4DeleteRejectedPath=Deletion refused: the target path is not exactly the current user's %LOCALAPPDATA%\BioDataAgent.
english.W4DeleteRejectedReparse=Deletion refused: the target is a junction/reparse point; following it could delete unrelated data.
english.W4DeleteRejectedBoundary=Deletion refused: the target equals the install directory or one of its ancestors, or contains the install directory.
english.W4DeleteFailed=Failed to delete local data: %1 (it may be in use).
english.W4NewerInstalled=An older installer cannot replace a newer installation.%n%nInstalled version: %1%nThis installer version: %2%n%nSetup will now exit.
english.W4DirMismatchInstalled=BioData Agent is already installed to a different directory.%n%nExisting installation: %1%nThis installer targets: %2%n%nUninstall the existing installation first, or run without /DIR to keep the current directory.

chinesesimplified.W4DataAskDelete=是否同时删除 BioData Agent 的本地数据？%n%n路径：%1%n%n删除操作不可恢复。点击「是」将删除（默认卸载会保留），点击「否」保留。
chinesesimplified.W4DeleteRejectedPath=已拒绝删除：目标路径并非当前用户的 %LOCALAPPDATA%\BioDataAgent。
chinesesimplified.W4DeleteRejectedReparse=已拒绝删除：目标是 junction/重解析点，跟随它会误删无关数据。
chinesesimplified.W4DeleteRejectedBoundary=已拒绝删除：目标等于安装目录或其上级，或包含安装目录。
chinesesimplified.W4DeleteFailed=删除本地数据失败：%1（数据可能正被占用）。
chinesesimplified.W4NewerInstalled=旧版安装器不能覆盖已安装的更高版本。%n%n已安装版本：%1%n本安装器版本：%2%n%n安装程序即将退出。
chinesesimplified.W4DirMismatchInstalled=检测到 BioData Agent 已安装在其他目录。%n%n现有安装：%1%n本次安装目标：%2%n%n请先卸载现有安装，或不指定 /DIR 以沿用当前安装目录。

[Code]
const
  W4_UNINSTALL_KEY = 'Software\Microsoft\Windows\CurrentVersion\Uninstall\{E249D2BA-8457-4B8A-B2C9-B7CFA234BA79}_is1';
  INVALID_FILE_ATTRIBUTES        = $FFFFFFFF;
  { W7：安装期把生效 data_root 持久化到卸载注册信息的自定义值名（E2E 数据根隔离钩子）}
  W4_DATA_ROOT_REG_VALUE = 'BioDataDataRoot';

var
  { W7：卸载期 data_root 缓存。卸载注册键在 usPostUninstall 前可能已被 Inno 清理
    （m10 实测：该阶段 RegQueryStringValue 读不回 BioDataDataRoot），故在
...
{ ── AppUserModelID（固定参数，Inno 无内置指令）──────────────────────
  ...
  后续批次（见 协同/交接/installer-w4-inno-签名图标.md）。 }
const
  W4_APP_USER_MODEL_ID = 'BioDataAgent.Desktop';

function GetFileAttributesW(lpFileName: WideString): DWORD;
  external 'GetFileAttributesW@kernel32.dll stdcall';

{ A1-M1：删除边界规范化的底层 API——GetFullPathNameW 做真实路径规范化
...
begin
  Result := RegQueryStringValue(HKEY_CURRENT_USER, W4_UNINSTALL_KEY, 'DisplayVersion', V);
  if not Result then
    Result := RegQueryStringValue(HKEY_LOCAL_MACHINE, W4_UNINSTALL_KEY, 'DisplayVersion', V);
end;
...
begin
  Result := RegQueryStringValue(HKEY_CURRENT_USER, W4_UNINSTALL_KEY, 'InstallLocation', Loc);
  if not Result then
    Result := RegQueryStringValue(HKEY_LOCAL_MACHINE, W4_UNINSTALL_KEY, 'InstallLocation', Loc);
end;
...
begin
  ...
  if TryReadInstalledVersion(Installed) then begin
    if CompareVersionStrings(Installed, '{#AppVersion}') > 0 then begin
      ReportSetupBlocked(FmtMessage(CustomMessage('W4NewerInstalled'), [Installed, '{#AppVersion}']), mbError);
    ...
    if Result and CommandLineHasDirOverride and ReadInstalledLocation(InstalledLoc)
      ...
      if not SameInstallDir(InstalledLoc, TargetDir) then begin
        ReportSetupBlocked(FmtMessage(CustomMessage('W4DirMismatchInstalled'), [InstalledLoc, TargetDir]), mbError);
        Result := False;
      end;
    end;
  end;
...
{ ── 删除边界：可选「同时删除我的本地数据」的精确路径校验 ──────────────
  A1-M1 加固：NormalizePath 不再是纯字符串变换——GetFullPathName 真实规范化 +
...
begin
  { A2-M3：[InstallDelete] 只在升级（已装旧版）时执行——全新安装不清用户指到的目录 }
  Result := RegKeyExists(HKEY_CURRENT_USER, W4_UNINSTALL_KEY)
         or RegKeyExists(HKEY_LOCAL_MACHINE, W4_UNINSTALL_KEY);
end;
...
begin
  { W7 数据根隔离钩子（E2E 联调前置，见 协同/交接/installer-w5-e2e-w4.md 验收 1）：
  ...
  if EnvRoot <> '' then
    Result := EnvRoot
  else if RegQueryStringValue(HKEY_CURRENT_USER, W4_UNINSTALL_KEY, W4_DATA_ROOT_REG_VALUE, Redirected)
          and (Redirected <> '') then
    Result := Redirected
  else
...
{ W7：安装期把生效 data_root 写入卸载注册信息（优先 BIODATA_DATA_ROOT env，
...
begin
  ...
  RegWriteStringValue(HKEY_CURRENT_USER, W4_UNINSTALL_KEY, W4_DATA_ROOT_REG_VALUE, EnvRoot);
end;

{ ── A2-M3：升级前清理 install_root 的 onedir 旧内容（排除 unins*）──────────────
...
{ 用户显式勾选 localmodel task 后才联网安装可选本地模型（om1）。
  UX 决策（edge-b1 评估，2026-08-21；om1 收口改 Exec 实现）：保留「安装完成才退出」的
...
begin
  if CurStep = ssInstall then
    CleanupStaleInstallRoot;   { A2-M3：升级前清理 onedir 旧内容（保留 unins*）}
...
begin
  { Target 用 InitializeUninstall 缓存的 U_DataRoot：usPostUninstall 阶段 Inno 已
    ...
    → 误触「拒绝删除」分支（m10 实测）。缓存值是卸载早期按 env 隔离钩子/注册表/
  ...
  if Target = '' then begin
    ReportDeleteFailure(CustomMessage('W4DeleteRejectedPath'), mbCriticalError);
    Exit;
  end;

  { ② A1-M1：删除目标先经 GetFullPathName 规范化 + 非法形态拒绝闸（含 `..` 段/
  ...
  if (Expected = '') or (Target = '') or (Target <> Expected) then begin
    ReportDeleteFailure(FmtMessage(CustomMessage('W4DeleteRejectedPath'), [U_DataRoot]), mbCriticalError);
    Exit;
  end;
  ...
  if (Attr and FILE_ATTRIBUTE_REPARSE_POINT) <> 0 then begin
    ReportDeleteFailure(FmtMessage(CustomMessage('W4DeleteRejectedReparse'), [U_DataRoot]), mbCriticalError);
    Exit;
  end;

  { ④ 拒绝等于安装目录或其上级，也拒绝安装目录位于目标内部的反向情形 }
  if PathIsAncestorOrSame(Target, AppDir) or PathIsAncestorOrSame(AppDir, Target) then begin
    ReportDeleteFailure(FmtMessage(CustomMessage('W4DeleteRejectedBoundary'), [U_DataRoot, AppDir]), mbCriticalError);
    Exit;
  end;

  if not DelTree(Target, True, True, True) then begin
    ReportDeleteFailure(FmtMessage(CustomMessage('W4DeleteFailed'), [Target]), mbError);
    Exit;
  end;
end;

{ ── 卸载删数据开关（W7 非交互验证，见 协同/交接/installer-w5-e2e-w4.md 验收 2）────
  卸载器支持命令行参数 /DELETEDATA：卸载完成后无条件执行「同时删除本地数据」（精确
  路径校验仍生效），供 E2E 矩阵 m10 非交互断言；不带该参数则维持原 GUI 询问、默认
  不勾（静默卸载下 MsgBox 被抑制 → 默认 No → 数据保留）。
  A1-L1：按 token 边界解析（空格/制表符分隔、大小写不敏感），不再用子串匹配——
...
begin
  { 可选「同时删除我的本地数据」：卸载完成后询问，默认 No（默认保留）。
    ...
    W7：卸载器带 /DELETEDATA 时跳过询问直接删除（E2E m10 非交互验证路径）。}
  if CurUninstallStep = usPostUninstall then begin
    if CommandLineWantsDeleteData then
      TryDeleteLocalData
    else if MsgBox(FmtMessage(CustomMessage('W4DataAskDelete'), [U_DataRoot]),
