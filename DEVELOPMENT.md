...
| POST | `/api/curate/check-updates` | 检查来源更新（2026-08-03；只读无 token：ArrayExpress/ENCODE/10x 在线比对最新清单，拉不到如实降级 snapshot；其余离线快照源如实报告本地快照+官网入口） | 否 |
...
| POST | `/api/admin/corpus-sync` | 定时任务用：启动/附着语料同步 job（2026-08-26 corpus-sync 批；恒全源，202 立即返回不阻塞；双闸自认证=env BIODATA_ADMIN_TOKEN 经 X-Admin-Token 头 hmac 比对 + 仅 loopback 对端，未配置 token → 403 fail-closed） | 是 |
